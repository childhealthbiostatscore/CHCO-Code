...
cv1 %>%
  ...
  mutate(
    name = factor(name,
      levels = plot_vars,
      labels = c(
        "HbA1c (%)", "Triglycerides (mg/dL)", "SHBG (mmol/L)",
        "ALT (U/L)", "Systolic Blood Pressure (mmHg)", "HDL (mg/dL)"
      )
    )
  ) %>%
  drop_na() %>%
...
plot_df <- cv1 %>%
  ...
  remove_labels() %>%
  mutate(
    cv_a1c = cut(cv_a1c, c(-Inf, 5.6, 6.4, Inf),
      labels = c("<=5.6%", "5.7-6.4%", ">=6.5%")
    ),
    cv_tg = cut(cv_tg, c(-Inf, 90, 150, Inf),
      labels = c("<90", "90-149", ">=150"), right = F
    ),
    cv_hdl = cut(cv_hdl, c(-Inf, 45, Inf),
      labels = c("<45", ">=45"), right = F
    ),
    cv_alt = cut(cv_alt, c(-Inf, 35, Inf),
      labels = c("<35", ">=35"), right = F
    ),
    cv_shbg = cut(cv_shbg, c(-Inf, 20, Inf),
      labels = c("<20", ">=20"), right = F
    ),
    cv_sbp = cut(cv_sbp, c(-Inf, 120, 130, 140, Inf),
      labels = c("<120", "120-129", "130-139", ">=140"), right = F
    )
  ) %>%
  pivot_longer(all_of(plot_vars)) %>%
...
plot_df %>%
  filter(name == "cv_a1c") %>%
  pivot_wider(names_from = value,values_from = perc,id_cols = weight_cat)%>%
  ggradar()
```
...
cv1 |>
  tbl_cross(
    row = cv_ft_above_uln, col = cv_tt_above_uln,
    percent = "cell", missing_text = "Missing"
  )
```
...
var_label(df$cv_osa_sx) <- "Symptoms of sleep apnea (snoring, frequent awakening, AM headaches, daytime fatigue, napping during day)"
# Variable lists
demo_vars <- c(
