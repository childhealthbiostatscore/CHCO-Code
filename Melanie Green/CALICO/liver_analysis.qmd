...
```{r}
m <- lm(cv_a1c ~ cv_alt * weight_cat, data = cv1)
m %>%
  tbl_regression() %>%
  add_n(location = "level")
plot_model(m, type = "pred", terms = c("cv_alt", "weight_cat"))
```

## Fasting glucose

```{r}
m <- lm(cv_fbg ~ cv_alt * weight_cat, data = cv1)
m %>%
  tbl_regression() %>%
  add_n(location = "level")
plot_model(m, type = "pred", terms = c("cv_alt", "weight_cat"))
...
```{r}
m <- lm(cv_2hrglucoseogtt ~ cv_alt * weight_cat, data = cv1)
m %>%
  tbl_regression() %>%
  add_n(location = "level")
plot_model(m, type = "pred", terms = c("cv_alt", "weight_cat"))
