...
# Recode specialty
df$pcosdx_specialty_combined <- fct_recode(
    df$pcosdx_specialty,
    "Primary Care (Pediatrics or Family Medicine)/Other" = "Primary Care (Pediatrics or Family Medicine)",
...
diagnostic_check <- c(
    "labs_this_visit___4",
    "labs_this_visit___5",
    "labs_this_visit___10",
    "labs_this_visit___11",
    "labs_this_visit___12",
    "labs_this_visit___33",
    "cv_acneother___0",
    "cv_acneother___1",
    "cv_acneother___2",
...
one_year <- df %>%
    ...
    summarise(
        ...
        cv_mood = any(
            cv_mood %in% c("Yes", "Yes, asked by clinician, but no scale used.")
        ),
        `Any glycemia measure` = any(
            !is.na(cv_a1c) | !is.na(cv_fbg) | !is.na(cv_2hrglucoseogtt)
        ),
        cv_a1c = any(!is.na(cv_a1c)),
        cv_fbg = any(!is.na(cv_fbg)),
        cv_2hrglucoseogtt = any(!is.na(cv_2hrglucoseogtt)),
        `Any lipid measure` = any(
            !is.na(cv_hdl) | !is.na(cv_ldl) | !is.na(cv_tg)
        ),
        cv_hdl = any(!is.na(cv_hdl)),
        cv_ldl = any(!is.na(cv_ldl)),
        cv_tg = any(!is.na(cv_tg)),
        cv_dietarycounseling = any(
            cv_dietarycounseling %in%
                c("Yes", "No, but referred to a dietitian")
        ),
        cv_exerciseplan = any(
            cv_exerciseplan %in%
        ...
        `Any other progesterone` = any(cv_medications___9 == "Checked") |
            any(cv_medications___10 == "Checked") |
            any(cv_medications___11 == "Checked") |
            any(cv_medications___12 == "Checked"),
        `Weight loss medication` = any(cv_medications___25 == "Checked") |
...
p <- one_year %>%
    ...
    summarise(across(
        all_of(c(diagnostic, diagnostic_check)),
        ~ mean(.x, na.rm = T)
    )) %>%
...
```{r results='asis'}
m <- glm(
    cv_newmeds___1 ~ pcosdx_specialty_combined * weight_cat,
    data = one_year[
        !(one_year$pcosdx_specialty_combined ==
...
```{r}
m <- glm(
    EC ~ pcosdx_specialty_combined * weight_cat,
    data = one_year,
    family = "binomial"
)
m %>% tbl_regression(., exponentiate = T)
...
```{r}
m <- glm(
    cv_alt ~ pcosdx_specialty_combined * weight_cat,
    data = one_year,
    family = "binomial"
)
m %>% tbl_regression(., exponentiate = T)
...
```{r}
m <- glm(
    `Any glycemia measure` ~ pcosdx_specialty_combined * weight_cat,
    data = one_year,
    family = "binomial"
)
m %>% tbl_regression(., exponentiate = T)
...
```{r}
m <- glm(
    `Any lipid measure` ~ pcosdx_specialty_combined * weight_cat,
    data = one_year,
    family = "binomial"
)
m %>% tbl_regression(., exponentiate = T)
...
m <- glm(
    cv_newmeds___1 ~ pcosdx_specialty_combined * weight_cat,
    data = ever_never[
        !(ever_never$pcosdx_specialty_combined ==
            "Gynecology/Adolescent Medicine" &
