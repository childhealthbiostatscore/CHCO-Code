...
# For weight loss medication, remove all visits after they first start
df <- df %>% filter(is.na(weight_loss_remove), is.na(atypical_remove))
# Additional labels
label(df$combined_race) <- "Race"
label(df$ethnicity) <- "Ethnicity"
label(df$insur_type) <- "Insurance Type"
...
larc_df <- df %>%
  ...
  select(
    ...
    cv_alt, cv_sbp, weight_cat, pcosdx_age
  )
# Create a spline model dataset
