...
visits_exclude <- function(
    ...
    if (nrow(r) == 1) {
        ...
        first_yes <- which(r$value == value & r$months > 3)[1]
        if (is.na(first_yes) & any(r$value == value & (r$end > r$start))) {
            # If there is no 3 month stretch, but there are consecutive visits, remove
            # everything after the first "Yes"
            data$remove[r$start[2]:nrow(data)] <- "Remove"
        } else if (
            is.na(first_yes) & !any(r$value == value & (r$end > r$start))
        ) {
            data$remove <- NA
        ...
        data$remove[r[r$value == value & r$length == 1, "start"]] <- "Remove"
    }
    ...
    if (remove_all_after) {
        data$remove <- NA
        if (value %in% r$value) {
            first_yes <- which(r$value == value)[1]
...
```{r data import}
# Load data
load("./Data_Clean/analysis_data_2025_06_26.RData")
...
dfs <- lapply(dfs, function(d) {
    ...
    d$progesterone_remove <- visits_exclude(
        d,
        "progesterone",
        remove_all_after = T
    )
    d$weight_loss_remove <- visits_exclude(
        d,
        "weight_loss",
        remove_all_after = T
    )
    d$atypical_remove <- visits_exclude(
        d,
        "cv_medications___17",
        remove_all_after = T
    )
    d$bariatric_surg_remove <- visits_exclude(
        d,
        "cv_surghx___3",
        value = "Checked",
        remove_all_after = T
...
df <- df %>%
    filter(
        is.na(weight_loss_remove),
        is.na(progesterone_remove),
        is.na(atypical_remove),
        is.na(bariatric_surg_remove)
    )
# Additional labels
...
write.csv(ec_df, file = "./Data_Clean/ec_data.csv", row.names = F, na = "")
write.csv(
    ec_12_month_spline,
    file = "./Data_Clean/ec_12_month_spline.csv",
    row.names = F,
    na = ""
)
```

## Participant characteristics at diagnostic visit
...
p <- ggplot(mod_df, aes(x = time_from_first_ec)) +
    geom_ribbon(
        aes(ymin = pop_pred_ll, ymax = pop_pred_ul),
        colour = "lightgrey",
        fill = "lightgrey",
        alpha = 0.5
    ) +
...
metformin_df <- df %>%
    ...
    mutate(
        time_of_first_metformin = first(
            cv_monthssincepcosdx[metformin == "Yes"],
            na_rm = T
        ),
        time_from_first_metformin = cv_monthssincepcosdx -
...
label(
    metformin_df$time_from_first_metformin
) <- "Months from First Visit on Metformin"
label(metformin_12_month_spline$x) <- "Change in Slope After Metformin Start"
# Write for Melanie and Grayson to check
write.csv(
    metformin_df,
    file = "./Data_Clean/metformin_data.csv",
    row.names = F,
    na = ""
)
write.csv(
    metformin_12_month_spline,
    file = "./Data_Clean/metformin_12_month_spline.csv",
    row.names = F,
    na = ""
)
```
...
t1_df <- metformin_df %>%
    ...
    filter(cv_monthssincepcosdx == 0)
t1_df$metformin_ever <- factor(
    t1_df$metformin_ever,
    levels = c(F, T),
...
lifestyle_df <- df %>%
    ...
    mutate(
        time_of_first_lifestyle = first(
            cv_monthssincepcosdx[lifestyle == "Yes"],
            na_rm = T
        ),
        time_from_first_lifestyle = cv_monthssincepcosdx -
...
label(
    lifestyle_df$time_from_first_lifestyle
) <- "Months from First Visit on Lifestyle"
label(lifestyle_12_month_spline$x) <- "Change in Slope After Lifestyle Start"
# Write for Melanie and Grayson to check
write.csv(
    lifestyle_df,
    file = "./Data_Clean/lifestyle_data.csv",
    row.names = F,
    na = ""
)
write.csv(
    lifestyle_12_month_spline,
    file = "./Data_Clean/lifestyle_12_month_spline.csv",
    row.names = F,
    na = ""
)
```
...
t1_df <- lifestyle_df %>%
    ...
    filter(cv_monthssincepcosdx == 0)
t1_df$lifestyle_ever <- factor(
    t1_df$lifestyle_ever,
    levels = c(F, T),
...
df <- do.call(rbind, dfs)
# Get baseline weight and BMI, ever LARC, and age at first LARC
df <- df %>%
    arrange(record_number, cv_monthssincepcosdx) %>%
    group_by(record_number) %>%
    mutate(
...
label(df$weight_loss) <- "On Weight Loss Medication?"
label(df$larc_ever) <- "Ever on LARC?"
label(df$age_first_larc) <- "Age at First LARC"
# Stratify by weight group
# df <- df %>% filter(weight_cat == "Obese")
# Create the LARC dataset
larc_df <- df %>%
    # Remove visits after weight loss or atypical antipsychotic start, and those
    # visits where we're not really sure if they're on LARC or not.
    filter(
        is.na(larc_remove),
        is.na(weight_loss_remove),
        is.na(atypical_remove),
    ...
    mutate(
        time_of_first_larc = first(
            cv_monthssincepcosdx[larc == "Yes"],
            na_rm = T
        ),
...
# Write for Melanie and Grayson to check
write.csv(larc_df, file = "./Data_Clean/larc_data.csv", row.names = F, na = "")
write.csv(
    larc_12_month_spline,
    file = "./Data_Clean/larc_12_month_spline.csv",
    row.names = F,
    na = ""
)
```
...
summary(
    t1,
    pfootnote = T,
    labelTranslations = list(age_first_larc = "Age at first LARC")
)
```
