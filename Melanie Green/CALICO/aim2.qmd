...
visits_exclude <- function(data = d, column, value = "Yes", remove_all_after = F) {
  # To dataframe
  ...
  if (nrow(r) == 1) {
    ...
    if (is.na(first_yes) & any(r$value == value & (r$end > r$start))) {
      ...
      data$remove[r$start[2]:nrow(data)] <- "Remove"
    } else if (is.na(first_yes) & !any(r$value == value & (r$end > r$start))) {
      data$remove <- NA
...
dfs <- lapply(dfs, function(d) {
  ...
  d$atypical_remove <- visits_exclude(d, "cv_medications___17", remove_all_after = T)
  d$bariatric_surg_remove <- visits_exclude(d,
    "cv_surghx___3",
    value = "Checked", remove_all_after = T
  )
...
df <- df %>%
  filter(
    is.na(weight_loss_remove), is.na(progesterone_remove),
    is.na(atypical_remove), is.na(bariatric_surg_remove)
  )
# Additional labels
...
df$larc_ever <- factor(df$larc_ever,
  levels = c(F, T),
  labels = c("No", "Yes")
)
# Additional labels
...
larc_df <- df %>%
  ...
  filter(
    is.na(larc_remove), is.na(weight_loss_remove), is.na(atypical_remove),
  ...
  select(
    ...
    metformin, cv_medications___16, cv_weight, cv_bmi, cv_a1c, cv_tg, cv_hdl,
    cv_alt, cv_sbp, weight_cat, pcosdx_age, age_first_larc, larc_ever
  )
# Create a spline model dataset
...
t1_df <- larc_df %>%
  ...
  filter(cv_monthssincepcosdx == 0)
t1 <- tableby(
  larc_ever ~ pcosdx_age + age_first_larc + combined_race + ethnicity +
    fe(insur_type) + cv_weight + cv_bmi + cv_a1c + cv_tg + cv_hdl + cv_alt +
    cv_sbp + cv_monthssincepcosdx,
  data = t1_df
)
summary(t1,
  pfootnote = T,
...
```{r results='asis'}
t1 <- tableby(
  weight_cat ~ pcosdx_age + age_first_larc + larc_ever + ethnicity +
    fe(insur_type) + cv_weight + cv_bmi + cv_a1c + cv_tg + cv_hdl + cv_alt +
    cv_sbp + cv_monthssincepcosdx,
  data = t1_df
)
...
mod <- lmer(
  cv_weight ~ cv_monthssincepcosdx * larc + cv_age + baseline_weight +
    combined_race + insur_type + cv_medications___16 + metformin +
    (1 | record_number),
  data = larc_df
)
# Model plot
...
change_mod <- lmer(cv_weight ~ time_from_first_larc + x + baseline_weight +
  cv_age + combined_race + insur_type + cv_medications___16 + metformin +
