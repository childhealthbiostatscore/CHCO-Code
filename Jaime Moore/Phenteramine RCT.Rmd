...
ht_wt_vitals$wt <- rowMeans(ht_wt_vitals[, c("Weight.1..kg.", "Weight.2..kg.", "Weight.3..kg.")], na.rm = TRUE)
...
# DEXA
dxa <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Moore/Phenteramine RCT/Data raw/PilotRCTPhentermineT-LPDEXA_DATA_2025-05-29_1730.csv")
dxa$Record.ID <- dxa$record_id
dxa$Record.ID <- str_pad(dxa$Record.ID, 3, pad = "0")
dxa$Record.ID <- ifelse(dxa$Record.ID == "002", "0002", dxa$Record.ID)
dxa$Record.ID <- ifelse(dxa$Record.ID == "045", "45", dxa$Record.ID)
dxa <- dxa %>% filter(!(record_id %in% c("Jacky Test","Jacky Test 2")))
dxa <- dxa %>% filter(redcap_event_name %in% c("study_visit_1_arm_1", "study_visit_4_arm_1"))
dxa_long <- dxa
dxa_long <- dxa_long %>% filter(!is.na(dexa_complete))
dxa_long <- dxa_long %>% filter(Record.ID %in% t1_data$Record.ID)
dxa_long <- dxa_long %>% select(Record.ID, redcap_event_name, totalmass, fatmass_absolute, ffm_absolute, pctfatmass, 
                                pctffm, vat_area, lean_mass, fmi, lmi, almi, abmd_femneck, zscore_abmd_femneck,
                                abmd_totalhip, zscore_abmd_totalhip, abmd_lumbarspine, zscore_abmd_lumbarspine)
...
dxa_wide$delta_pctffm <- dxa_wide$pctffm.study_visit_4_arm_1 - dxa_wide$pctffm.study_visit_1_arm_1
...
dxa_wide$delta_almi <- dxa_wide$almi.study_visit_4_arm_1 - dxa_wide$almi.study_visit_1_arm_1
dxa_wide$delta_abmd_femneck <- dxa_wide$abmd_femneck.study_visit_4_arm_1 - dxa_wide$abmd_femneck.study_visit_1_arm_1
dxa_wide$delta_zscore_abmd_femneck <- dxa_wide$zscore_abmd_femneck.study_visit_4_arm_1 - dxa_wide$zscore_abmd_femneck.study_visit_1_arm_1
dxa_wide$delta_abmd_totalhip <- dxa_wide$abmd_totalhip.study_visit_4_arm_1 - dxa_wide$abmd_totalhip.study_visit_1_arm_1
dxa_wide$delta_zscore_totalhip <- dxa_wide$zscore_abmd_totalhip.study_visit_4_arm_1 - dxa_wide$zscore_abmd_totalhip.study_visit_1_arm_1
dxa_wide$delta_abmd_lumbarspine <- dxa_wide$abmd_lumbarspine.study_visit_4_arm_1 - dxa_wide$abmd_lumbarspine.study_visit_1_arm_1
dxa_wide$delta_zscore_lumbarspine <- dxa_wide$zscore_abmd_lumbarspine.study_visit_4_arm_1 - dxa_wide$zscore_abmd_lumbarspine.study_visit_1_arm_1
...
#dd_summary <- dd %>% group_by(Record.ID, redcap_event_name) %>% summarise(mean_kcal = mean(dds_kcal), .groups = "drop")
...
# Try using aggregate instead of dplyr summarise
dd_summary <- aggregate(dds_kcal ~ Record.ID + redcap_event_name, 
                       data = dd, 
                       FUN = mean, 
                       na.rm = TRUE)
colnames(dd_summary)[3] <- "mean_kcal"

# Then pivot
dd_wide <- dd_summary %>%
  pivot_wider(names_from = redcap_event_name, 
              values_from = mean_kcal,
              names_prefix = "mean_kcal_")
...
labs_long_003$Record.ID <- "003"
labs_long_003$redcap_event_name <- "study_visit_2_arm_1" 
...
# CSSRS
cssrs <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Moore/Phenteramine RCT/Data raw/PilotRCTPhentermineT-LPCSSRS_DATA_2025-01-15_1037.csv")
cssrs <- cssrs %>% filter(!is.na(cssrs_id))
cssrs <- cssrs %>% filter(!(record_id %in% c("Jacky Test","Jacky Test 2")))
# exclude any "lifetime" responses
cssrs <- cssrs %>% filter(str_detect(cssrs_1, "2") == FALSE)
...
cssrs_long <- cssrs_long %>% group_by(Record.ID, redcap_event_name) %>% mutate(pos_screen = max(pos_screen)) %>% ungroup()
cssrs_long$visnum <- ifelse(cssrs_long$redcap_event_name == "screeningenrollmen_arm_1", 0, 
                            ifelse(cssrs_long$redcap_event_name == "study_visit_1_arm_1", 1,
                            ifelse(cssrs_long$redcap_event_name == "study_visit_2_arm_1", 2,
                            ifelse(cssrs_long$redcap_event_name == "study_visit_3_arm_1", 3, 4))))
# old syntax broke
#cssrs_long <- cssrs_long %>% complete(redcap_event_name, nesting(Record.ID))
#cssrs_long <- cssrs_long %>% complete(Record.ID, redcap_event_name)
...
#}
cssrs_wide <- cssrs_long %>%
  select(Record.ID, redcap_event_name, pos_screen) %>%
  pivot_wider(names_from = redcap_event_name, 
              values_from = pos_screen,
              values_fn = max,  # in case of duplicates, take the max
              names_prefix = "pos_screen_")
# Renaming of columns
#colnames(cssrs_wide)[-1] <- paste0("pos_screen_", times)
#colnames(cssrs_wide)[1] <- "Record.ID"

cssrs_wide$pos_CSSRS_v1_v4 <- as.factor(ifelse(cssrs_wide$pos_screen_study_visit_1_arm_1 == 1 | cssrs_wide$pos_screen_study_visit_2_arm_1 == 1 | cssrs_wide$pos_screen_study_visit_3_arm_1 == 1| cssrs_wide$pos_screen_study_visit_4_arm_1 == 1, 1, 
                                      ifelse(is.na(cssrs_wide$pos_screen_study_visit_1_arm_1) | is.na(cssrs_wide$pos_screen_study_visit_2_arm_1) | is.na(cssrs_wide$pos_screen_study_visit_3_arm_1) | is.na(cssrs_wide$pos_screen_study_visit_4_arm_1), NA, 0)))
...
write.csv(cssrs_wide, "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Moore/Phenteramine RCT/Data clean/CSSRS vars.csv", row.names = F)
...
# IWQOL - Self
iwqol <- read.csv("/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Moore/Phenteramine RCT/Data raw/PilotRCTPhentermineT-IWQOL_DATA_2025-05-29_2332.csv")
iwqol_self <- iwqol %>% filter(redcap_repeat_instrument == "iwqolkids")
iwqol_self <- iwqol_self %>% filter(!is.na(iwqolkids_dateofvisit) & !(iwqolkids_dateofvisit == ""))
iwqol_self$Record.ID <- iwqol_self$record_id
iwqol_self$Record.ID <- str_pad(iwqol_self$Record.ID, 3, pad = "0")
iwqol_self$Record.ID <- ifelse(iwqol_self$Record.ID == "002", "0002", iwqol_self$Record.ID)
iwqol_self$Record.ID <- ifelse(iwqol_self$Record.ID == "045", "45", iwqol_self$Record.ID)
# fix some duplicates
iwqol_self <- iwqol_self %>% filter(!(Record.ID == "0002" & redcap_repeat_instance == 1 & redcap_event_name == "study_visit_1_arm_1"))
iwqol_self <- iwqol_self %>% filter(!(Record.ID == "009" & redcap_repeat_instance == 1 & redcap_event_name == "study_visit_1_arm_1"))
...
iwqol_self_wide$delta_iwqol_kids_score <- iwqol_self_wide$iwqol_kids_score.study_visit_4_arm_1 - iwqol_self_wide$iwqol_kids_score.study_visit_1_arm_1
iwqol_self_wide$delta_iwqol_kids_physscore <- iwqol_self_wide$iwqol_kids_physscore.study_visit_4_arm_1 - iwqol_self_wide$iwqol_kids_physscore.study_visit_1_arm_1
iwqol_self_wide$delta_iwqol_kids_esteemscore <- iwqol_self_wide$iwqol_kids_esteemscore.study_visit_4_arm_1 - iwqol_self_wide$iwqol_kids_esteemscore.study_visit_1_arm_1
iwqol_self_wide$delta_iwqol_kids_socialscore <- iwqol_self_wide$iwqol_kids_socialscore.study_visit_4_arm_1 - iwqol_self_wide$iwqol_kids_socialscore.study_visit_1_arm_1
iwqol_self_wide$delta_iwqol_kids_famscore <- iwqol_self_wide$iwqol_kids_famscore.study_visit_4_arm_1 - iwqol_self_wide$iwqol_kids_famscore.study_visit_1_arm_1

# IWQOL - Parent
iwqol_parent <- iwqol %>% filter(redcap_repeat_instrument == "iwqolparent")
...
iwqol_parent$Record.ID <- str_pad(iwqol_parent$Record.ID, 3, pad = "0")
iwqol_parent$Record.ID <- ifelse(iwqol_parent$Record.ID == "002", "0002", iwqol_parent$Record.ID)
iwqol_parent$Record.ID <- ifelse(iwqol_parent$Record.ID == "045", "45", iwqol_parent$Record.ID)
# fix some duplicates
iwqol_parent <- iwqol_parent %>% filter(!(Record.ID == "0002" & redcap_repeat_instance == 1 & redcap_event_name == "study_visit_1_arm_1"))
...
iwqol_parent_wide$delta_iwqol_parent_score <- iwqol_parent_wide$iwqol_parent_score.study_visit_4_arm_1 - iwqol_parent_wide$iwqol_parent_score.study_visit_1_arm_1
iwqol_parent_wide$delta_iwqol_parent_physscore <- iwqol_parent_wide$iwqol_parent_physscore.study_visit_4_arm_1 - iwqol_parent_wide$iwqol_parent_physscore.study_visit_1_arm_1
iwqol_parent_wide$delta_iwqol_parent_esteemscore <- iwqol_parent_wide$iwqol_parent_esteemscore.study_visit_4_arm_1 - iwqol_parent_wide$iwqol_parent_esteemscore.study_visit_1_arm_1
iwqol_parent_wide$delta_iwqol_parent_socialscore <- iwqol_parent_wide$iwqol_parent_socialscore.study_visit_4_arm_1 - iwqol_parent_wide$iwqol_parent_socialscore.study_visit_1_arm_1
iwqol_parent_wide$delta_iwqol_parent_famscore <- iwqol_parent_wide$iwqol_parent_famscore.study_visit_4_arm_1 - iwqol_parent_wide$iwqol_parent_famscore.study_visit_1_arm_1
...
pedsql_self <- read.csv('/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Moore/Phenteramine RCT/Data raw/182793_PedsQLSelf_SCORED_3.5.251_DeId.csv')
#pedsql_self <- pedsql_self %>% filter(!is.na(pedsqlself_dateofvisit) & !(pedsqlself_dateofvisit == ""))
pedsql_self$Record.ID <- pedsql_self$STUDY.ID
pedsql_self$redcap_event_name <- pedsql_self$Event.Name
pedsql_self <- pedsql_self %>% select(Record.ID, redcap_event_name, PHYSICAL.SCORE, PSYCHOSOCIAL.SCORE,
                                      TOTAL.SCORE)
pedsql_self_wide <- reshape(pedsql_self, direction = "wide", idvar = "Record.ID", timevar = "redcap_event_name")
pedsql_self_wide$delta_pedsql_physical_self <- pedsql_self_wide$`PHYSICAL.SCORE.Study Visit 4` - pedsql_self_wide$`PHYSICAL.SCORE.Study Visit 1`
pedsql_self_wide$delta_pedsql_psych_self <- pedsql_self_wide$`PSYCHOSOCIAL.SCORE.Study Visit 4` - pedsql_self_wide$`PSYCHOSOCIAL.SCORE.Study Visit 1`
pedsql_self_wide$delta_pedsql_total_self <- pedsql_self_wide$`TOTAL.SCORE.Study Visit 4` - pedsql_self_wide$`TOTAL.SCORE.Study Visit 1`
...
pedsql_self_wide$Record.ID <- str_pad(pedsql_self_wide$Record.ID, 3, pad = "0")
pedsql_self_wide$Record.ID <- ifelse(pedsql_self_wide$Record.ID == "002", "0002", pedsql_self_wide$Record.ID)
pedsql_self_wide$Record.ID <- ifelse(pedsql_self_wide$Record.ID == "045", "45", pedsql_self_wide$Record.ID)
# PEDS-QL - Parent
pedsql_parent <- read.csv('/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Moore/Phenteramine RCT/Data raw/182793_PedsQLParent_SCORED_3.5.25_DeId.csv')
#pedsql_parent <- pedsql_parent %>% filter(!is.na(pedsqlparent_dateofvisit) & !(pedsqlparent_dateofvisit == ""))
pedsql_parent$Record.ID <- pedsql_parent$STUDY.ID
pedsql_parent$redcap_event_name <- pedsql_parent$Event.Name
pedsql_parent <- pedsql_parent %>% select(Record.ID, redcap_event_name, PSYCHOSOCIAL.SCORE, PHYSICAL.SCORE,
                                      TOTAL.SCORE)
pedsql_parent_wide <- reshape(pedsql_parent, direction = "wide", idvar = "Record.ID", timevar = "redcap_event_name")
pedsql_parent_wide$delta_pedsql_physical_parent <- pedsql_parent_wide$`PHYSICAL.SCORE.Study Visit 4` - pedsql_parent_wide$`PHYSICAL.SCORE.Study Visit 1`
pedsql_parent_wide$delta_pedsql_psych_parent <- pedsql_parent_wide$`PSYCHOSOCIAL.SCORE.Study Visit 4` - pedsql_parent_wide$`PSYCHOSOCIAL.SCORE.Study Visit 1`
pedsql_parent_wide$delta_pedsql_total_parent <- pedsql_parent_wide$`TOTAL.SCORE.Study Visit 4` - pedsql_parent_wide$`TOTAL.SCORE.Study Visit 1`
...
pedsql_parent_wide$Record.ID <- str_pad(pedsql_parent_wide$Record.ID, 3, pad = "0")
pedsql_parent_wide$Record.ID <- ifelse(pedsql_parent_wide$Record.ID == "002", "0002", pedsql_parent_wide$Record.ID)
pedsql_parent_wide$Record.ID <- ifelse(pedsql_parent_wide$Record.ID == "045", "45", pedsql_parent_wide$Record.ID)
...
write.csv(fig1_data, "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Moore/Phenteramine RCT/Data clean/fig1.csv")
...
t4_data_wide <- left_join(t4_data_wide, dxa_wide, by = "Record.ID")
t4_data_wide <- left_join(t4_data_wide, iwqol_self_wide, by = "Record.ID")
t4_data_wide <- left_join(t4_data_wide, iwqol_parent_wide, by = "Record.ID")
```
...
t2 <- tableby(randomization_grp ~ phentdose_tolerated + topdose_tolerated  +
                                              ...
                                              kwt(med_score_Caregiver, "median", "q1q3") +  kwt(med_score_Youth, "median", "q1q3") +
                                              as.factor(severeae) + as.factor(ae_dosereduce) + as.factor(ae_discont) + as.factor(aes_fever) + as.factor(aes_flushing) + 
                                   as.factor(aes_sweating) + as.factor(aes_cooldown) + as.factor(aes_fatigue) + as.factor(aes_energy) + as.factor(aes_vision) + 
                                   as.factor(aes_eye) + as.factor(nutrdifficult) + as.factor(aes_mouth) + as.factor(aes_taste) + as.factor(aes_abd) + as.factor(aes_nv) + 
                                   as.factor(aes_dia) + as.factor(aes_const) + as.factor(aes_cp) + as.factor(aes_hr) + as.factor(aes_sob) + as.factor(aes_exin) + as.factor(aes_cycle) + 
                                   as.factor(aes_back) + as.factor(aes_swell) + as.factor(aes_rash) + as.factor(aes_sleep) + as.factor(aes_tingle) + as.factor(aes_foggy) + 
                                   as.factor(aes_conc) + as.factor(aes_mem) + as.factor(aes_words) + as.factor(aes_ir)+ as.factor(aes_rest) + as.factor(aes_anx) + as.factor(aes_dep) + 
                                   as.factor(aes_allergy) + as.factor(cesd_screen) + as.factor(cssrs_screen) + as.factor(lowbicarb) + as.factor(lowk) + as.factor(highcr) + 
                                   as.factor(highsbp) + as.factor(highdbp) + as.factor(highhr) + as.factor(hightemp) + as.factor(covid_ae) + as.factor(aes_other_yn) + 
                                   as.factor(ae_count) + as.factor(any_ae), data = t2_data)
...
p <- ggplot(data = fig1_data, aes(x = Event.Name, y = percent_wt_change, group = Record.ID, 
  ...
  geom_point(aes(shape=randomization_grp), show.legend = F) + geom_line(guide = "none") + xlab("") + ylab("Percent weight change") + 
  scale_color_grey(name = "Randomization Assignment", start = 0, end = 0.7, guide = "none") +
...
t4 <- tableby(randomization_grp ~ delta_trig + delta_ldl + delta_hdl + delta_alt + delta_a1c + delta_mean_kcal + delta_rmr + delta_pedsql_physical_self +
                delta_pedsql_psych_self + delta_pedsql_total_self + delta_pedsql_physical_parent + delta_pedsql_psych_parent + delta_pedsql_total_parent +
                delta_percent_fat_mass + delta_pctffm + delta_fmi + delta_lmi + delta_vat + delta_almi + delta_abmd_femneck + 
                delta_zscore_abmd_femneck + delta_abmd_totalhip + delta_zscore_totalhip + delta_abmd_lumbarspine + delta_zscore_lumbarspine +
                delta_iwqol_kids_score + delta_iwqol_kids_physscore + delta_iwqol_kids_esteemscore + delta_iwqol_kids_socialscore +
                delta_iwqol_kids_famscore + delta_iwqol_parent_score + delta_iwqol_parent_physscore + delta_iwqol_parent_esteemscore + 
                delta_iwqol_parent_socialscore + delta_iwqol_parent_famscore, data = t4_data_wide)
