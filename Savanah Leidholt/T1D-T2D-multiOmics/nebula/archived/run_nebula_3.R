...
min_cells_gene <- 100
min_mean_gene <- 0.01
min_donors_detected_per_group <- 3 
...
#minimum gene counts per donor = 3 & min cells detected per donor =2
...
filter_genes_donor_aware <- function(counts_sub, meta_sub, pair) {
  ...
  function(g) {
    ...
    counts_g <- as.numeric(counts_basic[g, ])
    ...
    tibble(
      ...
      summarise(
        donor_sum_counts = sum(count),
        donor_n_cells_detected = sum(count > 0),
        detected_donor =
          donor_sum_counts >= 3 &
          donor_n_cells_detected >= 2,
        .groups = "drop"
      )
  }
)
...
put_object(
  file = tmp_rds,
  object = paste0(
    "results/nebula/nebula_3/","nebula_",cell_name,"_",contrast_name,".rds"),
  bucket = "triad",
