...
run_nebula <- function(cell_name, pair, contrast_name) {
  ...
  counts_sub <- counts_nebula_hvg[, meta_sub$cell_barcode, drop = FALSE]
  ...
  #run NEBULA one gene at a time so there isn't an error
  start_time <- Sys.time()
...
put_object(
  file = tmp_rds,
  object = paste0("results/nebula/","nebula_", cell_name,"_",contrast_name,".rds"),
  bucket = "triad",
