---
title: "Magnesium effect on ATTEMPT (Matthias Wolf)"
author: "Ye Ji Choi"
format: html
---
...
attempt_tal <- subset(attempt_so, KPMP_celltype_general == "TAL")
attempt_dct <- subset(attempt_so, KPMP_celltype %in% c("DCT", "dDCT"))
```

```{r echo = F}
gene_lists <- s3read_using_region(FUN = read.csv,
                                  object = "data_clean/sglt2i_magnesium_data.csv", 
                                  bucket = "scrna",
                                  region = "",
                                  na = "")

#create gene lists from Matthias Wolf's files
tal_genes <- c(gene_lists$tal, gene_lists$tal_2)[!is.na(c(gene_lists$tal, gene_lists$tal_2))]
dct_genes <- gene_lists$dct[!is.na(gene_lists$dct)]
mitochondria_genes <- gene_lists$mitochondria[!is.na(gene_lists$mitochondria)]
mtorc_genes <- gene_lists$mtorc1[!is.na(gene_lists$mtorc1)]

magnesium_gs <- c("tal", "dct", "mitochondria", "mtorc")
```
...
plot_volcano <- function(data, fc, p_col, title = NULL, x_axis, y_axis, file_suffix, p_thresh = 0.05,
                         ...
                         cell_type = "",
                         bucket = "scrna",
                         output_base_path = "Projects/Matthias Wolf Magnesium/Figures/",
                         geom_text_size = 3,
                         arrow_padding = 0.09,
...
## NEBULA results: TAL cells

```{r}
# nebula
nebula_compiled_tal <- list()
for (sig in magnesium_gs[1]) {
  message("Starting ", sig)
  sig_start_time <- Sys.time()
  
  genes <- get(paste0(sig, "_genes"))
  attempt_tal_subset <- subset(attempt_tal, features = genes)
  
  counts_subset <- round(GetAssayData(attempt_tal_subset, layer = "counts"))
  genes_list    <- rownames(counts_subset)
  meta_subset   <- attempt_tal_subset@meta.data
  ...
  s3saveRDS(nebula_res, 
            object = paste0("Projects/Matthias Wolf Magnesium/Tables/attempt_nebula_", sig, ".rds"),
            bucket = "scrna",
            region = "")
  
  nebula_compiled_tal[[sig]] <- nebula_res
...
nebula_tidy <- map_dfr(nebula_compiled_tal, function(sig_res) {
...
s3write_using_region(nebula_interaction, FUN = write.csv,
                     object = "Projects/Matthias Wolf Magnesium/Tables/attempt_nebula_interaction_tal.csv",
                     bucket = "scrna", region = "",
                     row.names = FALSE)
```


## NEBULA results: DCT cells

```{r}
# nebula
nebula_compiled_dct <- list()
for (sig in magnesium_gs[2]) {
  message("Starting ", sig)
  sig_start_time <- Sys.time()
  
  genes <- get(paste0(sig, "_genes"))
  attempt_dct_subset <- subset(attempt_dct, features = genes)
  
  counts_subset <- round(GetAssayData(attempt_dct_subset, layer = "counts"))
  genes_list    <- rownames(counts_subset)
  meta_subset   <- attempt_dct_subset@meta.data
  ...
  nebula_compiled_dct[[sig]] <- nebula_res
...
```{r echo = F}
# Compile all nebula results into one tidy dataframe
nebula_tidy <- map_dfr(nebula_compiled_dct, function(sig_res) {
  imap_dfr(sig_res, function(gene_summary, gene_name) {
    as_tibble(gene_summary) %>%
      mutate(Gene = gene_name, .before = 1)
  })
...
s3write_using_region(nebula_interaction, FUN = write.csv,
                     object = "Projects/Matthias Wolf Magnesium/Tables/attempt_nebula_interaction_dct.csv",
                     bucket = "scrna", region = "",
                     row.names = FALSE)
```
