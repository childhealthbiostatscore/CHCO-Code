...
test <- ZING_Model(r=Taxa.names[1],dat=Y2,exposure=exposure.names) #all good

# Check what taxon is causing the problem
problem_taxon <- Taxa.names[212]
print(problem_taxon)

# Check the values in that column
summary(Y2[[problem_taxon]])
table(Y2[[problem_taxon]])

# Check if it has non-integer values
any(!Y2[[problem_taxon]] %% 1 == 0, na.rm = TRUE)

# Which species contributed to genus6 in that iteration?
genus6_species <- which(Z.s.g[, "genus6"] == 1)
species_cols <- paste0("species", genus6_species)

# Look at those species in the problem row
extreme_row <- which(Y2[["genus6"]] == max(Y2[["genus6"]]))
Y2[extreme_row, species_cols]

# Look at the exposure values for row 6
Y2[6, exposure.names]

# Compare to the rest of the dataset
summary(Y2[, exposure.names])
