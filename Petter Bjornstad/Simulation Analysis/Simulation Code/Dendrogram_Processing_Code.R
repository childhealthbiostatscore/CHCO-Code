...
# Get all node labels from the tree
tree_labels <- data.frame(label = c(tree$tip.label, tree$node.label)) %>%
  filter(label != "root", label != "")

#Filter to pylum 1 only
# Get unique taxa at each level for phylum1
sp_names  <- unique(phy1_names$species)
g_names   <- unique(phy1_names$genus)
f_names   <- unique(phy1_names$family)
o_names   <- unique(phy1_names$order)
cl_names  <- unique(phy1_names$class)
p_names   <- unique(phy1_names$phylum)  # just "phylum1"
phy1_names <- c(sp_names,g_names,f_names,o_names,cl_names,p_names)

# Build Newick exactly as before but only for these taxa
genus_list <- list()
for(g in g_names){
  children <- sp_names[Z.s.g[sp_names, g] == 1]
  genus_list[[g]] <- paste0("(", paste(children, collapse=","), ")", g)
}

family_list <- list()
for(f in f_names){
  children <- g_names[Z.g.f[g_names, f] == 1]
  family_list[[f]] <- paste0("(", paste(unlist(genus_list[children]), collapse=","), ")", f)
}

order_list <- list()
for(o in o_names){
  children <- f_names[Z.f.o[f_names, o] == 1]
  order_list[[o]] <- paste0("(", paste(unlist(family_list[children]), collapse=","), ")", o)
}

class_list <- list()
for(cl in cl_names){
  children <- o_names[Z.o.c[o_names, cl] == 1]
  class_list[[cl]] <- paste0("(", paste(unlist(order_list[children]), collapse=","), ")", cl)
}

phylum_list <- list()
for(p in p_names){
  children <- cl_names[Z.c.p[cl_names, p] == 1]
  phylum_list[[p]] <- paste0("(", paste(unlist(class_list[children]), collapse=","), ")", p)
}
...
annotation_data_mixture <- results_data %>%
  ...
  mutate(
    abs_logodds   = abs(expectedLogOdds),
    association   = factor(indicator, levels = c("Not Associated", "Associated"))
  )

# --- Plot ---
annotation_data_mixture <- annotation_data_mixture %>%
  mutate(
...
if (exposure == "Individual"){
  ...
  # --- Plot ---
  annotation_data_individual <- annotation_data_individual %>%
    mutate(
      color_val = ifelse(indicator == "Associated", abs_logodds, NA)
    )
...
#Mixture dendrograms
dendro_fxn(results_data=all_formatted_results,scenario_num=1,exposure="Mixture")
dendro_fxn(results_data=all_formatted_results,scenario_num=2,exposure="Mixture")
dendro_fxn(results_data=all_formatted_results,scenario_num=3,exposure="Mixture")
dendro_fxn(results_data=all_formatted_results,scenario_num=4,exposure="Mixture")
dendro_fxn(results_data=all_formatted_results,scenario_num=5,exposure="Mixture")
dendro_fxn(results_data=all_formatted_results,scenario_num=6,exposure="Mixture")
dendro_fxn(results_data=all_formatted_results,scenario_num=7,exposure="Mixture")
dendro_fxn(results_data=all_formatted_results,scenario_num=8,exposure="Mixture")
dendro_fxn(results_data=all_formatted_results,scenario_num=9,exposure="Mixture")

#Individual dendrograms
dendro_fxn(results_data=all_formatted_results,scenario_num=1,exposure="Individual")
dendro_fxn(results_data=all_formatted_results,scenario_num=2,exposure="Individual")
dendro_fxn(results_data=all_formatted_results,scenario_num=3,exposure="Individual")
dendro_fxn(results_data=all_formatted_results,scenario_num=4,exposure="Individual")
dendro_fxn(results_data=all_formatted_results,scenario_num=5,exposure="Individual")
dendro_fxn(results_data=all_formatted_results,scenario_num=6,exposure="Individual")
dendro_fxn(results_data=all_formatted_results,scenario_num=7,exposure="Individual")
dendro_fxn(results_data=all_formatted_results,scenario_num=8,exposure="Individual")
dendro_fxn(results_data=all_formatted_results,scenario_num=9,exposure="Individual")
