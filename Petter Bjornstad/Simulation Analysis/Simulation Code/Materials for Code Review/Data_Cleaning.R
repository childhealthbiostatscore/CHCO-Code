...
simulation.parameters <- readRDS(fs::path(dir.dat2,"Simulation_Scenarios_03_12_25.rds"))
...
sim.par.all <- read.csv(fs::path(dir.dat2,"Simulation_Parameters.csv")) %>%
...
if (scenario_num=="all"){
for (scenario in 1:9) {
  # for (scenario in 1) { #To examinine one scenairo at a time
...
rm(formatted.results)
    ...
    for (scenario in scenario_num) { #To examinine one scenairo at a time
    results <- readRDS(fs::path(dir.dat,paste0("scn",scenario,"_iters1_to_1000.RDS")))
    formatted.results <- format.fxn(results)
    all_formatted_results <- rbind(all_formatted_results,formatted.results)
  }
