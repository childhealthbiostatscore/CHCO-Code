...
packageVersion("BiocManager") #‘1.30.27’
...
# Set directory 
dir.home <- here::here() 
dir.dat <- "/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Hailey Hampson/1_Ongoing Projects/Microbiome Manuscript/Data & Results"

# Load in data
Object <- readRDS(fs::path(dir.dat, "Formatted_Object_Cleaned.rds"))
table <- data.frame(Object$Table)
table <- table[, -178]  # Remove mitochondria

taxa_tab2 <- as.data.frame(t(table))
# taxa_tab2 <- taxa_tab2[c(1:206), ]
taxa_tab2 <- taxa_tab2[c(30:235), ]
...
tree1 <- (ggtree::ggtree(tree, layout = "fan", open.angle= 50))

tree1
# png("/Users/hehampson/Library/Mobile Documents/com~apple~CloudDocs/BHRM_microbiome/Blank_Dendrogram.png",width=4000,height=4000,res=300)
# print(tree1)
# dev.off()
...
n <- length(unique(rownames(table)))
m1 <- read.csv(fs::path(dir.dat,"BaH_ZING_Results_08_04_2025.csv")) %>%
...
# dd3 %>% filter(is.na(node_num))
# dd3$node_num <- as.numeric(dd3$node_num)
# dd3$Phylum <- sapply(strsplit(dd3$phylum_names, "--"), "[[", 2)
# 
# # For single-tip phyla, use the tip number as the node
# dd3$node_num <- ifelse(is.na(dd3$node_num), dd3$tip_num, dd3$node_num)
...
p_highlight <- p_no_highlight +
  ...
  # scale_fill_manual(values = rainbow(nrow(dd3))[c(2,1,4,3,5,6,7)])

p_highlight

# # What does the tree expect?
# head(tree$tip.label)
# 
# # What do your data have?
# head(psi_dd$Taxa)
# 
# # Any overlap?
# sum(psi_dd$Taxa %in% tree$tip.label)
# sum(beta_dd$Taxa %in% tree$tip.label)
psi_dd$Taxa <- gsub("^d__", "k__", psi_dd$Taxa)
beta_dd$Taxa <- gsub("^d__", "k__", beta_dd$Taxa)
...
p1 <- (ggtree::ggtree(tree, layout = "fan", open.angle= 20)) %<+% psi_dd + 
    ...
    scale_colour_manual(values=c("blue3","darkorange"),na.translate = F)

p2 <- (ggtree::ggtree(tree, layout = "fan", open.angle= 20)) %<+% beta_dd + 
    geom_point(aes(color=n_sig, size = n_sig), alpha=1) + #, size = Mean , size = 4
    scale_color_distiller(palette = "Blues", na.value = NA, direction = 1) + #, name = "Significant Individual PFAS"
...
#Fix naming
rownames(betas.ind.wide) <- gsub("^d__", "k__", rownames(betas.ind.wide))
...
# # max_value <- 3
# 
# range(pfna$pfna, na.rm = TRUE)
#Mixtures
# max_abs <- max(abs(betas.ind$Mean), na.rm = TRUE)
...
plot2 <- gheatmap(plot1, pfpes, offset = 0.6, width = 0.1, colnames_offset_y = -5) +
  scale_fill_gradient2(low = c("darkblue","blue3"), mid = c("blue3","white","orange"), high = c("orange","darkorange2"), midpoint = 0, na.value = "grey50",limits = c(min_value, max_value))
  # scale_fill_gradient2(low = "blue3", mid = "white", high = "orange", midpoint = 0, na.value = "grey50",limits = c(min_value, max_value))
plot2
...
png(fs::path(dir.dat,"/Mixture_Dendrogram.png"),width=5300,height=5300,res=300)
print(mixture.plot)
dev.off()

#Print legend
legend <- plot7
png(fs::path(dir.dat,"/Dendrogram_Legend.png"),width=5300,height=5300,res=300)
print(legend)
dev.off()
...
png(fs::path(dir.dat,"/Individual_Dendrogram.png"),width=5300,height=5300,res=300)
print(individual.plot)
dev.off()
legend <- plot14
png(fs::path(dir.dat,"/Dendrogram_Legend2.png"),width=5300,height=5300,res=300)
print(legend)
dev.off()
...
p1 <- (ggtree::ggtree(tree, layout = "fan", open.angle = 20)) %<+% psi_dd + 
  geom_point(aes(color = Direction_sig_only, size = abs(Mean)), alpha = 1) + 
  scale_colour_manual(values = c("blue3", "darkorange"), na.translate = F) +
  # geom_tiplab(aes(subset = (label %in% sig.only)), size = 2, offset = 0.1)
geom_tiplab(aes(subset = (label %in% sig.only), 
                label = gsub(".*--g__", "", label)), 
            size = 2, offset = 0.1)
png(fs::path(dir.dat,"/Dendrogram_TaxaNames.png"),width=5300,height=5300,res=300)
print(p1)
dev.off()

library(cowplot)
# combined.plot <- plot_grid(mixture.plot,individual.plot,ncol=2)
# ggsave(plot = combined.plot, "/Users/hhampson/Dropbox (USC Lab)/Chatzi Projects (Active)/Env Chem SOL-CHS/Analysis/2_ongoing/CHS PFAS Microbiome (Hailey)/2_Results/New_Dendrograms.jpeg",width=30,height=30)
...
names <- ggtree(tree, layout = "fan", open.angle= 20) %<+%  psi_dd + 
  geom_point(aes(color=Direction_sig_only,size=abs(Mean)), alpha=1) + 
  scale_colour_manual(values=c("blue3","darkorange"),na.translate = F)+
  geom_tiplab(aes(label = str_replace(label, ".*--g__", "")))

# ggsave(plot=names,"Names of sig points.jpeg",width=40,height=40)

png("/Users/hehampson/Library/Mobile Documents/com~apple~CloudDocs/BHRM_microbiome/Named_Mixture_Dendrogram3.png",width=7000,height=7000,res=300)
print(names)
dev.off()
