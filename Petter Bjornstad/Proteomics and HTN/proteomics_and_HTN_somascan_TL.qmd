...
deltas <- deltas %>% select(ID, sbp, dbp, bmi, seq.19563.3, seq.4498.62, seq.20161.41, seq.7957.2, seq.3343.1, 
                            ...
                            seq.13408.23, seq.5307.12, seq.23569.53, seq.3235.50, seq.6506.54, seq.7735.17,
                            seq.17755.5, seq.18896.23, seq.15487.164)
...
colnames_M <- top_htn_df %>%
  ...
  pull(Target)
#colnames_M <- paste0(colnames_M,"_",prot_heatmap)
# png(height = 3000, width = 2500, file = "./TODAY HTN and DKD output/local_T2D_BP_heatmap_TL_baseline_20.png", res = 170)
...
png(height = 3000, width = 2500, file = "./TODAY HTN and DKD output/local_T2D_BP_heatmap_TL_baseline_20noBMI.png", res = 170)
corr_plot_modified_nodict(data=df, 
                   Y=df[,prot_heatmap],   
                 X=df[,c("sbp","dbp")],
                 cor_method="spearman", adj_var = NA,
...
seq_include<-c("seq.24681.2","seq.15487.164","seq.13998.26",
                         "seq.2948.58","seq.23569.53","seq.17755.5",
                         "seq.3343.1","seq.7735.17","seq.24957.6","seq.5307.12")
prot_heatmap <- top_htn_df %>%
  filter(AptName %in% seq_include) %>%
  pull(AptName)
colnames_M <- top_htn_df %>%
  filter(AptName %in% seq_include) %>%
  pull(Target)
#split into two figures, one for risk factors and one for protective factors:
png(height = 3000, width = 2500, file = "/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward/TODAY HTN and DKD output/local_T2D_BP_heatmap_TL_baseline_20noBMI_risk.png", res = 170)
corr_plot_modified_nodict(data=df, 
                   Y=df[,prot_heatmap],   
                 X=df[,c("sbp","dbp")],
                 cor_method="spearman", adj_var = NA,
...
seq_include<-c("seq.19563.3","seq.13408.23",
                               "seq.4498.62","seq.7957.2",
                               "seq.18896.23","seq.6506.54",
                               "seq.3235.50","seq.8958.51",
                                "seq.20161.41","seq.2999.6")
prot_heatmap <- top_htn_df %>%
  filter(AptName %in% seq_include) %>%
  pull(AptName)
colnames_M <- top_htn_df %>%
  filter(AptName %in% seq_include) %>%
  pull(Target)
png(height = 3000, width = 2500, file = "/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward/TODAY HTN and DKD output/local_T2D_BP_heatmap_TL_baseline_20noBMI_protective.png", res = 170)
corr_plot_modified_nodict(data=df, 
                   Y=df[,prot_heatmap],   
                 X=df[,c("sbp","dbp")],
                 cor_method="spearman", adj_var = NA,
...
#### added on 6/4/2026: change in proteins vs. change in BMI correlations
#output table, not heatmap:
corr_table <- function(protein, label) {
  out <- rcorr(protein, deltas$bmi,type = "spearman")
  out_df <- data.frame("Aptamer" = label, "Correlation" = round(out$r[1,2],2), "Pval" = round(out$P[1,2],2))
  return(out_df)
}
...
corr_results_risk <- map2_dfr(
  prot_heatmap,
  colnames_M,  # now label = Target name directly
  ~ corr_table(deltas[, .x], .y)
)
write.csv(corr_results_risk,"/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward/TODAY HTN and DKD output/deltabmi_deltaproteins_risk.csv",row.names=F)
...
corr_results_protective<- map2_dfr(
  prot_heatmap,
  colnames_M,  # now label = Target name directly
  ~ corr_table(deltas[, .x], .y)
)
write.csv(corr_results_protective,"/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward/TODAY HTN and DKD output/deltabmi_deltaproteins_protective.csv",row.names=F)
```
...
colnames_M <- top_htn_df %>%
  ...
  pull(Target)
#colnames_M <- paste0(colnames_M,"_",prot_heatmap)
png(height = 3000, width = 2500, file = "./TODAY subaward/TODAY HTN and DKD output/local_T2D_BP_heatmap_TL_deltas.png", res = 170)
...
png('./TODAY subaward/TODAY HTN and DKD output/Teen-LABS_BP.png', res = 300, width = 14, height = 6, units = "in")
panel_p
dev.off()
...
```{r}
# write file
write.csv(lmm_res,"./TODAY subaward/TODAY HTN and DKD output/TeenLABS_mixed_models_HTN_proteins_BP.csv", row.names = F)

lmm_res_nobmi<-subset(lmm_res,lmm_res$bp_var!="BMI")
write.csv(lmm_res_nobmi,"./TODAY subaward/TODAY HTN and DKD output/TeenLABS_mixed_models_HTN_proteins_BP_top20noBMI.csv", row.names = F)
