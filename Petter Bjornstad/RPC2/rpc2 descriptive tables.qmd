...
#                            avg_pcascl + avg_c_adc + total_kidney_volume_ml + avg_c_r2 + bold_l_bl_cortex + bold_r_bl_cortex + avg_k_r2, 
#                          data = rpc_bx_dat),
#         digits = 1)
...
```{r echo = F}
vars <- c("age","weight","height","bmi","sbp","dbp","map","chol_base","ldl","hdl",
          "hba1c","triglycerides","eGFR_CKD_epi","acr_u","gfr_raw_plasma",
          "gfr_bsa_plasma","erpf_raw_plasma","erpf_bsa_plasma","avg_pcascl",
          "avg_c_adc","total_kidney_volume_ml","avg_c_r2","bold_l_bl_cortex",
          "bold_r_bl_cortex","avg_k_r2")

gg_miss_var(rpc_bx_dat[, c(vars, "visit")], facet = visit)

rpc_bx_dat %>%
  select(record_id, visit, all_of(vars)) %>%
  pivot_longer(-c(record_id, visit)) %>%
  ggplot(aes(visit, value, group = record_id)) +
...
outliers_3sd <- rpc_bx_dat %>%
  ...
  filter(!is.na(value), abs(z) > 3) %>%
  ungroup() %>%
  arrange(variable, desc(abs(z))) %>%
  select(record_id, visit, variable, value, mean_val, sd_val, z) %>%
...
plots <- purrr::imap(vars, ~ plot_rpc_box(rpc_bx_dat_hc, .y, .x))

purrr::walk(plots, print)
```

```{r echo = F}
mm_results <- map(names(vars), 
                  ~ fit_rpc_lmm(subset(rpc_bx_dat_hc, rpc2_group != "HC"), .x))
names(mm_results) <- names(vars)

fixed_all     <- map_dfr(mm_results, "fixed") %>%
  filter(term == "visitpost_treatment")
emmeans_all   <- map_dfr(mm_results, "emmeans")
contrasts_all <- map_dfr(mm_results, "contrasts")
icc_all       <- tibble(outcome = names(mm_results),
                        icc = map_dbl(mm_results, "icc"))
```
...
gg_miss_var(rpc_nonbx_dat[, c(vars, "visit")], facet = visit)
...
```{r echo = F, warning = F, results='asis'}
# BOLD MRI
# summary(arsenal::tableby(~avg_pcascl + avg_c_adc + total_kidney_volume_ml + avg_c_r2 + avg_k_r2, data = rpc2_dat))
```
