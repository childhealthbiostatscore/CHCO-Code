...
parsed_only <- parsed[parsed$Grammar != "NOT_PARSEABLE",]
```
...
## Made a separate list of all (parsed & non-parsed) lipids 
```{r}
...
```{r Getting standardized_name column into lipids_dat to look at expression across patients in duplicates}
...
## Back to the parsed lipids - removing all "NOT_PARSEABLE" values
```{r}
# Removing "NOT_PARSEABLE" values from Grammar column
parsed_rh <- parsed_rh[parsed_rh$Grammar != "NOT_PARSEABLE", ]

# Saving this version of parsed_rh to look back at duplicate values
library(writexl)
raw_data_wd <- "C:/Users/Henry Mangalapalli/UW/OneDrive - UW/Laura Pyle's files - Biostatistics Core Shared Drive/Renal HEIR"
setwd(raw_data_wd)
parsed_only <- as.data.frame(parsed_rh)
write_xlsx(parsed_rh, "parsed_only.xlsx")
```
