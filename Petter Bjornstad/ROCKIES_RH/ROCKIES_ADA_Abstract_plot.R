...
# ============================================================================
# MULTI-PANEL FIGURE: PET METRICS BY GROUP, PATHOLOGY, AND UACR
# Figure 1 - VOXEL-WISE VALUES (_vw suffix)
# ============================================================================
...
# Calculate PET averages - VOXEL-WISE for K1/K2, GLOBAL for F
# Note: Using pre-calculated avg_c_k2_wo_cyst_vw from dataset
PET_avg_vw <- function(data){
  # K2 voxel-wise (use pre-calculated averages if available, otherwise calculate)
  if("avg_c_k2_wo_cyst_vw" %in% names(data)) {
    avg_c_k2_vw <- data$avg_c_k2_wo_cyst_vw
    avg_m_k2_vw <- data$avg_m_k2_wo_cyst_vw
  } else {
    avg_c_k2_vw <- data %>%
  ...
  # F global (no voxel-wise available)
  avg_c_f <- data %>% 
    dplyr::select(lc_f, rc_f) %>% rowMeans(na.rm=T)
  avg_m_f <- data %>% 
    dplyr::select(lm_f, rm_f) %>% rowMeans(na.rm=T)
  
  # K1 voxel-wise (calculate from raw values)
  avg_c_k1_vw <- data %>% 
    dplyr::select(lc_k1_wo_cyst_vw, rc_k1_wo_cyst_vw) %>% rowMeans(na.rm=T)
  avg_m_k1_vw <- data %>% 
    dplyr::select(lm_k1_wo_cyst_vw, rm_k1_wo_cyst_vw) %>% rowMeans(na.rm=T)
  
  # Ratios: K2(vw)/F(global) and K2(vw)/K1(vw)
  avg_c_k2_f_vw <- avg_c_k2_vw / avg_c_f
  avg_m_k2_f_vw <- avg_m_k2_vw / avg_m_f
  avg_c_k2_k1_vw <- avg_c_k2_vw / avg_c_k1_vw
  avg_m_k2_k1_vw <- avg_m_k2_vw / avg_m_k1_vw
  
  results <- data.frame(
    avg_c_k2_vw = avg_c_k2_vw,
    avg_m_k2_vw = avg_m_k2_vw,
    avg_c_f = avg_c_f,
    avg_m_f = avg_m_f,
    avg_c_k1_vw = avg_c_k1_vw,
    avg_m_k1_vw = avg_m_k1_vw,
    avg_c_k2_f_vw = avg_c_k2_f_vw,
    avg_m_k2_f_vw = avg_m_k2_f_vw,
    avg_c_k2_k1_vw = avg_c_k2_k1_vw,
    avg_m_k2_k1_vw = avg_m_k2_k1_vw
  )
  return(results)
}


tmp_results <- PET_avg_vw(dat)

# Remove any existing PET average columns before binding new ones
dat_results <- dat %>% 
  select(-any_of(c('avg_c_k2_vw', 'avg_m_k2_vw', 'avg_c_f', 'avg_m_f', 
                   'avg_c_k1_vw', 'avg_m_k1_vw',
                   'avg_c_k2_f_vw', 'avg_m_k2_f_vw',
                   'avg_c_k2_k1_vw', 'avg_m_k2_k1_vw'))) %>%
  bind_cols(tmp_results)

# Filter for PET data and correct groups
dat_results <- dat_results %>% filter(!is.na(avg_c_k2_vw) | !is.na(avg_c_k2_wo_cyst_vw))
dat_results <- dat_results %>% filter(group %in% c('Lean Control', 'Type 2 Diabetes'))
...
# Calculate K1/K2 voxel-wise and F global from the FULL harmonized data (before SGLT2i filtering)
# This ensures all participants in dat_clinical get values
k1_vw_data <- dat %>%
  filter(!is.na(lc_k1_wo_cyst_vw) | !is.na(rc_k1_wo_cyst_vw) | !is.na(avg_c_k2_wo_cyst_vw)) %>%
  mutate(
    avg_c_k1_vw = rowMeans(select(., lc_k1_wo_cyst_vw, rc_k1_wo_cyst_vw), na.rm = TRUE),
    avg_c_k2_vw = ifelse(!is.na(avg_c_k2_wo_cyst_vw), avg_c_k2_wo_cyst_vw, 
                         rowMeans(select(., lc_k2_wo_cyst_vw, rc_k2_wo_cyst_vw), na.rm = TRUE)),
    avg_c_f = rowMeans(select(., lc_f, rc_f), na.rm = TRUE)
  ) %>%
  mutate(
    avg_c_k2_f_vw = avg_c_k2_vw / avg_c_f,
    avg_c_k2_k1_vw = avg_c_k2_vw / avg_c_k1_vw
  ) %>%
  dplyr::select(record_id, avg_c_k1_vw, avg_c_k2_vw, avg_c_f, avg_c_k2_f_vw, avg_c_k2_k1_vw) %>%
  distinct(record_id, .keep_all = TRUE)

# Merge voxel-wise values into dat_clinical
dat_clinical <- dat_clinical %>%
  left_join(k1_vw_data, by = "record_id")

# Check how many now have K2/K1
cat(sprintf("N with voxel-wise K2/K1 in dat_clinical: %d / %d\n", 
            sum(!is.na(dat_clinical$avg_c_k2_k1_vw)), nrow(dat_clinical)))
...
cat("Creating Panels A-D: PET by Group (Voxel-Wise K, Global F)\n")

# Prepare data for group comparison
aim1_df <- dat2 %>% 
  dplyr::select(record_id, group, avg_c_f, avg_c_k2_vw, avg_c_k2_f_vw, avg_c_k2_k1_vw)

aim1_long <- aim1_df %>%
  pivot_longer(cols = c(avg_c_f, avg_c_k2_vw, avg_c_k2_f_vw, avg_c_k2_k1_vw),
               names_to = "metric",
               values_to = "value") %>%
  mutate(metric = factor(metric, 
                         levels = c("avg_c_f", "avg_c_k2_vw", "avg_c_k2_f_vw", "avg_c_k2_k1_vw"),
                         labels = c("Cortical F", "Cortical K2 (vw)", "Cortical K2/F (vw)", "Cortical K2/K1 (vw)")))
...
# Panel B: Cortical K2 (voxel-wise)
plot_b <- aim1_long %>% filter(metric == "Cortical K2 (vw)") %>%
  ggplot(aes(x = group, y = value, fill = group)) +
  geom_boxplot(color = "black", alpha = 0.8, outlier.shape = NA, linewidth = 0.5) +
  geom_point(position = position_jitter(width = 0.2), alpha = 0.6, size = 2, shape = 21, color = "black", stroke = 0.3) +
  scale_fill_manual(values = group_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5) +
  labs(x = NULL, y = "Cortical K2 (vw)", tag = "B") +
  theme_classic(base_size = 14) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 12, color = "black"),
    axis.text.y = element_text(size = 12, color = "black"),
...
# Panel C: Cortical K2/F (voxel-wise K2, global F)
plot_c <- aim1_long %>% filter(metric == "Cortical K2/F (vw)") %>%
  ggplot(aes(x = group, y = value, fill = group)) +
  geom_boxplot(color = "black", alpha = 0.8, outlier.shape = NA, linewidth = 0.5) +
  geom_point(position = position_jitter(width = 0.2), alpha = 0.6, size = 2, shape = 21, color = "black", stroke = 0.3) +
  scale_fill_manual(values = group_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5) +
  labs(x = NULL, y = "Cortical K2/F (vw)", tag = "C") +
  theme_classic(base_size = 14) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 12, color = "black"),
    axis.text.y = element_text(size = 12, color = "black"),
...
# Panel D: Cortical K2/K1 (voxel-wise)
plot_d <- aim1_long %>% filter(metric == "Cortical K2/K1 (vw)") %>%
  ggplot(aes(x = group, y = value, fill = group)) +
  geom_boxplot(color = "black", alpha = 0.8, outlier.shape = NA, linewidth = 0.5) +
  geom_point(position = position_jitter(width = 0.2), alpha = 0.6, size = 2, shape = 21, color = "black", stroke = 0.3) +
  scale_fill_manual(values = group_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5) +
  labs(x = NULL, y = "Cortical K2/K1 (vw)", tag = "D") +
  theme_classic(base_size = 14) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 12, color = "black"),
    axis.text.y = element_text(size = 12, color = "black"),
...
cat("Creating Panels E-H: GBM Thickness (Voxel-Wise K, Global F)\n")
dat_clinical <- dat_clinical %>% dplyr::select(record_id, arteriosclerosis, `GBM thickness`)
combined_df_gbm <- dat_results %>% left_join(dat_clinical, by='record_id') %>%
  filter(!is.na(`GBM thickness`) & `GBM thickness` != "")

pathology_colors <- c("no" = "#00008B", "yes" = "#8B0000")

# Panel E: Cortical F (global)
plot_e <- ggplot(combined_df_gbm, aes(x = `GBM thickness`, y = avg_c_f, fill = `GBM thickness`)) +
  geom_boxplot(color = "black", alpha = 0.8, linewidth = 0.5) +
  scale_fill_manual(values = pathology_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5, p.adjust.method = "none") +
...
# Panel F: Cortical K2 (voxel-wise)
plot_f <- ggplot(combined_df_gbm, aes(x = `GBM thickness`, y = avg_c_k2_vw, fill = `GBM thickness`)) +
  geom_boxplot(color = "black", alpha = 0.8, linewidth = 0.5) +
  scale_fill_manual(values = pathology_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5, p.adjust.method = "none") +
  labs(x = "GBM Thickening", y = "Cortical K2 (vw)", fill = "GBM Thickening", tag = "F") +
  theme_classic(base_size = 14) +
  theme(
    axis.text = element_text(size = 12, color = "black"),
    axis.title = element_text(size = 14, face = "bold", color = "black"),
...
# Panel G: Cortical K2/F (voxel-wise K2, global F)
plot_g <- ggplot(combined_df_gbm, aes(x = `GBM thickness`, y = avg_c_k2_f_vw, fill = `GBM thickness`)) +
  geom_boxplot(color = "black", alpha = 0.8, linewidth = 0.5) +
  scale_fill_manual(values = pathology_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5, p.adjust.method = "none") +
  labs(x = "GBM Thickening", y = "Cortical K2/F (vw)", fill = "GBM Thickening", tag = "G") +
  theme_classic(base_size = 14) +
  theme(
    axis.text = element_text(size = 12, color = "black"),
    axis.title = element_text(size = 14, face = "bold", color = "black"),
...
# Panel H: Cortical K2/K1 (voxel-wise)
plot_h <- ggplot(combined_df_gbm, aes(x = `GBM thickness`, y = avg_c_k2_k1_vw, fill = `GBM thickness`)) +
  geom_boxplot(color = "black", alpha = 0.8, linewidth = 0.5) +
  scale_fill_manual(values = pathology_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5, p.adjust.method = "none") +
  labs(x = "GBM Thickening", y = "Cortical K2/K1 (vw)", fill = "GBM Thickening", tag = "H") +
  theme_classic(base_size = 14) +
  theme(
    axis.text = element_text(size = 12, color = "black"),
    axis.title = element_text(size = 14, face = "bold", color = "black"),
...
cat("Creating Panels I-L: Arteriosclerosis (Voxel-Wise K, Global F)\n")

combined_df_arterio <- dat_results %>% left_join(dat_clinical, by='record_id') %>%
  filter(!is.na(`GBM thickness`) & `GBM thickness` != "")

# Panel I: Cortical F (global)
plot_i <- ggplot(combined_df_arterio, aes(x = arteriosclerosis, y = avg_c_f, fill = arteriosclerosis)) +
  geom_boxplot(color = "black", alpha = 0.8, linewidth = 0.5) +
  scale_fill_manual(values = pathology_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5, p.adjust.method = "none") +
...
# Panel J: Cortical K2 (voxel-wise)
plot_j <- ggplot(combined_df_arterio, aes(x = arteriosclerosis, y = avg_c_k2_vw, fill = arteriosclerosis)) +
  geom_boxplot(color = "black", alpha = 0.8, linewidth = 0.5) +
  scale_fill_manual(values = pathology_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5, p.adjust.method = "none") +
  labs(x = "Arteriosclerosis", y = "Cortical K2 (vw)", fill = "Arteriosclerosis", tag = "J") +
  theme_classic(base_size = 14) +
  theme(
    axis.text = element_text(size = 12, color = "black"),
    axis.title = element_text(size = 14, face = "bold", color = "black"),
...
# Panel K: Cortical K2/F (voxel-wise K2, global F)
plot_k <- ggplot(combined_df_arterio, aes(x = arteriosclerosis, y = avg_c_k2_f_vw, fill = arteriosclerosis)) +
  geom_boxplot(color = "black", alpha = 0.8, linewidth = 0.5) +
  scale_fill_manual(values = pathology_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5, p.adjust.method = "none") +
  labs(x = "Arteriosclerosis", y = "Cortical K2/F (vw)", fill = "Arteriosclerosis", tag = "K") +
  theme_classic(base_size = 14) +
  theme(
    axis.text = element_text(size = 12, color = "black"),
    axis.title = element_text(size = 14, face = "bold", color = "black"),
...
# Panel L: Cortical K2/K1 (voxel-wise)
plot_l <- ggplot(combined_df_arterio, aes(x = arteriosclerosis, y = avg_c_k2_k1_vw, fill = arteriosclerosis)) +
  geom_boxplot(color = "black", alpha = 0.8, linewidth = 0.5) +
  scale_fill_manual(values = pathology_colors) +
  stat_compare_means(method = "wilcox.test", label = "p.format", size = 5, p.adjust.method = "none") +
  labs(x = "Arteriosclerosis", y = "Cortical K2/K1 (vw)", fill = "Arteriosclerosis", tag = "L") +
  theme_classic(base_size = 14) +
  theme(
    axis.text = element_text(size = 12, color = "black"),
    axis.title = element_text(size = 14, face = "bold", color = "black"),
...
cat("Creating Panel M: UACR Correlations (Voxel-Wise K, Global F)\n")

# Calculate correlations
uacr_f <- cor.test(dat2$acr_u, dat2$avg_c_f, method = "spearman", use = "complete.obs")
uacr_k2_vw <- cor.test(dat2$acr_u, dat2$avg_c_k2_vw, method = "spearman", use = "complete.obs")
uacr_k2f_vw <- cor.test(dat2$acr_u, dat2$avg_c_k2_f_vw, method = "spearman", use = "complete.obs")
uacr_k2k1_vw <- cor.test(dat2$acr_u, dat2$avg_c_k2_k1_vw, method = "spearman", use = "complete.obs")

# Create data frame for ggplot heatmap
corr_df <- data.frame(
  metric = factor(c('Cortical F', 'Cortical K2 (vw)', 'Cortical K2/F (vw)', 'Cortical K2/K1 (vw)'),
                  levels = c('Cortical F', 'Cortical K2 (vw)', 'Cortical K2/F (vw)', 'Cortical K2/K1 (vw)')),
  rho = c(round(uacr_f$estimate, 2), 
          round(uacr_k2_vw$estimate, 2), 
          round(uacr_k2f_vw$estimate, 2), 
          round(uacr_k2k1_vw$estimate, 2)),
  pval = c(uacr_f$p.value, uacr_k2_vw$p.value, uacr_k2f_vw$p.value, uacr_k2k1_vw$p.value)
)
...
ggsave(paste0(base_path, "Figure1_VoxelWise_PET_Analysis.pdf"), 
       plot = combined_plots, width = 22, height = 26, dpi = 300)

ggsave(paste0(base_path, "Figure1_VoxelWise_PET_Analysis.png"), 
       plot = combined_plots, width = 22, height = 26, dpi = 300)

ggsave(paste0(base_path, "Figure1_VoxelWise_PET_Analysis.tiff"), 
       plot = combined_plots, width = 22, height = 26, dpi = 300, compression = "lzw")
...
cat("\n=== SUMMARY STATISTICS FOR ABSTRACT (VOXEL-WISE) ===\n\n")
...
group_summary <- aim1_df %>%
  ...
  summarise(
    ...
    F_mean = mean(avg_c_f, na.rm = TRUE),
    F_sd = sd(avg_c_f, na.rm = TRUE),
    K2_mean = mean(avg_c_k2_vw, na.rm = TRUE),
    K2_sd = sd(avg_c_k2_vw, na.rm = TRUE),
    K2_F_mean = mean(avg_c_k2_f_vw, na.rm = TRUE),
    K2_F_sd = sd(avg_c_k2_f_vw, na.rm = TRUE),
    K2_K1_mean = mean(avg_c_k2_k1_vw, na.rm = TRUE),
    K2_K1_sd = sd(avg_c_k2_k1_vw, na.rm = TRUE)
  )
...
cat("\n--- P-VALUES (Wilcoxon tests) ---\n")
f_test <- wilcox.test(avg_c_f ~ group, data = aim1_df)
k2_test <- wilcox.test(avg_c_k2_vw ~ group, data = aim1_df)
k2f_test <- wilcox.test(avg_c_k2_f_vw ~ group, data = aim1_df)
k2k1_test <- wilcox.test(avg_c_k2_k1_vw ~ group, data = aim1_df)
...
if(nrow(combined_df_gbm) > 0) {
  ...
  cat("\nP-values:\n")
  cat(sprintf("F: p = %.4g\n", wilcox.test(avg_c_f ~ `GBM thickness`, data = combined_df_gbm)$p.value))
  cat(sprintf("K2: p = %.4g\n", wilcox.test(avg_c_k2_vw ~ `GBM thickness`, data = combined_df_gbm)$p.value))
  cat(sprintf("K2/F: p = %.4g\n", wilcox.test(avg_c_k2_f_vw ~ `GBM thickness`, data = combined_df_gbm)$p.value))
  cat(sprintf("K2/K1: p = %.4g\n", wilcox.test(avg_c_k2_k1_vw ~ `GBM thickness`, data = combined_df_gbm)$p.value))
}
...
if(nrow(combined_df_arterio) > 0) {
  ...
  cat("\nP-values:\n")
  cat(sprintf("F: p = %.4g\n", wilcox.test(avg_c_f ~ arteriosclerosis, data = combined_df_arterio)$p.value))
  cat(sprintf("K2: p = %.4g\n", wilcox.test(avg_c_k2_vw ~ arteriosclerosis, data = combined_df_arterio)$p.value))
  cat(sprintf("K2/F: p = %.4g\n", wilcox.test(avg_c_k2_f_vw ~ arteriosclerosis, data = combined_df_arterio)$p.value))
  cat(sprintf("K2/K1: p = %.4g\n", wilcox.test(avg_c_k2_k1_vw ~ arteriosclerosis, data = combined_df_arterio)$p.value))
}

# UACR Correlations
cat("\n--- UACR CORRELATIONS (Spearman) ---\n")
cat(sprintf("F: rho = %.2f, p = %.4g\n", uacr_f$estimate, uacr_f$p.value))
cat(sprintf("K2: rho = %.2f, p = %.4g\n", uacr_k2_vw$estimate, uacr_k2_vw$p.value))
cat(sprintf("K2/F: rho = %.2f, p = %.4g\n", uacr_k2f_vw$estimate, uacr_k2f_vw$p.value))
cat(sprintf("K2/K1: rho = %.2f, p = %.4g\n", uacr_k2k1_vw$estimate, uacr_k2k1_vw$p.value))

cat("\n=== FIGURE COMPLETE ===\n")
cat(sprintf("Saved to: %s\n", base_path))
cat("Files created:\n")
cat("  - Figure1_VoxelWise_PET_Analysis.pdf\n")
cat("  - Figure1_VoxelWise_PET_Analysis.png\n")
cat("  - Figure1_VoxelWise_PET_Analysis.tiff\n")
