...
boxplot_function <- function(data, variable, label){
  ...
  comparisons <- list(
    c("Control", "T2D Combined"),
    c('Control', 'T2D-No SGLTi2'),
    c("T2D-No SGLTi2", "T2D-SGLTi2")
  )
  ...
  ggplot(data %>% dplyr::filter(group2 %in% c('Control', 'T2D Combined')), aes(x = group2, y = Variable, fill = group2))  +
    ...
    theme_minimal()+
    theme(axis.text.x = element_blank(),
          text = element_text(size = 20)) + stat_compare_means(
            comparisons = comparisons,
            method = "wilcox.test",  # or "t.test" for parametric
            label = "p.format",      # shows actual p-values
            bracket.size = 0.8,
            tip.length = 0.02,
            size = 5
          )
