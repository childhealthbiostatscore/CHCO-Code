...
table1(~age + sex + bmi+epic_mfm_1+epic_insulin_1+epic_sglti2_1+epic_glp1ra_1| epic_sglti2_1,data=dat)
dat$record_id
...
dat <- dat[-which(duplicated(dat$kit_id)),]
...
save.image('C:/Users/netio/Documents/UofW/Rockies/ROCKIES_T2D_SGLT2_DylanEdits_Line685.RData')
...
#save.image('C:/Users/netio/Documents/UoFW/Rockies/SGLT2i_T2D_Line717.RData')
save.image('C:/Users/netio/Documents/UofW/Rockies/ROCKIES_T2D_SGLT2_DylanEdits_Line728.RData')
...
dat <- dat %>% filter(kit_id %in% so_kpmp_sc@meta.data$kit_id)

dat <- dat %>% filter(!duplicated(kit_id))

table1::table1(~age + sex + bmi+epic_mfm_1+epic_insulin_1+epic_sglti2_1+epic_glp1ra_1| epic_sglti2_1,data=dat)
...
save.image('C:/Users/netio/Documents/UofW/Rockies/pooledoffset_724.RData')
#----------------------------------------------------------------------------
...
save.image('C:/Users/netio/Documents/UofW/Rockies/pooledoffset_849.RData')
...
so_subset <- subset(so_kpmp_sc, cells = WhichCells(so_kpmp_sc, expression = RNA@data[genes, ] > 0))
...
# #Calculate K2 and F variables
 ...
 save.image('C:/Users/netio/Documents/UofW/Rockies/pooledoffset_879.RData')
```
...
label(dat$epic_sglti2_1) <- 'SGLTi2 Use'
...
#Check
table1(~age + sex + bmi + triglycerides + hba1c+ eGFR_CKD_epi+gfr_bsa_plasma +acr_u +epic_mfm_1+epic_insulin_1+group+epic_sglti2_1 | epic_sglti2_1,data=dat)
...
save.image('C:/Users/netio/Documents/UofW/Rockies/pooledoffset_1078.RData')

load('C:/Users/netio/Documents/UofW/Rockies/pooledoffset_1078.RData')
...
save.image('C:/Users/netio/Documents/UofW/Rockies/pooledoffset_1538.RData')

load('C:/Users/netio/Documents/UofW/Rockies/pooledoffset_1538.RData')
...
if(ncol(counts_matrix) > 0) {
  so_celltype[["RNA"]] <- CreateAssayObject(counts = counts_matrix)
}

identical(colnames(so_celltype), rownames(so_celltype@meta.data))
identical(colnames(so_celltype), colnames(so_celltype[["RNA"]]))

so_celltype[["RNA"]] <- so_celltype[["RNA"]][, colnames(so_celltype)]

expr_matrix <- as.matrix(GetAssayData(so_celltype, assay = "RNA", slot = "data"))
counts_matrix <- as.matrix(GetAssayData(so_celltype, assay = "RNA", slot = "counts"))
...
for(i in c(1:length(genes_list))){
  g <- genes_list[[i]]
  # Subset to your genes directly from the matrix
gene_expr <- expr_matrix[g, ]
gene_counts <- counts_matrix[g, ] %>% as.matrix()


  count_gene <- counts_path[g, , drop = FALSE]
  count_matrix <- t(Matrix(gene_counts, sparse = TRUE))
    ...
    data_g_gene <- group_cell(count = count_matrix, id = meta_gene$kit_id, pred = pred_gene ,offset=library)
    ...
    #With offset
    result <- nebula(count = count_matrix, id = meta_gene$kit_id, pred = pred_gene, reml=T,model="NBLMM",output_re = T,covariance=T,offset=library, 
                     verbose = TRUE, method = 'LN')
    
    results_list[[i]] <- list(gene = g, result = result)  # return both gene name and result
...
nebula_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    ...
    # library <- meta_gene$library_size
    library <- meta_gene$pooled_offset
    data_g_gene <- group_cell(count = count_gene, id = meta_gene$kit_id, pred = pred_gene,offsetlibrary)
