...
#Get same participants
t2d_part <- data.table::fread("C:/Users/netio/Documents/UofW/Rockies/T2D_participants.txt")
lc_part <- data.table::fread('C:/Users/netio/Documents/UofW/Rockies/LC_participants.txt')

combined_part <- bind_rows(t2d_part, lc_part)


so_kpmp_sc <- subset(so_kpmp_sc, kit_id %in% combined_part$kit_id)
...
dat_filt <- dat %>% dplyr::select(age, sex, bmi, epic_mfm_1, epic_insulin_1, epic_sglti2_1, epic_glp1ra_1, group2) %>% filter(!is.na(group2))


table1::table1(~age + sex + bmi+epic_mfm_1+epic_insulin_1+epic_sglti2_1+epic_glp1ra_1| group2,data=dat_filt)
dat$record_id



lc_imaging <- data.table::fread('C:/Users/netio/Documents/UofW/Rockies/LC_rawimagingdata.txt')
t2d_imaging <- data.table::fread('C:/Users/netio/Documents/UofW/Rockies/T2D_rawimagingdata.txt') %>%
  dplyr::select(mrn, lc_k2, rc_k2, lm_k2, rm_k2)

imaging_combined <- rbind(lc_imaging, t2d_imaging) %>% filter(!is.na(lc_k2))

dat2 <- dat %>% filter(mrn %in% imaging_combined$mrn)

dat2_filt <- dat2 %>% dplyr::select(age, sex, bmi, epic_mfm_1, epic_insulin_1, epic_sglti2_1, epic_glp1ra_1, group2) %>% filter(!is.na(group2))

table1::table1(~age + sex + bmi+epic_mfm_1+epic_insulin_1+epic_sglti2_1+epic_glp1ra_1| group2,data=dat2_filt)



 dat_results <- dat %>%
   filter(mrn %in% imaging_combined$mrn) %>% 
   rowwise() %>%
...
save.image('C:/Users/netio/Documents/UofW/Rockies/Line438_Boxplots_NoMed.RData')
...
meta.data <- so_kpmp_sc@meta.data %>% dplyr::select(record_id, mrn, group, kit_id,
                                                    epic_sglti2_1,
                                                    lc_k2, rc_k2, 
                                                    lm_k2, rm_k2, 
                                                    lc_f, rc_f, 
...
meta.data.test <- meta.data %>% filter(!duplicated(mrn))
write.table(meta.data.test %>% filter(group == 'Lean_Control') %>% dplyr::select(record_id, mrn, group, kit_id), 'C:/Users/netio/Documents/UofW/Rockies/LC_IDs.txt', row.names=F, quote=F, sep='\t')

lc_ids <- data.table::fread("C:/Users/netio/Documents/UofW/Rockies/LC_IDs.txt")

harmonized_data <- read.csv("C:/Users/netio/OneDrive - UW/Laura Pyle's files - Biostatistics Core Shared Drive/Data Harmonization/Data Clean/harmonized_dataset.csv", na = '')


test <- harmonized_data %>% filter(kit_id %in% lc_ids$kit_id)
...
write.table(test3, 'C:/Users/netio/Documents/UofW/Rockies/LC_rawimagingdata.txt', 
...
so_kpmp_sc@meta.data <- so_kpmp_sc@meta.data[, !colnames(so_kpmp_sc@meta.data) %in% c('avg_c_k2', 'avg_m_k2', 'avg_c_f', 'avg_m_f', 
...
imaging_combined <- rbind(lc_imaging, t2d_imaging)

so_kpmp_sc@meta.data <- so_kpmp_sc@meta.data %>%
  tibble::rownames_to_column("cell_id") %>%
  left_join(imaging_combined, by = "mrn") %>%
...
ggplot(dat_results %>% filter(!is.na(group2)), aes(x=group2, y=avg_c_k2, fill = group2))+
  geom_boxplot(alpha = 0.6, width = 0.4, size = 1.5)+
  geom_boxplot(data = dat_results_combined, aes(x=2.5, y=avg_c_k2), alpha=0.2, width = 1.75, size = 1.5)+
  scale_fill_manual(values = c('#708090', '#FF8C00', '#FF4500', '#9370DB'))+
#  annotate('rect', xmin = 1.5, xmax = 3.5, ymin = -Inf, ymax = Inf, alpha = 0.2, fill='brown')+
#  annotate('text', x = 2.5, y=min(dat_results_combined$avg_c_k2, na.rm=T) - (sd(dat_results_combined$avg_c_k2, na.rm=T)), label = 'T2D Combined')+
theme_minimal()


ggplot(dat_results %>% filter(!is.na(group2)), aes(x=group2, y=avg_c_k2, fill = group2)) +
  # Wide boxplot for Lean Control
  geom_boxplot(data = . %>% filter(group2 == "Lean Control"), 
               alpha = 0.6, width = 1.75, size = 1.5) + geom_point(data = . %>% filter(group2 == 'Lean Control'), shape = 21)+
  # Narrow boxplots for T2D groups
  geom_boxplot(data = . %>% filter(group2 %in% c("T2D-No SGLTi2", "T2D-SGLTi2")), 
               alpha = 0.6, width = 0.3, size = 1.5)+ geom_point(data = . %>% filter(group2 %in% c("T2D-No SGLTi2", "T2D-SGLTi2")), shape = 21)+
  # Combined background boxplot
  geom_boxplot(data = dat_results_combined, aes(x=2.5, y=avg_c_k2), 
               alpha=0.2, width = 1.75, size = 1.5, inherit.aes = FALSE) + geom_point(shape = 21)+
...
for(i in c(1:length(tests))){
  index <- which(names(dat_results) == tests[i])
  tmp <- dat_results %>% filter(!is.na(group2)) %>% 
    dplyr::select(record_id, group, group2, index)
  names(tmp) <- c('record_id', 'group', 'group2', 'index_var')
  tmp <- tmp %>% filter(!is.na(index_var)) %>% mutate(check_col = paste0(record_id, group, group2, index_var)) %>% filter(!duplicated(check_col))
tmp2 <- dat_results_combined %>% filter(!is.na(group2)) %>% 
  dplyr::select(record_id, group, group2, which(names(dat_results_combined) == tests[i]))
names(tmp2) <- c('record_id', 'group', 'group2', 'index_var')
tmp2 <- tmp2 %>% filter(!is.na(index_var)) %>% mutate(check_col = paste0(record_id, group, group2, 
                                                                         index_var)) %>% 
  filter(!duplicated(check_col))

  
  graphs[[i]] <-ggplot(tmp %>% filter(!is.na(group2)), aes(x=group2, y=index_var, fill = group2))+
  geom_boxplot(alpha = 0.6, width = 0.4, size = 1.5)+
  geom_boxplot(data = tmp2, aes(x=2.5, y=index_var), alpha=0.6, width = 0.4, size = 1.5)+
      scale_fill_manual(values = c('#708090', '#FF8C00', '#FF4500', '#9370DB'))+
      annotate('rect', xmin = 1.75, xmax = 3.25, ymin = -Inf, ymax = Inf, alpha = 0.2, fill='brown')+
  annotate('text', x = 2.5, y=min(tmp2$index_var, na.rm=T) - (sd(tmp2$index_var, na.rm=T)), label = 'T2D Combined')+
theme_minimal()+
    theme(text = element_text(size = 20))+
    labs(x='Study Group', y=tests[i])
}

gridExtra::grid.arrange(graphs[[1]], graphs[[2]], 
                        graphs[[3]], graphs[[4]], 
...
table1::table1(~ age + sex + race_ethnicity  + bmi + triglycerides + hba1c + epic_sglti2_1| group2, data=dat %>% filter(!is.na(group2)))
