...
for(i in c(1:length(tests))){
  index <- which(names(so_kpmp_sc@meta.data) == tests[i])
  tmp <- so_kpmp_sc@meta.data %>%
  ...
  graphs[[i]] <- ggplot(tmp, aes(x=group2, y=index_var, fill = group2))+geom_violin(alpha = 0.6)+
    geom_point(alpha=0.3, position = position_jitter(width = 0.2))+geom_boxplot(color = 'black', width = 0.3, alpha = 0.4)+
    theme_classic()+
