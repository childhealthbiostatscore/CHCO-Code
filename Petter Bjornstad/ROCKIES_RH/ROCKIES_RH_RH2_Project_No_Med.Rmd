...
length(unique(so_kpmp_sc$kit_id)) #24
...
so_kpmp_sc <- FindNeighbors(so_kpmp_sc, assay = "RNA", dims = 1:5)
...
#Table 1
table1(~age + sex + bmi + triglycerides + hba1c+pah_clear_bsa + eGFR_CKD_epi+gfr_bsa_plasma +acr_u +epic_mfm_1+epic_insulin_1| group,data=dat)
...
write.csv(full_results,fs::path(dir.results,"NEBULA_TCA_cycle_ALL_cells_LC_T2D_NoMed_unadjusted_pooled_offset.csv"))

full_results$color <- ifelse(full_results$fdr < 0.05 & full_results$`logFC_groupType 2 Diabetes` > 0, "lightcoral",
                             ifelse(full_results$fdr < 0.05 & full_results$`logFC_groupType 2 Diabetes` < 0, "lightblue", "gray"))
...
dot_plot <- ggplot(full_results, aes(
  y = reorder(gene, `logFC_groupType 2 Diabetes`),
  x = `logFC_groupType 2 Diabetes`,
  color = color,
  size = abs(`logFC_groupType 2 Diabetes`)
)) +
...
png(fs::path(dir.results, "Plot_TCA_cycle_NEBULA_All_Cells_T2D_LC_NoMed_unadjusted_pooled_offset.png"), 
...
write.csv(full_results,fs::path(dir.results,"NEBULA_Ox_Phos_cycle_ALL_cells_LC_T2D_NoMed_unadjusted_pooled_offset.csv"))
...
png(fs::path(dir.results, "Plot_Ox_Phos_cycle_NEBULA_All_Cells_T2D_LC_NoMed_unadjusted_pooled_offset.png"), 
    width = 2500, height = 2000, res = 300)
print(dot_plot)
dev.off()
```
...
write.csv(full_results,fs::path(dir.results,"NEBULA_TCA_cycle_PT_cells_LC_T2D_NoMed_unadjusted_pooled_offset.csv"))
...
png(fs::path(dir.results, "Plot_TCA_cycle_NEBULA_PT_Cells_T2D_LC_NoMed_unadjusted_pooled_offset.png"), 
    width = 2500, height = 2000, res = 300)
print(dot_plot)
dev.off()
```
...
#Filter to PT Cells
so_celltype <- subset(so_kpmp_sc,KPMP_celltype==celltype)
DefaultAssay(so_celltype) <- "RNA" 
...
for (celltype in celltypes) {
#Filter to PT Cells
so_celltype <- subset(so_kpmp_sc,celltype2==celltype)
DefaultAssay(so_celltype) <- "RNA" 
