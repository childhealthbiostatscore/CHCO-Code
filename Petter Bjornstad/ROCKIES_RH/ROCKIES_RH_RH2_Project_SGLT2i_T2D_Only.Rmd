...
biopsy[which(!biopsy %in% ids)] 

#Filter out followup visit for RH/RH2 participant: RH-93-T/RH2-09-T with kit ID KL-0029524
dat <- dat %>% 
 dplyr::filter(record_id!="RH-93-T")
...
table(so_subset@meta.data$KPMP_celltype,so_subset@meta.data$kit_id)
so_celltype <- subset(so_subset,KPMP_celltype=="PT-S3")
table(so_celltype@meta.data$KPMP_celltype,so_celltype@meta.data$kit_id)

# Define your variable set
pet <- c("var1", "var2", "var3")  # Replace with actual column names

# Subset metadata for those variables
meta_subset <- so_celltype@meta.data[, c("kit_id", pet)]

# Identify rows (cells) with any missing value in the pet variables
rows_with_missing <- apply(meta_subset[, pet], 1, function(x) any(is.na(x)))

# Get unique kit_ids from those rows
missing_kit_ids <- unique(meta_subset$kit_id[rows_with_missing])

# Count how many unique participants are missing any of the pet variables
length(missing_kit_ids)
...
# Subset metadata for those variables
meta_subset <- so_celltype@meta.data[, c("kit_id", k2_vars)]

# Identify rows (cells) with any missing value in the pet variables
rows_with_missing <- apply(meta_subset[, k2_vars], 1, function(x) any(is.na(x)))
...
# Count how many unique participants are missing any of the pet variables
length(missing_kit_ids)
length(unique(so_celltype$kit_id)) #26 
table(so_celltype$avg_c_k2,so_celltype$kit_id)
