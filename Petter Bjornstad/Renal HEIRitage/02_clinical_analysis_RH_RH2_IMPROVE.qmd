...
bx_meta <- s3readRDS(object = "data_clean/subset/pb90_ckd_analysis_subset_meta.rds", bucket = "scrna", region = "") %>%
  mutate(kit_id = gsub("kl", "KL", gsub("KI", "KL", kit_id)))

bx_subset <- rh_rh2_croc_improve_unique %>%
  filter(kit_id %in% bx_meta$kit_id)

unique(bx_meta$kit_id)[unique(bx_meta$kit_id) %nin% unique(bx_subset$kit_id)]
```
...
bx_subset$group_glp1 <- ifelse(bx_subset$epic_glp1ra_1 == "Yes", "T2D+", 
                               ifelse(bx_subset$epic_glp1ra_1 == "No" & bx_subset$group == "Type 2 Diabetes", "T2D-", 
                                      "HC"))
summary(arsenal::tableby(group_glp1 ~ age + sex + kwt(acr_u, "medianq1q3") + 
                           eGFR_CKD_epi + bmi + kwt(diabetes_duration, "medianq1q3") + 
                           diabetes_duration  + hba1c + epic_glp1ra_1 + sglt2i_ever +
                           sbp + dbp + map + waist_hip_ratio,
...
subset(bx_subset, is.na(waist_hip_ratio))$record_id
```
