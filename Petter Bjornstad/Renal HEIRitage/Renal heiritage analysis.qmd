...
```{r echo = F}
corr_plot_modified <- function(data, X, Y, cor_method = "pearson", dict = dict,
  ...
  p1_sub2 <- subset(p1, p.value <= 0.05 & abs(corr) >= 0.9)
  if (nrow(p1_sub) > 0) {
...
corr_plot_modified(data = subset(dat, group == "Type 2 Diabetes" & visit == "baseline" & elevated_albuminuria == "No"),
                   X = c("acr_u"),
                   Y = c("avg_c_k2",
                         "avg_c_k1",
                         "avg_c_f"),
