...
for (cell in names(celltype_groups)) {
  ...
  pathway_rev_summary <-  plot_df %>%
    ...
    group_by(pathway) %>%
    dplyr::summarise(n_genes = sum(!is.na(reversed)),
  ...
  plot_df <- plot_df %>%
    ...
    dplyr::mutate(pathway_lab = paste0(gsub("HALLMARK ", "", gsub("_", " ", pathway)), "\n(", sprintf("%.1f", pct_reversed), "% Reversed)"))
...
for (cell in names(celltype_groups)) {
  ...
  pathway_rev_summary <- plot_df %>%
    ...
    dplyr::mutate(reversed = sign(`T2D GLP- vs. HC`) != sign(`T2D GLP+ vs. GLP-`)) %>%
    group_by(pathway) %>%
    dplyr::summarise(
...
for (cell in names(celltype_groups)) {
  ...
  glpn_hc_df_sig <- glpn_hc_df %>%
    ...
    dplyr::mutate(celltype = cell, comparison = "T2D GLP (-) vs HC")
  glpy_glpn_df_sig <- glpy_glpn_df %>% 
    dplyr::select(logFC = logFC_glp_t2dobGLP_Y, p.val = p_glp_t2dobGLP_Y, fdr, Gene) %>%
    dplyr::mutate(celltype = cell, comparison = "T2D GLP (+) vs T2D GLP(-)")  
...
### Reversal scores
```{r}
# Reversal Score = (N reversed genes) / (N disease genes with GLP+ vs. GLP- data) × 100
# number of reversed genes
t2d_glp_rev_scores <- t2d_glp_rev_combined %>%
  dplyr::summarise(n = n(), 
                   .by = c(celltype, comparison, direction)) %>%
...
for (cell in names(celltype_groups)) {
  ...
  plot_df <- bind_rows(
    ...
    dplyr::select(Gene, logFC = logFC_glp_t2dobGLP_N,
      ...
      dplyr::mutate(comparison = "T2D GLP- vs. HC"),
    glpy_glpn_df %>%
    separate_rows(Gene, sep = ";") %>%
    dplyr::select(Gene, logFC = logFC_glp_t2dobGLP_Y,
                  p.val = p_glp_t2dobGLP_Y, fdr) %>%
      dplyr::mutate(comparison = "T2D GLP+ vs. GLP-")
  ) %>%
  ...
  pathway_rev_summary <- plot_df %>%
    dplyr::select(pathway, Gene, comparison, logFC) %>%
    pivot_wider(names_from = comparison, values_from = logFC) %>%
    dplyr::mutate(reversal_direction = case_when(`T2D GLP+ vs. GLP-` > 0 ~ "Enhanced", T ~ "Suppressed")) %>%
