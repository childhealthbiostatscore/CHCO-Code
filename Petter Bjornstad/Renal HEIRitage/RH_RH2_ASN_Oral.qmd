...
source("~/GitHub/CHCO-Code/Petter Bjornstad/ATTEMPT/resources/attempt_functions.R")
...
# OB/T2D only
summary(arsenal::tableby(study ~ age + sex + bmi + group + uacr_group + diabetes_duration + sbp + dbp + hba1c + gfr_bsa_plasma + erpf_bsa_plasma + htadjtkv_combined + avg_c_r2 + avg_m_r2  + avg_k_r2 + avg_c_fsoc + avg_m_fsoc + avg_c_r2 + avg_c_adc + avg_pcascl, data = subset(rh_rh2_unique, group != "Lean Control"), digits = 1))
```
...
summary(arsenal::tableby(study_visit ~ age + sex + bmi + group + uacr_group + diabetes_duration + sbp + dbp + hba1c + gfr_bsa_plasma + erpf_bsa_plasma + kwt(date_diff_years, "medianq1q3") + htadjtkv_combined + avg_c_r2 + avg_k_r2 + avg_c_adc + avg_pcascl + acr_u_cat3 + kwt(acr_u, "medianq1q3"), data = rh_rh2_unique_follow_up, digits = 1, strata = group))
```
...
df2 <- rh_rh2_unique[rh_rh2_unique$avg_k_r2 > cutoff,]
df2 <- df2 %>%
  filter(!is.na(avg_k_r2) & !is.na(study))
...
df$avg_k_r2 <- pmin(df$avg_k_r2, cutoff)
# Plot
p <- df %>%
  filter(!is.na(avg_k_r2) & !is.na(study)) %>% 
  # mutate(study = as.character(study)) %>%
  ggplot(aes(x=study, y=avg_k_r2, fill = study)) +
  geom_boxplot(outlier.shape = NA, color = "#3a5a40", size = 1, width = 0.8) +
  ggplot2::geom_jitter(size = 2.0, alpha = 0.5, width = 0) +
  ggplot2::labs(x = NULL, y = "Kidney R2*") +
  
  geom_point(aes(xmin = 0.5, xmax = 0.5,
                 ymin = 45, ymax = avg_k_r2+2), 
             data = df2,
             position = position_dodge(),
             color = "black", size = 2.0, alpha = 0.5) +
  # Use facets to resemble an axis interruption
  facet_grid(panel ~ ., scales = "free_y") +
  # force_panelsizes(rows = c(1,5))  +
  ggplot2::scale_fill_manual(values = c("#a3b18a", "#588157")) +
...
# If you want to manually add a specific p-value:
manual_p_value <- "p=0.0012"

# Create annotation data for the bracket and p-value
annotation_df <- data.frame(
  panel = factor("lower", levels = c("upper", "lower")),
  x = 1.5,  # Center between the two groups
  xmin = 1,  # Start of bracket
  xmax = 2,  # End of bracket
  y = cutoff - 1.5,  # Height of bracket
  label = manual_p_value
)

# Add manual p-value with bracket
p_manual <- p + 
  # Add horizontal lines for bracket
  geom_segment(data = annotation_df,
               aes(x = xmin, xend = xmax, y = y, yend = y),
               inherit.aes = FALSE,
               size = 0.5) +
  # Add vertical ticks for bracket
  geom_segment(data = annotation_df,
               aes(x = xmin, xend = xmin, y = y - 0.3, yend = y),
               inherit.aes = FALSE,
               size = 0.5) +
  geom_segment(data = annotation_df,
               aes(x = xmax, xend = xmax, y = y - 0.3, yend = y),
               inherit.aes = FALSE,
               size = 0.5) +
  # Add p-value text
...
# UACR
rh_rh2_unique$log_acr_u <- log(rh_rh2_unique$acr_u)
create_comparison_boxplot(
  data = rh_rh2_unique,
  x_var = "study",
  y_var = "log_acr_u",
