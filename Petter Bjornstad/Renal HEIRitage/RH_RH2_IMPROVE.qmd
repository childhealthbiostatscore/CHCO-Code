...
summary(arsenal::tableby(uacr_group ~ avg_c_r2 + avg_m_r2 + avg_k_r2 +
                           avg_c_adc + avg_c_fsoc + avg_m_fsoc + avg_k_fsoc + avg_pcascl, data=rh_rh2_unique),
        total = F)

summary(arsenal::tableby(uacr_group ~ bmi_cat + age + sex + kwt(acr_u, "medianq1q3") + eGFR_CKD_epi + bmi + egfr_cat + acr_u_cat3, data=rh_rh2_unique),
        total = F)
# 
# table(rh_rh2_unique$group,rh_rh2_unique$uacr_100)
# table(rh_rh2_unique$group,rh_rh2_unique$uacr_group)
...
# rh_rh2_croc_panther_unique$avg_pcascl 
# rh_rh2_croc_panther_unique$avg_k_r2
# rh_rh2_croc_panther_unique$avg_c_adc
# rh_rh2_croc_panther_unique$group

table(subset(rh_rh2_croc_panther_unique, !is.na(avg_k_r2))$bmi_cat)
table(subset(rh_rh2_croc_panther_unique, !is.na(avg_k_r2))$group)

avg_pcascl_fit <- (lm(avg_pcascl ~ bmi_cat + group + eGFR_fas_cr + log(acr_u), data = rh_rh2_croc_panther_unique))
summary(avg_pcascl_fit)
drop1(avg_pcascl_fit, ~ ., test = "F")
emm_bmi <- emmeans(avg_pcascl_fit, ~ bmi_cat)
emm_bmi

avg_k_r2_fit <- (lm(avg_k_r2 ~ bmi_cat + group + eGFR_fas_cr + log(acr_u), data = rh_rh2_croc_panther_unique))
summary(avg_k_r2_fit)
drop1(avg_k_r2_fit, ~ ., test = "F")
emm_bmi <- emmeans(avg_k_r2_fit, ~ bmi_cat)
emm_bmi
pairs(emm_bmi)

avg_c_r2_fit <- (lm(avg_c_r2 ~ bmi_cat + group + eGFR_fas_cr + log(acr_u), data = rh_rh2_croc_panther_unique))
summary(avg_c_r2_fit)
drop1(avg_c_r2_fit, ~ ., test = "F")
emm_bmi <- emmeans(avg_c_r2_fit, ~ bmi_cat)
emm_bmi

avg_c_adc_fit <- (lm(avg_c_adc ~ bmi_cat + group + eGFR_fas_cr + log(acr_u), data = rh_rh2_croc_panther_unique))
summary(avg_c_adc_fit)
drop1(avg_c_adc_fit, ~ ., test = "F")
emm_bmi <- emmeans(avg_c_adc_fit, ~ bmi_cat)
emm_bmi
```