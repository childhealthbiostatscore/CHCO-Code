...
```{r message=FALSE,warning=FALSE,dpi=1200}
volcano_jcem <- function(data, lab = "Target", xcol = "estimate", ycol = "p.value",
                    top = 11,
                    xlimit = c(-1, 1), ylimit = c(0, -log10(10e-7)),
                    xlabel = "HR", pCutoff = 0.05, overlaps = 50,
                    log_t = F) {
  ...
  p <- ggplot(data = data, aes(x = fc, y = logp, color = sig)) +
    ...
    geom_label_repel(
        data = data[data$top, ], aes(label = Target), color = "black",
  max.overlaps = overlaps,
  box.padding = 0.5,       # space between label box and other labels
  point.padding = 0.3,     # space between label and its point
  force = 2,               # how hard labels push away from each other
  force_pull = 0.5,        # how hard labels are pulled toward their point
  min.segment.length = 0,  # always draw the connecting line segment
  seed = 42                # makes layout reproducible
    ) +
    scale_color_manual(values = c("grey", "#3e6dbf")) +
    xlab(xlabel) + scale_x_continuous(labels = scaleFUN) +
    ylab(bquote(~ -Log[10] ~ italic(P))) +
...
v_glyc <- volcano_jcem(top_glyc_df)
v_glyc <- v_glyc +
...
#save figure for JCEM: 
tiff("/Users/kristenmiller/Library/CloudStorage/OneDrive-SharedLibraries-UW/bjornstad_pyle_tommerdahl_lab - Documents/Dissemination/Manuscripts/P028_TODAY_glycemia_km/02_Submitted/01_JCEM_r2/fig1_volcano.tif",
