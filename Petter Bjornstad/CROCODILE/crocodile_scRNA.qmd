...
# Import
so <- s3readRDS("data_clean/seurat_data_no_computations.RDS", "scrna", region = "")
# CROCODILE only
...
```{r pseudobulk data setup}
# Pseudobulk (modified code from Fadhl)
# raw_data = Seurat::GetAssayData(so, assay = "RNA", layer = "counts")
# av_counts <- apply(raw_data, 1, function(raw_data) {
#   by(raw_data, so$tubular_id, sum)
# })
# av_counts <- t(data.frame(av_counts))
# 
# group_list <- so@meta.data %>% dplyr::select(michigan_id, Group, tubular_id) %>% distinct(tubular_id, .keep_all = T)
# rownames(group_list) <- group_list$tubular_id
# so_pseudobulk <- CreateSeuratObject(meta.data = group_list, counts = av_counts)
# 
# so_pseudobulk <- NormalizeData(so_pseudobulk)
# so_pseudobulk <- ScaleData(so_pseudobulk)
# VariableFeatures(so_pseudobulk) <- rownames(so_pseudobulk)

# Pseudobulk (PT only)
so_pt <- subset(so, LR_clusters == "PT")
sample_id <- so_pt$michigan_id
raw_data = Seurat::GetAssayData(so_pt, assay = "RNA", layer = "counts")
av_counts <- apply(raw_data, 1, function(raw_data) {
...
raw_data = Seurat::GetAssayData(so_subset, assay = "RNA", layer = "counts")
av_counts <- apply(raw_data, 1, function(raw_data) {
  by(raw_data, sample_id, sum)
})
av_counts <- t(data.frame(av_counts))
...
```{r scRNA transcript definitions}
...
ggsave(tmp, plot = umap, scale = 3,width = 1200,height = 900,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/UMAP_Group_1200x900.jpeg",
  bucket = "scrna",
  region = ""
)
...
p 
#Save image
# ggsave(filename = "/home/yejichoi/Documents/lin_temp_folder/UMAP_General_Cell_Type_Labeled.jpeg",
#        plot = p,scale = 3,width = 1200,height = 900,units = "px")

tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = p, scale = 3,width = 1200,height = 900,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/UMAP_General_Cell_Type_Labeled.jpeg",
  bucket = "scrna",
  region = ""
)
...
# Save with group
gg.umap <- ggarrange(umap, p)

# ggsave(filename = "/home/yejichoi/Documents/lin_temp_folder/UMAP_General_Cell_Type_Labeled_Combined.jpeg",
#        plot = gg.umap, scale = 3.5,
#        width = 2400,height = 900, units = "px")

tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = gg.umap, scale = 3.5,
        width = 2400,height = 900, units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/UMAP_General_Cell_Type_Labeled_Combined.jpeg",
  bucket = "scrna",
  region = ""
)
...
p
#Save image
# ggsave(filename = "/home/yejichoi/Documents/lin_temp_folder/UMAP_General_Cell_Type_Unlabeled.jpeg",
#        plot = p, scale = 3,width = 1200,height = 900,units = "px")
...
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/UMAP_General_Cell_Type_Unlabeled.jpeg",
  bucket = "scrna",
  region = ""
)
```
...
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/UMAP_Cell_Type_1200x900.jpeg",
  bucket = "scrna",
  region = ""
)
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = cellcount, scale = 3,width = 500,height = 1000,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/PT_cell_proportions.jpeg",
  bucket = "scrna",
  region = ""
)
...
# ggsave(plot = cellcount, filename = "/home/yejichoi2/Documents/lin_temp_folder/PT_cell_proportions_by_grp.jpg", 
#        scale = 3,
#        width = 500,height = 1000,units = "px")
...
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/PT_cell_proportions_by_grp.jpeg",
  bucket = "scrna",
  region = ""
)

# By PT subtypes (by celltype (S subtypes))
cellcount_kpmp <- so@meta.data %>% 
  filter(generaltype == "PT" & KPMP_celltype != "PT_lowQuality") %>%
  dplyr::group_by(Group, KPMP_celltype) %>%
  dplyr::summarise(count = n()) %>%
  dplyr::mutate(n = case_when(Group == "HC" ~ n_HC, Group == "T1D" ~ n_T1D),
         ratio = count / n) %>% ungroup() %>%
  group_by(KPMP_celltype) %>%
  dplyr::mutate(proportion = ratio/sum(ratio)) %>%
  dplyr::select(proportion, Group, KPMP_celltype) %>%
  ggplot(aes(x= KPMP_celltype, y= proportion, fill = Group)) + 
  geom_bar(stat = "identity", position = "fill") +
  theme_classic() +
  labs(x = NULL,
       y = "Proportion",
  ...
  ggtitle("Proportion of cells") +
  scale_fill_manual(values = c("#264653", "#2a9d8f"))
# ggsave(plot = cellcount_kpmp, filename = "/home/yejichoi2/Documents/lin_temp_folder/PT_cell_proportions_by_kpmp.jpg", 
#        scale = 3,
#        width = 500,height = 1000,units = "px")

tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = cellcount_kpmp, scale = 3,width = 500,height = 1000,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/PT_cell_proportions_by_kpmp.jpeg",
  bucket = "scrna",
  region = ""
)
...
# ggsave(plot = cellcount_svg, filename = "/home/yejichoi/Documents/lin_temp_folder/celltype_distribution.jpg", 
#        scale = 3,
#        width = 1000,height = 500,units = "px")

tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = cellcount_svg, scale = 3,width = 500,height = 1000,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/celltype_distribution.jpeg",
  bucket = "scrna",
  region = ""
)
```
...
In the table below, T1D are group 1 and HC are group 2. Positive values indicate that the gene is more highly expressed in the first group (T1D), negative values indicate that the gene is less expressed in the first group (T1D.
...
split.vp <- function(seurat_object, genes, filepath, color1 = "#6c9a8b", color2 = "#e8998d") {
  for (i in 1:length(genes)){
    ...
    tmp <- tempfile(fileext = ".jpeg")
    ggsave(tmp, plot = p, scale = 5,
           width = 800,height = 600,units = "px")
    
    # Upload directly
    put_object(
      file = tmp,
      object = paste0("Projects/CROCODILE scRNA/results/figures/", filepath, "Violin_",genes[i],".jpeg"),
      bucket = "scrna",
      region = ""
    )
  }
...
split.vp.combined <- function(seurat_object, genes, filepath, color1 = "#6c9a8b", color2 = "#e8998d") {
  ...
  put_object(
    file = tmp,
    object = paste0("Projects/CROCODILE scRNA/results/figures/", filepath, "Violin_combined.jpeg"),
    bucket = "scrna",
    region = ""
  )
...
```{r}
split.vp(seurat_object = so, genes = genes_subset, filepath = "")
split.vp.combined(seurat_object = so, genes = genes_subset, filepath = "tca_")
```
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = rp,scale = 3,
       width = 1920,height = 1080,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Ridge_Group_1920x1080.jpeg",
  bucket = "scrna",
  region = ""
)
```
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = dp,scale = 3,
       width = 1600,height = 900,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_All_1600x900.jpeg",
  bucket = "scrna",
  region = ""
)
```
...
dp.formatted <- function(seurat_object, genes, celltype, group.by, m,
                         ...
                         limit_low = 10, limit_high = 100,
                         show_qvalue = TRUE, pct_as_percent = FALSE
                         ){
  ...
  pt.plot <- pt.combined %>% 
    ...
    scale_color_gradient2(low = colorlow, mid = colormid, high = colorhigh, midpoint = 2,
                          ...
                          limits = c(0, 4)) +
    scale_size(range = c(0, 15), limits = c(limit_low, limit_high)) +
  ...
  # Optionally multiply pct.1 and pct.2 by 100
  if (pct_as_percent) {
    pt.table <- pt.table %>%
      dplyr::mutate(pct.1 = pct.1 * 100,
                    pct.2 = pct.2 * 100)
  }
  
  # Select columns and set headers based on show_qvalue
  if (show_qvalue) {
    pt.table <- pt.table %>%
      dplyr::select(pct.1, pct.2, avg_log2FC, p_val, p_val_adj)
    col_names <- c("T1D", "HC", "Log2FC", "p-value", "q-value")
  } else {
    pt.table <- pt.table %>%
      dplyr::select(pct.1, pct.2, avg_log2FC, p_val)
    col_names <- c("T1D", "HC", "Log2FC", "p-value")
  }
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = pt.plot_table, scale = 3,
       width = 1000,height = 500,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_cells_1920x1080.jpeg",
  bucket = "scrna",
  region = ""
)
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = ptsub.plot_table, scale = 3.5,
       width = 1200,height = 700,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_subcells_tca.jpeg",
  bucket = "scrna",
  region = ""
)
...
## Violin plots
```{r}
split.vp(seurat_object = so, genes = genes_subset_acetylcoa, filepath = "")
split.vp.combined(seurat_object = so, genes = genes_subset_acetylcoa, filepath = "acetylcoa_")
```
## Dot Plot
```{r}
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = pt.plot_table_acetylcoa, scale = 3,
       width = 1000,height = 500,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_cells_acetylcoa.jpeg",
  bucket = "scrna",
  region = ""
)
...
gg.pt.table <- ggarrange(table_list[[1]], table_list[[2]], table_list[[3]], table_list[[4]], ncol = 1)
ptsub.plot_table <- ggarrange(pt.plot.subset, NULL, gg.pt.table,
          nrow = 1, widths = c(1.5,-0.1,1), common.legend = T)
...
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_subcells_acetylcoa.jpeg",
  bucket = "scrna",
  region = ""
)
...
## Violin plots
```{r}
split.vp(seurat_object = so, genes = genes_subset_ir, filepath = "")
split.vp.combined(seurat_object = so, genes = genes_subset_ir, filepath = "ir_")
```
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = ptsub.plot_table, scale = 3,
       width = 1000,height = 500,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_cells_IR.jpeg",
  bucket = "scrna",
  region = ""
)
...
## Violin plots
```{r}
split.vp(seurat_object = so, genes = genes_subset_ckd, filepath = "")
split.vp.combined(seurat_object = so, genes = genes_subset_ckd, filepath = "ckd_")
```
## Dot Plot
```{r}
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = pt.plot_table_ckd, scale = 3,
       width = 1000,height = 500,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_cells_ckd.jpeg",
  bucket = "scrna",
  region = ""
)
...
## Violin plots
```{r}
split.vp(seurat_object = so, genes = genes_subset_oxy_phos, filepath = "")
split.vp.combined(seurat_object = so, genes = genes_subset_oxy_phos, filepath = "oxy_phos_")
```
## Dot Plot
```{r}
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = pt.plot_table_oxy_phos, scale = 3,
       width = 1000,height = 500,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_cells_oxy_phos.jpeg",
  bucket = "scrna",
  region = ""
)
...
gg.pt.table <- ggarrange(table_list[[1]], table_list[[2]], table_list[[3]], table_list[[4]], table_list[[5]], ncol = 1)
ptsub.plot_table <- ggarrange(pt.plot.subset, NULL, gg.pt.table,
          nrow = 1, widths = c(1.5,-0.1,1), common.legend = T)
...
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_subcells_oxphos.jpeg",
  bucket = "scrna",
  region = ""
)
...
## Violin plots
```{r}
split.vp(seurat_object = so, genes = genes_subset_sglt2, filepath = "")
split.vp.combined(seurat_object = so, genes = genes_subset_sglt2, filepath = "sglt2_")
```
## Dot Plot
```{r}
...
#        width = 1000,height = 500,units = "px")
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = pt.plot_table_sglt2, scale = 3,
       width = 1000,height = 500,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_cells_sglt2.jpeg",
  bucket = "scrna",
  region = ""
)
...
## Violin plots
```{r}
split.vp(seurat_object = so, genes = genes_subset_leptin_adipo, filepath = "")
split.vp.combined(seurat_object = so, genes = genes_subset_leptin_adipo, filepath = "leptin_adipo_")
```
## Dot Plot
```{r}
...
tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = pt.plot_table_leptin_adipo, scale = 3,
       width = 1000,height = 500,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_cells_leptin_adipo.jpeg",
  bucket = "scrna",
  region = ""
)
...
s3load(object = "data_clean/croc_data.RData", 
                    bucket = "harmonized.dataset", region = "")
dict <- s3read_using_region(FUN = read.csv, 
                    object = "data_clean/data_dictionary_master.csv", 
                    bucket = "harmonized.dataset", region = "")
...
pt_md <- FetchData(subset(so, LR_clusters == "PT"), vars = var_list)
ec_md <- FetchData(subset(so, LR_clusters == "EC"), vars = var_list)
tubular_pb_md <- FetchData(so_pseudobulk, vars = var_list)
# %>% filter(ident=="tubular")
subset_pb_md <- FetchData(so_subset, vars = var_list) 

pt_md <- pt_md %>%
  group_by(michigan_id) %>%
  dplyr::summarise(across(where(is.numeric), ~ mean(.x > 0, na.rm = TRUE)))

subset_pt_md <- subset_pb_md %>%
  group_by(michigan_id) %>%
  dplyr::summarise(across(where(is.numeric), ~ mean(.x > 0, na.rm = TRUE)))
...
tubular_pb_md_tca <- tubular_pb_md %>%
  dplyr::select(michigan_id, genes_subset) %>% 
  dplyr::rename(record_id = michigan_id) %>%
  right_join(dat)

subset_pb_md_tca <- subset_pb_md %>%
  dplyr::select(michigan_id, genes_subset) %>% 
  dplyr::rename(record_id = michigan_id) %>%
  right_join(dat)
...
tubular_pb_md_ir <- tubular_pb_md %>%
  dplyr::select(michigan_id, genes_subset_ir) %>% 
  dplyr::rename(record_id = michigan_id) %>%
  right_join(dat)

subset_pb_md_ir <- subset_pb_md %>%
  dplyr::select(michigan_id, genes_subset_ir) %>% 
  dplyr::rename(record_id = michigan_id) %>%
  right_join(dat)

pt_md_leptin_adipo <- pt_md %>%
  dplyr::select(michigan_id, leptin_adipo_genes) %>% 
  dplyr::rename(record_id = michigan_id) %>%
  right_join(dat)

tubular_pb_md_leptin_adipo <- tubular_pb_md %>%
  dplyr::select(michigan_id, leptin_adipo_genes) %>% 
  dplyr::rename(record_id = michigan_id) %>%
  right_join(dat)

subset_pb_md_leptin_adipo <- subset_pb_md %>%
  dplyr::select(michigan_id, leptin_adipo_genes) %>% 
  dplyr::rename(record_id = michigan_id) %>%
  right_join(dat)
```
...
```{r}
dict <- label_harmonized_dict(dat, dict)[1,]
# Modify some labels
dict <- dict %>%
...
```{r message=F, warning=F}
citrate_plot <- subset(pt_md_tca, 
                       apply(!is.na(pt_md_tca[, c(spatial_metab_tca[1], blood_metab_tca[1])]), 1, all)) %>%
...
succinate_plot <- subset(pt_md_tca, 
                         apply(!is.na(pt_md_tca[, c(spatial_metab_tca[3], blood_metab_tca[2])]), 1, all)) %>%
...
fumarate_plot <- subset(pt_md_tca, 
                        apply(!is.na(pt_md_tca[, c(spatial_metab_tca[4], blood_metab_tca[3])]), 1, all)) %>%
...
malate_plot <- subset(pt_md_tca, 
                      apply(!is.na(pt_md_tca[, c(spatial_metab_tca[5], blood_metab_tca[4])]), 1, all)) %>%
...
```{r message=F, warning=F}
metabo_sc(data = label_harmonized(pt_md_tca, dict), gene = "gbm_thick_artmean", 
...
```{r message = F, warning = F}
metabo_sc(data = label_harmonized(pt_md_tca, dict), gene = "avg_c_k2", 
          transcripts = spatial_metab_tca,
          gene_name = "Cortex k2")
```
...
```{r message = F, warning = F}
metabo_sc(data = label_harmonized(pt_md_tca, dict), gene = "avg_c_k2_wo_cyst_vw", 
          transcripts = spatial_metab_tca,
          gene_name = "Voxel-wise Cortex k2")
```
### T1D only
#### Associate GBM width with spatial metabolomics
```{r message=F, warning=F}
metabo_sc(data = subset(label_harmonized(pt_md_tca, dict), group == "Type 1 Diabetes"), 
...
for (i in 1:length(leptin_adipo_genes)) {
  dict[1, leptin_adipo_genes[i]] <- leptin_adipo_genes[i]
}
...
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_scRNA.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200, 
    file = temp_file, res = 170)
corr_plot_modified(data = pt_md_tca,
                   X = genes_subset, 
                   Y = spatial_metab_tca,
                   adj_var = NULL,
                   dict = dict)
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_scRNA.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = pt_md_tca,
                   ...
                   dict = dict)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_blood_scRNA.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200, 
    file = temp_file, res = 170)
corr_plot_modified(pt_md_tca,
                   X = genes_subset, 
                   Y = blood_metab_tca,
                   adj_var = NULL,
                   dict = dict)
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_blood_scRNA.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = pt_md_tca,
                   ...
                   n_cols = length(blood_metab_tca),
                   dict = dict)
# png(height = 1200, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_blood_tissue.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200, 
    file = temp_file, res = 170)
corr_plot_modified(pt_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_blood_tissue.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = pt_md_tca,
                   ...
                   n_cols = 2,
                   dict = dict)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_scRNA_clin_t1dlc.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200, 
    file = temp_file, res = 170)
corr_plot_modified(pt_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_scRNA_clin_t1dlc.png",
  bucket = "scrna",
  region = ""
)
```
...
png(height = 2000, width = 1200, 
    file = temp_file, res = 170)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_scRNA_clin_t1d.png", res = 170)
corr_plot_modified(data = subset(pt_md_tca, group == "Type 1 Diabetes"),
                   X = genes_subset, 
                   Y = c("gbm_thick_artmean",
                         "avg_c_k2"),
                   adj_var = NULL,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_scRNA_clin_t1d.png",
  bucket = "scrna",
  region = ""
)
```
...
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1dlc.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200, 
    file = temp_file, res = 170)
corr_plot_modified(data = pt_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1dlc.png",
  bucket = "scrna",
  region = ""
)
# Reduced version for EDNSG abstract
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1dlc_reduced.png", res = 170)
# temp_file <- tempfile(fileext = ".png")
png(height = 1200, width = 1500, 
    file = temp_file, res = 170)
corr_plot_modified(data = pt_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1dlc_reduced.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = subset(pt_md_tca, group == "Type 1 Diabetes"),
                   ...
                   n_cols = 2,
                   dict = dict)
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1d.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 1200, width = 1500,
    file = temp_file, res = 170)
corr_plot_modified(data = subset(pt_md_tca, group == "Type 1 Diabetes"),
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1d.png",
  bucket = "scrna",
  region = ""
)
# Reduced version for EDNSG abstract
...
### T1D and LC
```{r}
correlation_table_minimal(data = pt_md_leptin_adipo,
                   relevant_vars = c("bmi",
                                     "p2_raw_m",
                                     leptin_adipo_genes),
                   n_cols = 2,
                   dict = dict)
# corr_plot_modified(data = pt_md_leptin_adipo,
#                    X = leptin_adipo_genes, 
#                    Y = c("bmi",
#                          "p2_raw_m"),
#                    adj_var = NULL,
#                    dict = dict)
...
### T1D only
```{r}
correlation_table_minimal(data = subset(pt_md_leptin_adipo, group == "Type 1 Diabetes"),
                   relevant_vars = c("bmi",
                                     "p2_raw_m",
                                     leptin_adipo_genes),
                   n_cols = 2,
                   dict = dict)
corr_plot_modified(data = subset(pt_md_leptin_adipo, group == "Type 1 Diabetes"),
                   X = leptin_adipo_genes, 
                   Y = c("bmi",
                         "p2_raw_m"),
                   adj_var = NULL,
...
# Pseudobulk tubular cells
```{r warning = F, message = F}
# Plots
metabo_sc(data = tubular_pb_md_tca, gene = "isocitric_citric_acid_tissue", 
          transcripts = c("ACO1", "ACO2", "IDH1"),
          gene_name = "Isocitric/citric acid")
metabo_sc(data = tubular_pb_md_tca, gene = "aconitic_acid_tissue", 
          transcripts = c("ACO1", "ACO2"),
          gene_name = "Aconitate")
metabo_sc(data = tubular_pb_md_tca, gene = "succinic_acid_tissue", 
          transcripts = c("OGDH", "OGDHL", "SUCLG1", "SUCLG2", "SDHB", "SDHC"),
          gene_name = "Succinate")
metabo_sc(data = tubular_pb_md_tca, gene = "fumaric_acid_tissue", 
          transcripts = c("SDHB", "SDHC"),
          gene_name = "Fumarate")
metabo_sc(data = tubular_pb_md_tca, gene = "malic_acid_tissue", 
          transcripts = c("ACO1", "ACO2", "IDH1"),
          gene_name = "Malate")
```
### Associate blood metabolites with sc PT transcripts
```{r message=F, warning=F}
metabo_sc(data = tubular_pb_md_tca, gene = "citrate", 
          transcripts = c("ACO1", "ACO2", "IDH1"),
          gene_name = "Citrate")

metabo_sc(data = tubular_pb_md_tca, gene = "succinate", 
          transcripts = c("OGDH", "OGDHL", "SUCLG1", "SUCLG2", "SDHB", "SDHC"),
          gene_name = "Succinate")

metabo_sc(data = tubular_pb_md_tca, gene = "fumarate", 
          transcripts = c("SDHB", "SDHC"),
          gene_name = "Fumarate")

metabo_sc(data = tubular_pb_md_tca, gene = "malate", 
          transcripts = c("ACO1", "ACO2", "IDH1"),
          gene_name = "Malate")
```
### Associate blood metabolites with spatial metabolomics
```{r message=F, warning=F}
citrate_plot <- subset(tubular_pb_md_tca, 
                       apply(!is.na(tubular_pb_md_tca[, c(spatial_metab_tca[1], blood_metab_tca[1])]), 1, all)) %>%
      ggplot(aes_string(y = spatial_metab_tca[1], x = blood_metab_tca[1], color = "group")) +
      geom_smooth(method = lm, se = F, aes(color=NULL), color = "black",
                  linetype = "dashed") +
      geom_point() +
...
succinate_plot <- subset(tubular_pb_md_tca, 
                         apply(!is.na(tubular_pb_md_tca[, c(spatial_metab_tca[3], blood_metab_tca[2])]), 1, all)) %>%
      ggplot(aes_string(y = spatial_metab_tca[3], x = blood_metab_tca[2], color = "group")) +
      geom_smooth(method = lm, se = F, aes(color=NULL), color = "black",
                  linetype = "dashed") +
      geom_point() +
...
fumarate_plot <- subset(tubular_pb_md_tca, 
                        apply(!is.na(tubular_pb_md_tca[, c(spatial_metab_tca[4], blood_metab_tca[3])]), 1, all)) %>%
      ggplot(aes_string(y = spatial_metab_tca[4], x = blood_metab_tca[3], color = "group")) +
      geom_smooth(method = lm, se = F, aes(color=NULL), color = "black",
                  linetype = "dashed") +
      geom_point() +
...
malate_plot <- subset(tubular_pb_md_tca, 
                      apply(!is.na(tubular_pb_md_tca[, c(spatial_metab_tca[5], blood_metab_tca[4])]), 1, all)) %>%
      ggplot(aes_string(y = spatial_metab_tca[5], x = blood_metab_tca[4], color = "group")) +
      geom_smooth(method = lm, se = F, aes(color=NULL), color = "black",
                  linetype = "dashed") +
      geom_point() +
...
```{r message=F, warning=F}
## Log GBM width, and group the two groups
metabo_sc(data = tubular_pb_md_tca, gene = "gbm_thick_artmean", 
          transcripts = genes_subset,
          gene_name = "GBM thickness (nm)")
```
...
#### Associate global k2 with sc PT transcripts
```{r message=F, warning=F}
metabo_sc(data = tubular_pb_md_tca, gene = "avg_c_k2", 
          transcripts = genes_subset,
          gene_name = "Cortex k2")
```
...
#### Associate voxelwise k2 with sc PT transcripts
```{r message=F, warning=F}
metabo_sc(data = tubular_pb_md_tca, gene = "avg_c_k2_wo_cyst_vw", 
          transcripts = genes_subset,
          gene_name = "Voxel-wise Cortex k2")
```
...
#### Associate insulin sensitivity with sc PT transcripts
```{r message=F, warning=F}
metabo_sc(data = tubular_pb_md_tca, gene = "p2_raw_m", 
          transcripts = genes_subset,
          gene_name = "M-value")

metabo_sc(data = tubular_pb_md_ir, gene = "p2_raw_m", 
          transcripts = genes_subset_ir,
          gene_name = "M-value")
```
...
#### Associate GBM width with spatial metabolomics
```{r message=F, warning=F}
metabo_sc(data = label_harmonized(tubular_pb_md_tca, dict), gene = "gbm_thick_artmean", 
          transcripts = spatial_metab_tca,
          gene_name = "GBM Thickness (nm)")
```
...
#### Associate global k2 with spatial metabolomics
```{r message = F, warning = F}
metabo_sc(data = label_harmonized(tubular_pb_md_tca, dict), gene = "avg_c_k2", 
          transcripts = spatial_metab_tca,
          gene_name = "Cortex k2")
```
...
#### Associate voxelwise k2 with spatial metabolomics
```{r message = F, warning = F}
metabo_sc(data = label_harmonized(tubular_pb_md_tca, dict), gene = "avg_c_k2_wo_cyst_vw", 
          transcripts = spatial_metab_tca,
          gene_name = "Voxel-wise Cortex k2")
```
### T1D only
#### Associate GBM width with spatial metabolomics
```{r message=F, warning=F}
metabo_sc(data = subset(label_harmonized(tubular_pb_md_tca, dict), group == "Type 1 Diabetes"), 
          gene = "gbm_thick_artmean", 
          transcripts = spatial_metab_tca,
          gene_name = "GBM Thickness (nm)")
```
...
## Associations: Blood metabolites
```{r message=F, warning=F, results='asis'}
summary(arsenal::tableby(group ~ citrate + succinate + kwt(fumarate, "Nmiss", "median", "q1q3", "range") + malate, data = tubular_pb_md_tca, total = F))
```
...
correlation_table_minimal(data = tubular_pb_md_tca,
                   relevant_vars = c(spatial_metab_tca, genes_subset),
                   n_cols = length(spatial_metab_tca),
                   dict = dict)
...
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
corr_plot_modified(tubular_pb_md_tca,
                   X = genes_subset, 
                   Y = spatial_metab_tca,
                   adj_var = NULL,
                   dict = dict)
...
correlation_table_minimal(data = tubular_pb_md_tca,
                   ...
                   n_cols = length(blood_metab_tca),
                   dict = dict)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_blood_scRNA_pseudobulk.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
corr_plot_modified(tubular_pb_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_blood_scRNA_pseudobulk.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = tubular_pb_md_tca,
                   ...
                   n_cols = length(blood_metab_tca),
                   dict = dict)
# png(height = 1200, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_blood_tissue_pseudobulk.png", res = 170)
temp_file <- tempfile(fileext = ".png")
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_blood_tissue_pseudobulk.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = tubular_pb_md_tca,
                   ...
                   n_cols = 2,
                   dict = dict)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_scRNA_clin_t1dlc_pseudobulk.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_scRNA_clin_t1dlc_pseudobulk.png",
  bucket = "scrna",
  region = ""
)
```
### T1D only
```{r}
correlation_table_minimal(data = subset(tubular_pb_md_tca, group == "Type 1 Diabetes"),
                   relevant_vars = c("gbm_thick_artmean",
                                     "avg_c_k2",
                                     genes_subset),
                   n_cols = 2,
                   dict = dict)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_scRNA_clin_t1d_pseudobulk.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
corr_plot_modified(data = subset(tubular_pb_md_tca, group == "Type 1 Diabetes"),
                   X = genes_subset, 
                   Y = c("gbm_thick_artmean",
                         "avg_c_k2"),
                   adj_var = NULL,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_scRNA_clin_t1d_pseudobulk.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = tubular_pb_md_tca,
                   ...
                   n_cols = 2,
                   dict = dict)
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1dlc_pseudobulk.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
corr_plot_modified(data = tubular_pb_md_tca,
                   X = spatial_metab_tca, 
                   Y = c("gbm_thick_artmean",
                         "avg_c_k2"),
                   adj_var = NULL,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1dlc_pseudobulk.png",
  bucket = "scrna",
  region = ""
)
# Reduced version for EDNSG abstract
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1dlc_reduced_pseudobulk.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 1200, width = 1500,
    file = temp_file, res = 170)
corr_plot_modified(data = tubular_pb_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1dlc_reduced_pseudobulk.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = subset(tubular_pb_md_tca, group == "Type 1 Diabetes"),
                   ...
                   n_cols = 2,
                   dict = dict)
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1d_pseudobulk.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 1200, width = 1500,
    file = temp_file, res = 170)
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1d_pseudobulk.png",
  bucket = "scrna",
  region = ""
)
# Reduced version for EDNSG abstract
...
### T1D and LC
```{r}
correlation_table_minimal(data = tubular_pb_md_leptin_adipo,
                   relevant_vars = c("bmi",
                                     "p2_raw_m",
                                     leptin_adipo_genes),
                   n_cols = 2,
                   dict = dict)
# corr_plot_modified(data = tubular_pb_md_leptin_adipo,
#                    X = leptin_adipo_genes, 
#                    Y = c("bmi",
#                          "p2_raw_m"),
#                    adj_var = NULL,
...
### T1D only
```{r}
correlation_table_minimal(data = subset(tubular_pb_md_leptin_adipo, group == "Type 1 Diabetes"),
                   relevant_vars = c("bmi",
                                     "p2_raw_m",
                                     leptin_adipo_genes),
                   n_cols = 2,
                   dict = dict)
corr_plot_modified(data = subset(tubular_pb_md_leptin_adipo, group == "Type 1 Diabetes"),
                   X = leptin_adipo_genes, 
                   Y = c("bmi",
                         "p2_raw_m"),
                   adj_var = NULL,
...
# Pseuobulk tubular cells (Method 2)
# Associations
## Associations: sc PT, spatial metabolomics, blood metabolites
### Associate spatial metabolomics with sc PT transcripts
```{r warning = F, message = F}
# Plots
metabo_sc(data = subset_pb_md_tca, gene = "isocitric_citric_acid_tissue", 
          transcripts = c("ACO1", "ACO2", "IDH1"),
          gene_name = "Isocitric/citric acid")
metabo_sc(data = subset_pb_md_tca, gene = "aconitic_acid_tissue", 
          transcripts = c("ACO1", "ACO2"),
          gene_name = "Aconitate")
metabo_sc(data = subset_pb_md_tca, gene = "succinic_acid_tissue", 
          transcripts = c("OGDH", "OGDHL", "SUCLG1", "SUCLG2", "SDHB", "SDHC"),
          gene_name = "Succinate")
metabo_sc(data = subset_pb_md_tca, gene = "fumaric_acid_tissue", 
          transcripts = c("SDHB", "SDHC"),
          gene_name = "Fumarate")
metabo_sc(data = subset_pb_md_tca, gene = "malic_acid_tissue", 
          transcripts = c("ACO1", "ACO2", "IDH1"),
          gene_name = "Malate")
```
### Associate blood metabolites with sc PT transcripts
```{r message=F, warning=F}
metabo_sc(data = subset_pb_md_tca, gene = "citrate", 
          transcripts = c("ACO1", "ACO2", "IDH1"),
          gene_name = "Citrate")

metabo_sc(data = subset_pb_md_tca, gene = "succinate", 
          transcripts = c("OGDH", "OGDHL", "SUCLG1", "SUCLG2", "SDHB", "SDHC"),
          gene_name = "Succinate")

metabo_sc(data = subset_pb_md_tca, gene = "fumarate", 
          transcripts = c("SDHB", "SDHC"),
          gene_name = "Fumarate")

metabo_sc(data = subset_pb_md_tca, gene = "malate", 
          transcripts = c("ACO1", "ACO2", "IDH1"),
          gene_name = "Malate")
```
### Associate blood metabolites with spatial metabolomics
```{r message=F, warning=F}
citrate_plot <- subset(subset_pb_md_tca, 
                       apply(!is.na(subset_pb_md_tca[, c(spatial_metab_tca[1], blood_metab_tca[1])]), 1, all)) %>%
      ggplot(aes_string(y = spatial_metab_tca[1], x = blood_metab_tca[1], color = "group")) +
      geom_smooth(method = lm, se = F, aes(color=NULL), color = "black",
                  linetype = "dashed") +
      geom_point() +
...
succinate_plot <- subset(subset_pb_md_tca, 
                         apply(!is.na(subset_pb_md_tca[, c(spatial_metab_tca[3], blood_metab_tca[2])]), 1, all)) %>%
      ggplot(aes_string(y = spatial_metab_tca[3], x = blood_metab_tca[2], color = "group")) +
      geom_smooth(method = lm, se = F, aes(color=NULL), color = "black",
                  linetype = "dashed") +
      geom_point() +
...
fumarate_plot <- subset(subset_pb_md_tca, 
                        apply(!is.na(subset_pb_md_tca[, c(spatial_metab_tca[4], blood_metab_tca[3])]), 1, all)) %>%
      ggplot(aes_string(y = spatial_metab_tca[4], x = blood_metab_tca[3], color = "group")) +
      geom_smooth(method = lm, se = F, aes(color=NULL), color = "black",
                  linetype = "dashed") +
      geom_point() +
...
malate_plot <- subset(subset_pb_md_tca, 
                      apply(!is.na(subset_pb_md_tca[, c(spatial_metab_tca[5], blood_metab_tca[4])]), 1, all)) %>%
      ggplot(aes_string(y = spatial_metab_tca[5], x = blood_metab_tca[4], color = "group")) +
      geom_smooth(method = lm, se = F, aes(color=NULL), color = "black",
                  linetype = "dashed") +
      geom_point() +
...
```{r message=F, warning=F}
## Log GBM width, and group the two groups
metabo_sc(data = subset_pb_md_tca, gene = "gbm_thick_artmean", 
          transcripts = genes_subset,
          gene_name = "GBM thickness (nm)")
```
...
#### Associate global k2 with sc PT transcripts
```{r message=F, warning=F}
metabo_sc(data = subset_pb_md_tca, gene = "avg_c_k2", 
          transcripts = genes_subset,
          gene_name = "Cortex k2")
```
...
#### Associate voxelwise k2 with sc PT transcripts
```{r message=F, warning=F}
metabo_sc(data = subset_pb_md_tca, gene = "avg_c_k2_wo_cyst_vw", 
          transcripts = genes_subset,
          gene_name = "Voxel-wise Cortex k2")
```
...
#### Associate insulin sensitivity with sc PT transcripts
```{r message=F, warning=F}
metabo_sc(data = subset_pb_md_tca, gene = "p2_raw_m", 
          transcripts = genes_subset,
          gene_name = "M-value")

metabo_sc(data = subset_pb_md_ir, gene = "p2_raw_m", 
          transcripts = genes_subset_ir,
          gene_name = "M-value")
```
...
#### Associate GBM width with spatial metabolomics
```{r message=F, warning=F}
metabo_sc(data = label_harmonized(subset_pb_md_tca, dict), gene = "gbm_thick_artmean", 
          transcripts = spatial_metab_tca,
          gene_name = "GBM Thickness (nm)")
```
...
#### Associate global k2 with spatial metabolomics
```{r message = F, warning = F}
metabo_sc(data = label_harmonized(subset_pb_md_tca, dict), gene = "avg_c_k2", 
          transcripts = spatial_metab_tca,
          gene_name = "Cortex k2")
```
...
#### Associate voxelwise k2 with spatial metabolomics
```{r message = F, warning = F}
metabo_sc(data = label_harmonized(subset_pb_md_tca, dict), gene = "avg_c_k2_wo_cyst_vw", 
          transcripts = spatial_metab_tca,
          gene_name = "Voxel-wise Cortex k2")
```
### T1D only
#### Associate GBM width with spatial metabolomics
```{r message=F, warning=F}
metabo_sc(data = subset(label_harmonized(subset_pb_md_tca, dict), group == "Type 1 Diabetes"), 
          gene = "gbm_thick_artmean", 
          transcripts = spatial_metab_tca,
          gene_name = "GBM Thickness (nm)")
```
...
## Associations: Blood metabolites
```{r message=F, warning=F, results='asis'}
summary(arsenal::tableby(group ~ citrate + succinate + kwt(fumarate, "Nmiss", "median", "q1q3", "range") + malate, data = subset_pb_md_tca, total = F))
```
...
correlation_table_minimal(data = subset_pb_md_tca,
                   relevant_vars = c(spatial_metab_tca, genes_subset),
                   n_cols = length(spatial_metab_tca),
                   dict = dict)

# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_scRNAsubset_pb.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
corr_plot_modified(subset_pb_md_tca,
                   X = genes_subset, 
                   Y = spatial_metab_tca,
                   adj_var = NULL,
                   dict = dict)
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_scRNAsubset_pb.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = subset_pb_md_tca,
                   ...
                   n_cols = length(blood_metab_tca),
                   dict = dict)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_blood_scRNAsubset_pb.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
corr_plot_modified(subset_pb_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_blood_scRNAsubset_pb.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = subset_pb_md_tca,
                   ...
                   n_cols = length(blood_metab_tca),
                   dict = dict)
# png(height = 1200, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_blood_tissuesubset_pb.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 1200, width = 1200,
    file = temp_file, res = 170)
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_blood_tissuesubset_pb.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = subset_pb_md_tca,
                   ...
                   n_cols = 2,
                   dict = dict)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_scRNA_clin_t1dlcsubset_pb.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
corr_plot_modified(subset_pb_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_scRNA_clin_t1dlcsubset_pb.png",
  bucket = "scrna",
  region = ""
)
```
### T1D only
```{r}
correlation_table_minimal(data = subset(subset_pb_md_tca, group == "Type 1 Diabetes"),
                   relevant_vars = c("gbm_thick_artmean",
                                     "avg_c_k2",
                                     genes_subset),
                   n_cols = 2,
                   dict = dict)
# png(height = 2000, width = 1200, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_scRNA_clin_t1dsubset_pb.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 2000, width = 1200,
    file = temp_file, res = 170)
corr_plot_modified(data = subset(subset_pb_md_tca, group == "Type 1 Diabetes"),
                   X = genes_subset, 
                   Y = c("gbm_thick_artmean",
                         "avg_c_k2"),
                   adj_var = NULL,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_scRNA_clin_t1dsubset_pb.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = subset_pb_md_tca,
                   ...
                   n_cols = 2,
                   dict = dict)
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1dlcsubset_pb.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 1200, width = 1500,
    file = temp_file, res = 170)
corr_plot_modified(data = subset_pb_md_tca,
                   X = spatial_metab_tca, 
                   Y = c("gbm_thick_artmean",
                         "avg_c_k2"),
                   adj_var = NULL,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1dlcsubset_pb.png",
  bucket = "scrna",
  region = ""
)
# Reduced version for EDNSG abstract
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1dlc_reducedsubset_pb.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 1200, width = 1500,
    file = temp_file, res = 170)
corr_plot_modified(data = subset_pb_md_tca,
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1dlc_reducedsubset_pb.png",
  bucket = "scrna",
  region = ""
)
```
...
correlation_table_minimal(data = subset(subset_pb_md_tca, group == "Type 1 Diabetes"),
                   ...
                   n_cols = 2,
                   dict = dict)
# png(height = 1200, width = 1500, file = "/home/yejichoi/Documents/lin_temp_folder/Correlation_tissue_clin_t1dsubset_pb.png", res = 170)
temp_file <- tempfile(fileext = ".png")
png(height = 1200, width = 1500,
    file = temp_file, res = 170)
corr_plot_modified(data = subset(subset_pb_md_tca, group == "Type 1 Diabetes"),
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/Correlation_tissue_clin_t1dsubset_pb.png",
  bucket = "scrna",
  region = ""
)
# Reduced version for EDNSG abstract
...
### T1D and LC
```{r}
correlation_table_minimal(data = subset_pb_md_leptin_adipo,
                   relevant_vars = c("bmi",
                                     "p2_raw_m",
                                     leptin_adipo_genes),
                   n_cols = 2,
                   dict = dict)
# corr_plot_modified(data = subset_pb_md_leptin_adipo,
#                    X = leptin_adipo_genes, 
#                    Y = c("bmi",
#                          "p2_raw_m"),
#                    adj_var = NULL,
...
### T1D only
```{r}
correlation_table_minimal(data = subset(subset_pb_md_leptin_adipo, group == "Type 1 Diabetes"),
                   relevant_vars = c("bmi",
                                     "p2_raw_m",
                                     leptin_adipo_genes),
                   n_cols = 2,
                   dict = dict)
corr_plot_modified(data = subset(subset_pb_md_leptin_adipo, group == "Type 1 Diabetes"),
                   X = leptin_adipo_genes, 
                   Y = c("bmi",
                         "p2_raw_m"),
                   adj_var = NULL,
...
# )
# 
# mapped_df <- subset_pb_md %>% data.table::transpose(make.names = "michigan_id")
# 
# gene_means <- rowMeans(mapped_df)
# mapped_df <- mapped_df %>%
#   dplyr::mutate(gene_means) %>%
...
# MTAP related figures for Petter (Kumar's) EDNSG presentation
```{r}
de.markers(so, "MTAP", "Group", id2 = "HC", id1 = "T1D", "PT", "")

mtap_dp <- dp.formatted(subset(so, LR_clusters == "PT"), 
                        "MTAP", "PT", "Group", m,
                        limit_low = 5, limit_high = 10,
                        show_qvalue = F, pct_as_percent = T)
mtap_dp

tmp <- tempfile(fileext = ".jpeg")
ggsave(tmp, plot = mtap_dp, scale = 3,
       width = 1000,height = 500,units = "px")

# Upload directly
put_object(
  file = tmp,
  object = "Projects/CROCODILE scRNA/results/figures/Dot_PT_cells_MTAP.jpeg",
  bucket = "scrna",
  region = ""
)
...
dict$adnine_tissue <- "Adenine"
dict$p2_raw_leanm <- "Insulin sensitivity (mg/lean kg/min)"
...
put_object(
  file = temp_file,
  object = "Projects/CROCODILE scRNA/results/figures/adenine_mvalue_corr.png",
  bucket = "scrna",
  region = ""
)

t.test(subset(dat, group == "Lean Control")$p2_raw_leanm,
       subset(dat, group == "Type 1 Diabetes")$p2_raw_leanm)
```