...
```{r include=F}
# reading harmonized dataset
df <- read.csv(file.path(root_path, "Data Harmonization", "Data Clean", "harmonized_dataset.csv"))

# reading exclude list
exclude <- read.csv(file.path(root_path, "CROCODILE", "Data_Cleaned", "exclude_list.csv"))

# reading variable names dictionary
var_names <- read.csv(file.path(root_path, "Data Harmonization", "data_dictionary_master.csv"))

# creating dictionary of variable names
var_dict <- setNames(var_names$label, var_names$variable_name)

# subsetting to CROCODILE study
df <- df %>% filter(study == "CROCODILE")  %>%
  group_by(record_id, visit) %>%
  dplyr::summarise(across(everything(), last), .groups = "drop") %>% 
  dplyr::mutate(aer_24 = (u24_mab * u24_vl) / 1440,
         height_adj_total_kidney_volume = total_kidney_volume_ml / (height / 100),
         height_adj_right_kidney_volume = right_kidney_volume_ml / (height / 100),
         height_adj_left_kidney_volume = left_kidney_volume_ml / (height / 100),
         ...
         m_i = p2_raw_m / p2_steady_state_insulin
  ) %>% 
  filter(participation_status == "Participated")

# merging with exclude list
excluded_df <- df %>% left_join(exclude, by = "record_id")

# defining categorical variables
cat <- c("sex", "race", "statin", "angiotensin_receptor_blocker", "ace_inhibitor",
         "insulin_pump_timepoint", "raasi_timepoint", "statin", "metformin_timepoint",
         "sglti_timepoint", "glp1_agonist_timepoint", "cgm_yn", "kidney_side")
groupby <- "group"
```
...
# note: the following commented code shows the original data processing
# the processed data is loaded from RData file below
# dat <- df
# crc_dxa <- read.csv(file.path(root_path, "CROCODILE", "Data_Cleaned", "CRC_extremities_lean_mass.csv"))
...
# save(dat, file = file.path(root_path, "CROCODILE", "Data_Cleaned", "croc_data.RData"))
...
croc_diabetes_duration <- subset(dat, group == "Type 1 Diabetes", select = c(record_id, group, diabetes_duration))
write.csv(croc_diabetes_duration, file.path(root_path, "CROCODILE/Data_Cleaned/croc_diabetes_duration.csv"),
          na = "", row.names = F)
```
...
summary(tableby(group ~ p2_raw_m + p2_raw_leanm + p2_gc_m + p2_gc_leanm + gir_190 + gir_200, data = dat))
```

```{r include = F}
# selecting variables for export tables
t1_dat <- dat %>%
...
# exporting tables (commented out)
# write.csv(t1_dat, file.path(root_path, "CROCODILE", "Data_Cleaned", "CRC_t1_dat.csv"), row.names = F)
# write.csv(t2_dat, file.path(root_path, "CROCODILE", "Data_Cleaned", "CRC_t2_dat.csv"), row.names = F)
# write.csv(t3_dat, file.path(root_path, "CROCODILE", "Data_Cleaned", "CRC_t3_dat.csv"), row.names = F)
# write.csv(fig1_dat, file.path(root_path, "CROCODILE", "Data_Cleaned", "CRC_fig1_dat.csv"), row.names = F)

```

```{r include = F}
# testing age difference between groups
t.test(dat$age ~ dat$group, var.equal=F)

# testing NEFA suppression adjusted for baseline and body fat
...
```{r include = F}
# testing M-value adjusted for GFR and body fat
...
```{r include = F}
# testing MRI outcomes adjusted for perfusion
...
```{r include = F}
# testing associations between PET and M-value
...
# testing associations between PET and GBM thickness
...
pdf(height = 4000, width = 3000, file = file.path(root_path, "CROCODILE", "Dissemination", "Figures", "YC", "Correlation_clinical.pdf"))
...
# partial correlation adjusting by age
png(height = 1200, width = 800, file = file.path(root_path, "CROCODILE", "Dissemination", "Figures", "YC", "Correlation_clinical_age.png"), res = 170)
pcor.plot(data = dat,
          X = c("avg_c_f", "avg_m_f","avg_c_k2", "avg_m_k2"),
          Y = c("gbm_thick_artmean", "p2_gc_m"),
...
# partial correlation adjusting by hba1c
png(height = 1200, width = 800, file = file.path(root_path, "CROCODILE", "Dissemination", "Figures", "YC", "Correlation_clinical_hba1c.png"), res = 170)
pcor.plot(data = dat,
          X = c("avg_c_f", "avg_m_f","avg_c_k2", "avg_m_k2"),
          Y = c("gbm_thick_artmean", "p2_gc_m"),
...
# correlation adjusting by sex
png(height = 1200, width = 800, file = file.path(root_path, "CROCODILE", "Dissemination", "Figures", "YC", "Correlation_clinical_sex.png"), res = 170)
corr_plot_modified(data = dat,
                   X = c("avg_c_f", "avg_m_f","avg_c_k2", "avg_m_k2"),
                   Y = c("gbm_thick_artmean", "p2_gc_m"),
                   adj_var = "sex",
...
# partial correlation adjusting by GFR and body fat
png(height = 1200, width = 800, file = file.path(root_path, "CROCODILE", "Dissemination", "Figures", "YC", "Correlation_clinical_gfr_bodyfat.png"), res = 170)
pcor.plot(data = dat,
          X = c("avg_c_f", "avg_m_f","avg_c_k2", "avg_m_k2"),
          Y = c("gbm_thick_artmean", "p2_gc_m"),
...
# testing PET associations with GBM and M-value adjusted by sex
...
# PT morphometrics
```{r echo = F, message=F}
# generating correlation plots with PT morphometrics
png(height = 1200, width = 800, file = file.path(root_path, "CROCODILE", "Dissemination", "Figures", "YC", "Correlation_clinical_w_pt.png"), res = 170)
corr_plot_modified(data = dat,
                   X = c("avg_c_f", "avg_m_f","avg_c_k2", "avg_m_k2"),
...
```{r echo = F, message = F}
# reading tissue metabolomics data
tissue_met <- read.csv(file.path(root_path, "CROCODILE", "Data_Cleaned", "CRC_tissue_metabolomics_011624.csv"), na.strings = c("NA", "")) %>%
...
# imputing missing values with min/5
...
# averaging tissue metabolites per participant
...
write.csv(dat_vb_hct, file.path(root_path, "CROCODILE", "Data_Cleaned", "CRC_vb_hct.csv"), row.names = F)
```
