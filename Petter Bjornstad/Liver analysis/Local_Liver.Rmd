...
#Mac Lap Top File Path
# dir.dat <- c("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive")
# dir.results <- c("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/Liver project/NEBULA Results")
...
## Local Load Seurat Object snRNA
...
p <- DimPlot(
  so_liver_sn,
  reduction = "umap",
  group.by = "steatosis_grade",
  label = F,
  raster = TRUE
) +
  # scale_color_manual(values = c("darkblue","lightgray")) +
  ggtitle("UMAP by Steatosis Grade")
print(p)

p <- DimPlot(
  so_liver_sn,
  reduction = "umap",
  group.by = "hepatocyte_ballooning_score",
  label = F,
  raster = TRUE
) +
  # scale_color_manual(values = c("darkblue","lightgray")) +
  ggtitle("UMAP by Hepatocyte Balooning Score")
print(p)

p <- DimPlot(
  so_liver_sn,
  reduction = "umap",
  group.by = "ballooning_steatosis_cat",
  label = F,
  raster = TRUE
) +
  # scale_color_manual(values = c("darkblue","lightgray")) +
  ggtitle("UMAP by Balooning & Steatosis Category")
print(p)

p <- DimPlot(
  so_liver_sn,
  reduction = "umap",
  group.by = "disease_cat",
  label = F,
  raster = TRUE
) +
  # scale_color_manual(values = c("darkblue","lightgray")) +
  ggtitle("UMAP by Balooning & Steatosis Category2")
print(p)
...
prop_plot <- ggplot(data=cellcount,aes(ballooning_steatosis_cat, fill = celltype)) + 
  geom_bar(stat = "count", position = "fill") +
  theme_classic() +
  labs(x = NULL,
       y = "Proportion of Cells",
...
propeller(clusters = cellcount$celltype, sample = cellcount$Kit_Lot, 
          group = cellcount$ballooning_steatosis_cat)
...
prop_plot <- ggplot(data=cellcount,aes(steatosis_cat, fill = celltype)) + 
  ...
  theme(plot.title = element_text(hjust=0.5, face="bold"),
        text = element_text(size = 20)) +
  scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51","darkred","darkmagenta"))
# png(fs::path(dir.results, "Barchart_Steatosis_Cat_All_Cells.png"),width = 3000, height = 3000, res = 300)
# print(prop_plot)
# dev.off()
...
prop_plot <- ggplot(data=cellcount,aes(ballooning_steatosis_cat, fill = celltype)) + 
  ...
  theme(plot.title = element_text(hjust=0.5, face="bold"),
        text = element_text(size = 20)) +
  scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51","darkred","darkmagenta"))
png(fs::path(dir.results, "Barchart_Ballooning_Steatosis_Cat_All_Cells.png"),width = 3000, height = 3000, res = 300)
print(prop_plot)
dev.off()
...
### iii. Explore Gene Expression Levels
```{r}
# First, subset to just hepatocytes
hepatocytes <- subset(so_liver_sn, subset = celltype_LR == "Hep")

# 1. Check overall SCIMP expression in hepatocytes
scimp_expr <- FetchData(hepatocytes, vars = "SCIMP")
summary(scimp_expr$SCIMP)

# Calculate percentage of cells expressing SCIMP (>0)
pct_expressing <- sum(scimp_expr$SCIMP > 0) / nrow(scimp_expr) * 100
cat("Percentage of hepatocytes expressing SCIMP:", round(pct_expressing, 2), "%\n")

# 2. Check SCIMP expression by hepatocyte subtype
# Assuming your subtypes are in a metadata column called "subtype" or similar
scimp_by_subtype <- FetchData(hepatocytes, vars = c("SCIMP", "celltype"))

# Get expression statistics by subtype
subtype_stats <- scimp_by_subtype %>%
  group_by(celltype) %>%
  summarise(
    mean_expression = mean(SCIMP),
    median_expression = median(SCIMP),
    pct_expressing = sum(SCIMP > 0) / n() * 100,
    n_cells = n()
  )
...
dat_long_carb <- dat_long_carb %>% 
  mutate(name = case_when(variable=="carbp_m" ~ "State 2 PM",
                          ...
                          variable=="carb_cytoc" ~ "State 3 PMG",
...
dat_long_carb$name <- factor(dat_long_carb$name, 
                             levels = c("State 2 PM", "State 3 PM", 
                                        "State 3 PMG", "State 3 PMGS", 
...
# 
# # Create faceted boxplots
# ggplot(dat_long, aes(x = steatosis_cat, y = value, fill = steatosis_cat)) +
#   geom_boxplot(alpha = 0.7) +
#   geom_jitter(width = 0.2, alpha = 0.5) +  # Show individual points
#   facet_wrap(~ variable, scales = "free_y", ncol = 3) +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1),
#         legend.position = "top") +
#   labs(title = "Lipid Parameters by Steatosis Category",
#        x = "Steatosis Category",
#        y = "Value",
#        fill = "Steatosis Category")
# 
...
#Lipid variables
lipid_vars <- oro_vars[c(16,18:22,24,25,30)]
dat_long_lipid <- dat %>%
  dplyr::select(steatosis_cat, all_of(lipid_vars)) %>%
  pivot_longer(cols = -steatosis_cat, 
               names_to = "variable", 
...
dat_long_lipid <- dat_long_lipid %>% 
  mutate(name = case_when(variable=="lipidpc_m" ~ "State 2 FAM",
                          variable=="lipid_cytoc" ~ "State 3 FAM",
                          variable=="lipid_g" ~ "State 3 FAMG",
...
dat_long_lipid$name <- factor(dat_long_lipid$name, 
                              levels = c("State 2 FAM", "State 3 FAM", "State 3 FAMG", 
...
# Save using png() device
png(fs::path(dir.results,"Lipid_Oroboros_Boxplots_Parameters.png"), 
    width = 3000, 
    height = 3000, 
    res = 300)
print(g)
...
##### REACTOME
b <- plot_fgsea_transpose(reactome_res_Hepatocytes, title = "Hepatocytes", xlimit = 5) + theme(legend.position = c(0.15, 0.5))
...
png(fs::path(dir.results,"Supplemental Results/Hep_Top_reactome.png"),res=300,height=3000,width=3500)
plot(b)
dev.off()
```
...
clinical_variables <- c("ballooning_steatosis_cat","fibrosis_cat")
...
# List of genes
genes_list <- rownames(counts_hvg)[1:10]
...
# Remove NULLs (failed genes)
nebula_results_list <- Filter(Negate(is.null), nebula_results_list)
cat("Successfully analyzed genes:", length(nebula_results_list), "\n")
...
hep_nebula_converged <- map_dfr(
  ...
  function(gene_name) {
    tryCatch({
      ...
      data.frame(
        Gene = gene_name,
        Converged = result_obj$convergence,
        Algorithm = result_obj$algorithm,
        Has_Results = !is.null(result_obj$summary) && nrow(result_obj$summary) > 0,
...
print(table(hep_nebula_converged$Converged))
cat("Genes with results:", sum(hep_nebula_converged$Has_Results), "\n")
# # set the names of results based on gene names
# nebula_results_list <- Filter(Negate(is.null), nebula_results_list)  # remove NULLs first
# names(nebula_results_list) <- sapply(nebula_results_list, function(x) x$gene)  # set names
# nebula_results_list <- lapply(nebula_results_list, function(x) x$result)  # clean list back to just results
...
#   function(gene_name) {
#     tryCatch({
#       result_obj <- nebula_results_list[[gene_name]]$result
#       
#       converged <- result_obj$convergence
#       algorithm <- result_obj$algorithm
#       has_results <- !is.null(result_obj$summary) && nrow(result_obj$summary) > 0
...
# The summary is in WIDE format - need to reshape to LONG format
# hep_nebula_results_long <- map_dfr(
#   names(nebula_results_list),
#   function(gene_name) {
#     tryCatch({
#       result_obj <- nebula_results_list[[gene_name]]$result
...
#         
#         # Extract coefficient names (remove "logFC_" prefix)
...
#       }
#     }, error = function(e) {
#       message("Error for gene ", gene_name, ": ", e$message)
#       return(NULL)
#     })
#   }
# )
...
hep_nebula_results_long <- hep_nebula_results_long %>%
  filter(!is.na(gene))

# Add FDR correction by coefficient type
hep_nebula_results_long <- hep_nebula_results_long %>%
...
full_results <- hep_nebula_results_long
# full_results <- as.data.frame(nebula_summaries)
#Calculate number of genes filtered out for low expression 
low_exp <- length(genes_list)-length(full_results$gene)
#Filter out non-converging genes
...
#   ungroup()
# mutate(fdr3=p.adjust(PValue3,method="fdr"))
full_results$PValue10 <- -log10(pmax(full_results$p_value, 1e-10))  # Avoid log(0)

# write.csv(full_results,fs::path(dir.results,"NEBULA_Hep_1_Ballooning_Steatosis_Cat_unadjusted_2000_pooled_offset.csv"))

clinical_variables <- c("ballooning_steatosis_cat","fibrosis_cat")
var <- clinical_variables[1]
# full_results$color <- ifelse(full_results$fdr < 0.05 & full_results$`logFC_steatosis_catHigh Steatosis (2+3)` > 0, "lightcoral",
#                              ifelse(full_results$fdr < 0.05 & full_results$`logFC_steatosis_catHigh Steatosis (2+3)` < 0, "lightblue", "gray"))
full_results$color <- ifelse(full_results$fdr < 0.05 & full_results$logFC_ballooning_steatosis_cat > 0, "#990000",
                             ifelse(full_results$fdr < 0.05 & full_results$logFC_ballooning_steatosis_cat < 0, "#003366", "lightgray"))
...
LR_celltypes <- unique(so_liver_sn$celltype_LR)[2:5]
for (cell_name in LR_celltypes) {
  ...
  full_results <- full_results %>% 
    ...
    dplyr::select(-c("x","low_exp","nebula_nonconverged_percent"))
...
for (cell_name in LR_celltypes) {
  ...
  full_results <- full_results[,-1]
  full_results$Celltype <- cell_name
  total_results2 <- rbind(total_results2,full_results)
}
# total_results <- rbind(total_results,total_results2)
# rm(total_results2)
...
# total_results$Celltype <- factor(total_results$Celltype, levels = unique(so_liver_sn$celltype_LR)[-1])
...
volcano_plots <- ggplot(total_results, aes(x = logFC, y = PValue10, color = color)) +
  ...
  labs(
    title = "High Steatosis (2+3) vs. Low Steatosis (0+1) in Hepatocytes",
    subtitle = "2000 HVGs",
    x = "Log Fold Change",
    y = "-log10(P-Value)"
  ) +
...
# Display the plot
print(volcano_plots)
png(fs::path(dir.results,"Supplemental Results", "Volcano_Plots_NEBULA_Other_Celltypess_unadjusted_pooled_offset_Steatosis2.png"),
    width = 2000, height = 2000, res = 300)
print(volcano_plots)
dev.off()

png(fs::path(dir.results,"Supplemental Results", "Volcano_Plots_NEBULA_Hepatocytes_unadjusted_pooled_offset_Steatosis2.png"),
    width = 2000, height = 2000, res = 300)
print(volcano_plots)
dev.off()
...
#Heat Map visualization of results
total_results <- data.frame()
HR_celltypes <- c("Hep_1", "Hep_2", "Hep_3", "Hep_4", "Hep_5", 
                  "dHep", "Hep_Immune")
# HR_celltypes <- str_replace_all(unique(so_liver_sn$celltype)[-7],"/","_")
...
volcano_plots <- ggplot(total_results, aes(x = logFC, y = p_value10, color = color)) +
  ...
  labs(
    title = "High Steatosis (2+3) vs. Low Steatosis (0+1) by Hepatocyte Subtype",
    subtitle = "2000 HVGs, Unadjusted",
    x = "Log Fold Change",
    y = "-log10(P-Value)"
  ) +
...
png(fs::path(dir.results, "Supplemental Results","Volcano_Plots_NEBULA_Hepatocytes_HR_unadjusted_pooled_offset_Steatosis.png"),
...
cell_name2 <- str_replace_all(cell_name,"_","/")
cell_name2
hep_immune <- plot_fgsea_transpose(reactome_res_dHep, title = paste0(cell_name2), xlimit = 5) + theme(legend.position.inside = c(0.15, 0.5))
...
# }
combined_plot <- Hep1 + Hep2 + Hep3 + Hep4 + Hep5 + dHep + hep_immune + plot_layout(ncol = 4)
png(fs::path(dir.results,"Supplemental Results",paste0("AllHep_top30_pathways_All_Hep_REACTOME.png")),
    width = 12000, height = 6000, res=300)
print(combined_plot)
dev.off()
...
```{r}
#Filter to celltype 
HR_celltypes <- unique(so_liver_sn$celltype)[-7]
#  [1] Hep-2        Hep-5        Hep-4        EC-2         Stellate-2   Hep-3        Cholang     
#  [8] Hep-1        NKC/NKT      Kup/MON      dHep         Kup/MAC      EC-1         Stellate-1  
# [15] B/Plasma     Hep/Immune   cFIB/cImmune
# cell_name <- "Hep-2"
...
volcano_plots <- ggplot(total_results, aes(x = logFC, y = p_value10, color = color)) +
  ...
  labs(
    title = "High Steatosis (2+3) vs. Low Steatosis (0+1) by Cell Subtype",
    subtitle = "2000 HVGs, Unadjusted",
    x = "Log Fold Change",
    y = "-log10(P-Value)"
  ) +
...
##C. Clinical Variables & Gene Expression
###i. LR Nebula
```{r}
#Filter to celltype 
LR_celltypes <- unique(so_liver_sn$celltype_LR)[-1]
# clinical_variables <- c("alt","ast","ggt","bmi","a1c","hepatocyte_ballooning_score","lobular_inflammation_grade","steatosis_grade","fibrosis_stage")
...
for (cell_name in LR_celltypes) {
  for (var in clinical_variables) {
    full_results <- read.csv(fs::path(dir.results,paste0("NEBULA_",cell_name,"_",var,"_unadjusted_2000_pooled_offset.csv")))
    full_results <- full_results %>% 
      clean_names() %>% 
      # dplyr::rename(logFC=log_fc_steatosis_cat_high_steatosis_2_3) %>% 
      dplyr::rename(Gene=gene)
...
# Set factor levels for cell type ordering
total_results$Celltype <- factor(total_results$Celltype, levels = unique(so_liver_sn$celltype_LR))
...
for (var in clinical_variables){
  ...
  volcano_plot <- ggplot(results, aes(x = log_fc, y = p_value10, color = color)) +
    ...
    labs(title = paste0("Gene Expression vs. ", str_to_upper(var))) +
    # xlim(-1, 1) +
    # xlim(min,max)+
  ...
  # Display the plot
  # print(volcano_plot)
  png(fs::path(dir.results, paste0("Volcano_Plots_NEBULA_",var,"_All_Celltypes_LR_unadjusted_pooled_offset.png")),
      width = 7000, height = 1500, res = 300)
  print(volcano_plot)
  dev.off()
}
...
for (var in clinical_variables){
  ...
  for (celltype in LR_celltypes) {
    ...
    ##### REACTOME
    b <- plot_fgsea_transpose(reactome_res_dHep, title = paste0(var," in ",celltype," , Top 30 REACTOME Pathways"), xlimit = 8)+ theme(legend.position.inside = c(0.1, 0.5))
    ...
    # combined_plot <- b + c + plot_layout(ncol = 1)
    print(b)
    ggsave(fs::path(dir.results,paste0(celltype,"_",var,"_top30_pathways_unadjusted_LR.jpeg")),
           width = 15, height = 10, scale = 1)
  }
}
```
###ii. HR Nebula
...
ox_phos <- unique(c(ox_phos1,ox_phos2))
#Filter to celltype 
LR_celltypes <- unique(so_oro$celltype_LR)[-1]
#  [1] Hep-2        Hep-5        Hep-4        EC-2         Stellate-2   Hep-3        Cholang     
#  [8] Hep-1        NKC/NKT      Kup/MON      dHep         Kup/MAC      EC-1         Stellate-1  
# [15] B/Plasma     Hep/Immune   cFIB/cImmune
meta <- so_oro@meta.data
...
for (cell_name in LR_celltypes){
  ...
  for (oro in oro_vars) {
    ...
    cell_name2 <- str_replace_all(cell_name,"/","_")
    cell_name2 <- str_replace_all(cell_name2,"-","_")
    write.csv(full_results,fs::path(dir.results,paste0("NEBULA_",oro,"_",cell_name2,"_unadjusted_MT_Genes.csv")))
  }
}
...
unique(so_celltype$StudyID) #"MANGO-06" "MANGO-18" "MANGO-05" "MANGO-07" "IT2D-19"  "IT2D-14"  "IT2D-15"  "MANGO-04"
table(so_celltype$steatosis_cat,so_celltype$carbp_m)
...
# saveRDS(key,"/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/Liver project/Data/gene_prot.RDS")
...
# today <- read.csv("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/TODAY subaward/Clinical data/TODAY/CBL.csv")
today <- today %>%
  filter(mvisit!="R")
#dplyr::select(c("releaseid","ALT","AST"))
#Load in Proteomics in Today
# prot <- readRDS(fs::path(dir.dat,"Liver project","Data","today_subeomics.rds"))
prot <- readRDS("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/Data/today_proteomics_log2.rds")
# prot <- readRDS("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/Liver project/Data/today_subeomics.rds")
...
# key <- readRDS("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/Liver project/Data/gene_prot.RDS")
...
#AST
today_sub$time_numeric <- as.numeric(factor(today_sub$mvisit)) - 1  # Start at 0 for baseline
m0 <- as.formula(paste0("AST ~ time_numeric + (1|releaseid)"))
model_simple <- lmer(m0, data = today_sub)
summary(model_simple)

# Scatter plot with individual trajectories
p3 <- ggplot(today_sub, aes(x = time_numeric, y = AST)) +
  geom_point(aes(color = factor(mvisit)), alpha = 0.6) +
  geom_path(aes(group = releaseid), alpha = 0.2) +
  geom_smooth(method = "lm", color = "red4", linewidth= 0.5,se=F) +
  scale_color_viridis_d(name = "Time") +
  theme_minimal() +
  labs(title = "AST Levels over Time",
       subtitle = "Beta = 1.319e-01, P-value = 1.55e-05***",
       x = "Time",
       y = "AST Level") +
...
# png(fs::path(dir.results,"AST_Over_Time_TODAY.png"),res=300,width=3000,height=2000)
# print(p3)
# dev.off()

#ALT
# today_sub$time_numeric <- as.numeric(factor(today_sub$mvisit)) - 1  # Start at 0 for baseline
m0 <- as.formula(paste0("ALT ~ time_numeric + (1|releaseid)"))
model_simple <- lmer(m0, data = today_sub)
summary(model_simple)

# Scatter plot with individual trajectories
p3 <- ggplot(today_sub, aes(x = time_numeric, y = ALT)) +
  geom_point(aes(color = factor(mvisit)), alpha = 0.6) +
  geom_path(aes(group = releaseid), alpha = 0.2) +
  geom_smooth(method = "lm", color = "red4", linewidth= 0.5,se=F) +
  scale_color_viridis_d(name = "Time") +
  theme_minimal() +
  labs(title = "ALT Levels over Time",
       subtitle = "Beta = 2.697e-01, P-value = 1.72e-08***",
       x = "Time",
       y = "ALT Level") +
  theme(plot.title = element_text(face = "bold"))
# legend.position="none")
...
# png(fs::path(dir.results,"ALT_Over_Time_TODAY.png"),res=300,width=3000,height=2000)
# print(p3)
# dev.off()
...
###Test proteins over time
```{r}
#RIPK2
today_sub$time_numeric <- as.numeric(factor(today_sub$mvisit)) - 1  # Start at 0 for baseline
m0 <- as.formula(paste0("AST ~ stage + time_numeric + (1|releaseid)"))
model_simple <- lmer(m0, data = today_sub)
summary(model_simple)
...
results_alt <- tidylog::left_join(results_alt,key,by="Protein")
results_ratio <- tidylog::left_join(results_ratio,key,by="Protein")
# write.csv(results_ast,fs::path(dir.results,"today_proteomics_AST_adj_fdr_01.csv"))
# write.csv(results_alt,fs::path(dir.results,"today_proteomics_ALT_adj_fdr_01.csv"))
# write.csv(results_ratio,fs::path(dir.results,"today_proteomics_AST_ALT_ratio_adj_fdr_01.csv"))
...
#ALT
full_results <- read.csv(fs::path(dir.results,"today_proteomics_ALT_adj_fdr_01.csv")) %>%
  clean_names() %>% 
  dplyr::select(-x) %>% 
  dplyr::rename(Gene=entrez_gene_symbol,LogHR=log_hr)

#Keep aptamer with largest HR
full_results_processed <- full_results %>%
  group_by(Gene) %>%
  filter(abs(LogHR) == max(abs(LogHR))) %>%
  dplyr::slice(1) %>%  # In case of ties, keep first
  ungroup()
length(unique(full_results$Gene)) #3351 unique gene names
length(full_results$Gene) #3770 total aptamers


full_results <- full_results_processed
rm(full_results_processed)
...
##### REACTOME
plot_fgsea_transpose(reactome_res_dHep, title = "TODAY ALT Proteomics - Top 30 REACTOME", xlimit = 5)

ggsave(fs::path(dir.results,"TODAY_top30_reactome_pathways_ALT.jpeg"),
       width = 13, height = 10, scale = 1)
...
#AST
full_results <- read.csv(fs::path(dir.results,"today_proteomics_AST_adj_fdr_01.csv")) %>%
  clean_names() %>% 
  dplyr::select(-x) %>% 
  dplyr::rename(Gene=entrez_gene_symbol,LogHR=log_hr)
...
##### REACTOME
plot_fgsea_transpose(reactome_res_dHep, title = "TODAY AST Proteomics - Top 30 REACTOME", xlimit = 5)

ggsave(fs::path(dir.results,"TODAY_top30_reactome_pathways_AST.jpeg"),
       width = 13, height = 10, scale = 1)
...
load(fs::path(dir.dat,"Teen Labs","Data_Cleaned","analysis_dataset_log2.RData"))
# saveRDS(analyte_info,fs::path(dir.dat,"Teen Labs","Data_Cleaned","analyte_info_log2.rds"))
# saveRDS(df,fs::path(dir.dat,"Teen Labs","Data_Cleaned","df_log2.rds"))
...
df <- df %>% 
  dplyr::select(c("ID","visit","SEX","ETHN","bmi","HBA1C","NAFLD0","NAFLD1",
...
#Test ALT and AST levels over time
#AST
df$time_numeric <- as.numeric(factor(df$visit)) - 1  # Start at 0 for baseline
m0 <- as.formula(paste0("AST ~ time_numeric + (1|ID)"))
model_simple <- lmer(m0, data = df)
summary(model_simple)

# Scatter plot with individual trajectories
p3 <- ggplot(df, aes(x = time_numeric, y = AST)) +
  ...
  theme_minimal() +
  labs(title = "AST Levels over Time",
       subtitle = "Beta = -2.95, P-value = 8.52e-07***",
       x = "Time",
       y = "AST Level") +
  theme(plot.title = element_text(face = "bold"))
...
png(fs::path(dir.results,"AST_Over_Time_TeenLABS.png"),res=300,width=3000,height=2000)
print(p3)
dev.off()

#ALT
# df$time_numeric <- as.numeric(factor(df$visit)) - 1  # Start at 0 for baseline
m0 <- as.formula(paste0("ALT ~ time_numeric + (1|ID)"))
model_simple <- lmer(m0, data = df)
summary(model_simple)

# Scatter plot with individual trajectories
p3 <- ggplot(df, aes(x = time_numeric, y = ALT)) +
  geom_point(aes(color = factor(visit)), alpha = 0.6) +
  geom_path(aes(group = ID), alpha = 0.2) +
  geom_smooth(method = "lm", color = "black", linewidth= 0.5,se=F) +
  scale_color_viridis_d(name = "Time") +
  theme_minimal() +
  labs(title = "ALT Levels over Time",
       subtitle = "Beta = -2.07, P-value = 0.00111**",
       x = "Time",
       y = "ALT Level") +
  theme(plot.title = element_text(face = "bold"))
...
png(fs::path(dir.results,"ALT_Over_Time_TeenLABS.png"),res=300,width=3000,height=2000)
print(p3)
dev.off()
...
for (protein in gene_proteins) {
  m0 <- as.formula(paste0("ALT ~",protein,"+ time_numeric + (1|ID)"))
  model_simple <- lmer(m0, data = df)
...
#Do again but removing problematic nonconvregent protesins
alt_results <- data.frame()
...
alt_results <- alt_results %>% 
  dplyr::rename(Gene=EntrezGeneSymbol)
# write.csv(alt_results,fs::path(dir.results,"TeenLABS_ALT_Crude_Results_Proteomics_New.csv"))
...
volcano_plot <- ggplot(alt_results, aes(x = Beta, y = -log10(fdr), color = color)) +
  ...
  theme_bw() +  # Minimal theme
  labs(
    title = "Mixed-Effect Model for ALT (U/L) vs. Proteins over Time",
    x = "Beta",
    y = "-log10(FDR)",
...
# volcano_plot_alt
png(fs::path(dir.results,"ALT_Proteins_TeenLabs_New.png"), width = 2200, height = 2000,res=300)
print(volcano_plot)
dev.off()
...
alt_results <- data.frame()
for (protein in gene_proteins) {
  m0 <- as.formula(paste0("ALT ~",protein, "+ time_numeric + (1|ID)+age+SEX"))
  model_simple <- lmer(m0, data = df)
  beta <- summary(model_simple)$coef[2,1]
  pval <- summary(model_simple)$coef[2,5]
  results <- data.frame(Protein=protein,Beta=beta,Pval=pval)
...
alt_results <- alt_results %>%
  dplyr::rename(Gene=EntrezGeneSymbol)
write.csv(alt_results,fs::path(dir.results,"TeenLABS_ALT_Adj_Results_Proteomics_New.csv"))
...
alt_results <- alt_results %>%
  dplyr::rename(Gene=EntrezGeneSymbol)
write.csv(alt_results,fs::path(dir.results,"TeenLABS_ALT_Adj_Results_Proteomics_FDR_01_New.csv"))
...
# volcano_plot_alt
png(fs::path(dir.results,"ALT_Proteins_TeenLabs_Adjusted_FDR_01_New.png"), width = 2200, height = 2000,res=300)
print(volcano_plot)
dev.off()
# write.csv(alt_results,fs::path(dir.results,"ALT_Proteins_Teenlabs.csv"))
...
for (protein in gene_proteins) {
  m0 <- as.formula(paste0("AST ~",protein, "+ time_numeric + (1|ID)+age+SEX"))
...
ast_results <- ast_results %>%
  dplyr::rename(Gene=EntrezGeneSymbol)
write.csv(ast_results,fs::path(dir.results,"TeenLABS_AST_Adj_Results_Proteomics_FDR_01_New.csv"))
...
volcano_plot <- ggplot(ast_results, aes(x = Beta, y = -log10(fdr), color = color)) +
  ...
  theme_bw() +
  labs(
    title = "Mixed-Effect Model for AST (U/L) vs. Proteins over Time",
    x = "Beta",
    y = "-log10(FDR)",
    color = "Beta Direction",
    caption = paste0("fdr<0.1, Proteins = ",num_proteins,", N = ",N)
...
# volcano_plot_AST
png(fs::path(dir.results,"AST_Proteins_TeenLabs_Adjusted_FDR_01_New.png"), width = 2200, height = 2000,res=300)
print(volcano_plot)
dev.off()
# write.csv(alt_results,fs::path(dir.results,"ALT_Proteins_Teenlabs.csv"))
```
...
```{r echo = F}
#ALT
full_results <- read.csv(fs::path(dir.results,"TeenLABS_ALT_Adj_Results_Proteomics_FDR_01_New.csv")) %>%
  clean_names() %>% 
  dplyr::select(-x) %>% 
  dplyr::rename(Gene=gene,Beta=beta)
...
# rank genes by t-stats in DiD
rankings_dHep <- full_results$Beta
names(rankings_dHep) <- full_results$Gene
rankings_dHep <- sort(rankings_dHep, decreasing = TRUE)
plot(rankings_dHep)
min(rankings_dHep)
...
##### REACTOME
plot_fgsea_transpose(reactome_res_dHep, title = "Teen-LABS ALT Proteomics - Top 30 REACTOME", xlimit = 5)

ggsave(fs::path(dir.results,"TEENLABS_top30_reactome_pathways_ALT.jpeg"),
       width = 13, height = 10, scale = 1)
...
#AST
full_results <- read.csv(fs::path(dir.results,"TeenLABS_AST_Adj_Results_Proteomics_FDR_01_New.csv")) %>%
  clean_names() %>% 
  dplyr::select(-x) %>% 
  dplyr::rename(Gene=gene,Beta=beta)
...
##### REACTOME
plot_fgsea_transpose(reactome_res_dHep, title = "Teen-LABS AST Proteomics - Top 30 REACTOME", xlimit = 5)

ggsave(fs::path(dir.results,"TEENLABS_top30_reactome_pathways_AST.jpeg"),
       width = 13, height = 10, scale = 1)
