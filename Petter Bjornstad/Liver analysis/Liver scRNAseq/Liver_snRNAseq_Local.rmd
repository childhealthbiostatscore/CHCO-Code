...
#Filter to celltype 
LR_celltypes <- unique(so_liver_sn$celltype_LR)[1]
clinical_variables <- c("alt","ast","ggt","bmi","a1c","hepatocyte_ballooning_score","lobular_inflammation_grade","steatosis_grade","fibrosis_stage","fibrosis_cat")
...
<<<<<<< HEAD
  # List of genes
  genes_list <- rownames(counts_hvg)[1001:2000]
  
  # for (var in clinical_variables[1]){
  ...
  start_time <- Sys.time()
    ...
    #Bind results from multiple runs together
    # full_results_full <- full_results
    full_results_full <- rbind(full_results_full,full_results)
    # full_results <- full_results_full
    # rm(full_results_full)
...
=======
  for (var in clinical_variables[1]){
    # List of genes
    genes_list <- rownames(counts_hvg)[11:100]
    # total_results <- data.frame()
    # for (i in 1:length(genes_list)) {
    # for (i in 1:10) {
      # gene_select <- genes_list[i]
    ...
    # full_results <- full_results_full
    # rm(full_results_full)
    full_results <- total_results
    rm(total_results)
>>>>>>> 654af8c6 (fix targeted analysis code)
    ...
    full_results$celltype <- cell_name2
    #Save Results
    write.csv(full_results,paste0("NEBULA_",cell_name,"_",var,"_unadjusted_2000_pooled_offset.csv"))
...
#   out_key <- file.path(paste0("Liver transcriptomics/Results/NEBULA_",cell_name,"_",var,"_unadjusted_2000_pooled_offset.rds"))
#   tmp <- tempfile(fileext = ".rds")
#   saveRDS(full_results, tmp)
