...
length(which(!is.na(harm_meta_data$lc_k2))) #53
length(which(!is.na(harm_meta_data$rc_k2))) #55
length(which(!is.na(harm_meta_data$lm_k2))) #53
length(which(!is.na(harm_meta_data$rm_k2))) #54



#Filter to those with Pet data - remove "mm_ir","mm_si","airg because complete missing data 
pet_dat <- harm_meta_data %>% 
  dplyr::select(c("record_id","kit_id","visit","screen_date","lc_k2","rc_k2","lm_k2","rm_k2",
                  "lc_f","rc_f","lm_f","rm_f","search_eis",
