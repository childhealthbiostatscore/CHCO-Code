...
# Sys.setenv(RETICULATE_PYTHON = "/mmfs1/gscratch/togo/hhampson/miniconda3/envs/py310_env/bin/python")
library(reticulate)
...
keys <- fromJSON("/mmfs1/gscratch/togo/hhampson/keys.json") # replace with your Lambda username
...
## b. Local Load Seurat Object snRNA
...
# Perform UMAP 
#Print plot
png(fs::path(dir.results, "UMAP_by_celltype_ratio1.png"),
...
# Display the plot
print(volcano_plots)
png(fs::path(dir.results, "Volcano_Plots_NEBULA_Other_Celltypess_unadjusted_pooled_offset_Steatosis.png"),
...
# Display the plot
print(volcano_plots)
png(fs::path(dir.results, "Volcano_Plots_NEBULA_All_Celltypes_HR_unadjusted_pooled_offset_Steatosis.png"),
...
# is.numeric(so_liver_sn$fibrosis_stage)
...
for (cell_name in LR_celltypes){
  ...
  for (var in clinical_variables){
    ...
    full_results$celltype <- cell_name2
    #Save Results
    write.csv(full_results,fs::path(dir.results,paste0("NEBULA_",cell_name,"_",var,"_unadjusted_2000_pooled_offset.csv")))
...
# # Display the plot
# print(volcano_plots)
# png(fs::path(dir.results, "Volcano_Plots_NEBULA_Other_Celltypess_LR_unadjusted_pooled_offset_Clinical_vars.png"),
#     width = 5000, height = 3000, res = 300)
# print(volcano_plots)
# dev.off()
# 
...
saveRDS(base_log,"/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/Data/today_subeomics.rds")
...
prot <- readRDS("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/Data/today_eomics.rds")
...
for (x in hvg_proteins) {
  ...
  M1 <- coxph(M0,data=dat_ratio)
  logHR <- summary(M1)$coef[1,1]
  pval <- summary(M1)$coef[1,5]
  results_full_ratio <- data.frame(Protein=x,Outcome=paste0("Time to ALT/AST Ratio = 1.5"),LogHR=logHR,Pvalue=pval)
...
# write.csv(results_alt,fs::path(dir.results,"today_subeomics_ALT_hvg.csv"))
...
results_ast <- results_ast %>%
  mutate(sig=ifelse(fdr<0.05,"*",""))
# write.csv(results_ast,fs::path(dir.results,"today_subeomics_AST_hvg.csv"))

#Ratio
results_ratio <- results_ratio %>%
  filter(Outcome=="Time to ALT/AST Ratio = 1.5") %>% 
  mutate(fdr = p.adjust(Pvalue,method="fdr"))
results_ratio <- results_ratio %>%
  mutate(sig=ifelse(fdr<0.05,"*",""))
# write.csv(results_ratio,fs::path(dir.results,"today_subeomics_ALT_AST_ratio_hvg.csv"))
...
png(fs::path(dir.results, "Plot_ALT_Proteomics_2000_hvg.png"), 
...
#Ratio
# Avoid log(0)
results_ratio$PValue10 <- -log10(pmax(results_ratio$Pvalue, 1e-10))
...
N <- length(unique(dat_ratio$releaseid))
...
volcano_plot <- ggplot(results_ratio, aes(x = LogHR, y = PValue10, color = color)) +
  ...
  # # Add labels for significant points
  # geom_text(data = significant_df, aes(label = Gene),
  #           vjust = 1, hjust = 1, size = 3, check_overlap = FALSE, color = "black")+
  # geom_text(data = top_labels, aes(label = Gene),
  #           vjust = 1, hjust = 1, size = 3, check_overlap = TRUE, color = "black") +
  geom_text_repel(data = significant_df, 
                  ...
                  force = 1,
                  max.overlaps = Inf,
                  # Optional: limit how far labels can be from points
                  max.time = 1,
                  max.iter = 10000) +
  facet_wrap(~Outcome)

volcano_plot
png(fs::path(dir.results, "Plot_Ratio_Proteomics_2000_hvg.png"), 
    width = 2200, height = 2000, res = 300)
print(volcano_plot)
dev.off()
...
png(fs::path(dir.results, "Plot_AST_Proteomics_2000_hvg.png"), 
    width = 2200, height = 2000, res = 300)
print(volcano_plot)
dev.off()
```
...
# volcano_plot_alt
png(fs::path(dir.results,"ALT_Proteins_TeenLabs.png"),width=4000,height=4000,res=300)
print(volcano_plot)
dev.off()
...
for (protein in gene_proteins) {
  m0 <- as.formula(paste0(protein,"~ AST + time_numeric + (1|ID)"))
  model_simple <- lmer(m0, data = df)
  beta <- summary(model_simple)$coef[2,1]
  pval <- summary(model_simple)$coef[2,5]
  results <- data.frame(Protein=protein,Beta=beta,Pval=pval)
...
# volcano_plot_ast
png(fs::path(dir.results,"AST_Proteins_TeenLabs.png"),width=4000,height=4000,res=300)
print(volcano_plot)
dev.off()
...
#Number of participants
length(unique(harm_meta_data$record_id)) #651 changed to 574 on 08/25
...
#Filter to those with Pet data - remove "mm_ir","mm_si","airg because complete missing data 
pet_dat <- harm_meta_data %>% 
  dplyr::select(c("record_id","kit_id","visit","procedure","date","liver_k1","liver_k2","liver_k3","search_eis",
                  ...
                  "p2_steady_state_ffa"))
# Create lookup table of screening dates
screen_dates <- pet_dat %>%
  filter(procedure == "screening") %>%
  dplyr::select(record_id, screen_date = date) %>%
  distinct()  # In case there are multiple screening records per person

# Join back to original data
pet_dat <- tidylog::left_join(pet_dat,screen_dates, by = "record_id")

# pet_dat <- harm_meta_data %>% 
#   dplyr::select(c("record_id","kit_id","visit","screen_date","liver_k1","liver_k2","liver_k3","search_eis",
#                   "homa_ir","p1_ffa_suppression","gir_190","gir_200",
#                   "p1_gc_leanm","p1_gc_m","p1_raw_leanm","p1_raw_m","p1_steady_state_ffa",
#                   "p2_ffa_suppression","p2_gc_leanm","p2_gc_m","p2_raw_leanm","p2_raw_m",
#                   "p2_steady_state_ffa"))
...
pet_variables <- c("lc_k2","rc_k2","lm_k2","rm_k2",
                   "lc_f","rc_f","lm_f","rm_f")
pet_dat3 <- pet_dat2 %>%
...
#Calculate K2 and F variables
pet_dat4$avg_c_k2 <- (pet_dat4$lc_k2+pet_dat4$rc_k2)/2 
pet_dat4$avg_m_k2 <- (pet_dat4$lm_k2+pet_dat4$rm_k2)/2
pet_dat4$avg_c_f <- (pet_dat4$lc_f+pet_dat4$rc_f)/2 
pet_dat4$avg_m_f <- (pet_dat4$lm_f+pet_dat4$rm_f)/2
pet_dat4$avg_c_k2_f <- pet_dat4$avg_c_k2/pet_dat4$avg_c_f
pet_dat4$avg_m_k2_f <- pet_dat4$avg_m_k2/pet_dat4$avg_m_f
pet_vars_avg <- c("avg_c_k2","avg_m_k2","avg_c_f",
                  "avg_m_f","avg_c_k2_f","avg_m_k2_f")

#Correlation/association between each pet var and each insulin var
total_results <- data.frame()
for(pet in pet_vars_avg) {
  for (insulin in insulin_vars){
    # Pearson correlation test
    pearson_test <- cor.test(pet_dat4[[pet]], pet_dat4[[insulin]], 
                             method = "pearson",
                             conf.level = 0.95)
    ...
    spearman_test <- cor.test(pet_dat4[[pet]], pet_dat4[[insulin]], 
                              method = "spearman",
                              conf.level = 0.95)
...
png(fs::path(dir.results,"Heatmaps_Pearson_Corr_Liver_Pet_IR.png"),width=3000,height=2000,res=300)
print(heatmap_pearson)
dev.off()
png(fs::path(dir.results,"Heatmaps_Spearman_Corr_Liver_Pet_IR.png"),width=3000,height=2000,res=300)
