...
#Filter to RH2 and CRC 
harm_meta_data <- harm_meta_data %>%
  filter(grepl("RH",record_id)|grepl("RH2",record_id)|grepl("CRC",record_id))
# harm_meta_data <- harm_meta_data %>% 
#   filter(grepl("CRC",record_id))

#Number of participants
length(unique(harm_meta_data$record_id)) #117, 60 CRC people, 211 including RH and RH2

#Number of participants with Pet data
length(which(!is.na(harm_meta_data$liver_k1))) #57, 27 crc
length(which(!is.na(harm_meta_data$liver_k2))) #57, 27 crc
length(which(!is.na(harm_meta_data$liver_k3))) #57, 27 crc

#Filter to those with Pet data - remove "mm_ir","mm_si","airg because complete missing data 
pet_dat <- harm_meta_data %>% 
  dplyr::select(c("record_id","mrn","co_enroll_id","visit","screen_date","liver_k1","liver_k2","liver_k3","search_eis",
...
png(fs::path(dir.results,"Heatmaps_Pearson_Corr_Liver_Pet_IR_CRC.png"),width=3000,height=2000,res=300)
print(heatmap_pearson)
dev.off()
png(fs::path(dir.results,"Heatmaps_Spearman_Corr_Liver_Pet_IR_CRC.png"),width=3000,height=2000,res=300)
