...
prot_id <- unique(prot$ID)

df_sub <- df %>% 
  filter(ID %in% prot_id)

# Scatter plot with individual trajectories
p3 <- ggplot(your_data, aes(x = protein, y = alt)) +
  geom_point(aes(color = factor(time)), alpha = 0.6) +
  geom_path(aes(group = subject_id), alpha = 0.2) +
...
# # Basic spaghetti plot
# ggplot(df_sub, aes(x = visit, y = ALT, group = ID)) +
#   geom_line(alpha = 0.5, color = "gray40") +
#   geom_point(alpha = 0.5, size = 1.5, color = "gray40") +
#   labs(title = "ALT Values Over Time by Individual",
#        x = "Visit",
