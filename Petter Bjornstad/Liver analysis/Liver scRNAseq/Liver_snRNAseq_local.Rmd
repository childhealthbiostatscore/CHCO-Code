...
# List of genes
genes_list <- rownames(counts_hvg)[1:20]
# # count_gene <- counts_hvg[g, , drop = FALSE]
# meta_gene_hvg <- .subset(so_celltype_hvg,features=g)@meta.data
# pred_gene_hvg <- model.matrix(~steatosis_cat, data = meta_gene_hvg)
# data_g_hvg = list(count = counts_hvg, id = id, pred = pred_gene_hvg)
...
nebula_results_list <- foreach(g = genes_list, .packages = c("nebula", "Matrix")) %dopar% {
  tryCatch({
    count_gene <- counts_hvg[g, , drop = FALSE]
    meta_gene <- subset(so_celltype,features=g)@meta.data
    pred_gene <- model.matrix(~carbfccp, data = meta_gene)
    # library <- meta_gene$pooled_offset
    library <- meta_gene$pooled_offset
    data_g_gene <- group_cell(count = count_gene, id = meta_gene$Kit_Lot, pred = pred_gene,offset=library)
...
write.csv(total_results,fs::path(dir.results,"NEBULA_Hepatocytes_Oroboros_unadjusted_2000_hvg.csv"))
...
# 
# umap_keep <- rownames(as.data.frame(so_Hep@reductions$umap@cell.embeddings) %>%
...
#Unsupervised 
sce_Hep_unsupervised <- as.SingleCellExperiment(so_Hep)
sce_Hep_unsupervised <- PrepareTotem(sce_Hep_unsupervised)
print(dim(sce_Hep_unsupervised)) #11224 genes, 103101 hepatocytes
sce_Hep_unsupervised <- RunDimRed(object = sce_Hep_unsupervised,
                 dim.red.method = "lmds",
...
# own_dim_red <- reducedDim(sce_Hep_unsupervised)
# reducedDim(sce_Hep_unsupervised,type = "lmds") <- own_dim_red # use any name

#Clustering
sce_Hep_unsupervised <- RunClustering(sce_Hep_unsupervised,
                         k.range = 3:20,
                         min.cluster.size = 5,
                         N.clusterings=10000)
VizCellConnectivity(sce_Hep_unsupervised,viz.dim.red = dim_red)



sce_Hep_unsupervised <- slingshot(sce_Hep_unsupervised, reducedDim = 'PCA', clusterLabels = colData(sce_Hep_unsupervised)$celltype)
shuffle <- sample(ncol(sce_Hep_unsupervised))
plot(reducedDims(sce_Hep_unsupervised)$UMAP[shuffle, ], asp = 1, pch = 16,
     xlab = "UMAP-1", ylab = "UMAP-2",
     col = hcl.colors(100, alpha = 0.5, palette = "PinkYl")[cut(sce_Hep_unsupervised$slingPseudotime_1, breaks = 100)][shuffle])
lines(SlingshotDataSet(sce_Hep_unsupervised), type = "lineages")
lines(SlingshotDataSet(sce_Hep_unsupervised))
SlingshotDataSet(sce_Hep_unsupervised)
...
write.csv(results_total,fs::path(dir.results,"TODAY_proteomics_alt_ast_adj.csv"))
```
###ii. Old
```{r}
#Load in AST and ALT in today 
