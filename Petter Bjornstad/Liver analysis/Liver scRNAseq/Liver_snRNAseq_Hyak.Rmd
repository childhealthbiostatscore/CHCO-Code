...
so_liver_sn <- readRDS("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/scRNA/data_raw/NoRef_PetterLiver_ClinData_Labels_Char_041924.RDS")
# #Lambda
# so_liver_sn <- readRDS(fs::path(dir.dat,"scRNA","data_raw","NoRef_PetterLiver_ClinData_Labels_Char_041924.RDS"))
DefaultAssay(so_liver_sn) <- "RNA"
dim(so_liver_sn)#36601 130124
...
# meta_liver_raw <- read.csv("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/scRNA/data_clean/liver_biopsy_metadata_PN.csv")
...
#Create general hepatocyte cell type variable
so_oro$celltype2 <- ifelse(grepl("Hep-",so_oro$celltype) | grepl("dHep",so_oro$celltype),"Hep","Non-Hep")
...
# colData(sce)$treatment <- attempt_so_pt$treatment
# colData(sce)$visit <- attempt_so_pt$visit
...
plot(reducedDims(sce_celltype)$PCA, asp=0.5, col = 'grey75', pch = 16)
...
pca_df$celltype <- factor(pca_df$celltype, levels = c("Hep-1", "Hep-2", "Hep-3", "Hep-4", "Hep-5","dHep"))
...
prot <- readRDS("/Users/hhampson/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Biostatistics Core Shared Drive/Liver project/Data/TODAY_proteomics.rds")
...
results_ast <- results_ast %>%
  mutate(sig=ifelse(fdr<0.05,"*",""))
# write.csv(results_ast,fs::path(dir.results,"TODAY_proteomics_AST_hvg.csv"))
...
results_ratio <- results_ratio %>%
  mutate(sig=ifelse(fdr<0.05,"*",""))
# write.csv(results_ratio,fs::path(dir.results,"TODAY_proteomics_ALT_AST_ratio_hvg.csv"))
...
data <- read.csv("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/NEBULA Results/TODAY_proteomics_ALT_hvg.csv")
data <- data %>% 
  dplyr::rename(LogFC=LogHR) %>% 
  dplyr::rename(Gene=EntrezGeneSymbol) %>% 
  mutate(dataset="ALT")
write.csv(data,"/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/NEBULA Results/Formatted_TODAY_proteomics_ALT_hvg.csv")

data <- read.csv("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/NEBULA Results/TODAY_proteomics_AST_hvg.csv")
data <- data %>% 
  dplyr::rename(LogFC=LogHR) %>% 
  dplyr::rename(Gene=EntrezGeneSymbol) %>% 
  mutate(dataset="AST")
write.csv(data,"/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/NEBULA Results/Formatted_TODAY_proteomics_AST_hvg.csv")
```
...
file_paths <- c(
  ...
  "/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/NEBULA Results/Formatted_TODAY_proteomics_ALT_hvg.csv",
  "/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver project/NEBULA Results/Formatted_TODAY_proteomics_AST_hvg.csv"
)
