...
dir.dat <- c("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Kidney scRNAseq Project/Data")
# dir.dat2 <- c("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/scRNA")
dir.results <- c("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Kidney scRNAseq Project/Organoid Results/Organoid Results 02_12")
...
# 
# # Output directory
# # dir.gsea <- "GSEA_T2DvsLC_Reactome"
...
```{r}
#Load Biopsy Data
so_kpmp_sc <- readRDS(fs::path(dir.dat,"cleaned_biopsy.rds"))

#Load Oragnoid Data
so_combined <- readRDS(fs::path(dir.dat,"annotated_cleaned_organoid.rds"))

#Load Meta Data
harm_meta_data2 <- read.csv(fs::path(dir.dat,"harmonized_dataset.csv"))

#Load Pathway Databases
# Read in the Reactome .gmt file
reactome_sets <- gmtPathways(fs::path(dir.dat,"gmt_files","c2.cp.reactome.v2024.1.Hs.symbols.gmt"))
```
...
# write.csv(top_40_markers,"combined_gluc_top_40_markers_res_05_clusters_19_outlier_removed.csv")
...
# png("dotplot_avg_exp_top40_gluc_combined_cluster18_res05_outlier_removed.png",res=300,width=10000,height=2500)
# print(dotplot_40)
# dev.off()
...
# Save the plot
ggsave("UMAP_celltype_abbreviated_v2.png", width = 10, height = 10, dpi = 300)
...
print(p)
ggsave("UMAP_celltype_bold_labels_v2.png", p, width = 10, height = 10, dpi = 300)
...
write.csv(abbrev_legend, "celltype_abbreviations_legend_corrected_v2.csv", row.names = FALSE)
...
# Save as text
writeLines(abbrev_legend_formatted$Legend, "celltype_abbreviations_legend_corrected_v2.txt")
...
# Save the plot
ggsave("UMAP_celltype_with_full_legend_v2.png", p, width = 16, height = 8, dpi = 300)
...
png(file.path("Barchart_Celltypes_by_Group.png"), 
...
# Save results
write.csv(propeller_group, "Propeller_T2DvsLC_results.csv")
...
for (cell_name in celltypes) {
  ...
  # Save volcano plot
  plot_file <- file.path(paste0("Plot_Organoid_NEBULA_", cell_name_clean, 
...
for (cell_name in celltypes) {
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  input_file <- file.path(paste0("Results?NEBULA_T2DvsLC_", cell_name_clean, 
                                 "_unadjusted_2000.csv"))
  ...
  # Save the plot
  ggsave("DE_genes_by_celltype_barplot.png", 
...
for (cell_name in celltypes) {
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  input_file <- file.path(paste0("Results?NEBULA_T2DvsLC_", 
...
# Output directory
dir.gsea.kegg <- "GSEA_T2DvsLC_KEGG"
...
# Save plot
ggsave("DiD_CategoryCounts_StackedBar_AllCelltypes.png", 
...
write.csv(summary_table, 
          "DiD_CategoryCounts_Summary.csv", 
          row.names = FALSE)
...
# Save plot
ggsave("DiD_SignificantOnly_StackedBar_AllCelltypes.png", 
...
write.csv(summary_table, 
          "DiD_SignificantOnly_Summary.csv", 
          row.names = FALSE)
...
pdf("DiD_Volcano_GSEA_AllCelltypes.pdf", 
    width = 22, height = 10)
...
# Open PDF
pdf("DiD_DGSEA_Comparative_AllCelltypes.pdf", 
    width = 22, height = 10)
...
# Open PDF - one page per cell type
pdf("T2DvsLC_Volcano_GSEA_AllCelltypes.pdf", width = 22, height = 10)
...
so_combined$group <- factor(so_combined$group)
so_combined$group <- relevel(so_combined$group, ref = "Lean Control")
...
png(fs::path(dir.results,"Barchart_Celltypes_by_Group_Treatment.png"), 
...
cell_type_abbrev <-c("DMus","EEC","ProlPod","MesV")

# Prepare the metadata
props_data <- so_combined@meta.data %>%
  dplyr::select(record_id, group, treatment, cell_type_abbrev) %>%
  mutate(group_treatment = paste(group, treatment, sep = "_"))

# Get counts per record_id per cell type per condition
props_df <- table(props_data$record_id, 
                  props_data$cell_type_abbrev,
                  props_data$group_treatment)

# Convert to format for propeller
props_list <- lapply(1:dim(props_df)[3], function(i) props_df[,,i])
names(props_list) <- dimnames(props_df)[[3]]

# Create sample metadata
sample_info <- props_data %>%
  dplyr::distinct(record_id, group, treatment, group_treatment) %>%
  dplyr::arrange(record_id)

# Create design matrix
design <- model.matrix(~ 0 + group_treatment, data = sample_info)

# Run propeller for overall comparison across all 4 groups
propeller_results <- propeller(
  clusters = props_data$cell_type_abbrev,
  sample = props_data$record_id,
  group = props_data$group_treatment,
  robust = TRUE,
  trend = FALSE
)
...
write.csv(propeller_results, 
          fs::path(dir.results, "Propeller_CellType_Proportions_4Groups.csv"),
          row.names = TRUE)

# For pairwise comparisons, we need to run propeller separately for each comparison

# 1. T2D vs LC at 15mM
props_15mM <- props_data %>% dplyr::filter(treatment == "15mM")
propeller_T2D_vs_LC_15mM <- propeller(
  clusters = props_15mM$cell_type_abbrev,
  sample = props_15mM$record_id,
  group = props_15mM$group,
  robust = TRUE,
  trend = FALSE
)
propeller_T2D_vs_LC_15mM$comparison <- "T2D_vs_LC_15mM"

# 2. T2D vs LC at 30mM
props_30mM <- props_data %>% dplyr::filter(treatment == "30mM")
propeller_T2D_vs_LC_30mM <- propeller(
  clusters = props_30mM$cell_type_abbrev,
  sample = props_30mM$record_id,
  group = props_30mM$group,
  robust = TRUE,
  trend = FALSE
)
propeller_T2D_vs_LC_30mM$comparison <- "T2D_vs_LC_30mM"

# 3. Effect of glucose in LC (30mM vs 15mM)
props_LC <- props_data %>% dplyr::filter(group == "LC")
propeller_Glucose_LC <- propeller(
  clusters = props_LC$cell_type_abbrev,
  sample = props_LC$record_id,
  group = props_LC$treatment,
  robust = TRUE,
  trend = FALSE
)
propeller_Glucose_LC$comparison <- "Glucose_effect_LC"

# 4. Effect of glucose in T2D (30mM vs 15mM)
props_T2D <- props_data %>% dplyr::filter(group == "T2D")
propeller_Glucose_T2D <- propeller(
  clusters = props_T2D$cell_type_abbrev,
  sample = props_T2D$record_id,
  group = props_T2D$treatment,
  robust = TRUE,
  trend = FALSE
)
propeller_Glucose_T2D$comparison <- "Glucose_effect_T2D"

# Combine all pairwise results
propeller_contrasts <- bind_rows(
  propeller_T2D_vs_LC_15mM,
  propeller_T2D_vs_LC_30mM,
  propeller_Glucose_LC,
  propeller_Glucose_T2D
)

# Save contrast results
write.csv(propeller_contrasts, 
          fs::path(dir.results, "Propeller_CellType_Proportions_Contrasts.csv"),
          row.names = FALSE)

# Print significant results (FDR < 0.05)
cat("\nSignificant cell type proportion differences (FDR < 0.05):\n")
sig_results <- propeller_contrasts %>%
  dplyr::filter(FDR < 0.05) %>%
  dplyr::arrange(comparison, FDR)
print(sig_results)

# Create visualization of propeller results
library(pheatmap)

# Reshape for heatmap
pval_wide <- propeller_contrasts %>%
  dplyr::select(comparison, Clusters, FDR) %>%
  tidyr::pivot_wider(names_from = comparison, values_from = FDR) %>%
  tibble::column_to_rownames("Clusters")

# Convert to -log10(FDR) for better visualization
logp_matrix <- -log10(as.matrix(pval_wide))
logp_matrix[logp_matrix > 10] <- 10  # Cap very significant values

png(fs::path(dir.results, "Propeller_Heatmap_CellType_Proportions.png"),
    res = 300, width = 2500, height = 2000)
pheatmap(logp_matrix,
         cluster_rows = TRUE,
         cluster_cols = FALSE,
         color = colorRampPalette(c("white", "orange", "red"))(100),
         breaks = seq(0, 10, length.out = 101),
         main = "Cell Type Proportion Differences\n(-log10 FDR)",
         angle_col = 45)
dev.off()

```
### ii. Analysis
####1. Untargeted Gene Set
```{r}
...
ggsave("DE_genes_by_celltype_interaction_barplot.png", 
       plot = p_interaction,
       width = 8, height = 6, dpi = 300)
...
ggsave("DE_genes_by_celltype_groupType.2.Diabetes_barplot.png", 
       plot = p_group,
       width = 8, height = 6, dpi = 300)
...
```{r}
# Get cell types to analyze
# celltypes <- c("EEC","ProlPod","MesV","DT")
celltypes <- c("DT")
...
for (cell_name in celltypes) {
  ...
  # Save results with all coefficients
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  output_file <- file.path(fs::path(dir.results,"Interaction Model", 
                                    paste0("NEBULA_T2DvsLC_interaction_", cell_name_clean, 
                                           "_2000.csv")))
  write.csv(full_results, output_file, row.names = FALSE)
...
#                 vjust = 1, hjust = 1, size = 3, check_overlap = TRUE, color = "black")
#     
#     plot_file_group <- file.path("//Interaction Model", 
#                                  paste0("Plot_Organoid_NEBULA_", cell_name_clean, 
#                                         "_T2DvsLC_interaction_maineffect_2000.png"))
#     png(plot_file_group, width = 3000, height = 2100, res = 300)
#     print(volcano_plot_group)
...
#                 vjust = 1, hjust = 1, size = 3, check_overlap = TRUE, color = "black")
#     
#     plot_file_interaction <- file.path("//Interaction Model", 
#                                        paste0("Plot_Organoid_NEBULA_", cell_name_clean, 
#                                               "_T2DvsLC_interaction_effect_2000.png"))
#     png(plot_file_interaction, width = 3000, height = 2100, res = 300)
#     print(volcano_plot_interaction)
...
for (cell_name in celltypes) {
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  input_file <- file.path(fs::path(dir.dat,"Interaction Model", 
                                   paste0("NEBULA_T2DvsLC_interaction_", cell_name_clean, 
                                          "_unadjusted_2000.csv")))
...
for (cell_name in celltypes) {
  ...
  # Get counts for HVGs and metadata
  counts_hvg <- round(GetAssayData(so_celltype, layer = "counts"))
  genes_list <- hvgs[1:20]
  meta_all <- so_celltype@meta.data
  
  # Set up model matrix for ALL genes at once
  formula <- as.formula("~ group + treatment + group:treatment")
  pred_all <- model.matrix(formula, data = meta_all)
  library_all <- meta_all$pooled_offset
  
  # Sort by record_id
  sort_order <- order(meta_all$record_id)
  counts_sorted <- counts_hvg[genes_list, sort_order]
  meta_sorted <- meta_all[sort_order, ]
  pred_sorted <- pred_all[sort_order, , drop = FALSE]
  library_sorted <- library_all[sort_order]
  
  # Create data structure
  data_nebula <- list(
    count = counts_sorted,
    id = meta_sorted$record_id,
    pred = pred_sorted,
    offset = library_sorted
  )
  
  cat("Running NEBULA on", length(genes_list), "genes...\n")
  start_time <- Sys.time()
  
  # Run NEBULA on ALL genes at once
  nebula_result <- nebula(
    count = data_nebula$count,
    id = data_nebula$id,
    pred = data_nebula$pred,
    ncore = 10,
    reml = TRUE,
    model = "NBLMM",
    output_re = FALSE,
    covariance = TRUE,  # Request covariance matrix
    offset = data_nebula$offset
  )
  ...
  # Extract summary
  nebula_summary <- as.data.frame(nebula_result$summary)
  nebula_summary$Gene <- rownames(nebula_summary)
  
  # Now compute contrasts for each gene
  # Treatment effect in LC = beta_treatment
  # Treatment effect in T2D = beta_treatment + beta_interaction
  
  cat("Computing contrasts...\n")
  
  # Get coefficient names
  coef_names <- colnames(pred_sorted)
  treat_idx <- which(coef_names == "treatment30")
  int_idx <- which(grepl("treatment30", coef_names) & grepl(":", coef_names))
  
  cat("Treatment coefficient index:", treat_idx, "\n")
  cat("Interaction coefficient index:", int_idx, "\n")
  
  # Extract coefficients and covariance for each gene
  contrast_results <- map_dfr(genes_list, function(g) {
    
    # Get gene-specific results
    gene_row <- which(nebula_summary$Gene == g)
    
    if (length(gene_row) == 0) {
      return(data.frame(
        Gene = g,
        logFC_Treatment_LC = NA,
        logFC_Treatment_T2D = NA,
        SE_Treatment_T2D = NA,
        p_Treatment_T2D = NA
      ))
    }
    
    # Extract logFC values from summary
    treat_col <- paste0("logFC_", coef_names[treat_idx])
    int_col <- paste0("logFC_", coef_names[int_idx])
    
    logFC_treat <- nebula_summary[gene_row, treat_col]
    logFC_int <- nebula_summary[gene_row, int_col]
    
    # LC effect = treatment coefficient
    logFC_LC <- logFC_treat
    
    # T2D effect = treatment + interaction
    logFC_T2D <- logFC_treat + logFC_int
    
    # Get SEs for approximate variance
    se_treat_col <- paste0("se_", coef_names[treat_idx])
    se_int_col <- paste0("se_", coef_names[int_idx])
    
    se_treat <- nebula_summary[gene_row, se_treat_col]
    se_int <- nebula_summary[gene_row, se_int_col]
    
    # Approximate SE (conservative - assumes independence)
    se_T2D <- sqrt(se_treat^2 + se_int^2)
    
    # Z-test and p-value
    z_T2D <- logFC_T2D / se_T2D
    p_T2D <- 2 * pnorm(-abs(z_T2D))
    
    data.frame(
      Gene = g,
      logFC_Treatment_LC = logFC_LC,
      logFC_Treatment_T2D = logFC_T2D,
      SE_Treatment_T2D = se_T2D,
      p_Treatment_T2D = p_T2D
    )
  })
  
  # Merge with original summary
  full_results <- nebula_summary %>%
    left_join(contrast_results, by = "Gene")
  ...
  # FDR correction
  full_results$fdr_Treatment_LC <- 
    p.adjust(full_results[[paste0("p_", coef_names[treat_idx])]], method = "fdr")
  full_results$fdr_Treatment_T2D <- 
    p.adjust(full_results$p_Treatment_T2D, method = "fdr")
  
  # Save results
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  output_file <- file.path(dir.results, "2. Interaction Model",
                          paste0("NEBULA_interaction_", cell_name_clean, "_contrasts.csv"))
...
celltypes <- c("ProlPod","EEC","DT","MesV")   # modify if needed

pdf(file.path(fs::path(dir.results,"2. Interaction Model",
                       "DiD_Volcano_with_T2D_LC_Heatmap_AllCelltypes.pdf")),
    width = 14, height = 10)
...
for (cell_name in celltypes) {
  ...
  input_file <- file.path(
    fs::path(dir.results,"2. Interaction Model",
             paste0("NEBULA_T2DvsLC_interaction_",
                    cell_name_clean,
                    "_2000_with_contrast.csv"))
  )
  ...
  plot_df <- full_results %>%
    mutate(
      ...
      Glucose_T2D = logFC_Treatment_T2D,
      Glucose_LC  = logFC_Treatment_LC
    )
  ...
  volcano <- ggplot(plot_df,
                    aes(x = DiD, y = DiD_pval10)) +
    geom_point(aes(color = DiD_fdr < 0.05),
               alpha = 0.7, size = 2) +
    scale_color_manual(values = c("gray75", "#D32F2F")) +
    geom_vline(xintercept = 0, linetype = "dashed") +
    theme_minimal() +
    labs(
      title = paste0(cell_name, " – Interaction (DiD)"),
      x = "Difference-in-Differences\n(T2D glucose − LC glucose)",
      y = "-log10(p-value)"
    ) +
    theme(
  ...
  heat_df <- plot_df %>%
    filter(DiD_fdr < 0.05) %>%
    arrange(DiD) %>%
    dplyr::select(Gene, DiD, Glucose_T2D, Glucose_LC)
  
  # If too many genes, limit to top 50 strongest interactions
  if (nrow(heat_df) > 50) {
    heat_df <- heat_df %>%
      slice_max(abs(DiD), n = 50)
  }
  ...
  if (nrow(heat_df) > 0) {
    ...
    heat_long$Effect <- factor(
      heat_long$Effect,
      levels = c("DiD", "Glucose_T2D", "Glucose_LC")
    )
    
    heatmap_plot <- ggplot(heat_long,
                           aes(x = Effect,
                               y = factor(Gene, levels = heat_df$Gene),
                               fill = logFC)) +
      geom_tile() +
    ...
    combined_plot <- volcano + heatmap_plot +
      plot_layout(widths = c(2, 1))
    
  } else {
    
    combined_plot <- volcano +
      ggtitle(paste0(cell_name, " – No significant interaction genes"))
  }
...
pdf(file.path(fs::path(dir.results,"2. Interaction Model",
                       "DiD_GSEA_fgsea_Reactome_pval0.05_AllCelltypes.pdf")),
    width = 14, height = 10)
...
for (cell_name in celltypes) {
  ...
  # ===============================
  # PREP RANKING VECTOR (DiD logFC)
  # ===============================
  
  rank_df <- full_results %>%
    dplyr::select(Gene,
                  DiD = logFC_groupType.2.Diabetes.treatment30) %>%
    filter(!is.na(DiD)) %>%
    distinct() %>%
  ...
  if (nrow(sig_pathways) == 0) {
    plot.new()
    title(paste0(cell_name, " – No significant Reactome pathways (p < 0.05)"))
    next
  }
  ...
  barplot_plot <- ggplot(top_pathways,
                         aes(x = reorder(gs_name, NES),
                             y = NES,
                             fill = NES)) +
    geom_col() +
    coord_flip() +
    ...
    theme_minimal() +
    labs(
      title = paste0(cell_name, " – Reactome GSEA (DiD ranking)"),
      x = "",
  ...
  top_pathway_name <- sig_pathways$gs_name[1]
  
  enrichment_plot <- plotEnrichment(
    reactome_list[[top_pathway_name]],
    ranks
  ) +
    labs(title = paste0(cell_name, " – ", top_pathway_name))
...
# Cell types to analyze
# celltypes <- c("Prolif","DMus","EEC","Neur","DPEC","DT","Mes","MesV","DKE","PEC_N","Strom","MusV","NsKE","ProlPod")
celltypes <- c("ProlPod","EEC","DT","MesV")
# Open PDF
pdf(fs::path(dir.results,"2. Interaction Model","T2DvsLC_DiD_FourCategory_Volcano_AllCelltypes.pdf"), 
    width = 14, height = 10)
...
for (cell_name in celltypes) {
  ...
  #                         paste0("NEBULA_T2DvsLC_interaction_", cell_name_clean, 
  #                                "_unadjusted_2000.csv"))
  input_file <- file.path(fs::path(dir.results,"2. Interaction Model", 
                                   paste0("NEBULA_T2DvsLC_interaction_", cell_name_clean, 
                                          "_2000.csv")))
...
# Open PDF - one page per cell type
pdf(paste0("GlucoseConc_Volcano_GSEA_", effect_type, "_AllCelltypes.pdf"), 
    width = 22, height = 10)
...
for (cell_name in celltypes) {
  ...
  input_file <- file.path(paste0("NEBULA_GlucoseConc_", 
...
pdf("T2DvsLC_Interaction_CategoryCounts_AllCelltypes.pdf", 
    width = 14, height = 8)
...
pdf("T2DvsLC_Interaction_Volcano_Categorized_AllCelltypes.pdf", 
    width = 22, height = 10)
...
write.csv(summary_table, 
          "T2DvsLC_Interaction_CategorySummary.csv", 
          row.names = FALSE)
...
# Create stacked barchart
png("Barchart_Celltypes_by_Treatment.png", 
    res = 300, width = 2000, height = 2000)
...
for (cell_name in celltypes) {
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  input_file <- file.path(paste0("NEBULA_30vs15mM_", cell_name_clean, 
                                 "_unadjusted_2000.csv"))
  # Check if file exists before reading
  if (file.exists(input_file)) {
    file <- read.csv(input_file)
  ...
  # Save the plot
  ggsave("DE_genes_by_celltype_barplot_treatment.png", 
         width = 8, height = 6, dpi = 300)
}
...
# Open PDF - one page per cell type
pdf("30vs15_Volcano_GSEA_AllCelltypes.pdf", width = 22, height = 10)

for (cell_name in celltypes) {
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  input_file <- file.path(paste0("NEBULA_30vs15mM_", 
                                 cell_name_clean, "_unadjusted_2000.csv"))
...
# 1. Aggregate raw counts by cell type and sample
pseudo_bulk <- AggregateExpression(
  so_combined,
  assays = "RNA",
  return.seurat = FALSE,
  group.by = c("cell_type", "record_id")
)
counts_mat <- pseudo_bulk$RNA

# 2. Parse column names and merge metadata
col_info <- data.frame(group = colnames(counts_mat))
col_info$cell_type <- sapply(strsplit(col_info$group, "_"), `[`, 1)
col_info$record_id <- sapply(strsplit(col_info$group, "_"), function(x) paste(x[-1], collapse = "_"))

# Extract unique sample-level metadata from the Seurat object
sample_meta <- so_combined@meta.data %>%
  dplyr::select(record_id, group, any_of(c("sex", "age", "bmi", "ethnicity"))) %>%  # adjust columns as needed
  dplyr::distinct(record_id, .keep_all = TRUE)

# Now merge
col_info <- merge(col_info, sample_meta, by = "record_id")
rownames(col_info) <- col_info$group.x
col_info <- col_info %>% 
  dplyr::rename(group=group.x) %>% 
  dplyr::rename(diabetes=group.y)

# 3. Run lm() per cell type
results_list <- list()

for (ct in unique(col_info$cell_type)) {
  idx <- col_info$cell_type == ct
  sub_counts <- counts_mat[, col_info$group[idx]]
  sub_meta <- col_info[col_info$group[idx], ]
  
  # Normalize to CPM (or log-CPM)
  cpm <- t(t(sub_counts) / colSums(sub_counts)) * 1e6
  log_cpm <- log2(cpm + 1)
  ...
  res <- do.call(rbind, lapply(rownames(log_cpm), function(gene) {
    df <- data.frame(
      expr = as.numeric(log_cpm[gene, ]),
      group = sub_meta$condition  # T2D vs Control
    )
    fit <- lm(expr ~ group, data = df)
    s <- summary(fit)
    coefs <- s$coefficients
    # The group effect is the second row
    data.frame(
      gene = gene,
      logFC = coefs[2, "Estimate"],
...
# Create stacked barchart
png("Barchart_Celltypes_by_Group_Biopsy.png", 
    res = 300, width = 2000, height = 2000)
...
for (cell_name in celltypes) {
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  input_file <- file.path(paste0("NEBULA_Biopsy_T2DvsLC_", 
                                 cell_name_clean, "_unadjusted_2000.csv"))
...
# Save the plot
ggsave("DE_genes_by_celltype_barplot_Biopsy_T2DvsLC.png", 
...
# Open PDF - one page per cell type
pdf("Biopsy_T2DvsLC_Volcano_GSEA_AllCelltypes.pdf", width = 22, height = 10)
...
pdf("Comparative_Organoid_Biopsy_T2DvsLC_Volcano_GSEA.pdf", 
    width = 24, height = 20)
...
for (pair in comparative_pairs) {
  ...
  organoid_file <- file.path(paste0("Results?NEBULA_T2DvsLC_", 
  ...
  biopsy_file <- file.path(paste0("NEBULA_Biopsy_T2DvsLC_", 
...
png("Barchart_Celltypes_by_Diabetes_15_Glucose.png",res=300,width=2000,height=2000)
...
# Create the stacked barchart
png("Barchart_Celltypes_by_Diabetes_30_Glucose.png",res=300,width=2000,height=2000)
ggplot(celltype_counts, aes(x = group, y = proportion, fill = celltype2)) +
  geom_col(width = 0.6) +
  scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
  theme_minimal() +
