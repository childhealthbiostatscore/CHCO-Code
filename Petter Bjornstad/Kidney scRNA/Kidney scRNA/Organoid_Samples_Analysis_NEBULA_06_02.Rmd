...
sc_org <- annotate_celltypes(sc_org, marker_genes, min_score_diff = 0.1)
...
# saveRDS(sc_org, "sc_org_annotated.rds")
# ggsave("celltype_annotation_umap.pdf", plots$annotation_plot, width = 12, height = 8)

# Check annotation quality
cat("\nAnnotation Quality Check:\n")
cat("Total cells:", ncol(sc_org), "\n")
cat("Annotated cells:", sum(sc_org@meta.data$celltype_annotation != "Unknown"), "\n")
cat("Unknown cells:", sum(sc_org@meta.data$celltype_annotation == "Unknown"), "\n")
cat("Annotation rate:", round(100 * sum(sc_org@meta.data$celltype_annotation != "Unknown") / ncol(sc_org), 2), "%\n")

# Optional: Manual refinement based on additional criteria
# You can further refine annotations by examining specific markers
# Example:
# FeaturePlot(sc_org, features = c("CUBN", "UMOD", "SLC12A3", "EMCN"), ncol = 2)

cat("Cell type annotation completed!\n")
cat("Results stored in sc_org@meta.data$celltype_annotation\n")
cat("Module scores and confidence metrics also available in metadata\n")

# Check specific kidney markers
FeaturePlot(sc_org, features = c("CUBN", "UMOD", "SLC12A3", "EMCN"), ncol = 2)

#Try exteracting marker genes from seurat object in biopsies
# Example usage:
# Assuming you have a reference Seurat object called 'reference_seurat' 
# with cell type annotations in a column (e.g., 'celltype', 'seurat_clusters', etc.)

# Method 1: Extract from existing cell type annotations
# Replace 'reference_seurat' with your annotated Seurat object name
# Replace 'celltype_annotation' with your cell type column name
...
# Method 2: If you want to use cluster-based markers instead
# marker_results <- extract_top_markers(
...
# Save the marker genes for future use
# saveRDS(extracted_marker_genes, "extracted_marker_genes.rds")

# Optional: Export to CSV for manual review
# write.csv(marker_results$full_markers, "all_markers.csv", row.names = FALSE)
...
cat("Marker extraction functions loaded!\n")
cat("Use extract_top_markers() to get markers from your reference dataset\n")
