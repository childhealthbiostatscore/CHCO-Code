...
so_kpmp_sc$celltype_annotate <- case_when(
...
final_counts <- table(so_kpmp_sc$celltype_annotate)
print(final_counts)
...
sc_org_annotated <- annotate_celltypes(sc_org, marker_genes, min_score_diff = 0.1)
sc_org_annotated <- NormalizeData(sc_org_annotated)
sc_org_annotated <- ScaleData(sc_org_annotated, features = VariableFeatures(sc_org_annotated))
sc_org_annotated <- FindVariableFeatures(object = sc_org_annotated)
sc_org_annotated <- RunPCA(sc_org_annotated, features = VariableFeatures(object = sc_org_annotated),assay="RNA")
ElbowPlot(sc_org_annotated)

# # Find neighbors and clusters (again using integrated data)
sc_org_annotated <- FindNeighbors(sc_org_annotated, assay = "RNA", dims = 1:20)
sc_org_annotated <- FindClusters(sc_org_annotated, resolution = 0.5)

# Perform UMAP and tSNE
#sc_org_annotated <- RunTSNE(sc_org_annotated, dims = 1:20)
sc_org_annotated@reductions
DimPlot(sc_org_annotated, reduction = "umap", raster = T)

DimPlot(sc_org_annotated, reduction = "umap", group.by = "celltype_annotation", raster = T)+
  ggtitle(label="Low-Resolution KPMP Celltypes")
DimPlot(sc_org_annotated, reduction = "pca", group.by = "celltype_annotation", raster = T)+
  ggtitle(label="Low-Resolution KPMP Celltypes")
# DimPlot(sc_org_annotated, reduction = "harmony", group.by = "celltype_annotation_dual", raster = T)+
#   ggtitle(label="Low-Resolution KPMP Celltypes")
# DimPlot(sc_org_annotated, reduction = "pca_for_harmony", group.by = "celltype_annotation_dual", raster = T)+
#   ggtitle(label="Low-Resolution KPMP Celltypes")
# DimPlot(sc_org_annotated, reduction = "pca_for_harmony", group.by = "celltype_annotation_dual", raster = T)+
#   ggtitle(label="Low-Resolution KPMP Celltypes")
# # Generate plots
# plots <- plot_celltype_results(sc_org, reduction = "umap")
# 
# # Display plots
...
validation_results <- validate_markers(sc_org, marker_df)
...
sc_org_annotated <- annotation_results$seurat_object
...
# Perform UMAP and tSNE
# so_liver_sc <- RunTSNE(sc_org_annotated, dims = 1:20)
sc_org_annotated@reductions
DimPlot(sc_org_annotated, reduction = "umap", raster = F)

DimPlot(sc_org_annotated, reduction = "umap", group.by = "celltype_annotation", raster = FALSE)+
  ggtitle(label="Low Resolution KPMP Celltypes")
DimPlot(sc_org_annotated, reduction = "pca", group.by = "celltype_annotation", raster = FALSE)+
  ggtitle(label="Low Resolution KPMP Celltypes")
DimPlot(sc_org_annotated, reduction = "harmony", group.by = "celltype_annotation", raster = FALSE)+
  ggtitle(label="Low Resolution KPMP Celltypes")
DimPlot(sc_org_annotated, reduction = "pca_for_harmony", group.by = "celltype_annotation", raster = FALSE)+
  ggtitle(label="Low Resolution KPMP Celltypes")
DimPlot(sc_org_annotated, reduction = "pca_for_harmony", group.by = "celltype_annotation", raster = FALSE)+
  ggtitle(label="Low Resolution KPMP Celltypes")
...
# Step 3: Extract results
sc_org_annotated <- dual_results$seurat_object
...
DimPlot(sc_org_annotated, reduction = "umap", group.by = "celltype_annotation_dual", raster = T)+
  ggtitle(label="High-Resolution KPMP Celltypes")
DimPlot(sc_org_annotated, reduction = "pca", group.by = "celltype_annotation_dual", raster = T)+
  ggtitle(label="High-Resolution KPMP Celltypes")
DimPlot(sc_org_annotated, reduction = "harmony", group.by = "celltype_annotation_dual", raster = T)+
  ggtitle(label="High-Resolution KPMP Celltypes")
DimPlot(sc_org_annotated, reduction = "pca_for_harmony", group.by = "celltype_annotation_dual", raster = T)+
...
# Annotate cell types (adjust min_score_diff as needed)
sc_org_annotated <- annotate_celltypes(sc_org, organoid_markers_simple, min_score_diff = 0.1)
...
DimPlot(sc_org_annotated, reduction = "umap", group.by = "celltype_annotation", raster = FALSE)+
  ggtitle(label="Low Resolution Organoid Markers Celltypes")
...
###############################################################################
#6. From FindMarkers of KPMP celltypes in SingleCell - Do again without filtering down
marker_results <- extract_top_markers(
  reference_seurat = so_kpmp_sc,
  cell_type_column = "celltype_annotate",  # or "seurat_clusters", "cell_type", etc.
  top_n = 20,
  min_pct = 0.25,
  logfc_threshold = 0.25
)

# Extract the marker gene list for annotation
extracted_marker_genes <- marker_results$marker_genes

# View the results
print("Extracted marker genes:")
print(extracted_marker_genes)

# View marker quality
plots <- plot_marker_quality(marker_results)
print(plots$count_plot)

# Now use these markers with the annotation function from previous code:
sc_org <- annotate_celltypes(sc_org, extracted_marker_genes, min_score_diff = 0.1)
sc_org$celltype_annotation
...
## c. Explore Distribution & Zero-Inflation
### i. Biopsy Data
```{r}
...
DimPlot(sc_org, reduction = "umap", group.by = "celltype_annotation", raster = FALSE)
DimPlot(sc_org, reduction = "pca", group.by = "celltype_annotation", raster = FALSE)
