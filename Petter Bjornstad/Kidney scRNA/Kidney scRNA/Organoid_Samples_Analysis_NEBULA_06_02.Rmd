...
head(gene_annotations$NA.)  # Column with gene names you want to use
gene_annotations$Gene <- gene_annotations$NA.
gene_annotations <- gene_annotations[,-c(2:4)]

# Reorder gene annotations to match Seurat feature order
gene_annotations_ordered <- gene_annotations[match(rownames(sc_org[["RNA"]]), gene_annotations$input), ]
# Extract gene names from column named "NA"
gene_symbols <- make.unique(gene_annotations_ordered$Gene)

# Apply new names to all assay slots
match(rownames(sc_org@assays$RNA@counts),gene_annotations_ordered$input)
match(rownames(sc_org@assays$RNA@scale.data),gene_annotations_ordered$input)
rownames(sc_org@assays$RNA@counts) <- gene_symbols
rownames(sc_org@assays$RNA@data) <- gene_symbols
if (!is.null(sc_org@assays$RNA@scale.data)) {
  rownames(sc_org@assays$RNA@scale.data) <- gene_symbols
}
...
#Low resolution human KPMP marker genes annotation
marker.genes <- list(
  PT = c("CUBN", "LRP2"),
  DTL = c("AQP1", "ID1"),
  ATL = c("CLDN3", "CLCNKA"),
  ...
  EC = c("EMCN"),
  Immune = c("PTPRC"),
  Interstitial = c("ACTA2", "RGS5")
)

# Add module scores
for (celltype in names(marker.genes)) {
  sc_org <- AddModuleScore(sc_org, features = list(marker.genes[[celltype]]),
                               name = paste0(celltype, "_Score"))
}

# Extract scores from metadata
score_cols <- grep("_Score1", colnames(sc_org@meta.data), value = TRUE)

# Assign cell type with highest score
sc_org$predicted_celltype <- apply(sc_org@meta.data[, score_cols], 1, function(x) {
  names(marker.genes)[which.max(x)]
})

DimPlot(sc_org, group.by = "predicted_celltype", label = TRUE, repel = TRUE)

# Map dominant predicted type per cluster
cluster_annotations <- sc_org@meta.data %>%
...
cluster_to_celltype <- setNames(cluster_annotations$predicted_celltype, cluster_annotations$seurat_clusters)

# Assign to object metadata
sc_org$cluster_celltype <- cluster_to_celltype[as.character(sc_org$seurat_clusters)]

DimPlot(seurat_obj, group.by = "cluster_celltype", label = TRUE, repel = TRUE)
