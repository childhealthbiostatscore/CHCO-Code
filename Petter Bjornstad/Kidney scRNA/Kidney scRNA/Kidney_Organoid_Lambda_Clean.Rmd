...
#Summarize the celltypes as follows: 
# |------------------------------|-------------------------------------------------------------------------------------------|
# | Kidney Epithelium            | developing_kidney_epithelium, developing_kidney_epithelium2, non_specific_kidney_epithelium |
# | Mesangial & VSMC             | mesangial_cells_smooth_muscle, developing_muscle, mesangial_cells_vsmcs, muscle_vsmc      |
# | Proliferating Cells          | proliferating_mitotic_cells, proliferating_podocyte_epithelial_cells                      |
# | Mixed / Review Required      | early_endothelial_cells_vsmc, mixed_pec_general_neural                                    |
# | Distal Tubular Epithelium    | distal_tubular_epithelial_cells_                                                          |
# | Stromal                      | stromal                                                                                   |
# | Neural                       | neural                                                                                    |
# | Developing PEC               | developing_pec
```
...
#5. Reannotate Clusters
```{r}
#Keep only Strom, DKE and DKE2, DPEC, EEC and Mes
#Filter so to these clusters
#First, visualize the umap of all the annotated clusters
print(paste("Number of clusters:", length(unique(Idents(so_combined)))))
print(table(Idents(so_combined)))  # Show distribution of cells per cluster
Idents(so_combined) <- "cell_type_abbrev"
...
DimPlot(so_combined, reduction = "umap", raster = T, group.by = "record_id")
DimPlot(so_combined, reduction = "umap", raster = T, group.by = "cell_type_abbrev")
...
png("/home/hhampson/Results/Organoid_UMAP_Annotation1.png",width=2000,height=2000,res=300)
print(p)
dev.off()

Idents(so_combined) <- "record_id"
p <- DimPlot(so_combined, reduction = "umap", raster = TRUE, label = FALSE) +
  labs(title = "Cell Type Annotations",
       color = "Cell Type")+ theme(legend.position = "none")
...
png("/home/hhampson/Results/Organoid_UMAP_Annotation1_participants.png",width=2000,height=2000,res=300)
print(p)
dev.off()

#Fitler out & recluster
#Celltypes to keep: Strom, DKE and DKE2, DPEC, EEC and Mes
unique(so_combined$cell_type_abbrev)
so_combined2 <- subset(so_combined, subset = cell_type_abbrev %in% c("Strom", "DKE", "DKE2", "DPEC", "EEC", "Mes"))

# 1. Re-normalize and find variable features on the subset
so_combined2 <- NormalizeData(so_combined2)
so_combined2 <- FindVariableFeatures(so_combined2, selection.method = "vst", nfeatures = 2000)
so_combined2 <- ScaleData(so_combined2)

# 2. Re-run PCA
so_combined2 <- RunPCA(so_combined2, npcs = 30)

# 3. Check elbow plot to choose dims
ElbowPlot(so_combined2, ndims = 30)

# 4. Re-cluster (adjust dims and resolution as needed)
so_combined2 <- FindNeighbors(so_combined2, dims = 1:20)
so_combined2 <- FindClusters(so_combined2, resolution = 0.5)

# 5. Re-run UMAP
so_combined2 <- RunUMAP(so_combined2, dims = 1:20)

# 6. Visualize
p1 <- DimPlot(so_combined2, reduction = "umap", label = TRUE, group.by = "seurat_clusters")
p2 <- DimPlot(so_combined2, reduction = "umap", label = TRUE, group.by = "cell_type_abbrev")  # old annotations
p3 <- DimPlot(so_combined2, reduction = "umap", label = TRUE, group.by = "record_id")

png("/home/hhampson/Results/Organoid_UMAP_Annotation2_seurat_clusters.png",width=2200,height=2000,res=300)
print(p1)
dev.off()
png("/home/hhampson/Results/Organoid_UMAP_Annotation2_old_celltypes.png",width=2200,height=2000,res=300)
print(p2)
dev.off()
png("/home/hhampson/Results/Organoid_UMAP_Annotation2_participants.png",width=2200,height=2000,res=300)
print(p3)
dev.off()
...
# 8. View top markers per cluster
top_markers <- markers |>
  dplyr::group_by(cluster) |>
  dplyr::slice_max(avg_log2FC, n = 40)
