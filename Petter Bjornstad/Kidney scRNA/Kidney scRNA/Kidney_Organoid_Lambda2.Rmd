...
## b. Annotate Organoid Data from SE & Beno-approved annotations
```{r}
#Annotate cell types
#Start by clustering
...
# Save the plot
# ggsave("/home/hhampson/Results/UMAP_celltype_abbreviated.png", width = 10, height = 10, dpi = 300)
...
print(p)
# ggsave("/home/hhampson/Results/UMAP_celltype_bold_labels.png", p, width = 10, height = 10, dpi = 300)
...
# write.csv(abbrev_legend, "/home/hhampson/Results/celltype_abbreviations_legend_corrected.csv", row.names = FALSE)
...
# Save as text
# writeLines(abbrev_legend_formatted$Legend, "/home/hhampson/Results/celltype_abbreviations_legend_corrected.txt")
...
# Save the plot
# ggsave("/home/hhampson/Results/UMAP_celltype_with_full_legend.png", p, width = 16, height = 8, dpi = 300)
...
## c. Annotate Organoid Data - Bottom Up
...
# Get top 40 markers per cluster
top_40_markers <- get_top_markers(all_markers, top_n = 40)
write.csv(top_40_markers,"/home/hhampson/Results/combined_gluc_top_40_markers_res_02_clusters_8.csv")
...
png("/home/hhampson/Results/dotplot_avg_exp_top40_gluc_combined_cluster8_res02.png.png",res=300,width=10000,height=2500)
print(dotplot_40)
dev.off()

#Break dot plot down by sets of clusters 
...
png("/home/hhampson/Results/UMAP_combined_unlabeled_res_035_clusters11_removed_outlier.png",res=300,width=2000,height=2000)
DimPlot(so_combined, reduction = "umap", raster = T)+  ggtitle("Combined Treatment by Cluster",
          subtitle = "Res = 0.35, Clusters = 11")
dev.off()

png("/home/hhampson/Results/UMAP_combined_unlabeled_res_035_clusters11_removed_outlier_by_record_ID.png",res=300,width=2000,height=2000)
DimPlot(so_combined, reduction = "umap", raster = T, group.by="record_id")+  ggtitle("Combined Treatment by Record ID",
          subtitle = "Res = 0.35, Clusters = 11")
dev.off()
...
# Get top 20 markers per cluster
top_40_markers <- get_top_markers(all_markers, top_n = 40)
write.csv(top_40_markers,"/home/hhampson/Results/combined_gluc_top_40_markers_res_035_clusters_13.csv")
...
png("/home/hhampson/Results/dotplot_avg_exp_top40_gluc_combined_cluster13_res035.png.png",res=300,width=10000,height=2500)
print(dotplot_40)
dev.off()
...
# png("/home/hhampson/Results/UMAP_combined_unlabeled_res_05_clusters18_removed_outlier.png",res=300,width=2000,height=2000)
# DimPlot(so_combined, reduction = "umap", raster = T)+  ggtitle("Combined Treatment by Cluster",
#           subtitle = "Res = 0.5, Clusters = 18")
# dev.off()
# 
# png("/home/hhampson/Results/UMAP_combined_unlabeled_res_05_clusters18_removed_outlier_by_record_ID.png",res=300,width=2000,height=2000)
# DimPlot(so_combined, reduction = "umap", raster = T, group.by="record_id")+  ggtitle("Combined Treatment by Record ID",
#           subtitle = "Res = 0.5, Clusters = 18")
# dev.off()
...
# # Get top 20 markers per cluster
# top_20_markers <- get_top_markers(all_markers, top_n = 20)
# write.csv(top_20_markers,"/home/hhampson/Results/combined_gluc_top_20_markers_res_05_clusters_19_outlier_removed.csv")
# 
# # Create dot plot with top 10 markers (might be crowded)
# dotplot_20 <- create_marker_dotplot(so_combined, all_markers, top_n = 20,
#                                     title = "Top 20 Marker Genes by Cluster; Combined Glucose Treatment")
# print(dotplot_20)
# 
# png("/home/hhampson/Results/dotplot_avg_exp_top20_gluc_combined_cluster18_res05_outlier_removed.png",res=300,width=10000,height=2500)
# print(dotplot_20)
# dev.off()
...
# Create split dot plots - adjust clusters_per_plot as needed
# For 19 clusters split into ~4 plots, use 5 clusters per plot
dotplots_split <- create_marker_dotplot_split(
  so_combined, 
  all_markers, 
  top_n = 40,
  title = "Top 40 Marker Genes by Cluster; Combined Glucose Treatment",
...
# Print each plot individually (they'll be larger)
for (i in seq_along(dotplots_split)) {
  print(dotplots_split[[i]])
}

# Or save them to files with larger dimensions
for (i in seq_along(dotplots_split)) {
  ggsave(
    filename = paste0("dotplot_part_", i, ".png"),
    plot = dotplots_split[[i]],
    width = 16,
    height = 10,
    dpi = 300
  )
...
#Unlabeled cluster UMAP
png("/home/hhampson/Results/UMAP_combined_unlabeled_res_05_clusters18.png",res=300,width=2000,height=2000)
DimPlot(so_combined, reduction = "umap", raster = T)+ 
  ggtitle("Combined Diabetes Status + Combined Treatment Group",
          subtitle = "Res = 0.5, Clusters = 18")
dev.off()
...
#Label clusters with cluster number
#Unlabeled cluster UMAP
png("/home/hhampson/Results/UMAP_combined_labeled_res_05_clusters18.png",res=300,width=2000,height=2000)
DimPlot(so_combined, reduction = "umap", raster = T,label = T)+ 
  ggtitle("Combined Diabetes Status + Combined Treatment Group",
          subtitle = "Res = 0.5, Clusters = 18")
dev.off()
```
...
current_names <- rownames(so_combined)

# Option A: Keep just gene symbols, make unique if needed
gene_symbols <- str_extract(current_names, "^[^-]+") %>% str_trim()
new_names <- make.unique(gene_symbols, sep = "-")

# Option B: Keep full name but fix formatting (replace problematic characters)
# new_names <- str_replace_all(current_names, " - ", "_") %>%
#              str_replace_all("_", "-")  # preemptively do what Seurat would do

# Check for duplicates that would be created
if (any(duplicated(new_names))) {
  message(paste("Making", sum(duplicated(new_names)), "duplicate names unique"))
}

# Rebuild the assay with clean names
counts <- GetAssayData(so_combined, slot = "counts")
rownames(counts) <- new_names

# Create fresh assay and replace
so_combined[["RNA"]] <- CreateAssayObject(counts = counts)

# Now normalize - should work without warnings
so_combined <- NormalizeData(so_combined)
...
FeaturePlot(so_combined, 
            features = "NOX5",raster=T) +
  # scale_color_viridis_c() +
  ggtitle("NOX5 Expression")

FeaturePlot(so_combined, 
            features = "NOX5-1",raster=T) +
  # scale_color_viridis_c() +
  ggtitle("NOX5 Expression")

# Check the exact feature name after cleaning
grep("NOX5", rownames(so_combined), value = TRUE)

# Basic dotplot
DotPlot(so_combined, features = "NOX5") + 
  RotatedAxis()

# If you have multiple NOX5 entries (NOX5, NOX5-1, etc.)
nox5_features <- grep("^NOX5", rownames(so_combined), value = TRUE)
DotPlot(so_combined, features = nox5_features) + 
  RotatedAxis()
...
DotPlot(so_combined, features = "NOX5") + 
  ...
  ggtitle("NOX5 Expression Across Clusters")

nox5_1 <- grep("NOX5-1", rownames(so_combined), value = TRUE)
FeaturePlot(so_combined, features = nox5_1, raster = TRUE)
```
...
de_summary_treatment <- combined_results_interaction %>%
  filter(fdr_treatment30 < 0.05) %>%
  mutate(direction = ifelse(logFC_treatment30 > 0, "Positive", "Negative")) %>%
  group_by(cell_type, direction) %>%
  summarise(count = n(), .groups = "drop") %>%
  mutate(count = ifelse(direction == "Negative", -count, count))

# Create the plot for treatment30 main effect
p_treatment <- ggplot(de_summary_treatment, aes(x = count, y = reorder(cell_type, abs(count)), fill = direction)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("Negative" = "#A8C9D9", "Positive" = "#E89A9A")) +
  geom_vline(xintercept = 0, color = "black", size = 0.5) +
  geom_hline(yintercept = 9.5, linetype = "dotted", color = "black", size = 0.5) +
  labs(x = "Number of DE genes (Treatment: 30mM vs 15mM)", 
       y = NULL,
       fill = NULL) +
  theme_minimal() +
  theme(
...
ggsave("/home/hhampson/Results/DE_genes_by_celltype_treatment30_barplot.png", 
       plot = p_treatment,
       width = 8, height = 6, dpi = 300)
...
cat("\n=== Treatment30 Main Effect Summary ===\n")
...
for (cell_name in celltypes) {
  ...
  nebula_results_list <- foreach(g = genes_list, 
    ...
    tryCatch({
      ...
      # MODEL: comparing treatment (30mM vs 15mM)
  ...
  full_results$Variable <- "30mMvs15mM"
  full_results$Celltype <- cell_name
  ...
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  output_file <- file.path("/home/hhampson/Results", 
                           paste0("NEBULA_30mMvs15mM_", cell_name_clean, 
                                  "_unadjusted_2000.csv"))
  write.csv(full_results, output_file, row.names = FALSE)
  ...
  volcano_plot <- ggplot(full_results, aes(x = logFC, y = PValue10, color = color)) +
    ...
    theme_minimal() +
    labs(
      title = paste("30mM vs. 15mM Glucose Treatment -", cell_name),
      subtitle = "Organoids, Unadjusted (REML, Log Normal, Offset)",
      x = "Log Fold Change",
      y = "-log10(P-Value)",
      color = "FC Direction",
  ...
  plot_file <- file.path("/home/hhampson/Results", 
                         paste0("Plot_Organoid_NEBULA_", cell_name_clean, 
                                "_30mMvs15mM_unadjusted_2000.png"))
  png(plot_file, width = 3000, height = 2100, res = 300)
  print(volcano_plot)
  dev.off()
...
for (cell_name in celltypes) {
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  input_file <- file.path(paste0("/home/hhampson/Results/NEBULA_30mMvs15mM_", cell_name_clean, 
                                  "_unadjusted_2000.csv"))
 # Check if file exists before reading
  if (file.exists(input_file)) {
    file <- read.csv(input_file)
...
for (cell_name in celltypes) {
  cell_name_clean <- str_replace_all(cell_name, "[/ -]", "_")
  input_file <- file.path(paste0("/home/hhampson/Results/NEBULA_30mMvs15mM_", 
                                  cell_name_clean, "_unadjusted_2000.csv"))
  ...
  volcano_plot <- ggplot(full_results, aes(x = logFC, y = PValue10, color = color)) +
    ...
    theme_minimal() +
    labs(
      title = paste0(cell_name, " - 30mM vs 15mM"),
      subtitle = paste0("Genes = ", Genes, " | Cells = ", n_cells, 
                        " | Non-Conv: ", Nonconvergence_Rate),
      x = "Log Fold Change",
      y = "-log10(P-Value)"
