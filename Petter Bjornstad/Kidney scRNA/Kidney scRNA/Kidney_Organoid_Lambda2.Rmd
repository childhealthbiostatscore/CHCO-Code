...
####ii. Limma
```{r}
# For RNA-seq: start with raw counts
dge <- DGEList(counts = count_matrix, group = group_factor)
dge <- calcNormFactors(dge)  # TMM normalization
logcpm <- cpm(dge, log = TRUE, prior.count = 1)  # voom alternative

# Extract raw counts
counts <- GetAssayData(so_combined, assay = "RNA", layer = "counts")

# Extract metadata
meta <- so_combined@meta.data

# Aggregate by sample + cell type (adjust grouping vars to your column names)
pseudo <- AggregateExpression(
  so_combined,
  assays = "RNA",
  return.seurat = TRUE,
  group.by = c("record_id", "cell_type_abbrev")  # adjust to your metadata columns
)

# Extract pseudobulk counts
pb_counts <- GetAssayData(pseudo, assay = "RNA", layer = "counts")

# Extract one row per sample from original metadata
sample_meta <- so_combined@meta.data %>%
  dplyr::distinct(record_id, .keep_all = TRUE) %>%
  dplyr::select(record_id, group, treatment)  # keep your key variables

# Match to pseudobulk column order
# pseudobulk cols are named like "record_id_celltype"
# You may need to parse them:
sample_ids <- sub("_.*", "", colnames(pb_counts))  # extract record_id prefix
sample_meta_aligned <- sample_meta[match(sample_ids, sample_meta$record_id), ]

dge <- DGEList(counts = pb_counts)
dge <- calcNormFactors(dge, method = "TMM")

keep <- filterByExpr(dge, group = sample_meta_aligned$group)
dge <- dge[keep, , keep.lib.sizes = FALSE]

group     <- factor(sample_meta_aligned$group)
treatment <- factor(sample_meta_aligned$treatment)

# Option A: additive model (group effect, controlling for treatment)
design <- model.matrix(~ treatment + group)

# Option B: interaction model (if you want group × treatment effect)
# design <- model.matrix(~ treatment * group)
...
# For the group coefficient (last coef by default with reference coding)
results <- topTable(fit2, coef = "groupType 2 Diabetes", number = Inf, adjust.method = "BH")

# Significant DEGs
sig <- dplyr::filter(results, adj.P.Val < 0.05, abs(logFC) > 1)
nrow(sig)
...
ggplot(results, aes(x = logFC, y = -log10(adj.P.Val), color = sig)) +
  ...
  labs(
    title    = "T2D vs Lean Control",
    subtitle = "limma-voom | adj.P.Val < 0.05, |logFC| > 1",
    x        = "log2 Fold Change",
    y        = "-log10(adjusted p-value)",
    color    = NULL
  ) +
