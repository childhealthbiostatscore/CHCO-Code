...
for (pair in comparative_pairs) {
  ...
  biopsy_res_filtered <- biopsy_res %>%
    ...
    mutate(
      ...
      color = ifelse(sig & logFC_groupType_2_Diabetes > 0, "#DC143C",
                     ifelse(sig & logFC_groupType_2_Diabetes < 0, "#4682B4", "#D3D3D3"))
    )
  ...
  organoid_res_filtered <- organoid_res %>%
    ...
    mutate(
      ...
      sig = fdr < fdr_threshold,
      color = ifelse(sig & logFC_group > 0, "#DC143C",
                     ifelse(sig & logFC_group < 0, "#4682B4", "#D3D3D3"))
    )
  ...
  if (length(shared_sig_genes) == 0) {
    ...
    combined_all <- volcano_biopsy +
      ...
      plot_layout(ncol = 3, widths = c(1, 1, 0.9))
  }
