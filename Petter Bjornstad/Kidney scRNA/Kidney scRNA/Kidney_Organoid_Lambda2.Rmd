...
for (pair in comparative_pairs) {
  ...
  # Get significant genes from either source
  sig_genes_organoid <- organoid_res %>% filter(fdr_groupType.2.Diabetes < 0.05) %>% pull(Gene)
  sig_genes_biopsy <- biopsy_res %>% filter(fdr_groupType.2.Diabetes < 0.05) %>% pull(Gene)
  all_sig_genes <- unique(c(sig_genes_organoid, sig_genes_biopsy))
  
  if (length(all_sig_genes) == 0) {
    cat("No significant genes found for", label, "\n")
    next
  }
  
  # Create matrix with organoid and biopsy effects
  heatmap_data <- data.frame(
    Gene = all_sig_genes
  ) %>%
    left_join(organoid_res %>% select(Gene, logFC_organoid = logFC_groupType.2.Diabetes), 
              by = "Gene") %>%
    left_join(biopsy_res %>% select(Gene, logFC_biopsy = logFC_groupType.2.Diabetes), 
              by = "Gene") %>%
    as.data.frame()
  ...
  # Replace NA with 0 for non-significant genes
  ...
  # Annotate which are significant in each
  annotation_row <- data.frame(
    Organoid = ifelse(all_sig_genes %in% sig_genes_organoid, "Sig", "NS"),
    Biopsy = ifelse(all_sig_genes %in% sig_genes_biopsy, "Sig", "NS"),
    row.names = all_sig_genes
  )
  
  ann_colors <- list(
    Organoid = c(Sig = "#DC143C", NS = "grey90"),
    Biopsy = c(Sig = "#4682B4", NS = "grey90")
  )
