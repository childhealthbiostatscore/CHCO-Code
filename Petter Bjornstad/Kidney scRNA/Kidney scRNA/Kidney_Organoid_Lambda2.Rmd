...
table(so_kpmp_sc$cell_type_abbrev)
head(so_kpmp_sc@meta.data[, c("group", "cell_type_abbrev", "kit_id")])
...
# Calculate pooled offset if not already done
if (!"pooled_offset" %in% colnames(so_kpmp_sc@meta.data)) {
  so_kpmp_sc$pooled_offset <- log(so_kpmp_sc$nCount_RNA + 1)
}
...
```{r}
# Get cell types to analyze
celltypes <- unique(so_kpmp_sc$cell_type_abbrev)
...
for (cell_name in celltypes) {
  ...
  # Subset to current cell type
  so_celltype <- subset(so_kpmp_sc, cell_type_abbrev == cell_name)
  DefaultAssay(so_celltype) <- "RNA"
