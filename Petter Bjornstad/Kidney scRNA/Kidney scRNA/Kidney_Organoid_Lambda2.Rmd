...
celltypes <- unique(so_combined$cell_type_abbrev)[12:14]
...
for (cell_name in celltypes) {
  ...
  # Get counts for HVGs
  counts_hvg <- round(GetAssayData(so_celltype, layer = "counts"))
  genes_list <- hvgs[1:100]  # Changed from hvgs[1] - you want all genes!
  ...
  nebula_results_list <- foreach(g = genes_list, 
    ...
    tryCatch({
      ...
      # CRITICAL: Sort by kit_id to group cells from same subject
      sort_order <- order(meta_gene$kit_id)
      count_gene_sorted <- count_gene[, sort_order, drop = FALSE]
      meta_gene_sorted <- meta_gene[sort_order, ]
      pred_gene_sorted <- pred_gene[sort_order, , drop = FALSE]
      library_sorted <- library[sort_order]
      
      # CHECK: Make sure there's variation in predictors
      if (length(unique(meta_gene_sorted$group)) < 2) {
        return(NULL)
      }
      
      # Check for zero variance in any predictor column
      pred_variances <- apply(pred_gene_sorted, 2, var)
      if (any(pred_variances == 0)) {
        return(NULL)
      }
      
      # Create data structure with sorted data
      data_g_gene <- list(count = count_gene_sorted, 
                          id = meta_gene_sorted$kit_id, 
                          pred = pred_gene_sorted, 
                          offset = library_sorted)
