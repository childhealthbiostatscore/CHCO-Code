#!/usr/bin/env Rscript
# =============================================================================
# Generate job_config_normal_vs_ow_obese.txt for T1D Adiposity NEBULA array jobs
# Normal vs Overweight+Obese comparisons only (BMI and DXA)
# =============================================================================
# Run this locally to generate the config file before submitting SLURM jobs.
# Output: config/job_config_normal_vs_ow_obese.txt (tab-delimited: analysis_type celltype celltype_var)
# =============================================================================

# Normal vs Overweight+Obese analysis types (must match names in run_nebula_single.R)
analysis_types <- c(
...
config_file <- file.path(config_dir, "job_config_normal_vs_ow_obese.txt")
...
# Print summary
cat("\nAnalysis breakdown:\n")
cat(sprintf("  Normal vs Overweight+Obese (BMI, unadjusted): 1\n"))
cat(sprintf("  Normal vs Overweight+Obese (DXA, unadjusted): 1\n"))
cat(sprintf("  Normal vs Overweight+Obese (BMI, adjusted for age): 1\n"))
cat(sprintf("  Normal vs Overweight+Obese (DXA, adjusted for age): 1\n"))
cat(sprintf("  Normal vs Overweight+Obese (BMI, adjusted for age + sex): 1\n"))
cat(sprintf("  Normal vs Overweight+Obese (DXA, adjusted for age + sex): 1\n"))
