...
# Re-set dirs immediately — RData may overwrite them
dir.sensitivity <- 'C:/Users/netio/Documents/UofW/Projects/pioglitazone/Sensitivity/'
dir.interaction <- 'C:/Users/netio/Documents/UofW/Projects/pioglitazone/Sensitivity/Interaction_PIO_GLP1/'
dir.root        <- 'C:/Users/netio/Documents/UofW/Projects/pioglitazone/GLP1_Extended/'
...
so_subset$celltype1 <- dplyr::case_when(
...
harmonized_data <- harmonized_data %>% dplyr::filter(group == 'Type 2 Diabetes')
...
harmonized_data <- harmonized_data %>%
  dplyr::filter(rh_id %in% meta.data$record_id | croc_id %in% meta.data$record_id |
...
medications <- medications %>%
  ...
  dplyr::filter(mrn %in% harmonized_data$mrn)

harmonized_data$mrn <- as.character(harmonized_data$mrn)

final_df <- medications %>% dplyr::left_join(harmonized_data, by = 'mrn')
final_df$combined_id <- paste0(final_df$mrn, '_', final_df$record_id)
final_df <- final_df %>% dplyr::filter(!duplicated(combined_id))

# ============================================================================
# QC & APPLY COVARIATES TO SEURAT OBJECT
# ============================================================================
...
# Drop conflicting columns before join to prevent .x/.y clobbering
meta.data <- as.data.frame(scrna_small@meta.data) %>%
  dplyr::select(-dplyr::any_of(c("bmi", "sex", "epic_mfm_1", "epic_glp1ra_1", "group_labels",
...
meta_joined <- meta.data %>%
  dplyr::left_join(covar_lookup, by = 'record_id')
...
scrna_small$four_group <- dplyr::case_when(
...
# Pull patient-level data as a plain data frame — avoids Seurat metadata list issues
patient_meta <- as.data.frame(scrna_small@meta.data)
patient_meta$four_group <- as.character(patient_meta$four_group)
patient_meta <- patient_meta[!duplicated(patient_meta$record_id), ]
patient_meta <- patient_meta[!is.na(patient_meta$four_group), ]
patient_meta$four_group <- factor(patient_meta$four_group,
                                  levels = c("PIO-GLP1-", "PIO+GLP1-", "PIO-GLP1+", "PIO+GLP1+"))

# Patient counts
group_counts <- as.data.frame(table(patient_meta$four_group))
colnames(group_counts) <- c("four_group", "n_patients")
group_counts$pct <- round(100 * group_counts$n_patients / sum(group_counts$n_patients), 1)
cat("\nPatient counts:\n"); print(group_counts)
...
if (length(missing_vars) > 0)
  cat("Skipping missing vars:", paste(missing_vars, collapse = ", "), "\n")
...
demo_table <- patient_meta %>%
  dplyr::select(four_group, dplyr::all_of(demo_vars_present)) %>%
  tbl_summary(
    by        = four_group,
    type      = type_list,
...
demo_table %>%
  as_gt() %>%
  gt::tab_options(table.font.size = 11, heading.title.font.size = 14,
                  column_labels.font.size = 12) %>%
  gt::gtsave(paste0(dir.root, 'Demographics/FourGroup_Demographics.png'), vwidth = 1400, vheight = 1000)
demo_table %>%
  as_gt() %>%
  gt::gtsave(paste0(dir.root, 'Demographics/FourGroup_Demographics.html'))

cat("Demographics table saved.\n")

# ---- Cell counts bar plot --------------------------------------------------
cell_meta <- as.data.frame(scrna_small@meta.data)
cell_meta$four_group <- as.character(cell_meta$four_group)
cell_counts <- cell_meta %>%
  dplyr::filter(!is.na(four_group)) %>%
  dplyr::count(four_group, celltype2, name = "n_cells")
...
if ("UMAP_1" %in% colnames(scrna_small@meta.data)) {
  umap_df <- as.data.frame(scrna_small@meta.data) %>%
    dplyr::filter(!is.na(four_group)) %>%
...
run_nebula_glp1_main <- function(so_obj, dir.out, celltype,
  ...
  meta_g     <- as.data.frame(so_c@meta.data)
  ...
  tmp_df  <- meta_g %>% dplyr::distinct(record_id, .keep_all = TRUE)
...
load_glp1_effect <- function(dir.root, ct_clean, model_label = "base") {
  ...
  df %>% dplyr::transmute(gene   = .data[[gene_col]],
...
for (ct in celltypes_vec) {
  ...
  for (m in names(GLP1_COVARIATES)) {
    ...
    if (is.null(df)) next
    df <- df %>%
      dplyr::mutate(diffexp = dplyr::case_when(
        pvalue < P_THRESH & logFC > 0 ~ "Up",
        pvalue < P_THRESH & logFC < 0 ~ "Down",
        TRUE ~ "NS")) %>%
      dplyr::arrange(pvalue) %>%
      dplyr::mutate(label = ifelse(dplyr::row_number() <= 8, gene, NA)) %>%
...
for (ct in celltypes_vec) {
  ...
  for (m in names(GLP1_COVARIATES)) {
    df <- load_glp1_effect(dir.root, ct_clean, m)
    if (is.null(df)) next
    ranked <- df %>% dplyr::filter(!is.na(logFC)) %>% dplyr::arrange(desc(logFC)) %>%
      { setNames(.$logFC, .$gene) }
...
write.csv(combined_glp1_gsea,
          paste0(dir.root, 'GLP1_Main/GSEA/Raw/GLP1_all_gsea.csv'), row.names = FALSE)
...
scrna_small$pt_subtype <- dplyr::case_when(
...
observed_pt <- as.data.frame(scrna_small@meta.data) %>%
  dplyr::filter(!is.na(pt_subtype)) %>%
  dplyr::count(pt_subtype) %>%
  dplyr::filter(n > 50)
...
load_pio_sensitivity <- function(dir.sensitivity, ct_clean, model_label = "base") {
  ...
  df <- read.csv(fp)
  df %>% dplyr::transmute(gene   = summary.gene,
...
for (ct in celltypes_vec) {
  ...
  pio_df  <- pio_df  %>% dplyr::filter(!is.na(logFC) & !is.na(pvalue))
  glp1_df <- glp1_df %>% dplyr::filter(!is.na(logFC) & !is.na(pvalue))
  ...
  overlap_rows[[ct]] <- data.frame(
    celltype = ct, n_genes_tested = nrow(merged),
...
if (length(overlap_rows) > 0) {
  overlap_summary <- dplyr::bind_rows(overlap_rows)
  write.csv(overlap_summary,
            paste0(dir.root, 'Comparison/PIO_vs_GLP1_DEG_Overlap.csv'), row.names = FALSE)
...
for (ct in celltypes_vec) {
  ...
  shared <- dplyr::inner_join(
    pio_df  %>% dplyr::filter(!is.na(pvalue) & pvalue < P_THRESH) %>%
      dplyr::select(gene, logFC_pio = logFC, pval_pio = pvalue, fdr_pio = fdr),
    glp1_df %>% dplyr::filter(!is.na(pvalue) & pvalue < P_THRESH) %>%
      dplyr::select(gene, logFC_glp1 = logFC, pval_glp1 = pvalue, fdr_glp1 = fdr),
    by = "gene") %>%
    dplyr::filter(sign(logFC_pio) == sign(logFC_glp1)) %>%
    dplyr::mutate(direction = ifelse(logFC_pio > 0, "Up in both", "Down in both"),
                  celltype  = ct) %>%
    dplyr::arrange(pval_pio)
  if (nrow(shared) > 0) shared_degs[[ct]] <- shared
}
if (length(shared_degs) > 0) {
  shared_combined <- dplyr::bind_rows(shared_degs)
  write.csv(shared_combined,
            paste0(dir.root, 'Comparison/SharedConcordantDEGs_PIO_GLP1.csv'), row.names = FALSE)
  cat("Concordant DEGs:\n")
  print(shared_combined %>% dplyr::count(celltype, direction))
}
...
for (ct in celltypes_vec) {
  ...
  int_df  <- if (file.exists(int_fp)) {
    ...
    if (!is.na(logfc_c) && !is.na(p_c))
      df %>% dplyr::transmute(gene = .data[[gene_c]], logFC = .data[[logfc_c]],
...
sig_count_df <- dplyr::bind_rows(sig_count_rows)
write.csv(sig_count_df,
          paste0(dir.root, 'Comparison/SigGeneCounts_PIO_GLP1_Interaction.csv'), row.names = FALSE)
