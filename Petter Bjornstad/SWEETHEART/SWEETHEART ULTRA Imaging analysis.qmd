...
harm_dat_subset <- harm_dat %>%
  ...
  dplyr::select(swht_id, record_id, mrn, group, sex, age, imaging_visit_id, date, imaging_date, cmri_date, study, all_of(im_vars)) %>%
  ...
  mutate(uacr_100 = case_when(acr_u >= 100 ~ "UACR >=100 mg/g",
         ...
         tr2 = (1/tt2)*1000,
         lspeak   = case_when(lspeak   == 0 ~ NA_real_, TRUE ~ lspeak),
         cspeak = case_when(cspeak == 0 ~ NA_real_, TRUE ~ cspeak),
         rspeak = case_when(rspeak == 0 ~ NA_real_, TRUE ~ rspeak),
         log_acr_u = log(acr_u),
...
harm_dat_improve <- harm_dat %>%
  filter(study == "IMPROVE" & visit == "baseline") %>%
    dplyr::summarise(across(where(negate(is.numeric)), ~ ifelse(all(is.na(.x)), NA_character_, last(na.omit(.x)))),
                   across(where(is.numeric), ~ ifelse(all(is.na(.x)), NA_real_, mean(.x, na.rm = TRUE))),
                   .by = c(record_id)) %>%
  dplyr::select(record_id, lvedv, lvesv, lv_stroke_volume, lvef, lved_mass, lves_mass,
                lv_cardiac_output, lv_cardiac_index, rvedv, rvesv, rv_stroke_volume, rvef, rv_cardiac_output, rv_cardiac_index,
...
# hist(subset(harm_dat_subset, imaging_visit_id != "ultra_follow_up" & group != "Lean Control")$rv_stroke_volume_bsa)
