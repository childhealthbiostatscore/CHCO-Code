...
#1. Set Up - RH2 & CRC ##a. Load Libraries & Directores \## a. Libraries & Directories
...
if (user == "shivaniramesh") {
  ...
  dir.results <- c("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver Pet IR/Results")
}
...
CROC <- read.csv(fs::path(dir.dat,"/Data Harmonization/Data Clean/harmonized_dataset.csv"),na.strings = c("", "NA", "N/A", "n/a", " ","na")) %>% 
  filter(study=="CROCODILE")

CROC <- CROC %>% 
  dplyr::select(c("record_id","study","procedure","visit","group","age","sex","bmi","hba1c","liver_k1","liver_k2","liver_k3","p1_gc_leanm","p1_steady_state_insulin","p1_gc_m","p1_ffa_suppression","m_i_p2_raw_lean","p2_gc_leanm","p2_steady_state_insulin","p2_gc_m","p2_ffa_suppression")) 
# filter(procedure=="ivgtt")

#For Phase 1 M/I normalized to lean mass:
CROC$m_i_p1_lean = CROC$p1_gc_leanm / CROC$p1_steady_state_insulin

#For Phase 1 M/I using total body weight:
CROC$m_i_p1 = CROC$p1_gc_m / CROC$p1_steady_state_insulin

# Where:
# `p1_gc_leanm` = Phase 1 glucose clamp rate normalized to lean mass (mg/kg lean/min)
#  `p1_gc_m` = Phase 1 glucose clamp rate normalized to total mass (mg/kg/min)
# `p1_steady_state_insulin` = Steady-state insulin concentration during Phase 1 (μU/mL or pmol/L)

# Hepatic insulin sensitivity index 
CROC$hepatic_is_ffa = CROC$p1_ffa_suppression / CROC$p1_steady_state_insulin

#Phase 2 M/I normalized to lean mass:
CROC$m_i_p2_lean = CROC$p2_gc_leanm / CROC$p2_steady_state_insulin

#For Phase 1 M/I using total body weight:
CROC$m_i_p2 = CROC$p2_gc_m / CROC$p2_steady_state_insulin

#Phase 2 FFA-based insulin sensitivity:**
CROC$peripheral_is_ffa = CROC$p2_ffa_suppression / CROC$p2_steady_state_insulin

#Measures adipose tissue insulin sensitivity at higher insulin levels
#Reflects how well peripheral insulin action suppresses lipolysis
insulin <- c("m_i_p1_lean","m_i_p1","hepatic_is_ffa","m_i_p2_lean","m_i_p2","peripheral_is_ffa")
...
CROC_collapsed2 <- CROC_collapsed %>% 
  filter(!is.na(liver_k1)) %>%  #27 participants have complete pet data
  # filter(!is.na(m_i_p2_raw_lean)) #24 participants have complete pet and ir data
  filter(if_any(c(m_i_p1_lean, m_i_p1, hepatic_is_ffa, 
                  m_i_p2_lean, m_i_p2, peripheral_is_ffa), ~ !is.na(.))) #27 have at least 1 mi variable or is variable 
rm(CROC,CROC_collapsed,harm_dat,harm_dat_collapsed,harm_meta_data)
CROC <- CROC_collapsed2
rm(CROC_collapsed2)
#Save formatted data 
# saveRDS(CROC,"/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Liver Pet IR/Data/Formatted_CROCODILE_data.RDS")
# 
# #Collapse RH
...
#2. Analysis ##a. Descriptive Statistics

```{r}
#CROCODILE
table1(~age+sex+bmi+hba1c+liver_k1+liver_k2+liver_k3+m_i_p1_lean+m_i_p1+hepatic_is_ffa+
                  m_i_p2_lean+m_i_p2+peripheral_is_ffa| group, data=CROC,
...
##b. Associations Between Pet Vars & IRs ###i. CROCODILE

```{r}
pet_variables <- c("liver_k1","liver_k2","liver_k3")
insulin_vars <- c("m_i_p1_lean", "m_i_p1", "hepatic_is_ffa", 
                  "m_i_p2_lean", "m_i_p2", "peripheral_is_ffa")
...
for(pet in pet_variables) {
  for (insulin in insulin_vars){
    ...
    pearson_test <- cor.test(CROC[[pet]], CROC[[insulin]], 
                             method = "pearson",
                             conf.level = 0.95)
    ...
    spearman_test <- cor.test(CROC[[pet]], CROC[[insulin]], 
                              method = "spearman",
                              conf.level = 0.95)
...
for(pet in pet_variables) {
  for (insulin in insulin_vars){
    ...
    m1_adj <- lm(m0_adj,data=CROC)
    beta <- summary(m1)$coef[2,1]
    pval <- summary(m1)$coef[2,4]
    beta_adj <- summary(m1_adj)$coef[2,1]
    pval_adj <- summary(m1_adj)$coef[2,4]
    full_results <- data.frame(Insulin=insulin,Pet=pet,Beta=beta,Pval=pval,Beta_Adj=beta_adj,Pval_adj=pval_adj)
...
for(pet in pet_variables) {
  for (insulin in insulin_vars){
    tot_sub <- total_results %>% 
      filter(Insulin==insulin & Pet==pet)
plot <- ggplot(CROC, aes_string(x=insulin, y=pet)) +
      geom_point(size=3, alpha=0.6) +
      geom_smooth(method="lm", se=F, color="blue") +
      labs(x = gsub("_", " ", insulin),
           y = gsub("_", " ", pet),
           title = paste(pet, "vs", insulin),
           subtitle = paste0("β = ", round(tot_sub$Beta_Adj, 3), ", p = ", round(tot_sub$Pval_adj, 3))) +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5),
            plot.subtitle = element_text(hjust = 0.5))
png(fs::path(dir.results,paste0("Linear_Reg_",insulin,"_vs_",pet,"_adj_age_sex.png")),res=300,height=1000,width=1000)
print(plot)
dev.off()
  }
}

#Interaction for Diabetes status
#Linear regression & scatterplots
pet_variables <- c("liver_k1","liver_k2","liver_k3")
insulin_vars <- c("m_i_p1_lean", "m_i_p1", "hepatic_is_ffa", 
                  "m_i_p2_lean", "m_i_p2", "peripheral_is_ffa")
...
for(pet in pet_variables) {
  for (insulin in insulin_vars){
    m0 <- as.formula(paste0(pet,"~",insulin,"*group"))
    m1 <- lm(m0,data=CROC)
    m0_adj <- as.formula(paste0(pet,"~",insulin,"*group+age+sex"))
    m1_adj <- lm(m0_adj,data=CROC)
    beta <- summary(m1)$coef[2,1]
    pval <- summary(m1)$coef[2,4]
    beta_adj <- summary(m1_adj)$coef[2,1]
    pval_adj <- summary(m1_adj)$coef[2,4]
    beta_int <- summary(m1)$coef[4,1]
    beta_int_adj <- summary(m1_adj)$coef[6,1]
    pval_int <- summary(m1)$coef[4,4]
    pval_int_adj <- summary(m1_adj)$coef[6,4]
    full_results <- data.frame(Insulin=insulin,Pet=pet,Beta=beta,Pval=pval,Beta_Adj=beta_adj,Pval_adj=pval_adj,Beta_Int=beta_int,PVal_Int=pval_int,Beta_Int_Adj=beta_int_adj,Pval_Int_Adj=pval_int_adj)
    total_results <- rbind(total_results,full_results)
  }
}
...
for(pet in pet_variables) {
  for (insulin in insulin_vars){
    tot_sub <- total_results %>% 
      filter(Insulin==insulin & Pet==pet)
plot <- ggplot(CROC, aes_string(x=insulin, y=pet,color="group")) +
      geom_point(size=3, alpha=0.6) +
      geom_smooth(method="lm", se=F,aes(color=group)) +
      labs(x = gsub("_", " ", insulin),
           y = gsub("_", " ", pet),
           title = paste(pet, "vs", insulin),
           subtitle = paste0("β = ", round(tot_sub$Beta_Adj, 3), ", p = ", round(tot_sub$Pval_adj, 3),", Int = ", round(tot_sub$Beta_Int_Adj,3),", p-int = ",round(tot_sub$Pval_Int_Adj,3))) +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5),
            plot.subtitle = element_text(hjust = 0.5))
# png(fs::path(dir.results,paste0("Linear_Reg_",insulin,"_vs_",pet,"_adj_age_sex.png")),res=300,height=1000,width=1000)
