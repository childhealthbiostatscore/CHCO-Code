...
if (user == "laurapyle") {
  ...
  results_path <- '/Users/laurapyle/Library/CloudStorage/OneDrive-UW/Bjornstad/Biostatistics Core Shared Drive/Proteomics and Glycemia/Figures'
} else if (user == "lpyle") {
  ...
  results_path <- '/Users/lpyle/Library/CloudStorage/OneDrive-UW/Bjornstad/Biostatistics Core Shared Drive/Proteomics and Glycemia/Figures'
} else if (user == "pylell") {
  ...
  results_path <- '/Users/pylell/Library/CloudStorage/OneDrive-UW/Bjornstad/Biostatistics Core Shared Drive/Proteomics and Glycemia/Figures'
} else {
...
corr_plot_modified_nodict <- function(data, X, Y, cor_method = "spearman", adj_var = NULL, 
  ...
  # Add these lines to apply custom row and column names:
  if (!missing(rownames_M)) {
...
filename <- "/local_T2D_glycemia_heatmap.png"
full_path <- file.path(results_path, filename)
#colnames_M <- paste0(colnames_M,"_",prot_heatmap)
png(height = 3000, width = 6000, file = full_path, res = 300)
corr_plot_modified_nodict(data=df_t2d, 
                   Y=df_t2d[,seq_include], 
                 X=df_t2d[,c("acprg", "m_i_gir_190", "airg","di")],
                 cor_method="spearman", adj_var = NA,
                 colnames_M=prot_include,
                 rownames_M=c("ACPRg", "M/I", "AIRg","DI"))
dev.off()

#View(df_t2d[,c("record_id", "visit", "acprg", "acpr", "gir_190", "airg", "mm_airg", "air", "di", "glu_disposal")])
View(df_t2d[,c("record_id", "visit", "acprg", "gir_190", "airg", "mm_airg", "di")])

# acprg (Acute C-peptide response to glucose) has data, data dictionary does not say where it comes from
# acpr (ACPR from clamp) is not in my data set. Looking at the shiny app, it is present in IMPROVE but missing in RH
# gir_190 (Raw M-value) has data, data dictionary does not say where it comes from
# airg (Acute insulin response to glucose) has data, data dictionary does not say where it comes from
# mm_airg (from IVGTT) is missing in all participants - I think this comes from PANTHER
# di (Disposition Index, from clamp) has data
