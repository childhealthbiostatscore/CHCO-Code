...
dir.results <- c(fs::path(dir.dat,"Brain Biomarkers and Proteomics","Results"))
...
table_demographics <- table1(~ age + sex + race + bmi + triglycerides + hba1c + 
                               eGFR_CKD_epi + gfr_bsa_plasma + total_kidney_volume_ml + 
                               ht_adj_tkv | group, data = data_full)

table_biomarkers <- table1(~ ab40_avg_conc + ab42_avg_conc + nfl_avg_conc + 
                             gfap_avg_conc + ptau_181_avg_conc + ptau_217_avg_conc + 
                             tau_avg_conc | group, data = data_full)

# Create and save tables
table1(~ age + sex + race + bmi + triglycerides + hba1c +
                            eGFR_CKD_epi + gfr_bsa_plasma + total_kidney_volume_ml +
                            ht_adj_tkv | study, data = data_full)

table1(~ ab40_avg_conc + ab42_avg_conc + nfl_avg_conc +
                          gfap_avg_conc + ptau_181_avg_conc + ptau_217_avg_conc +
                          tau_avg_conc | study, data = data_full)

# # Save the data and table objects
# save(data_full, table_demographics, table_biomarkers, file = "/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Brain Biomarkers and Proteomics/Results/tables_for_report.RData")
```


#4. Analysis
##a. PENGUIN & CROCODILE
###i. Explore Distributions by proteomics
```{r}
...
# #Scatterplots
# pdf(fs::path(dir.results,"CROCODILE_Prot_Quanterix_Scatterplots.pdf"))
# for (y in qx_var) {
#   for(x in proteins[1:10]) {
...
###ii. Explore distributions by group
```{r}
#Make group a factor variable
data_plasma$group <- factor(data_plasma$group)
data_plasma$group <- relevel(data_plasma$group,"Lean Control")

#Linear regression
total_results <- data.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ group"))
  model0 <- lm(m0,data=data_plasma)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
  results <- data.frame(Variable=y,Group=names(model0$coefficients[2]),Beta=beta,PValue=pval,Significant=sig)
  total_results <- rbind(total_results,results)
}
...
#Scatterplots
ggplot(data= data_plasma, aes(x = group, y = ab40_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "AB40 Concentration by PKD Status",
    x = "",
    y = "Average AB40 Concentration"
  ) +
  theme_minimal()

ggplot(data= data_plasma, aes(x = group, y = ab42_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "AB42 Concentration by PKD Status",
    x = "",
    y = "Average AB42 Concentration"
  ) +
  theme_minimal()

ggplot(data= data_plasma, aes(x = group, y = tau_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#e9c46a", "#f4a261", "#e76f51")) +
  labs(
    title = "Total Tau Concentration by PKD Status",
    x = "",
    y = "Average Tau Concentration"
  ) +
  theme_minimal()

ggplot(data= data_plasma, aes(x = group, y = nfl_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "Nfl Concentration by PKD Status",
    x = "",
    y = "Average Nfl Concentration"
  ) +
  theme_minimal()

ggplot(data= data_plasma, aes(x = group, y = gfap_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#e76f51", "darkred","#264653")) +
  labs(
    title = "GFAP Concentration by PKD Status",
    x = "",
    y = "Average GFAP Concentration"
  ) +
  theme_minimal()

ggplot(data= data_plasma, aes(x = group, y = ptau_181_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("darkred", "#264653","#2a9d8f")) +
  labs(
    title = "PTAU 181 Concentration by PKD Status",
    x = "",
    y = "Average PTAU 181 Concentration"
  ) +
  theme_minimal()
################################################
ggplot(data= data_plasma, aes(x = group, y = ptau_217_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#2a9d8f", "#264653","#e9c46a")) +
  labs(
    title = "PTAU 217 Concentration by Disease Status",
    x = "",
    y = "Average PTAU 217 Concentration"
  ) +
  theme_minimal()

summary(aov(ptau_217_avg_conc~group,data_plasma))
# First, save the ANOVA model
aov_model <- aov(ptau_217_avg_conc ~ group, data = data_plasma)
...
total_results <- data.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ group + age"))
  model0 <- lm(m0,data=data_plasma)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
...
total_results <- data.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ group + sex"))
  model0 <- lm(m0,data=data_plasma)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
...
total_results <- data.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ group + age + sex"))
  model0 <- lm(m0,data=data_plasma)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
...
total_results <- data.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ group + eGFR_CKD_epi"))
  model0 <- lm(m0,data=data_plasma)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
...
total_results <- data.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ group + gfr_bsa_plasma"))
  model0 <- lm(m0,data=data_plasma)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
...
hist(data2$ab40_avg_conc)
hist(data2$ab42_avg_conc)
hist(data2$tau_avg_conc)
hist(data2$nfl_avg_conc)
hist(data2$gfap_avg_conc)
hist(data2$ptau_181_avg_conc)
hist(data2$ptau_217_avg_conc)
# 
# data <- data %>% 
#   mutate(across(all_of(qx_var),~log(.),.names = "{.col}_log"))

# data$group <- factor(data$group)
# data$group <- relevel(data$group,"Lean Control")

#Linear regression
total_results <- data.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ total_kidney_volume_ml"))
  model0 <- lm(m0,data=data2)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
  results <- data.frame(Variable=y,Group=names(model0$coefficients[2]),Beta=beta,PValue=pval,Significant=sig)
...
total_results <- data.frame()
for (y in qx_var) {
  # Pearson correlation instead of linear regression
  cor_result <- cor.test(data2[[y]], data2$total_kidney_volume_ml)
  
  # Extract correlation coefficient and p-value
  beta <- cor_result$estimate  # Pearson correlation coefficient
  pval <- cor_result$p.value
  sig <- ifelse(pval < 0.05, "*", "")
  results <- data.frame(Variable=y,TKV="Total Kidney Volume",Corr=beta,PValue=pval,Significant=sig)
  total_results <- rbind(total_results,results)
}

total_results2 <- data.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ ht_adj_tkv"))
  model0 <- lm(m0,data=data2)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
  results2 <- data.frame(Variable=y,Group=names(model0$coefficients[2]),Beta=beta,PValue=pval,Significant=sig)
...
for (y in qx_var) {
  # Pearson correlation instead of linear regression
  cor_result <- cor.test(data2[[y]], data2$ht_adj_tkv)
  
  # Extract correlation coefficient and p-value
  beta <- cor_result$estimate  # Pearson correlation coefficient
  pval <- cor_result$p.value
  sig <- ifelse(pval < 0.05, "*", "")
  results2 <- data.frame(Variable=y,TKV="Height-Adjusted Total Kidney Volume",Corr=beta,PValue=pval,Significant=sig)
  total_results2 <- rbind(total_results2,results2)
}

total_results3 <- data.frame()
for (y in qx_var) {
  # Pearson correlation instead of linear regression
  cor_result <- cor.test(data2[[y]], data2$bmi)
  
  # Extract correlation coefficient and p-value
  beta <- cor_result$estimate  # Pearson correlation coefficient
  pval <- cor_result$p.value
  sig <- ifelse(pval < 0.05, "*", "")
  results3 <- data.frame(Variable=y,TKV="BMI",Corr=beta,PValue=pval,Significant=sig)
  total_results3 <- rbind(total_results3,results3)
}

total_results4 <- data.frame()
for (y in qx_var) {
  # Pearson correlation instead of linear regression
  cor_result <- cor.test(data2[[y]], data2$eGFR_CKD_epi)
  
  # Extract correlation coefficient and p-value
  beta <- cor_result$estimate  # Pearson correlation coefficient
  pval <- cor_result$p.value
  sig <- ifelse(pval < 0.05, "*", "")
  results4 <- data.frame(Variable=y,TKV="eGFR",Corr=beta,PValue=pval,Significant=sig)
  total_results4 <- rbind(total_results4,results4)
}

total_results5 <- data.frame()
for (y in qx_var) {
  # Pearson correlation instead of linear regression
  cor_result <- cor.test(data2[[y]], data2$gfr_bsa_plasma)
  
  # Extract correlation coefficient and p-value
  beta <- cor_result$estimate  # Pearson correlation coefficient
  pval <- cor_result$p.value
  sig <- ifelse(pval < 0.05, "*", "")
  results5 <- data.frame(Variable=y,TKV="mGFR",Corr=beta,PValue=pval,Significant=sig)
  total_results5 <- rbind(total_results5,results5)
}

write.csv(total_results,"Total_Kidney_Volume_Brain_Biomarkers.csv")
write.csv(total_results2,"Ht_adj_Total_Kidney_Volume_Brain_Biomarkers.csv")
write.csv(total_results3,"BMI_Brain_Biomarkers.csv")
write.csv(total_results4,"eGFR.csv")
write.csv(total_results5,"mGFR.csv")

#Check assumptions
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ total_kidney_volume_ml"))
  model0 <- lm(m0,data=data)
  residuals <- resid(model0)
  # 2. QQ plot to check normality of residuals
  print(qqnorm(residuals))
  qqline(residuals, col = "red")
...
#Scatterplots
ggplot(data = data, aes(x = total_kidney_volume_ml, y = ab40_avg_conc)) +
  geom_point()+
  geom_smooth(method="lm",color="#2a9d8f",se=F) +
  # scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "AB40 Concentration vs. Total Kidney Volume (mL)",
    subtitle=paste0("Beta = ",round(total_results$Beta[1],3),", P = ",round(total_results$PValue[1],3),total_results$Significant[1]),
    x = "Total Kidney Volume (mL)",
    y = "Average AB40 Concentration"
  ) +
  theme_minimal()

ggplot(data = data, aes(x = ht_adj_tkv, y = ab40_avg_conc)) +
  geom_point()+
  geom_smooth(method="lm",color="#2a9d8f",se=F) +
  # scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "AB40 Concentration vs. Height-Adjusted Total Kidney Volume",
    subtitle=paste0("Beta = ",round(total_results2$Beta[1],3),", P = ",round(total_results2$PValue[1],3),total_results2$Significant[1]),
    x = "Height-Adjusted Total Kidney Volume",
    y = "Average AB40 Concentration"
  ) +
  theme_minimal()


ggplot(data = data, aes(x = total_kidney_volume_ml, y = ab42_avg_conc)) +
  geom_point()+
  geom_smooth(method="lm",color="#f4a261",se=F) +
  # scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "AB42 Concentration vs. Total Kidney Volume (mL)",
    subtitle=paste0("Beta = ",round(total_results$Beta[2],3),", P = ",round(total_results$PValue[2],3),total_results$Significant[2]),
    x = "Total Kidney Volume (mL)",
    y = "Average AB42 Concentration"
  ) +
  theme_minimal()

ggplot(data = data, aes(x = ht_adj_tkv, y = ab42_avg_conc)) +
  geom_point()+
  geom_smooth(method="lm",color="#f4a261",se=F) +
  # scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "AB42 Concentration vs. Height-Adjusted Total Kidney Volume",
    subtitle=paste0("Beta = ",round(total_results2$Beta[2],3),", P = ",round(total_results2$PValue[2],3),total_results2$Significant[2]),
    x = "Height-Adjusted Total Kidney Volume",
    y = "Average AB42 Concentration"
  ) +
  theme_minimal()
...
ggplot(data = data, aes(x = total_kidney_volume_ml, y = ab42_avg_conc)) +
  geom_point()+
  geom_smooth(method="lm",color="#e9c46a",se=F) +
  # scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "Tau Concentration vs. Total Kidney Volume (mL)",
    subtitle=paste0("Beta = ",round(total_results$Beta[3],3),", P = ",round(total_results$PValue[3],3),total_results$Significant[3]),
    x = "Total Kidney Volume (mL)",
    y = "Average Tau Concentration"
  ) +
  theme_minimal()

ggplot(data = data, aes(x = ht_adj_tkv, y = tau_avg_conc)) +
  geom_point()+
  geom_smooth(method="lm",color="#e9c46a",se=F) +
  # scale_fill_manual(values = c("#264653", "#2a9d8f", "#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "Tau Concentration vs. Height-Adjusted Total Kidney Volume",
    subtitle=paste0("Beta = ",round(total_results2$Beta[3],3),", P = ",round(total_results2$PValue[3],3),total_results2$Significant[3]),
    x = "Height-Adjusted Total Kidney Volume",
    y = "Average Tau Concentration"
  ) +
  theme_minimal()
...
ggplot(data = data, aes(x = group, y = nfl_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "Nfl Concentration by PKD Status",
...
ggplot(data = data, aes(x = group, y = gfap_avg_conc, fill = group)) +
  geom_boxplot() +
...
ggplot(data = data, aes(x = group, y = ptau_181_avg_conc, fill = group)) +
  geom_boxplot() +
...
ggplot(data = data, aes(x = group, y = ptau_217_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#2a9d8f", "#264653")) +
  labs(
    title = "PTAU 217 Concentration by PKD Status",
    x = "",
    y = "Average PTAU 217 Concentration"
  ) +
  theme_minimal()
