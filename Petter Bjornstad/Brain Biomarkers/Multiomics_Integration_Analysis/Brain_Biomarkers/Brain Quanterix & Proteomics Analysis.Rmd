...
#Linear regression
total_results <- data_plasma.frame()
for (y in qx_var) {
  m0 <- as.formula(paste0(y,"~ group"))
  model0 <- lm(m0,data_plasma=data_plasma)
  beta <- summary(model0)$coef[2,1]
  pval <- summary(model0)$coef[2,4]
  sig <- ifelse(pval<0.05,"*","")
  results <- data_plasma.frame(Variable=y,Group=names(model0$coefficients[2]),Beta=beta,PValue=pval,Significant=sig)
...
#Scatterplots
ggplot(data_plasma = data_plasma, aes(x = group, y = ab40_avg_conc, fill = group)) +
  geom_boxplot() +
...
ggplot(data_plasma = data_plasma, aes(x = group, y = ab42_avg_conc, fill = group)) +
  geom_boxplot() +
...
ggplot(data_plasma = data_plasma, aes(x = group, y = tau_avg_conc, fill = group)) +
  geom_boxplot() +
  scale_fill_manual(values = c("#e9c46a", "#f4a261", "#e76f51", "darkred")) +
  labs(
    title = "Tau Concentration by PKD Status",
    x = "",
    y = "Average Tau Concentration"
  ) +
  theme_minimal()

ggplot(data_plasma = data_plasma, aes(x = group, y = nfl_avg_conc, fill = group)) +
  geom_boxplot() +
...
ggplot(data_plasma = data_plasma, aes(x = group, y = gfap_avg_conc, fill = group)) +
  geom_boxplot() +
...
ggplot(data_plasma = data_plasma, aes(x = group, y = ptau_181_avg_conc, fill = group)) +
  geom_boxplot() +
...
ggplot(data_plasma = data_plasma, aes(x = group, y = ptau_217_avg_conc, fill = group)) +
  geom_boxplot() +
