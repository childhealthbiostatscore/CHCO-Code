...
ggsave(file.path(root_path, "ATTEMPT/Results/Figures/Panels/urine_proteomics_panel.pdf"), width = 10, height = 6)
ggsave(file.path(root_path, "ATTEMPT/Results/Figures/Panels/urine_proteomics_panel.eps"), device = cairo_ps,family = "Arial", width = 10, height = 6)
...
```{r echo = F}
# HALLMARK
soma_DiD_fgsea_hallmark <- soma_DiD_fgsea$hallmark %>%
  as.data.frame() %>%
  mutate(leadingEdge = vapply(leadingEdge, paste, collapse = ", ", FUN.VALUE = character(1))) %>%
  arrange(pval)

write.csv(soma_DiD_fgsea_hallmark, file.path(root_path, "ATTEMPT/Results/GSEA/Proteomics/urine_mixed_DiD_HALLMARK.csv"), row.names = F, na = "")
...
soma_DiD_fgsea_kegg <- soma_DiD_fgsea$kegg %>%
  as.data.frame() %>%
  mutate(leadingEdge = vapply(leadingEdge, paste, collapse = ", ", FUN.VALUE = character(1))) %>%
  arrange(pval)

write.csv(soma_DiD_fgsea_kegg, file.path(root_path, "ATTEMPT/Results/GSEA/Proteomics/urine_mixed_DiD_KEGG.csv"), row.names = F, na = "")
...
soma_DiD_fgsea_reactome <- soma_DiD_fgsea$reactome %>%
  as.data.frame() %>%
  mutate(leadingEdge = vapply(leadingEdge, paste, collapse = ", ", FUN.VALUE = character(1))) %>%
  arrange(pval)

write.csv(soma_DiD_fgsea_reactome, file.path(root_path, "ATTEMPT/Results/GSEA/Proteomics/urine_mixed_DiD_REACTOME.csv"), row.names = F, na = "")
...
soma_DiD_fgsea_go <- soma_DiD_fgsea$go %>%
  as.data.frame() %>%
  mutate(leadingEdge = vapply(leadingEdge, paste, collapse = ", ", FUN.VALUE = character(1))) %>%
  arrange(pval)

write.csv(soma_DiD_fgsea_go, file.path(root_path, "ATTEMPT/Results/GSEA/Proteomics/urine_mixed_DiD_GO.csv"), row.names = F, na = "")
