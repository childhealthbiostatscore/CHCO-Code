...
vsmc_plasma <- process_nebula_results(vsmc_plasma_raw$results, pval_col = "p_protein")$results
vsmc_plasma <- vsmc_plasma %>%
  mutate(Gene = sub("_.*", "", Gene))
```

```{r echo = F}
plot_volcano(vsmc_plasma, fc = "logFC_protein", p_col = "p_protein", x_axis = "logFC protein", y_axis = "-log10(p.value)",
             file_suffix = "_kpmp_vsmc_plasma",
             formula = "protein + treatment", cell_type = "VSMC") + 
...
glom_plasma <- process_nebula_results(glom_plasma_raw$results, pval_col = "p_protein")$results
glom_plasma <- glom_plasma %>%
  mutate(Gene = sub("_.*", "", Gene))
```

```{r echo = F}
plot_volcano(glom_plasma, fc = "logFC_protein", p_col = "p_protein", x_axis = "logFC protein", y_axis = "-log10(p.value)", 
             file_suffix = "_kpmp_glom_plasma",
             formula = "protein + treatment", cell_type = "Glomerular cells") + 
  geom_vline(xintercept = 0, color = "grey", linetype = "dashed", size = 0.2)
```

```{r echo = F}
immune_plasma_raw <- s3readRDS("Proteomics/immune_nebula_plasma_soma_res_kpmp.rds", "attempt", region = "")
immune_plasma <- process_nebula_results(immune_plasma_raw$results, pval_col = "p_protein")$results
immune_plasma <- immune_plasma %>%
  mutate(Gene = sub("_.*", "", Gene))
```

```{r echo = F}
plot_volcano(immune_plasma, fc = "logFC_protein", p_col = "p_protein", x_axis = "logFC protein", y_axis = "-log10(p.value)", file_suffix = "_kpmp_immune_plasma",
             formula = "protein + treatment", cell_type = "Immune") + 
  geom_vline(xintercept = 0, color = "grey", linetype = "dashed", size = 0.2)
```
```{r echo = F}
ec_plasma_raw <- s3readRDS("Proteomics/ec_nebula_plasma_soma_res_kpmp.rds", "attempt", region = "")
ec_plasma <- process_nebula_results(ec_plasma_raw$results, pval_col = "p_protein")$results
ec_plasma <- ec_plasma %>%
  mutate(Gene = sub("_.*", "", Gene))
```

```{r echo = F}
plot_volcano(ec_plasma, fc = "logFC_protein", p_col = "p_protein", x_axis = "logFC protein", y_axis = "-log10(p.value)", 
             file_suffix = "_kpmp_ec_plasma",
             formula = "protein + treatment", cell_type = "EC") + 
  geom_vline(xintercept = 0, color = "grey", linetype = "dashed", size = 0.2)
```
