...
plot_mixedmodel_ci_stars <- function(data, y_var, y_axis_title, 
                                     legend_position = c(0.6, 0.8), 
                                     baseline_visit = 0, 
                                     visits_to_plot = c(-4, 0, 4, 16, 18)) {
  dodge_val <- 0.08
  ...
  # 1. Get EMMs from mixed model per visit
  emm_results <- purrr::map_dfr(visits_to_plot, function(v) {
    df_sub <- data %>% filter(visit == v)
    tryCatch({
      model <- lmer(as.formula(paste(y_name, "~ treatment +", baseline_var, "+ (1|subject_id)")), data = df_sub)
      em <- emmeans(model, ~ treatment)
      broom::tidy(em, conf.int = TRUE) %>%
        mutate(visit = v)
    }, error = function(e) {
      message(glue::glue("Mixed model failed for visit {v}: {e$message}"))
      tibble(treatment = c("Placebo", "Dapagliflozin 5mg"),
             estimate = NA_real_, conf.low = NA_real_, conf.high = NA_real_,
             visit = v)
    })
  })

  # 2. Get contrast results + stars
  contrast_results <- purrr::map_dfr(visits_to_plot, function(v) {
    df_sub <- data %>% filter(visit == v)
    tryCatch({
      model <- lmer(as.formula(paste(y_name, "~ treatment +", baseline_var, "+ (1|subject_id)")), data = df_sub)
      em <- emmeans(model, ~ treatment)
      cont <- pairs(em)
      broom::tidy(cont, conf.int = TRUE) %>%
        mutate(visit = v)
    }, error = function(e) {
      tibble(visit = v, estimate = NA, p.value = NA, conf.low = NA, conf.high = NA, contrast = NA)
    })
  }) %>%
    filter(contrast == "Placebo - Dapagliflozin 5mg") %>%
    mutate(
  ...
  # 3. Compute y-position for stars
  ttest_results <- contrast_results %>%
    left_join(emm_results %>%
                group_by(visit) %>%
                summarize(max_upper = max(conf.high, na.rm = TRUE), .groups = "drop"),
              by = "visit") %>%
    mutate(y_position = max_upper + 0.05 * max_upper)

  # 3. Plot
  p <- ggplot(emm_results, aes(x = factor(visit, levels = visits_to_plot), y = estimate, color = treatment)) +
    geom_errorbar(aes(ymin = conf.low, ymax = conf.high, group = treatment),
                  width = 0.1, size = 0.3, position = position_dodge(width = dodge_val),
                  color = "black") +
    geom_line(aes(group = treatment), position = position_dodge(width = dodge_val), size = 1.2) +
    geom_point(aes(shape = treatment), size = 11, position = position_dodge(width = dodge_val), color = "white") +
    geom_point(aes(shape = treatment), size = 7, position = position_dodge(width = dodge_val)) +
    geom_text(data = ttest_results,
              aes(x = factor(visit, levels = visits_to_plot), y = y_position, label = stars),
              inherit.aes = FALSE,
              size = 6, vjust = 0) +
...
# 1. Change in cell type proportions
...
## 1b. T1D PRE vs. POST in Dapagliflozin
...
# 3. Associate DEGs with clinical markers adjusting for treatment group

---

## 3a. Change in mGFR (raw)
...
## 4a. Baseline Table (Full cohort, Dapa vs. Placebo)
...
kable(summary(arsenal::tableby(treatment ~ age + sex + weight+ height + bmi + hba1c + kwt(emu_urine_acr_mean, "Nmiss","medianq1q3", "range") + creatinine_s + cystatin_c_s + albumin_serum_gl + cgm_tir + diabetes_dx_duration  + creatinine_s + cystatin_c_s + sbp + dbp + cholesterol_serum_mmoll + triglycerides_serum_mmoll + pulse, data = subset(attempt_dat, visit == 0)), digits = 1))
...
## 4b. MRI Mixed model (Full cohort, Dapa vs. Placebo over time, Unadjusted)
...
## 4c. MRI Mixed model (Full cohort, Dapa vs. Placebo over time, Adjusted for age, sex, diabetes duration, site, and BMI)
...
## 4d. Baseline Table (Denver only cohort, Dapa vs. Placebo)

```{r echo = F, results='asis'}
kable(summary(arsenal::tableby(treatment ~ age + sex + weight+ height + bmi + hba1c + kwt(emu_urine_acr_mean, "Nmiss","medianq1q3", "range") + creatinine_s + cystatin_c_s + albumin_serum_gl + cgm_tir + diabetes_dx_duration  + creatinine_s + cystatin_c_s + sbp + dbp + cholesterol_serum_mmoll + triglycerides_serum_mmoll + pulse, data = subset(attempt_dat, visit == 0 & site == "Denver")), digits = 1))
...
## 4e. MRI Mixed model (Denver only cohort, Dapa vs. Placebo over time)
...
## 4f. Baseline Table (Biopsy only cohort, Dapa vs. Placebo)
...
## 4g. MRI Mixed model (Biopsy only cohort, Dapa vs. Placebo over time)
...
## 4h. Plots over time (Full cohort)
...
plot_mean_ci_stars2 <- function(data, y_var, y_axis_title, legend_position = c(0.6, 0.8)) {
  ...
  # 4. Calculate mean and 95% CI (t-based)
  mean_dat <- data %>%
...
## 4i. Plots over time (Denver cohort)
