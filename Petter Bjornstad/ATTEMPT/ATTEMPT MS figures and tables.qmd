...
ggarrange(delta_avg_k_r2_overtime, avg_k_r2_by_mgfr_category_bars,
          delta_avg_k_r2_overtime_denver, delta_avg_m_r2_overtime_denver, delta_avg_c_r2_overtime_denver, ncol=3, nrow=2, common.legend = TRUE, legend="bottom", labels = c("A", "B", "C", "D", "E"))

ggsave("/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/Figures/Panels/avg_k_r2_panel.png", width = 12, height = 7, dpi = 300)
```
...
#| fig-cap: "Segmental distribution of kidney cell populations"
#| fig-dpi: 300
...
ggsave(file.path(base_path, "Panels/pt_panel.png"), p, height = 20, width = 14, dpi = 600)
```
...
#| fig-cap: "TAL gene expression correlations with clinical outcomes. Volcano plots showing gene expression changes correlated with (A) change in mGFR, (B) change in HbA1c, (C) change in time in range, and (D) change in kidney R2*"
#| fig-dpi: 300
...
#| fig-cap: "EC gene expression correlations with clinical outcomes. Volcano plots showing gene expression changes correlated with (A) change in mGFR, (B) change in HbA1c, (C) change in time in range, and (D) change in kidney R2*"
#| fig-dpi: 300
...
#| fig-cap: "IC gene expression correlations with clinical outcomes. Volcano plots showing gene expression changes correlated with (A) change in mGFR, (B) change in HbA1c, (C) change in time in range, and (D) change in kidney R2*"
#| fig-dpi: 300
...
#| fig-cap: "POD gene expression correlations with clinpodal outcomes. Volcano plots showing gene expression changes correlated with (A) change in mGFR, (B) change in HbA1c, (C) change in time in range, and (D) change in kidney R2*"
#| fig-dpi: 300
...
baseline_comp <- comp_df %>%
  filter(group %in% c("Lean Control", "Type 1 Diabetes"),
         visit == "baseline")
baseline_comp$group <- factor(baseline_comp$group, levels = c("Type 1 Diabetes", "Lean Control"))

tt_t1d <- t.test(avg_m_r2 ~ group, data = baseline_comp)  # Welch
...
baseline_diff_row <- tibble(
  ...
  n          = n_t1d + n_lean,                         # total N as integer
  mean_delta = unname(diff(tt_t1d$estimate)),          # T1D − Lean
  sd_delta   = NA_real_,
  se         = NA_real_,
  ci_low     = tt_t1d$conf.int[1],
  ci_high    = tt_t1d$conf.int[2],
...
# ----- 4) DiD -----
# t-test for difference in mean deltas (Dapa vs Placebo)
...
did_row <- tibble(
  ...
  n          = nrow(paired_changes),   # total N across both groups
  mean_delta = unname(diff(tt_did$estimate)),  # mean ΔDapa − mean ΔPlacebo
  sd_delta   = NA_real_,
  se         = NA_real_,
  ci_low     = tt_did$conf.int[1],
  ci_high    = tt_did$conf.int[2],
...
final_summary2$group <- factor(final_summary2$group, labels = c("HC - T1D", "DiD\n(D - P)",
