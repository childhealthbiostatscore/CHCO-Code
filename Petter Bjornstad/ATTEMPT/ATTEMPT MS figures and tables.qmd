...
```{r, out.height="100%"}
#| label: fig-ipa-main
#| fig-cap: "Pathway analysis using IPA on key cell types. Pathway names have been shortened using standard abbreviations where necessary. Full names for pathways truncated are listed below each panel."
#| fig-dpi: 300
...
ggarrange(ggarrange(p2, labels = "A", ncol = 2, widths = c(3,2),
                    ggarrange(p1, p3, nrow = 2, heights = c(1,1), labels = c("B", "C"))),
          ggarrange(p4, p5, p6, p7, 
                    ncol = 2, nrow = 2, 
                    labels = c("D", "E", "F", "G")), ncol = 1,
          nrow = 2, heights = c(2,3))
ggsave(file.path(base_path, "Panels/slingshot_panel.png"), dpi = 300, height = 12)

```

```{r, out.width="80%"}
#| label: fig-pt-pseudotime
#| fig-cap: "Proximal tubule pseudotime trajectory analysis. (A) Cell density distribution along pseudotime. Cell type proportions are shown as pie charts at bins of pseudotime in increments of 10. (B) PCA plot with inferred lineage trajectory. (C) Proportion plot of PT cells in each treatment/visit groups. (D-G) Clinical parameter changes along proximal tubule pseudotime. Correlation of pseudotime progression with change in mGFR and change in HbA1c."
#| fig-dpi: 300
