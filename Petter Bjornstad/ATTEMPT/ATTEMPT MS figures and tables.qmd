...
ggsave(file.path(base_path, "Panels/mgfr_hba1c_tir_overtime.png"), width = 10, height = 7, dpi = 300)
```
...
ggarrange(delta_avg_k_r2_overtime, avg_k_r2_by_mgfr_category_bars,
          delta_avg_k_r2_overtime_denver, delta_avg_c_r2_overtime_denver, ncol=2, nrow=2, common.legend = TRUE, legend="bottom", labels = c("A", "B", "C", "D"))

ggsave("/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/Figures/Panels/avg_k_r2_panel.png", width = 7, height = 7, dpi = 300)
```
...
```{r eval = F}
ggarrange(ggarrange(ggarrange(pt_plots$volcano_plot, pt_plots$heatmap_plot, ncol = 2, widths = c(4,1)),
...
ggsave(file.path(base_path, "Panels/key_celltypes_panel.png"), height = 22, width = 20)

```

```{r}
#| label: fig-key-cells-panel
#| fig-cap: ""
#| fig-dpi: 300

knitr::include_graphics(file.path(base_path, "Panels/key_celltypes_panel.png"))
```
...
plot_params <- list(
  mgfr = list(
    data = pt_mgfr_jodal_bsa_delta_hvg_kpmp,
    fc_col = "logFC_mgfr_jodal_bsa_delta",
    p_col = "p_mgfr_jodal_bsa_delta",
    x_axis = "\n\nlogFC \u0394 mGFR (BSA)",
    y_axis = "-log10(p-value)",
    file_suffix = "nebula/kpmp_pt_mgfr_jodal_bsa_trtcolors",
  ...
  hba1c = list(
    data = pt_hba1c_delta_hvg_kpmp,
    fc_col = "logFC_hba1c_delta",
    p_col = "p_hba1c_delta",
    x_axis = "\n\nlogFC \u0394 HbA1c",
    y_axis = "-log10(p-value)",
    file_suffix = "nebula/kpmp_pt_hba1c_trtcolors",
    positive_text = "Positively associated \nw/ \u0394 HbA1c",
    negative_text = "Negatively associated \nw/ \u0394 HbA1c",
    clinical_var = "hba1c",
    arrow_label = "HbA1c\n-0.47",
  ...
  tir = list(
    data = pt_tir_delta_hvg_kpmp,
    fc_col = "logFC_tir_delta",
    p_col = "p_tir_delta",
    x_axis = "\n\nlogFC \u0394 TIR",
    y_axis = "-log10(p-value)",
    file_suffix = "nebula/kpmp_pt_tir_trtcolors",
    positive_text = "Positively associated \nw/ \u0394 TIR",
    negative_text = "Negatively associated \nw/ \u0394 TIR",
    clinical_var = "tir",
    arrow_label = "TIR\n+9.3",
  ...
  avg_k_r2 = list(
    data = pt_avg_k_r2_delta_hvg_kpmp,
    fc_col = "logFC_avg_k_r2_delta",
    p_col = "p_avg_k_r2_delta",
    x_axis = "\n\nlogFC \u0394 avg kidney R2*",
    y_axis = "-log10(p-value)",
    file_suffix = "nebula/kpmp_pt_avg_k_r2_trtcolors",
    positive_text = "Positively associated \nw/ \u0394 avg kidney R2*",
    negative_text = "Negatively associated \nw/ \u0394 avg kidney R2*",
...
# Size parameters - adjust these to increase/decrease sizes
size_params <- list(
  label_size = 6,           # Gene labels
  arrow_symbol_size = 12,    # Up/down arrow symbols
  main_arrow_size = 15,     # Central arrow
  axis_title_size = 15,      # Axis titles
  axis_text_size = 14,       # Axis tick labels
  legend_text_size = 15,     # Legend text
  caption_size = 15,         # Caption text
  annotation_text_size = 5, # Annotation text (positive/negative text)
  text_size = 7,            # Arrow label text
  title_size = 9,            # Overall theme text
  arrow_padding = 0.15,
...
for (var_name in names(plot_params)) {
  ...
  # Create plot
  plots[[var_name]] <- plot_volcano_concordance(
...
common_legend <- get_legend(p3_with_solid_legend)
...
# Create the combined plots for rows 2 and 3
middle_row <- ggarrange(p2_no_legend, p3_no_legend, ncol = 2, labels = c("A", "B"),
                        font.label = list(size = 25))
bottom_row <- ggarrange(p4_no_legend, p5_no_legend, ncol = 2, labels = c("C", "D"),
                        font.label = list(size = 25))

# Combine middle and bottom rows with the common legend at the bottom
rows_2_3_with_legend <- ggarrange(
  ggarrange(middle_row, bottom_row, nrow = 2),
  nrow = 2,
  heights = c(1, 0.1),  # Adjust the height ratio between plots and legend
...
ggsave(file.path(base_path, "Panels/pt_panel.png"), final_plot, width = 18, height = 12, dpi = 600)
```
...
ggarrange(
  p2, labels = "A", nrow = 2, label.x = 0.15,
  ggarrange(p1, p3, ncol = 2, labels = c("B", "C")))
ggsave(file.path(base_path, "Panels/slingshot_panel.png"), dpi = 600)

```

```{r, out.width="100%"}
#| label: fig-pt-pseudotime
#| fig-cap: "Proximal tubule pseudotime trajectory analysis. (A) Cell density distribution along pseudotime. (B) PCA plot with inferred lineage trajectory. (C) Proportion plot of PT cells in each treatment/visit groups."
#| fig-dpi: 300
...
# Read images
img1 <- image_read(file.path(base_path, "Slingshot/attempt_density_pt_kpmp_mgfr_jodal_bsa_deltadelta_slingshot.png"))
img2 <- image_read(file.path(base_path, "Slingshot/attempt_density_pt_kpmp_hba1c_deltadelta_slingshot.png"))
img3 <- image_read(file.path(base_path, "Slingshot/attempt_density_pt_kpmp_tir_deltadelta_slingshot.png"))
...
```{r, eval = F}
ggarrange(ggarrange(ggarrange(pod_plots$volcano_plot, pod_plots$heatmap_plot, ncol = 2, widths = c(4,1)),
                    ggarrange(pod_dot_legend, pod_plots$bar_plot, pod_plots$dot_plot, nrow = 3, heights = c(1, 10,20)), 
                    nrow = 2, heights = c(3,1), labels = "A", font.label = list(size = 30)), 
          ggarrange(ggarrange(pc_plots$volcano_plot, pc_plots$heatmap_plot, ncol = 2, widths = c(4,1)), 
                    ggarrange(pc_dot_legend, pc_plots$bar_plot, pc_plots$dot_plot, nrow = 3, heights = c(1, 10,20)), 
                    nrow = 2, heights = c(3,1), labels = "B", font.label = list(size = 30)), 
          ggarrange(ggarrange(vsmc_p_fib_plots$volcano_plot, vsmc_p_fib_plots$heatmap_plot, ncol = 2, widths = c(4,1)),
                    ggarrange(vsmc_p_fib_dot_legend, vsmc_p_fib_plots$bar_plot, vsmc_p_fib_plots$dot_plot, nrow = 3, heights = c(1, 10,20)), 
                    nrow = 2, heights = c(3,1), labels = "C", font.label = list(size = 30)), 
          ggarrange(ggarrange(immune_plots$volcano_plot, immune_plots$heatmap_plot, ncol = 2, widths = c(4,1)),
                    ggarrange(immune_dot_legend, immune_plots$bar_plot, immune_plots$dot_plot, nrow = 3, heights = c(1, 10,20)), 
                    nrow = 2, heights = c(3,1), labels = "D", font.label = list(size = 30)), 
          nrow = 2, ncol = 2)
