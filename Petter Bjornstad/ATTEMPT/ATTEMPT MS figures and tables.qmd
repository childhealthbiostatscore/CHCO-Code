...
#| fig-cap: "Summary of Hallmark gene set enrichment analysis findings in segmental distribution of kidney cell populations. Red overlay (positive NES) indicates upregulation of pathways in dapagliflozin treated cohort vs. placebo treated cohort, blue overlay (negative NES) indicates downregulation of pathways in the dapagliflozin treated cohort vs. placebo treated cohort, and light pink or light blue overlay indicates mixed results (both upregulation and downregulation)."
#| fig-dpi: 300

knitr::include_graphics(file.path(base_path, "Pathways in ATTEMPT overlay.png"))
```


```{r, out.height="100%"}
#| label: fig-hallmark-segments-croc
#| fig-cap: "Summary of Hallmark gene set enrichment analysis findings in segmental distribution of kidney cell populations in CROCODILE comparing T1D to HC. Red overlay (positive NES) indicates upregulation of pathways in T1D vs. HC, blue overlay (negative NES) indicates downregulation of pathways in T1D vs. HC, and light pink or light blue overlay indicates mixed results (both upregulation and downregulation)."
#| fig-dpi: 300

knitr::include_graphics(file.path(base_path, "Pathways in CROC overlay.png"))
```
...
Figure S3: Gene set enrichment using Reactome on key cell types. Pathway names have been shortened using standard abbreviations where necessary. Full names for pathways truncated are listed below each panel.

![](`r file.path(base_path, "Panels/key_combined_hallmark.png")`)

Figure S4: Gene set enrichment using Hallmark on key cell types. Pathway names have been shortened using standard abbreviations where necessary. Full names for pathways truncated are listed below each panel.

![](`r file.path(base_path, "Panels/key_combined_reactome.png")`)

Figure S5: Gene set enrichment using Gene Ontology on key cell types. Pathway names have been shortened using standard abbreviations where necessary. Full names for pathways truncated are listed below each panel.
