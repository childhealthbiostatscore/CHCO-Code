...
table_labs <- list(
  ...
  emu_urine_acr_mean_mgg = "Urine ACR (mg/g)",
...
kable(summary(arsenal::tableby(
  treatment ~
    weight +
    height +
    kwt(emu_urine_acr_mean, "medianq1q3", digits = 2) +
    creatinine_s +
    cystatin_c_s +
    albumin_serum_gl +
    cgm_tir +
...
```{r}
#| label: fig-consort
#| fig-cap: "CONSORT flow diagram showing participant enrollment, allocation, follow-up, and analysis. The left panel (pink background) illustrates the ancillary study within the parent trial conducted in Toronto, ON; London, ON; and Aurora, CO (total n=98), with participants randomized to either placebo (n=49) or dapagliflozin 5 mg (n=49). The right panel (yellow background) highlights the kidney biopsy-eligible cohort, a subset of participants from Colorado (n=18), randomized to placebo (n=9) or dapagliflozin 5 mg (n=9). "
...
delta_avg_k_r2_overtime_denver$p
```
...
#| fig-cap: "(A-F) Longitudinal change in mGFR, HbA1c, TIR, and R2* in kidney, medulla, and cortex by treatment in biopsy eligible cohort. Visit-specific means and 95% CIs are shown for dapagliflozin and placebo at baseline and week 16. Stars above panels indicate between-arm differences at each visit from a linear mixed-effects model with treatment × visit interaction, age, sex, diabetes duration, BMI, and subject-level random intercepts (emmeans pairwise contrasts; Bonferroni adjusted for models with more than 2 visits;  * P<0.05, ** P<0.01, *** P<0.001). "
...
Figure S1: Pathway analysis using IPA on other cell types. Pathway names have been shortened using standard abbreviations where necessary. Full names for pathways truncated are listed below each panel.
...
Figure S2: Gene set enrichment using Reactome on key cell types. Pathway names have been shortened using standard abbreviations where necessary. Full names for pathways truncated are listed below each panel.

![](`r file.path(base_path, "Panels/key_combined_hallmark.png")`)

Figure S3: Gene set enrichment using Hallmark on key cell types. Pathway names have been shortened using standard abbreviations where necessary. Full names for pathways truncated are listed below each panel.

![](`r file.path(base_path, "Panels/key_combined_reactome.png")`)

Figure S4: Gene set enrichment using Gene Ontology on key cell types. Pathway names have been shortened using standard abbreviations where necessary. Full names for pathways truncated are listed below each panel.
