...
fibvsmcp_plots <- create_gene_expression_plots(
  main_results = fibvsmc_nebula_res_combined_reml_pooled,
  subtype_results_list = list(
...
for (ct in celltype_list) {
  for (v in vars) {
    ...
    # Read RDS file
    message("Reading: ", s3_path)
    rds_obj <- s3readRDS(s3_path, bucket = "attempt", region = "")
    
    # Process results
    processed <- process_nebula_results(rds_obj, pval_col = paste0("p_", v))$results
...
croc_res_select_vars$pt_hba1c %>%
  ggplot(aes(x = Gene, y = 1, fill = logFC_hba1c)) +
  geom_tile()
