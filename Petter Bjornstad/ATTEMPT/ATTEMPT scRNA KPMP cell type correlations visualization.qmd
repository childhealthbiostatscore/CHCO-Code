...
for (cell in names(celltype_groups))  {
  ...
  var_name <- paste0(tolower(cell), "hvg_kpmp")
...
```{r echo = F}
plot_volcano_concordance(pt_mgfr_jodal_res_combined, 
...
```{r echo = F}
plot_volcano_concordance(pt_mgfr_jodal_bsa_res_combined, 
                         "logFC_mgfr_jodal_bsa_delta", 
                         "p_mgfr_jodal_bsa_delta",
                         "logFC \u0394 mGFR (BSA)", 
                         "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(pt_tir_res_combined, 
                          "logFC_tir_delta", 
                          "p_tir_delta",
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(pt_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(pt_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(pt_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta",
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
pt_avg_k_r2_res_combined <- subset(pt_avg_k_r2_res_combined, logFC_avg_k_r2_delta >-10)
...
```{r echo = F}
plot_volcano_concordance(pt_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
# Read in nebula results saved to kopah
pt_avg_ketones_res <- s3readRDS('associations/nebula/avg_ketones/pt_attempt_nebula_avg_ketones.rds', bucket = "attempt", region = "")

# nebula only keeps results for converged
pt_avg_ketones_convergence <- map_dfr(
  names(pt_avg_ketones_res),
  function(gene_name) {
    converged <- pt_avg_ketones_res[[gene_name]]$convergence
    df <- data.frame(Gene = gene_name,
                     Convergence_Code = converged)
    return(df)
  }
...
pt_avg_ketones_converged <- pt_avg_ketones_convergence %>%
  filter(Convergence_Code >=-10)

pt_avg_ketones_res_combined <- map_dfr(
  names(pt_avg_ketones_res),
  function(gene_name) {
    df <- pt_avg_ketones_res[[gene_name]]$summary
    df <- df %>% filter(gene_name %in% pt_avg_ketones_converged$Gene) %>%
      mutate(Gene = gene_name)
    return(df)
  }
)

pt_avg_ketones_res_combined <- pt_avg_ketones_res_combined %>%
  ungroup() %>%
  mutate(fdr_interaction = p.adjust(p_avg_ketones_delta, method = "fdr"))

pt_avg_ketones_res_combined_disp <- map_dfr(
  names(pt_avg_ketones_res),
  function(gene_name) {
    df <- pt_avg_ketones_res[[gene_name]]$overdispersion
    df <- df %>% mutate(Gene = gene_name)
    return(df)
  }
)

pt_avg_ketones_res_combined <- subset(pt_avg_ketones_res_combined, logFC_avg_ketones_delta >-10)

write.csv(pt_avg_ketones_res_combined, "/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/nebula/pt_avg_ketones.csv", row.names = F)
```

```{r echo = F}
plot_volcano_associations(subset(pt_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "PT ketones", 
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
                          "nebula/pt_avg_ketones_trtcolors",
                          treatment_results = pt_nebula_res_combined_reml_pooled,
                          treatment_p_col = "p_treatmentDapagliflozin:visitPOST",
...
```{r echo = F}
plot_volcano_concordance(pt_avg_ketones_res_combined, 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
...
```{r echo =F, eval = F}
plot_volcano(subset(pt_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), "logFC_avg_ketones_delta", 
             "p_avg_ketones_delta",
             NULL,
             "logFC \u0394 ketones", 
             "-log10(p-value)",
             "nebula/pt_avg_ketones",
             formula = "\u0394 ketones + treatment",
             positive_text = "Positively associated with \n\u0394 ketones",
             negative_text = "Negatively associated with \n\u0394 ketones",
             cell_type = "PT")
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
kegg_legacy <- prepare_gmt(gmt_files[1], unique(pt_avg_ketones_res_combined$Gene), savefile = FALSE)
reactome <- prepare_gmt(gmt_files[3], unique(pt_avg_ketones_res_combined$Gene), savefile = FALSE)
go <- prepare_gmt(gmt_files[4], unique(pt_avg_ketones_res_combined$Gene), savefile = FALSE)

# rank genes by logFC
rankings_pt_avg_ketones <- pt_avg_ketones_res_combined$logFC_avg_ketones_delta
names(rankings_pt_avg_ketones) <- pt_avg_ketones_res_combined$Gene
...
```{r echo = F}
plot_volcano_concordance(tal_mgfr_jodal_res_combined, 
                          "logFC_mgfr_jodal_delta", 
                          "p_mgfr_jodal_delta",
                          "logFC \u0394 mGFR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(tal_mgfr_jodal_bsa_res_combined, 
                          "logFC_mgfr_jodal_bsa_delta", 
                          "p_mgfr_jodal_bsa_delta",
                          "logFC \u0394 mGFR (BSA)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(tal_tir_res_combined, 
                          "logFC_tir_delta", 
                          "p_tir_delta",
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(tal_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(tal_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(tal_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta",
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_associations(tal_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "TAL avg kidney R2*", 
                          "logFC \u0394 avg kidney R2*", 
...
```{r echo = F}
plot_volcano_concordance(tal_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
# Read in nebula results saved to kopah
tal_avg_ketones_res <- s3readRDS('associations/nebula/avg_ketones/tal_attempt_nebula_avg_ketones.rds', bucket = "attempt", region = "")

# nebula only keeps results for converged
tal_avg_ketones_convergence <- map_dfr(
  names(tal_avg_ketones_res),
  function(gene_name) {
    converged <- tal_avg_ketones_res[[gene_name]]$convergence
    df <- data.frame(Gene = gene_name,
                     Convergence_Code = converged)
    return(df)
  }
...
tal_avg_ketones_converged <- tal_avg_ketones_convergence %>%
  filter(Convergence_Code >=-10)

tal_avg_ketones_res_combined <- map_dfr(
  names(tal_avg_ketones_res),
  function(gene_name) {
    df <- tal_avg_ketones_res[[gene_name]]$summary
    df <- df %>% filter(gene_name %in% tal_avg_ketones_converged$Gene) %>%
      mutate(Gene = gene_name)
    return(df)
  }
)

tal_avg_ketones_res_combined <- tal_avg_ketones_res_combined %>%
  ungroup() %>%
  mutate(fdr_interaction = p.adjust(p_avg_ketones_delta, method = "fdr"))

tal_avg_ketones_res_combined_disp <- map_dfr(
  names(tal_avg_ketones_res),
  function(gene_name) {
    df <- tal_avg_ketones_res[[gene_name]]$overdispersion
    df <- df %>% mutate(Gene = gene_name)
    return(df)
  }
)

tal_avg_ketones_res_combined <- subset(tal_avg_ketones_res_combined, logFC_avg_ketones_delta >-10)

write.csv(tal_avg_ketones_res_combined, "/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/nebula/tal_avg_ketones.csv", row.names = F)
```

```{r echo = F}
plot_volcano_associations(subset(tal_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "TAL ketones", 
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
                          "nebula/tal_avg_ketones_trtcolors",
                          treatment_results = tal_nebula_res_combined_reml_pooled,
                          treatment_p_col = "p_treatmentDapagliflozin:visitPOST",
                          formula = "\u0394 ketones + treatment",
                          positive_text = "Positively associated with \n\u0394 ketones",
...
```{r echo = F}
plot_volcano_concordance(tal_avg_ketones_res_combined, 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
...
```{r echo =F, eval = F}
plot_volcano(subset(tal_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), "logFC_avg_ketones_delta", 
             "p_avg_ketones_delta",
             NULL,
             "logFC \u0394 ketones", 
             "-log10(p-value)",
             "nebula/tal_avg_ketones",
             formula = "\u0394 ketones + treatment",
             positive_text = "Positively associated with \n\u0394 ketones",
             negative_text = "Negatively associated with \n\u0394 ketones",
             cell_type = "TAL")
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
kegg_legacy <- prepare_gmt(gmt_files[1], unique(tal_avg_ketones_res_combined$Gene), savefile = FALSE)
reactome <- prepare_gmt(gmt_files[3], unique(tal_avg_ketones_res_combined$Gene), savefile = FALSE)
go <- prepare_gmt(gmt_files[4], unique(tal_avg_ketones_res_combined$Gene), savefile = FALSE)

# rank genes by logFC
rankings_tal_avg_ketones <- tal_avg_ketones_res_combined$logFC_avg_ketones_delta
names(rankings_tal_avg_ketones) <- tal_avg_ketones_res_combined$Gene
...
```{r echo = F}
plot_volcano_concordance(immune_mgfr_jodal_res_combined, 
                          "logFC_mgfr_jodal_delta", 
                          "p_mgfr_jodal_delta",
                          "logFC \u0394 mGFR (Raw)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_mgfr_jodal_bsa_res_combined, 
                          "logFC_mgfr_jodal_bsa_delta", 
                          "p_mgfr_jodal_bsa_delta",
                          "logFC \u0394 mGFR (BSA)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_tir_res_combined, 
                          "logFC_tir_delta", 
                          "p_tir_delta",
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta",
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
immune_avg_k_r2_res_combined <- subset(immune_avg_k_r2_res_combined, logFC_avg_k_r2_delta >-10)
...
```{r echo = F}
plot_volcano_associations(immune_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
...
```{r echo = F}
plot_volcano_concordance(immune_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
# Read in nebula results saved to kopah
immune_avg_ketones_res <- s3readRDS('associations/nebula/avg_ketones/immune_attempt_nebula_avg_ketones.rds', bucket = "attempt", region = "")

# nebula only keeps results for converged
immune_avg_ketones_convergence <- map_dfr(
  names(immune_avg_ketones_res),
  function(gene_name) {
    converged <- immune_avg_ketones_res[[gene_name]]$convergence
    df <- data.frame(Gene = gene_name,
                     Convergence_Code = converged)
    return(df)
  }
...
immune_avg_ketones_converged <- immune_avg_ketones_convergence %>%
  filter(Convergence_Code >=-10)

immune_avg_ketones_res_combined <- map_dfr(
  names(immune_avg_ketones_res),
  function(gene_name) {
    df <- immune_avg_ketones_res[[gene_name]]$summary
    df <- df %>% filter(gene_name %in% immune_avg_ketones_converged$Gene) %>%
      mutate(Gene = gene_name)
    return(df)
  }
)

immune_avg_ketones_res_combined <- immune_avg_ketones_res_combined %>%
  ungroup() %>%
  mutate(fdr_interaction = p.adjust(p_avg_ketones_delta, method = "fdr"))

immune_avg_ketones_res_combined_disp <- map_dfr(
  names(immune_avg_ketones_res),
  function(gene_name) {
    df <- immune_avg_ketones_res[[gene_name]]$overdispersion
    df <- df %>% mutate(Gene = gene_name)
    return(df)
  }
)

immune_avg_ketones_res_combined <- subset(immune_avg_ketones_res_combined, logFC_avg_ketones_delta >-10)

write.csv(immune_avg_ketones_res_combined, "/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/nebula/immune_avg_ketones.csv", row.names = F)
```

```{r echo = F}
plot_volcano_associations(subset(immune_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "Immune ketones", 
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
                          "nebula/immune_avg_ketones_trtcolors",
                          treatment_results = immune_nebula_res_combined_reml_pooled,
                          treatment_p_col = "p_treatmentDapagliflozin:visitPOST",
                          formula = "\u0394 ketones + treatment",
                          positive_text = "Positively associated with \n\u0394 ketones",
...
```{r echo = F}
plot_volcano_concordance(immune_avg_ketones_res_combined, 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
...
```{r echo =F, eval = F}
plot_volcano(subset(immune_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), "logFC_avg_ketones_delta", 
             "p_avg_ketones_delta",
             NULL,
             "logFC \u0394 ketones", 
             "-log10(p-value)",
             "nebula/immune_avg_ketones",
             formula = "\u0394 ketones + treatment",
             positive_text = "Positively associated with \n\u0394 ketones",
             negative_text = "Negatively associated with \n\u0394 ketones",
             cell_type = "Immune")
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
kegg_legacy <- prepare_gmt(gmt_files[1], unique(immune_avg_ketones_res_combined$Gene), savefile = FALSE)
reactome <- prepare_gmt(gmt_files[3], unique(immune_avg_ketones_res_combined$Gene), savefile = FALSE)
go <- prepare_gmt(gmt_files[4], unique(immune_avg_ketones_res_combined$Gene), savefile = FALSE)

# rank genes by logFC
rankings_immune_avg_ketones <- immune_avg_ketones_res_combined$logFC_avg_ketones_delta
names(rankings_immune_avg_ketones) <- immune_avg_ketones_res_combined$Gene
...
```{r echo = F}
plot_volcano_concordance(immune_myeloid_mgfr_jodal_res_combined, 
                          "logFC_mgfr_jodal_delta", 
                          "p_mgfr_jodal_delta",
                          "logFC \u0394 mGFR (Raw)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_myeloid_mgfr_jodal_bsa_res_combined, 
                          "logFC_mgfr_jodal_bsa_delta", 
                          "p_mgfr_jodal_bsa_delta",
                          "logFC \u0394 mGFR (BSA)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_myeloid_tir_res_combined, 
                          "logFC_tir_delta", 
                          "p_tir_delta", 
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_myeloid_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_myeloid_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_myeloid_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta",
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
immune_myeloid_avg_k_r2_res_combined <- subset(immune_myeloid_avg_k_r2_res_combined, logFC_avg_k_r2_delta >-10)
...
```{r echo = F}
plot_volcano_associations(immune_myeloid_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
...
```{r echo = F}
plot_volcano_concordance(immune_myeloid_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_lymphoid_mgfr_jodal_res_combined, 
                          "logFC_mgfr_jodal_delta", 
                          "p_mgfr_jodal_delta",
                          "logFC \u0394 mGFR (Raw)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_lymphoid_mgfr_jodal_bsa_res_combined, 
                          "logFC_mgfr_jodal_bsa_delta", 
                          "p_mgfr_jodal_bsa_delta",
                          "logFC \u0394 mGFR (BSA)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_lymphoid_tir_res_combined, 
                          "logFC_tir_delta", 
                          "p_tir_delta",
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_lymphoid_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_lymphoid_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(immune_lymphoid_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta", 
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
immune_lymphoid_avg_k_r2_res_combined <- subset(immune_lymphoid_avg_k_r2_res_combined, logFC_avg_k_r2_delta >-10)
...
```{r echo = F}
plot_volcano_associations(immune_lymphoid_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
...
```{r echo = F}
plot_volcano_concordance(immune_lymphoid_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(pc_mgfr_jodal_res_combined, 
                          "logFC_mgfr_jodal_delta", 
                          "p_mgfr_jodal_delta",
                          "logFC \u0394 mGFR (Raw)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(pc_mgfr_jodal_bsa_res_combined, 
                          "logFC_mgfr_jodal_bsa_delta", 
                          "p_mgfr_jodal_bsa_delta",
                          "logFC \u0394 mGFR (BSA)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(subset(pc_tir_res_combined, logFC_tir_delta >-10), 
                          "logFC_tir_delta", 
                          "p_tir_delta",
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
pc_hba1c_res_combined <- subset(pc_hba1c_res_combined, logFC_hba1c_delta >-10)
...
```{r echo = F}
plot_volcano_associations(pc_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "PC HbA1c", 
                          "logFC \u0394 HbA1c", 
...
```{r echo = F}
plot_volcano_concordance(pc_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(pc_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(pc_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta",
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
pc_avg_k_r2_res_combined <- subset(pc_avg_k_r2_res_combined, logFC_avg_k_r2_delta >-10)
...
```{r echo = F}
plot_volcano_associations(pc_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "PC avg kidney R2*", 
                          "logFC \u0394 avg kidney R2*", 
...
```{r echo = F}
plot_volcano_concordance(pc_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
# Read in nebula results saved to kopah
pc_avg_ketones_res <- s3readRDS('associations/nebula/avg_ketones/pc_attempt_nebula_avg_ketones.rds', bucket = "attempt", region = "")

# nebula only keeps results for converged
pc_avg_ketones_convergence <- map_dfr(
  names(pc_avg_ketones_res),
  function(gene_name) {
    converged <- pc_avg_ketones_res[[gene_name]]$convergence
    df <- data.frame(Gene = gene_name,
                     Convergence_Code = converged)
    return(df)
  }
...
pc_avg_ketones_converged <- pc_avg_ketones_convergence %>%
  filter(Convergence_Code >=-10)

pc_avg_ketones_res_combined <- map_dfr(
  names(pc_avg_ketones_res),
  function(gene_name) {
    df <- pc_avg_ketones_res[[gene_name]]$summary
    df <- df %>% filter(gene_name %in% pc_avg_ketones_converged$Gene) %>%
      mutate(Gene = gene_name)
    return(df)
  }
)

pc_avg_ketones_res_combined <- pc_avg_ketones_res_combined %>%
  ungroup() %>%
  mutate(fdr_interaction = p.adjust(p_avg_ketones_delta, method = "fdr"))

pc_avg_ketones_res_combined_disp <- map_dfr(
  names(pc_avg_ketones_res),
  function(gene_name) {
    df <- pc_avg_ketones_res[[gene_name]]$overdispersion
    df <- df %>% mutate(Gene = gene_name)
    return(df)
  }
)

pc_avg_ketones_res_combined <- subset(pc_avg_ketones_res_combined, logFC_avg_ketones_delta >-10)

write.csv(pc_avg_ketones_res_combined, "/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/nebula/pc_avg_ketones.csv", row.names = F)
```

```{r echo = F}
plot_volcano_associations(subset(pc_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "PC ketones", 
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
                          "nebula/pc_avg_ketones_trtcolors",
                          treatment_results = pc_nebula_res_combined_reml_pooled,
                          treatment_p_col = "p_treatmentDapagliflozin:visitPOST",
                          formula = "\u0394 ketones + treatment",
                          positive_text = "Positively associated with \n\u0394 ketones",
...
```{r echo = F}
plot_volcano_concordance(pc_avg_ketones_res_combined, 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
...
```{r echo =F, eval = F}
plot_volcano(subset(pc_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), "logFC_avg_ketones_delta", 
             "p_avg_ketones_delta",
             NULL,
             "logFC \u0394 ketones", 
             "-log10(p-value)",
             "nebula/pc_avg_ketones",
             formula = "\u0394 ketones + treatment",
             positive_text = "Positively associated with \n\u0394 ketones",
             negative_text = "Negatively associated with \n\u0394 ketones",
             cell_type = "PC")
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
kegg_legacy <- prepare_gmt(gmt_files[1], unique(pc_avg_ketones_res_combined$Gene), savefile = FALSE)
reactome <- prepare_gmt(gmt_files[3], unique(pc_avg_ketones_res_combined$Gene), savefile = FALSE)
go <- prepare_gmt(gmt_files[4], unique(pc_avg_ketones_res_combined$Gene), savefile = FALSE)

# rank genes by logFC
rankings_pc_avg_ketones <- pc_avg_ketones_res_combined$logFC_avg_ketones_delta
names(rankings_pc_avg_ketones) <- pc_avg_ketones_res_combined$Gene
...
```{r echo = F}
plot_volcano_concordance(ic_mgfr_jodal_res_combined, 
                          "logFC_mgfr_jodal_delta", 
                          "p_mgfr_jodal_delta",
                          "logFC \u0394 mGFR (Raw)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ic_mgfr_jodal_bsa_res_combined, 
                          "logFC_mgfr_jodal_bsa_delta", 
                          "p_mgfr_jodal_bsa_delta",
                          "logFC \u0394 mGFR (BSA)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ic_tir_res_combined, 
                          "logFC_tir_delta", 
                          "p_tir_delta",
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ic_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ic_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ic_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta", 
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
ic_avg_k_r2_res_combined <- subset(ic_avg_k_r2_res_combined, logFC_avg_k_r2_delta >-10)
...
```{r echo = F}
plot_volcano_associations(ic_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "IC avg kidney R2*", 
                          "logFC \u0394 avg kidney R2*", 
...
```{r echo = F}
plot_volcano_concordance(ic_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
# Read in nebula results saved to kopah
ic_avg_ketones_res <- s3readRDS('associations/nebula/avg_ketones/ic_attempt_nebula_avg_ketones.rds', bucket = "attempt", region = "")

# nebula only keeps results for converged
ic_avg_ketones_convergence <- map_dfr(
  names(ic_avg_ketones_res),
  function(gene_name) {
    converged <- ic_avg_ketones_res[[gene_name]]$convergence
    df <- data.frame(Gene = gene_name,
                     Convergence_Code = converged)
    return(df)
  }
...
ic_avg_ketones_converged <- ic_avg_ketones_convergence %>%
  filter(Convergence_Code >=-10)

ic_avg_ketones_res_combined <- map_dfr(
  names(ic_avg_ketones_res),
  function(gene_name) {
    df <- ic_avg_ketones_res[[gene_name]]$summary
    df <- df %>% filter(gene_name %in% ic_avg_ketones_converged$Gene) %>%
      mutate(Gene = gene_name)
    return(df)
  }
)

ic_avg_ketones_res_combined <- ic_avg_ketones_res_combined %>%
  ungroup() %>%
  mutate(fdr_interaction = p.adjust(p_avg_ketones_delta, method = "fdr"))

ic_avg_ketones_res_combined_disp <- map_dfr(
  names(ic_avg_ketones_res),
  function(gene_name) {
    df <- ic_avg_ketones_res[[gene_name]]$overdispersion
    df <- df %>% mutate(Gene = gene_name)
    return(df)
  }
)

ic_avg_ketones_res_combined <- subset(ic_avg_ketones_res_combined, logFC_avg_ketones_delta >-10)

write.csv(ic_avg_ketones_res_combined, "/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/nebula/ic_avg_ketones.csv", row.names = F)
```

```{r echo = F}
plot_volcano_associations(subset(ic_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "IC ketones", 
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
                          "nebula/ic_avg_ketones_trtcolors",
                          treatment_results = ic_nebula_res_combined_reml_pooled,
                          treatment_p_col = "p_treatmentDapagliflozin:visitPOST",
                          formula = "\u0394 ketones + treatment",
                          positive_text = "Positively associated with \n\u0394 ketones",
...
```{r echo = F}
plot_volcano_concordance(ic_avg_ketones_res_combined, 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
...
```{r echo =F, eval = F}
plot_volcano(subset(ic_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), "logFC_avg_ketones_delta", 
             "p_avg_ketones_delta",
             NULL,
             "logFC \u0394 ketones", 
             "-log10(p-value)",
             "nebula/ic_avg_ketones",
             formula = "\u0394 ketones + treatment",
             positive_text = "Positively associated with \n\u0394 ketones",
             negative_text = "Negatively associated with \n\u0394 ketones",
             cell_type = "IC")
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
kegg_legacy <- prepare_gmt(gmt_files[1], unique(ic_avg_ketones_res_combined$Gene), savefile = FALSE)
reactome <- prepare_gmt(gmt_files[3], unique(ic_avg_ketones_res_combined$Gene), savefile = FALSE)
go <- prepare_gmt(gmt_files[4], unique(ic_avg_ketones_res_combined$Gene), savefile = FALSE)

# rank genes by logFC
rankings_ic_avg_ketones <- ic_avg_ketones_res_combined$logFC_avg_ketones_delta
names(rankings_ic_avg_ketones) <- ic_avg_ketones_res_combined$Gene
...
```{r echo = F}
plot_volcano_concordance(ec_mgfr_jodal_res_combined, 
                          "logFC_mgfr_jodal_delta", 
                          "p_mgfr_jodal_delta",
                          "logFC \u0394 mGFR (Raw)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ec_mgfr_jodal_bsa_res_combined, 
                          "logFC_mgfr_jodal_bsa_delta", 
                          "p_mgfr_jodal_bsa_delta",
                          "logFC \u0394 mGFR (BSA)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ec_tir_res_combined, 
                          "logFC_tir_delta", 
                          "p_tir_delta",
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ec_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ec_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(ec_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta", 
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
ec_avg_k_r2_res_combined <- subset(ec_avg_k_r2_res_combined, logFC_avg_k_r2_delta >-10)
...
```{r echo = F}
plot_volcano_associations(ec_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "EC avg kidney R2*", 
                          "logFC \u0394 avg kidney R2*", 
...
```{r echo = F}
plot_volcano_concordance(ec_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
# Read in nebula results saved to kopah
ec_avg_ketones_res <- s3readRDS('associations/nebula/avg_ketones/ec_attempt_nebula_avg_ketones.rds', bucket = "attempt", region = "")

# nebula only keeps results for converged
ec_avg_ketones_convergence <- map_dfr(
  names(ec_avg_ketones_res),
  function(gene_name) {
    converged <- ec_avg_ketones_res[[gene_name]]$convergence
    df <- data.frame(Gene = gene_name,
                     Convergence_Code = converged)
    return(df)
  }
...
ec_avg_ketones_converged <- ec_avg_ketones_convergence %>%
  filter(Convergence_Code >=-10)

ec_avg_ketones_res_combined <- map_dfr(
  names(ec_avg_ketones_res),
  function(gene_name) {
    df <- ec_avg_ketones_res[[gene_name]]$summary
    df <- df %>% filter(gene_name %in% ec_avg_ketones_converged$Gene) %>%
      mutate(Gene = gene_name)
    return(df)
  }
)

ec_avg_ketones_res_combined <- ec_avg_ketones_res_combined %>%
  ungroup() %>%
  mutate(fdr_interaction = p.adjust(p_avg_ketones_delta, method = "fdr"))

ec_avg_ketones_res_combined_disp <- map_dfr(
  names(ec_avg_ketones_res),
  function(gene_name) {
    df <- ec_avg_ketones_res[[gene_name]]$overdispersion
    df <- df %>% mutate(Gene = gene_name)
    return(df)
  }
)

ec_avg_ketones_res_combined <- subset(ec_avg_ketones_res_combined, logFC_avg_ketones_delta >-10)

write.csv(ec_avg_ketones_res_combined, "/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/nebula/ec_avg_ketones.csv", row.names = F)
```

```{r echo = F}
plot_volcano_associations(subset(ec_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "EC ketones", 
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
                          "nebula/ec_avg_ketones_trtcolors",
                          treatment_results = ec_nebula_res_combined_reml_pooled,
                          treatment_p_col = "p_treatmentDapagliflozin:visitPOST",
                          formula = "\u0394 ketones + treatment",
                          positive_text = "Positively associated with \n\u0394 ketones",
...
```{r echo = F}
plot_volcano_concordance(ec_avg_ketones_res_combined, 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
...
```{r echo =F, eval = F}
plot_volcano(subset(ec_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), "logFC_avg_ketones_delta", 
             "p_avg_ketones_delta",
             NULL,
             "logFC \u0394 ketones", 
             "-log10(p-value)",
             "nebula/ec_avg_ketones",
             formula = "\u0394 ketones + treatment",
             positive_text = "Positively associated with \n\u0394 ketones",
             negative_text = "Negatively associated with \n\u0394 ketones",
             cell_type = "EC")
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
kegg_legacy <- prepare_gmt(gmt_files[1], unique(ec_avg_ketones_res_combined$Gene), savefile = FALSE)
reactome <- prepare_gmt(gmt_files[3], unique(ec_avg_ketones_res_combined$Gene), savefile = FALSE)
go <- prepare_gmt(gmt_files[4], unique(ec_avg_ketones_res_combined$Gene), savefile = FALSE)

# rank genes by logFC
rankings_ec_avg_ketones <- ec_avg_ketones_res_combined$logFC_avg_ketones_delta
names(rankings_ec_avg_ketones) <- ec_avg_ketones_res_combined$Gene
...
```{r echo = F}
plot_volcano_concordance(fibvsmc_mgfr_jodal_res_combined, 
                          "logFC_mgfr_jodal_delta", 
                          "p_mgfr_jodal_delta",
                          "logFC \u0394 mGFR (Raw)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(fibvsmc_mgfr_jodal_bsa_res_combined, 
                          "logFC_mgfr_jodal_bsa_delta", 
                          "p_mgfr_jodal_bsa_delta",
                          "logFC \u0394 mGFR (BSA)", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(fibvsmc_tir_res_combined, 
                          "logFC_tir_delta", 
                          "p_tir_delta",
                          "logFC \u0394 TIR", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(fibvsmc_hba1c_res_combined, 
                          "logFC_hba1c_delta", 
                          "p_hba1c_delta",
                          "logFC \u0394 HbA1c", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(fibvsmc_weight_res_combined, 
                          "logFC_weight_delta", 
                          "p_weight_delta",
                          "logFC \u0394 Weight", 
                          "-log10(p-value)",
...
```{r echo = F}
plot_volcano_concordance(fibvsmc_avg_c_r2_res_combined, 
                          "logFC_avg_c_r2_delta", 
                          "p_avg_c_r2_delta",
                          "logFC \u0394 avg cortical R2*", 
                          "-log10(p-value)",
...
fibvsmc_avg_k_r2_res_combined <- subset(fibvsmc_avg_k_r2_res_combined, logFC_avg_k_r2_delta >-10)
...
```{r echo = F}
plot_volcano_associations(fibvsmc_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
...
```{r echo = F}
plot_volcano_concordance(fibvsmc_avg_k_r2_res_combined, 
                          "logFC_avg_k_r2_delta", 
                          "p_avg_k_r2_delta",
                          "logFC \u0394 avg kidney R2*", 
                          "-log10(p-value)",
...
```{r echo = F}
# Read in nebula results saved to kopah
fibvsmcp_avg_ketones_res <- s3readRDS('associations/nebula/avg_ketones/fibvsmcp_attempt_nebula_avg_ketones.rds', bucket = "attempt", region = "")

# nebula only keeps results for converged
fibvsmcp_avg_ketones_convergence <- map_dfr(
  names(fibvsmcp_avg_ketones_res),
  function(gene_name) {
    converged <- fibvsmcp_avg_ketones_res[[gene_name]]$convergence
    df <- data.frame(Gene = gene_name,
                     Convergence_Code = converged)
    return(df)
  }
...
fibvsmcp_avg_ketones_converged <- fibvsmcp_avg_ketones_convergence %>%
  filter(Convergence_Code >=-10)

fibvsmcp_avg_ketones_res_combined <- map_dfr(
  names(fibvsmcp_avg_ketones_res),
  function(gene_name) {
    df <- fibvsmcp_avg_ketones_res[[gene_name]]$summary
    df <- df %>% filter(gene_name %in% fibvsmcp_avg_ketones_converged$Gene) %>%
      mutate(Gene = gene_name)
    return(df)
  }
)

fibvsmcp_avg_ketones_res_combined <- fibvsmcp_avg_ketones_res_combined %>%
  ungroup() %>%
  mutate(fdr_interaction = p.adjust(p_avg_ketones_delta, method = "fdr"))

fibvsmcp_avg_ketones_res_combined_disp <- map_dfr(
  names(fibvsmcp_avg_ketones_res),
  function(gene_name) {
    df <- fibvsmcp_avg_ketones_res[[gene_name]]$overdispersion
    df <- df %>% mutate(Gene = gene_name)
    return(df)
  }
)

fibvsmcp_avg_ketones_res_combined <- subset(fibvsmcp_avg_ketones_res_combined, logFC_avg_ketones_delta >-10)

write.csv(fibvsmcp_avg_ketones_res_combined, "/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/nebula/fibvsmcp_avg_ketones.csv", row.names = F)
```

```{r echo = F}
plot_volcano_associations(subset(fibvsmcp_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "FIB, VSMC/P ketones", 
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
                          "nebula/fibvsmcp_avg_ketones_trtcolors",
                          treatment_results = fibvsmcp_nebula_res_combined_reml_pooled,
                          treatment_p_col = "p_treatmentDapagliflozin:visitPOST",
                          formula = "\u0394 ketones + treatment",
                          positive_text = "Positively associated with \n\u0394 ketones",
                          negative_text = "Negatively associated with \n\u0394 ketones")
...
```{r echo = F}
plot_volcano_concordance(fibvsmcp_avg_ketones_res_combined, 
                          "logFC_avg_ketones_delta", 
                          "p_avg_ketones_delta",
                          "logFC \u0394 ketones", 
                          "-log10(p-value)",
...
```{r echo =F, eval = F}
plot_volcano(subset(fibvsmcp_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), "logFC_avg_ketones_delta", 
             "p_avg_ketones_delta",
             NULL,
             "logFC \u0394 ketones", 
             "-log10(p-value)",
             "nebula/fibvsmcp_avg_ketones",
             formula = "\u0394 ketones + treatment",
             positive_text = "Positively associated with \n\u0394 ketones",
             negative_text = "Negatively associated with \n\u0394 ketones",
             cell_type = "FIB, VSMC/P")
...
plot_volcano(subset(fibvsmcp_avg_ketones_res_combined, logFC_avg_ketones_delta >-10), "logFC_avg_ketones_delta", 
             ...
             "logFC \u0394 ketones", 
             "-log10(FDR adjusted p-value)",
             "nebula/fibvsmcp_avg_ketones_fdr",
             formula = "\u0394 ketones + treatment",
             positive_text = "Positively associated with \n\u0394 ketones",
             negative_text = "Negatively associated with \n\u0394 ketones",
             cell_type = "FIB, VSMC/P")
...
plot_fgsea_transpose <- function(fgsea_res,
  ...
  fgsea_res <- fgsea_res %>%
    ...
    mutate(
      ...
      pathway_clean = str_replace_all(pathway_clean, "\\bv\\b", "V"),
      pathway_clean = str_replace_all(pathway_clean, regex("\\(immune\\)", ignore_case = TRUE), "(IMMUNE)"),
      pathway_clean = capifibvsmcpize_acronyms(pathway_clean, acronyms),
      pathway_clean = replace_mixed_case(pathway_clean, special_mixed, special_replacements),
      pathway_clean = paste0(pathway_clean, " (", size, ")")
    ) %>%
    arrange(pval)
  ...
  fgsea_res %>%
    ggplot(aes(x = abs(NES), y = fct_rev(pathway_clean), label = pathway_clean)) +
    geom_point(aes(size = -log10(pval), color = direction, alpha = 0.8)) +
    # geom_vline(xintercefibvsmcp = 2, linetype = "dashed") +
    geom_text(aes(group = pathway_clean, color = direction, fontface = face), 
              hjust = 0, size = text1, nudge_x = xnudge) +
    scale_size_binned() +
    scale_color_manual(values = c("Positive" = "#c75146", "Negative" = "#2c7da0", 
...
gmt_files <- list.files(path = bg_path, pattern = '.gmt', full.names = TRUE)
gmt_files
kegg_legacy <- prepare_gmt(gmt_files[1], unique(fibvsmcp_avg_ketones_res_combined$Gene), savefile = FALSE)
reactome <- prepare_gmt(gmt_files[3], unique(fibvsmcp_avg_ketones_res_combined$Gene), savefile = FALSE)
go <- prepare_gmt(gmt_files[4], unique(fibvsmcp_avg_ketones_res_combined$Gene), savefile = FALSE)

# rank genes by logFC
rankings_fibvsmcp_avg_ketones <- fibvsmcp_avg_ketones_res_combined$logFC_avg_ketones_delta
names(rankings_fibvsmcp_avg_ketones) <- fibvsmcp_avg_ketones_res_combined$Gene
rankings_fibvsmcp_avg_ketones <- sort(rankings_fibvsmcp_avg_ketones, decreasing = TRUE)
plot(rankings_fibvsmcp_avg_ketones)
min(rankings_fibvsmcp_avg_ketones)
max(rankings_fibvsmcp_avg_ketones)
```

```{r echo = F, eval = F}
set.seed(1234)

kegg_legacy_res_fibvsmcp_avg_ketones <- fgsea(pathways = kegg_legacy,
                                 stats = rankings_fibvsmcp_avg_ketones,
                                 scoreType = 'std', 
                                 minSize = 3,
                                 maxSize = 500,
                                 nproc = 1)

reactome_res_fibvsmcp_avg_ketones <- fgsea(pathways = reactome,
                              stats = rankings_fibvsmcp_avg_ketones,
                              scoreType = 'std', 
                              minSize = 3,
                              maxSize = 500,
                              nproc = 1)
go_res_fibvsmcp_avg_ketones <- fgsea(pathways = go,
                        stats = rankings_fibvsmcp_avg_ketones,
                        scoreType = 'std', 
                        minSize = 5,
                        maxSize = 500,
...
fgsea <- data.frame("KEGG Legacy"=c(sum(kegg_legacy_res_fibvsmcp_avg_ketones[, padj < 0.05], na.rm = T), sum(kegg_legacy_res_fibvsmcp_avg_ketones[, pval < 0.05], na.rm = T)),
                         "REACTOME"=c(sum(reactome_res_fibvsmcp_avg_ketones[, padj < 0.05], na.rm = T), sum(reactome_res_fibvsmcp_avg_ketones[, pval < 0.05], na.rm = T)),
                         "GO"=c(sum(go_res_fibvsmcp_avg_ketones[, padj < 0.05], na.rm = T), sum(go_res_fibvsmcp_avg_ketones[, pval < 0.05], na.rm = T)))
rownames(fgsea) <- c("adj.pval", "p.val")
```

##### KEGG Legacy
```{r echo = F, eval = F}
kegg_legacy_res_fibvsmcp_avg_ketones_filtered <- filter_redundant_pathways(kegg_legacy_res_fibvsmcp_avg_ketones, overlap_pct = 0.3)

plot_fgsea_transpose(kegg_legacy_res_fibvsmcp_avg_ketones_filtered, title = "FIB, VSMC/P Top 30 ketones Associations KEGG", xmin = 1, xmax = 3)

ggsave("/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/Figures/Pathways/nebula/fibvsmcp_avg_ketones_top30_kegg_pathways.jpeg",
       width = 27.5, height = 14, scale = 1)
```

##### REACTOME
```{r echo = F, eval = F}
reactome_res_fibvsmcp_avg_ketones_filtered <- filter_redundant_pathways(reactome_res_fibvsmcp_avg_ketones, overlap_pct = 0.3)

plot_fgsea_transpose(reactome_res_fibvsmcp_avg_ketones_filtered, title = "FIB, VSMC/P Top 30  ketones Associations REACTOME", xmin = 1.1, xmax = 3.5)

ggsave("/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/Figures/Pathways/nebula/fibvsmcp_avg_ketones_top30_reactome_pathways.jpeg",
       width = 27.5, height = 14, scale = 1)
```
##### GO
```{r echo = F, eval = F}
go_res_fibvsmcp_avg_ketones_filtered <- filter_redundant_pathways(go_res_fibvsmcp_avg_ketones, overlap_pct = 0.3)

plot_fgsea_transpose(go_res_fibvsmcp_avg_ketones_filtered, title = "FIB, VSMC/P Top 30  ketones Associations GO", xmin = 1.4, xmax = 5)

ggsave("/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura Pyle - Bjornstad/Biostatistics Core Shared Drive/ATTEMPT/Results/Figures/Pathways/nebula/fibvsmcp_avg_ketones_top30_go_pathways.jpeg",
       width = 27.5, height = 14, scale = 1)
```
