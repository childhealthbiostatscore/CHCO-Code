...
(summary(arsenal::tableby(treatment ~ mgfr_jodal_bsa + mgfr_jodal + mri_r2_cortex_l + mri_r2_cortex_r + avg_c_r2 + avg_k_r2 + avg_m_r2, data = subset(attempt_dat, visit == -4 & site == "Denver")), digits = 1))
```
...
![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Plot%20over%20time/delta_avg_m_r2_overtime.jpeg){width="100%"}
...
![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_ec_plasma.png)

```{r echo= F}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/ec_plasma_concordance.csv"))$concordance)
```

![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_vsmc_plasma.png)
```{r echo= F, eval = T}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/vsmc_plasma_concordance.csv"))$concordance)
```

![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_combined_plasma.png)
```{r echo= F, eval = T}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/combined_plasma_concordance.csv"))$concordance)
```
...
![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_glom_plasma.png)
```{r echo= F, eval = T}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/glom_plasma_concordance.csv"))$concordance)
```

![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_immune_plasma.png)

```{r echo= F}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/immune_plasma_concordance.csv"))$concordance)
```
...
![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_pt_urine.png)

```{r echo= F}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/pt_urine_concordance.csv"))$concordance)
```

![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_dct_urine.png)
```{r echo= F}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/dct_urine_concordance.csv"))$concordance)
```

![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_pod_urine.png)
```{r echo= F}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/pod_urine_concordance.csv"))$concordance)
```
...
![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_tal_urine.png)
```{r echo= F}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/tal_urine_concordance.csv"))$concordance)
```

![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_cd_urine.png)
```{r echo= F, eval = T}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/cd_urine_concordance.csv"))$concordance)
```

![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Volcano Plots/_kpmp_combined_urine.png)
```{r echo= F, eval = T}
table(read.csv(file.path(root_path, "ATTEMPT/Results/Somascan/Transcript concordance/combined_urine_concordance.csv"))$concordance)
```
:::
...
captions <- c("mgfr_jodal_bsa" = "A positive change in mGFR was consistently associated with later/stressed PT states across all quantiles. Effects were highly significant at τ = 0.25, 0.65, and 0.85 (all p < 0.0001).",
"hba1c" = "Higher HbA1c was associated with earlier/less differentiated PT states. Effects were significant at τ = 0.25 (β = -0.38, p < 0.0001) and τ = 0.85 (β = -1.82, p < 0.0001), but not at τ = 0.65 (β = -1.02, p = 0.378).",
"tir" = "Higher TIR was inconsistently associated with PT states across quantiles. Effects were not significant at τ = 0.25 (β = 0.002, p = 0.770), but were significantly associated with earlier PT states at τ = 0.65 (β = -0.03, p < 0.0001) and τ = 0.85 (β = -0.10, p < 0.0001).",
"weight" = "Weight change showed opposing effects across quantiles. Higher weight change was associated with earlier PT states at τ = 0.25 (β = -0.08, p < 0.0001), showed no significant effect at τ = 0.65 (β = -0.14, p = 0.084), but was associated with later PT states at τ = 0.85 (β = 0.13, p = 0.0003).")

for (v in vars) {
  key <- v
  v_delta <- paste0(v, "_delta")

  # Load model + summary
  model_path <- paste0("Results/pt_kpmp_rq_fit_", v_delta, ".rds")
  pt_kpmp_rq_fit <- s3readRDS(model_path, bucket = "attempt", region = "")
  pt_kpmp_rq_summary <- quantreg:::summary.rqs(pt_kpmp_rq_fit, se = "nid")
  coef_table <- dplyr::bind_rows(lapply(seq_along(pt_kpmp_rq_summary), function(i) {
    coef_df <- as.data.frame(pt_kpmp_rq_summary[[i]]$coefficients)
    coef_df$term <- rownames(coef_df)
  ...
  # Open a two-column layout
  cat("::: {.columns}\n")
  
  # Left column = figure
  cat("::: {.column width=\"50%\"}\n")
  cat(paste0("![](/Users/choiyej/Library/CloudStorage/OneDrive-SharedLibraries-UW/Laura%20Pyle%20-%20Bjornstad/Biostatistics%20Core%20Shared%20Drive/ATTEMPT/Results/Figures/Slingshot/attempt_density_pt_kpmp_", key, "_deltadelta_slingshot.jpeg){width=100%}\n"))
  cat(":::\n")
  
  # Right column = table + caption
  cat("::: {.column width=\"50%\"}\n")
  print(knitr::kable(coef_table, row.names = FALSE))
  cat("\n\n", captions[[key]], "\n")
  cat(":::\n")
