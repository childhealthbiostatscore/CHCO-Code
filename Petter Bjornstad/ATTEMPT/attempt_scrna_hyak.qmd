...
inflammatory_response_hallmark <- 
  msigdb_hallmark[msigdb_hallmark$gs_name == "HALLMARK_INFLAMMATORY_RESPONSE",] %>%
  pull(gene_symbol)

apoptosis_response_hallmark <- 
  msigdb_hallmark[msigdb_hallmark$gs_name == "HALLMARK_APOPTOSIS",] %>%
  pull(gene_symbol)

oxphos_hallmark <- 
  msigdb_hallmark[msigdb_hallmark$gs_name == "HALLMARK_OXIDATIVE_PHOSPHORYLATION",] %>%
  pull(gene_symbol)


# Curated immune and metabolism gene sets for kidney biology
kidney_gene_sets <- list(
  inflammatory_response = inflammatory_response_hallmark,
  apoptosis = apoptosis_response_hallmark,
  oxphos = oxphos_hallmark)
...
score_df <- attempt_so@meta.data %>%
  ...
  summarise(
    across(
      c(inflammatory_response1, apoptosis2, oxphos3),
      mean,  # or mean if you prefer
      .names = "{.col}"
    ),
    .groups = "drop"
  )
...
score_long <- score_df %>%
  pivot_longer(
    cols = c(inflammatory_response1, apoptosis2, oxphos3),
    names_to = "score_type",
...
score_delta <- score_long %>%
  group_by(subject_id, treatment, KPMP_celltype, score_type) %>%
...
score_delta %>%
  ggplot(aes(x = treatment, y = score_type, fill = delta)) +
  geom_tile() +
...
#   ggplot(aes(x = treatment, y = inflammatory_response1, fill = visit)) +
#   geom_boxplot() +
#   theme_minimal() +
#   facet_wrap(~KPMP_celltype)
# 
# attempt_so@meta.data %>%
#   filter(KPMP_celltype %in% c("C-TAL-1", "C-TAL-2", "dTAL")) %>%
#   ggplot(aes(x = treatment, y = inflammatory_response1, fill = visit)) +
#   geom_boxplot() +
#   theme_minimal() +
#   facet_wrap(~KPMP_celltype)
# 
# attempt_so@meta.data %>%
#   ggplot(aes(x = treatment, y = apoptosis2, fill = visit)) +
#   geom_boxplot() +
#   theme_bw()
# 
# attempt_so@meta.data %>%
#   ggplot(aes(x = treatment, y = oxphos3, fill = visit)) +
#   geom_boxplot() +
#   theme_bw()
```
