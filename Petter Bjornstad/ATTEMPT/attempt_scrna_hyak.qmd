...
```{r echo = F, eval = T}
counts_pt <- round(GetAssayData(attempt_so_pt, layer = "counts")) # load counts and round
genes_list <- rownames(counts_pt)

# meta_pt <- attempt_so_pt@meta.data
# pred = model.matrix(~treatment*visit, data = meta_pt)
# data_g_pt = group_cell(count=counts_pt, id=meta_pt$subject, pred=pred)
...
nebula_pt_results_list_reml_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
  ...
  tryCatch({
    count_gene <- counts_pt[g, , drop = FALSE]
    meta_gene <- subset(attempt_so_pt, features = g)@meta.data
    pred_gene <- model.matrix(~treatment * visit, data = meta_gene)
    data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(nebula_pt_results_list_reml_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'PT/nebula/pt_attempt_nebula_res_reml_pooled.rds')
```
...
temp_file <- tempfile(fileext = ".jpeg")
ggsave(temp_file, height = 10, width = 8)
s3$upload_file(temp_file, "attempt", "Figures/Proportions/attempt_pod_treatment_visit_proportions_plot.jpeg")
...
temp_file <- tempfile(fileext = ".jpeg")
ggsave(temp_file, height = 10)
s3$upload_file(temp_file, "attempt", "Figures/Proportions/attempt_pod_treatment_proportions_plot.jpeg")
...
for (ct in celltype_vec) {
  ...
  so_ct <- subset(attempt_so_hvg_pod, celltype == ct)
  ...
  tmp <- tempfile(fileext = ".rds")
  saveRDS(nebula_res_list, tmp)
  s3_key <- sprintf("POD/%s/nebula/%s_nebula_res_reml_pooled.rds", ct, ct)
  s3$upload_file(tmp, "attempt", s3_key)
}
...
for (ct in celltype_vec) {
  ...
  so_ct <- subset(attempt_so_pod, celltype == ct)
  ...
  tmp <- tempfile(fileext = ".rds")
  saveRDS(nebula_res_list, tmp)
  s3_key <- sprintf("POD/%s/nebula/%s_nebula_res_reml_pooled_full.rds", ct, ct)
  s3$upload_file(tmp, "attempt", s3_key)
}
