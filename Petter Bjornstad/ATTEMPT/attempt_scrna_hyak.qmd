...
# module scores
# msigdb Hallmark
msigdb_hallmark <- msigdbr(category = "H")

hallmark_pathway_sets <- c(
  ...
  HALLMARK_IL6_JAK_STAT3_SIGNALING = "IL6_JAK_STAT3_Signaling",
  HALLMARK_HYPOXIA                 = "Hypoxia",
  HALLMARK_REACTIVE_OXYGEN_SPECIES_PATHWAY = "ROS_Pathway",
  HALLMARK_PI3K_AKT_MTOR_SIGNALING = "PI3K_AKT_MTOR_Signaling"
)

hallmark_groups <- list(
  Inflammatory = c(
    "HALLMARK_INFLAMMATORY_RESPONSE",
    "HALLMARK_IL6_JAK_STAT3_SIGNALING",
    "HALLMARK_PI3K_AKT_MTOR_SIGNALING"   # overlaps immune/metabolic, but included here
  ),
  ...
  Stress_Adaptive = c(   # not purely one or the other
    "HALLMARK_APOPTOSIS",
    "HALLMARK_HYPOXIA",
...
msigdb_h <- msigdbr(species = "Homo sapiens", category = "H") %>%
  filter(gs_name %in% names(hallmark_pathway_sets))

# build named list: short keys → character vectors of genes
kidney_gene_sets <- setNames(
  lapply(names(hallmark_pathway_sets), \(nm) msigdb_h %>% filter(gs_name == nm) %>% pull(gene_symbol)),
  unname(hallmark_pathway_sets)
)

grouped_kidney_gene_sets <- lapply(hallmark_groups, function(gs_vec) {
  short_names <- unname(hallmark_pathway_sets[gs_vec])
  unique(unlist(kidney_gene_sets[short_names], use.names = FALSE))
  }
)


attempt_so <- AddModuleScore(attempt_so, features = kidney_gene_sets, name = names(kidney_gene_sets))
attempt_so <- AddModuleScore(attempt_so, features = grouped_kidney_gene_sets, name = names(grouped_kidney_gene_sets))
md_names <- colnames(attempt_so@meta.data)

# Collect both sets of names
all_gene_set_names <- c(
  names(kidney_gene_sets),        # e.g. "Inflammatory_Response", "OxPhos", ...
  names(grouped_kidney_gene_sets)         # e.g. "Inflammatory", "Metabolic", "Stress_Adaptive"
)

# Build regex pattern
pattern <- paste0("^(", paste(all_gene_set_names, collapse="|"), ")_?\\d+$")
...
```{r echo = F}
score_df <- attempt_so@meta.data %>%
  group_by(subject_id, treatment, visit, KPMP_celltype_general) %>%
...
score_long <- score_df %>%
  pivot_longer(
    cols = (all_gene_set_names),
    names_to = "score_type",
    values_to = "score_value"
  )

# Step 3: compute deltas only if paired
score_delta <- score_long %>%
  group_by(subject_id, treatment, KPMP_celltype_general, score_type) %>%
  # excludes the 0 deltas
  filter(n() >= 2) %>%
  arrange(visit) %>%
  summarise(
    delta = dplyr::last(score_value) - dplyr::first(score_value),
    .groups = "drop"
  )

# Step 1: summarise subject × visit × cell type × score
# 1) Compute DiD per celltype × score_type
did_tbl <- score_long %>%
  group_by(KPMP_celltype_general, visit, score_type, treatment) %>%
  summarise(mean = mean(score_value, na.rm = TRUE),
            n = dplyr::n(), .groups = "drop") %>%
  pivot_wider(names_from = c(treatment, visit), values_from = c(mean, n)) %>%
  mutate(
    mean_delta_Dapagliflozin = mean_Dapagliflozin_POST - mean_Dapagliflozin_PRE,
    mean_delta_Placebo = mean_Placebo_POST - mean_Placebo_PRE,
    DiD = `mean_delta_Dapagliflozin` - `mean_delta_Placebo`,
    score_type = gsub("_", " ", score_type)
  ) %>%
  pivot_longer(
    cols = c(starts_with("mean_delta_"), "DiD"),
    names_to   = "treatment",
    values_to  = "mean_delta",
    names_prefix = "mean_delta_"
  ) %>%
  dplyr::mutate(treatment = fct_relevel(factor(treatment), "DiD", "Dapagliflozin", "Placebo"),
                KPMP_celltype_general = fct_relevel(factor(KPMP_celltype_general), "PT", "TAL", "EC", "IC",
                                                    "POD", "Immune (L)", "Immune (M)", "PC", "VSMC/P, FIB"))

# 4) Rebuild your per-celltype plots (now includes a DiD column)
plots <- did_tbl %>%
  split(.$KPMP_celltype_general) %>%
  purrr::map(~ ggplot(.x, aes(x = treatment, y = score_type, fill = mean_delta)) +
               geom_tile() +
               ...
               ggtitle(unique(.x$KPMP_celltype_general))+
               labs(fill = "\u0394Score")
  )
...
did_tbl %>%
  ggplot(aes(x = treatment, y = score_type, fill = mean_delta)) +
  geom_tile() +
  scale_fill_gradient2(low = "#89c2d9", mid = "white", high = "#ee7674", midpoint = 0) +
  theme_minimal() +
  theme(panel.grid = element_blank(),
        ...
        axis.title.y = element_blank(),
        axis.text.x = element_text(angle = 60, hjust = 1)) +
  facet_wrap(~KPMP_celltype_general) +
  labs(fill = "\u0394Score")

temp_file <- tempfile(fileext = ".png")
ggsave(temp_file, width = 10, height = 7)
s3$upload_file(temp_file, "attempt", "Figures/attempt_geneset_scores.png") 
```

```{r echo = F}
attempt_so_meta <- attempt_so@meta.data

inflammatory_fit <- lmer(Inflammatory ~ treatment*visit*KPMP_celltype_general + (1|subject_id), data = attempt_so_meta)
inflammatory_emm <- emmeans(inflammatory_fit, ~treatment*visit | KPMP_celltype_general)
inflammatory_contrasts <- contrast(inflammatory_emm, interaction = "trt.vs.ctrl", by = "KPMP_celltype_general")
inflammatory_contrasts_df <- tidy(inflammatory_contrasts)%>%
  filter(KPMP_celltype_general != "Other") %>%
  mutate(
    CI_low = estimate - 1.96*std.error,
    CI_high = estimate + 1.96*std.error,
    KPMP_celltype_general = factor(KPMP_celltype_general, levels = rev(inflammatory_contrasts_df$KPMP_celltype_general[order(inflammatory_contrasts_df$estimate)])),
    sig = case_when(p.value < 0.05 & estimate > 0 ~ "+", 
                    p.value < 0.05 & estimate < 0 ~ "-", 
                    T ~ "NS")) 

metabolic_fit <- lmer(Metabolic ~ treatment*visit*KPMP_celltype_general + (1|subject_id), data = attempt_so_meta)
...
metabolic_contrasts_df <- tidy(metabolic_contrasts) %>%
  ...
  mutate(
    CI_low = estimate - 1.96*std.error,
    CI_high = estimate + 1.96*std.error,
    KPMP_celltype_general = factor(KPMP_celltype_general, levels = rev(metabolic_contrasts_df$KPMP_celltype_general[order(metabolic_contrasts_df$estimate)])),
    sig = case_when(p.value < 0.05 & estimate > 0 ~ "+", 
                    p.value < 0.05 & estimate < 0 ~ "-", 
                    T ~ "NS")) 
```
...
inflammatory_contrasts_df %>%
  ...
  labs(x = "DiD (Inflammatory score)", y = NULL)
