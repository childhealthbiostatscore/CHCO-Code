...
dat <- s3readRDS("attempt_clinical_data.RDS",
                 bucket = "harmonized.dataset", region = "")
so_attempt_pt <- readRDS(file = file.path(root_path, "ATTEMPT/Data Clean/attempt_so_pt.RDS"))
all_data <- readRDS(file = file.path(root_path, "ATTEMPT/Data Clean/attempt_pb.RDS")) %>%
...
summary(tableby(treatment_arm ~  race + ethnicity , data = subset(merged_data, visit == "baseline")), test = F, total = F)
```
