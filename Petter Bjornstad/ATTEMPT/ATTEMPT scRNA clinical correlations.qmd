...
attempt_so_meta <- left_join(attempt_so_meta, attempt_dat) %>%
  dplyr::select(-c(sex, demographics_complete, hct, age, ethnicity, randomization_order, randomization_date, randomization_time, treatment,
randomization_user, randomization_site, randomization_filtration_status)) %>%
  left_join(attempt_dat2)
```
...
delta_df <-  attempt_so_meta %>%
  group_by(subject_id) %>%
  summarise(gfr_post = mean(gfr_bsa_plasma[visit == "POST"]),
            gfr_pre  = mean(gfr_bsa_plasma[visit == "PRE"]),
            gfr_bsa_plasma_delta = gfr_post - gfr_pre,
            gfr_raw_post = mean(gfr_raw_plasma[visit == "POST"]),
            gfr_raw_pre  = mean(gfr_raw_plasma[visit == "PRE"]),
            gfr_raw_plasma_delta = gfr_raw_post - gfr_raw_pre,
            hba1c_post = mean(hba1c[visit == "POST"]),
            hba1c_pre  = mean(hba1c[visit == "PRE"]),
            hba1c_delta = hba1c_post - hba1c_pre,
            weight_post = mean(weight[visit == "POST"]),
            weight_pre  = mean(weight[visit == "PRE"]),
            weight_delta = weight_post - weight_pre,
            avg_c_r2_post = mean(avg_c_r2[visit == "POST"]),
            avg_c_r2_pre  = mean(avg_c_r2[visit == "PRE"]),
            avg_c_r2_delta = avg_c_r2_post - avg_c_r2_pre,
            avg_k_r2_post = mean(avg_k_r2[visit == "POST"]),
            avg_k_r2_pre  = mean(avg_k_r2[visit == "PRE"]),
            avg_k_r2_delta = avg_k_r2_post - avg_k_r2_pre,
            .groups = "drop") %>%
  dplyr::select(subject_id, ends_with("_delta"))
delta_df[delta_df == 0] <- NA # not truly zero
...
```{r echo = F}
# Subset the Seurat object to nonmissing gfr_raw_plasma_delta
attempt_so_hvg_pt_meta <- attempt_so_hvg_pt@meta.data
attempt_so_hvg_pt_nonmissing_names <- rownames(attempt_so_hvg_pt_meta)[!is.na(attempt_so_hvg_pt_meta$gfr_raw_plasma_delta)]
attempt_so_hvg_pt_nonmissing <- attempt_so_hvg_pt[, attempt_so_hvg_pt_nonmissing_names, drop = FALSE]
...
pt_mgfr_raw_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_pt_nonmissing[g, , drop = FALSE]
                              meta_gene <- subset(attempt_so_hvg_pt_nonmissing, features = g)@meta.data
                              pred_gene <- model.matrix(~gfr_raw_plasma_delta, data = meta_gene)
...
names(pt_mgfr_raw_pooled) <- sapply(pt_mgfr_raw_pooled, function(x) x$gene)  # set names
pt_mgfr_raw_pooled <- lapply(pt_mgfr_raw_pooled, function(x) x$result)  # clean list back to just results
pt_mgfr_raw_pooled <- Filter(Negate(is.null), pt_mgfr_raw_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pt_mgfr_raw_filtered <- (length(genes_list)-length(pt_mgfr_raw_pooled))/length(genes_list)
print(paste0(pt_mgfr_raw_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_mgfr_raw_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_raw/pt_attempt_nebula_mgfr_raw.rds')
```

#### mGFR (BSA)

```{r echo = F}
# Subset the Seurat object to nonmissing gfr_bsa_plasma_delta
attempt_so_hvg_pt_meta <- attempt_so_hvg_pt@meta.data
attempt_so_hvg_pt_nonmissing_names <- rownames(attempt_so_hvg_pt_meta)[!is.na(attempt_so_hvg_pt_meta$gfr_bsa_plasma_delta)]
attempt_so_hvg_pt_nonmissing <- attempt_so_hvg_pt[, attempt_so_hvg_pt_nonmissing_names, drop = FALSE]
counts_hvg_pt_nonmissing <- round(GetAssayData(attempt_so_hvg_pt_nonmissing, layer = "counts")) # load counts and round
...
pt_mgfr_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_pt_nonmissing[g, , drop = FALSE]
                              meta_gene <- subset(attempt_so_hvg_pt_nonmissing, features = g)@meta.data
                              pred_gene <- model.matrix(~gfr_bsa_plasma_delta, data = meta_gene)
                              data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
# set the names of results based on gene names
names(pt_mgfr_bsa_pooled) <- sapply(pt_mgfr_bsa_pooled, function(x) x$gene)  # set names
pt_mgfr_bsa_pooled <- lapply(pt_mgfr_bsa_pooled, function(x) x$result)  # clean list back to just results
pt_mgfr_bsa_pooled <- Filter(Negate(is.null), pt_mgfr_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pt_mgfr_bsa_filtered <- (length(genes_list)-length(pt_mgfr_bsa_pooled))/length(genes_list)
print(paste0(pt_mgfr_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_mgfr_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_bsa/pt_attempt_nebula_mgfr_bsa.rds')
```
...
pt_hba1c_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_pt_nonmissing[g, , drop = FALSE]
                              meta_gene <- subset(attempt_so_hvg_pt_nonmissing, features = g)@meta.data
                              pred_gene <- model.matrix(~hba1c_delta, data = meta_gene)
                              data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
pt_weight_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_pt_nonmissing[g, , drop = FALSE]
                              meta_gene <- subset(attempt_so_hvg_pt_nonmissing, features = g)@meta.data
                              pred_gene <- model.matrix(~weight_delta, data = meta_gene)
                              data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
pt_avg_c_r2_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_pt_nonmissing[g, , drop = FALSE]
                              meta_gene <- subset(attempt_so_hvg_pt_nonmissing, features = g)@meta.data
                              pred_gene <- model.matrix(~avg_c_r2_delta, data = meta_gene)
                              data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
pt_avg_k_r2_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_pt_nonmissing[g, , drop = FALSE]
                              meta_gene <- subset(attempt_so_hvg_pt_nonmissing, features = g)@meta.data
                              pred_gene <- model.matrix(~avg_k_r2_delta, data = meta_gene)
                              data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
attempt_so_hvg_tal_nonmissing_names <- rownames(attempt_so_hvg_tal_meta)[!is.na(attempt_so_hvg_tal_meta$gfr_raw_plasma_delta)]
attempt_so_hvg_tal_nonmissing <- attempt_so_hvg_tal[, attempt_so_hvg_tal_nonmissing_names, drop = FALSE]
...
tal_mgfr_raw_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_tal_nonmissing[g, , drop = FALSE]
...
## After the loop:
for (res in tal_mgfr_raw_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(tal_mgfr_raw_pooled) <- sapply(tal_mgfr_raw_pooled, function(x) x$gene)  # set names
tal_mgfr_raw_pooled <- lapply(tal_mgfr_raw_pooled, function(x) x$result)  # clean list back to just results
tal_mgfr_raw_pooled <- Filter(Negate(is.null), tal_mgfr_raw_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

tal_mgfr_raw_filtered <- (length(genes_list)-length(tal_mgfr_raw_pooled))/length(genes_list)
print(paste0(tal_mgfr_raw_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_mgfr_raw_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_raw/tal_attempt_nebula_mgfr_raw.rds')
```
...
# Subset the Seurat object to nonmissing gfr_bsa_plasma_delta
attempt_so_hvg_tal_meta <- attempt_so_hvg_tal@meta.data
attempt_so_hvg_tal_nonmissing_names <- rownames(attempt_so_hvg_tal_meta)[!is.na(attempt_so_hvg_tal_meta$gfr_bsa_plasma_delta)]
attempt_so_hvg_tal_nonmissing <- attempt_so_hvg_tal[, attempt_so_hvg_tal_nonmissing_names, drop = FALSE]
counts_hvg_tal_nonmissing <- round(GetAssayData(attempt_so_hvg_tal_nonmissing, layer = "counts")) # load counts and round
...
tal_mgfr_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
...
## After the loop:
for (res in tal_mgfr_bsa_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(tal_mgfr_bsa_pooled) <- sapply(tal_mgfr_bsa_pooled, function(x) x$gene)  # set names
tal_mgfr_bsa_pooled <- lapply(tal_mgfr_bsa_pooled, function(x) x$result)  # clean list back to just results
tal_mgfr_bsa_pooled <- Filter(Negate(is.null), tal_mgfr_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

tal_mgfr_bsa_filtered <- (length(genes_list)-length(tal_mgfr_bsa_pooled))/length(genes_list)
print(paste0(tal_mgfr_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_mgfr_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_bsa/tal_attempt_nebula_mgfr_bsa.rds')
```
...
```{r echo = F}
# Subset the Seurat object to nonmissing gfr_raw_plasma_delta
attempt_so_hvg_immune_meta <- attempt_so_hvg_immune@meta.data
attempt_so_hvg_immune_nonmissing_names <- rownames(attempt_so_hvg_immune_meta)[!is.na(attempt_so_hvg_immune_meta$gfr_raw_plasma_delta)]
attempt_so_hvg_immune_nonmissing <- attempt_so_hvg_immune[, attempt_so_hvg_immune_nonmissing_names, drop = FALSE]
counts_hvg_immune_nonmissing <- round(GetAssayData(attempt_so_hvg_immune_nonmissing, layer = "counts")) # load counts and round
...
immune_mgfr_raw_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_immune_nonmissing[g, , drop = FALSE]
                              meta_gene <- subset(attempt_so_hvg_immune_nonmissing, features = g)@meta.data
                              pred_gene <- model.matrix(~gfr_raw_plasma_delta, data = meta_gene)
                              data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
## After the loop:
for (res in immune_mgfr_raw_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(immune_mgfr_raw_pooled) <- sapply(immune_mgfr_raw_pooled, function(x) x$gene)  # set names
immune_mgfr_raw_pooled <- lapply(immune_mgfr_raw_pooled, function(x) x$result)  # clean list back to just results
immune_mgfr_raw_pooled <- Filter(Negate(is.null), immune_mgfr_raw_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

immune_mgfr_raw_filtered <- (length(genes_list)-length(immune_mgfr_raw_pooled))/length(genes_list)
print(paste0(immune_mgfr_raw_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_mgfr_raw_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_raw/immune_attempt_nebula_mgfr_raw.rds')
```
...
# Subset the Seurat object to nonmissing gfr_bsa_plasma_delta
attempt_so_hvg_immune_meta <- attempt_so_hvg_immune@meta.data
attempt_so_hvg_immune_nonmissing_names <- rownames(attempt_so_hvg_immune_meta)[!is.na(attempt_so_hvg_immune_meta$gfr_bsa_plasma_delta)]
attempt_so_hvg_immune_nonmissing <- attempt_so_hvg_immune[, attempt_so_hvg_immune_nonmissing_names, drop = FALSE]
counts_hvg_immune_nonmissing <- round(GetAssayData(attempt_so_hvg_immune_nonmissing, layer = "counts")) # load counts and round
...
immune_mgfr_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
...
## After the loop:
for (res in immune_mgfr_bsa_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(immune_mgfr_bsa_pooled) <- sapply(immune_mgfr_bsa_pooled, function(x) x$gene)  # set names
immune_mgfr_bsa_pooled <- lapply(immune_mgfr_bsa_pooled, function(x) x$result)  # clean list back to just results
immune_mgfr_bsa_pooled <- Filter(Negate(is.null), immune_mgfr_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

immune_mgfr_bsa_filtered <- (length(genes_list)-length(immune_mgfr_bsa_pooled))/length(genes_list)
print(paste0(immune_mgfr_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_mgfr_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_bsa/immune_attempt_nebula_mgfr_bsa.rds')
```
...
# Subset the Seurat object to nonmissing hba1c_delta
attempt_so_hvg_immune_meta <- attempt_so_hvg_immune@meta.data
attempt_so_hvg_immune_nonmissing_names <- rownames(attempt_so_hvg_immune_meta)[!is.na(attempt_so_hvg_immune_meta$hba1c_delta)]
attempt_so_hvg_immune_nonmissing <- attempt_so_hvg_immune[, attempt_so_hvg_immune_nonmissing_names, drop = FALSE]
counts_hvg_immune_nonmissing <- round(GetAssayData(attempt_so_hvg_immune_nonmissing, layer = "counts")) # load counts and round
...
# Subset the Seurat object to nonmissing weight_delta
attempt_so_hvg_immune_meta <- attempt_so_hvg_immune@meta.data
attempt_so_hvg_immune_nonmissing_names <- rownames(attempt_so_hvg_immune_meta)[!is.na(attempt_so_hvg_immune_meta$weight_delta)]
attempt_so_hvg_immune_nonmissing <- attempt_so_hvg_immune[, attempt_so_hvg_immune_nonmissing_names, drop = FALSE]
counts_hvg_immune_nonmissing <- round(GetAssayData(attempt_so_hvg_immune_nonmissing, layer = "counts")) # load counts and round
...
# Subset the Seurat object to nonmissing avg_c_r2_delta
attempt_so_hvg_immune_meta <- attempt_so_hvg_immune@meta.data
attempt_so_hvg_immune_nonmissing_names <- rownames(attempt_so_hvg_immune_meta)[!is.na(attempt_so_hvg_immune_meta$avg_c_r2_delta)]
attempt_so_hvg_immune_nonmissing <- attempt_so_hvg_immune[, attempt_so_hvg_immune_nonmissing_names, drop = FALSE]
counts_hvg_immune_nonmissing <- round(GetAssayData(attempt_so_hvg_immune_nonmissing, layer = "counts")) # load counts and round
...
# Subset the Seurat object to nonmissing avg_k_r2_delta
attempt_so_hvg_immune_meta <- attempt_so_hvg_immune@meta.data
attempt_so_hvg_immune_nonmissing_names <- rownames(attempt_so_hvg_immune_meta)[!is.na(attempt_so_hvg_immune_meta$avg_k_r2_delta)]
attempt_so_hvg_immune_nonmissing <- attempt_so_hvg_immune[, attempt_so_hvg_immune_nonmissing_names, drop = FALSE]
counts_hvg_immune_nonmissing <- round(GetAssayData(attempt_so_hvg_immune_nonmissing, layer = "counts")) # load counts and round
...
attempt_so_hvg_pc_nonmissing_names <- rownames(attempt_so_hvg_pc_meta)[!is.na(attempt_so_hvg_pc_meta$gfr_raw_plasma_delta)]
attempt_so_hvg_pc_nonmissing <- attempt_so_hvg_pc[, attempt_so_hvg_pc_nonmissing_names, drop = FALSE]
...
pc_mgfr_raw_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_pc_nonmissing[g, , drop = FALSE]
...
# set the names of results based on gene names
names(pc_mgfr_raw_pooled) <- sapply(pc_mgfr_raw_pooled, function(x) x$gene)  # set names
pc_mgfr_raw_pooled <- lapply(pc_mgfr_raw_pooled, function(x) x$result)  # clean list back to just results
pc_mgfr_raw_pooled <- Filter(Negate(is.null), pc_mgfr_raw_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pc_mgfr_raw_filtered <- (length(genes_list)-length(pc_mgfr_raw_pooled))/length(genes_list)
print(paste0(pc_mgfr_raw_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_mgfr_raw_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_raw/pc_attempt_nebula_mgfr_raw.rds')
```
...
# Subset the Seurat object to nonmissing gfr_bsa_plasma_delta
attempt_so_hvg_pc_meta <- attempt_so_hvg_pc@meta.data
attempt_so_hvg_pc_nonmissing_names <- rownames(attempt_so_hvg_pc_meta)[!is.na(attempt_so_hvg_pc_meta$gfr_bsa_plasma_delta)]
attempt_so_hvg_pc_nonmissing <- attempt_so_hvg_pc[, attempt_so_hvg_pc_nonmissing_names, drop = FALSE]
counts_hvg_pc_nonmissing <- round(GetAssayData(attempt_so_hvg_pc_nonmissing, layer = "counts")) # load counts and round
...
pc_mgfr_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
...
# set the names of results based on gene names
names(pc_mgfr_bsa_pooled) <- sapply(pc_mgfr_bsa_pooled, function(x) x$gene)  # set names
pc_mgfr_bsa_pooled <- lapply(pc_mgfr_bsa_pooled, function(x) x$result)  # clean list back to just results
pc_mgfr_bsa_pooled <- Filter(Negate(is.null), pc_mgfr_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pc_mgfr_bsa_filtered <- (length(genes_list)-length(pc_mgfr_bsa_pooled))/length(genes_list)
print(paste0(pc_mgfr_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_mgfr_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_bsa/pc_attempt_nebula_mgfr_bsa.rds')
```
...
attempt_so_hvg_ic_nonmissing_names <- rownames(attempt_so_hvg_ic_meta)[!is.na(attempt_so_hvg_ic_meta$gfr_raw_plasma_delta)]
attempt_so_hvg_ic_nonmissing <- attempt_so_hvg_ic[, attempt_so_hvg_ic_nonmissing_names, drop = FALSE]
...
ic_mgfr_raw_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_ic_nonmissing[g, , drop = FALSE]
...
# set the names of results based on gene names
names(ic_mgfr_raw_pooled) <- sapply(ic_mgfr_raw_pooled, function(x) x$gene)  # set names
ic_mgfr_raw_pooled <- lapply(ic_mgfr_raw_pooled, function(x) x$result)  # clean list back to just results
ic_mgfr_raw_pooled <- Filter(Negate(is.null), ic_mgfr_raw_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

ic_mgfr_raw_filtered <- (length(genes_list)-length(ic_mgfr_raw_pooled))/length(genes_list)
print(paste0(ic_mgfr_raw_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_mgfr_raw_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_raw/ic_attempt_nebula_mgfr_raw.rds')
```
...
# Subset the Seurat object to nonmissing gfr_bsa_plasma_delta
attempt_so_hvg_ic_meta <- attempt_so_hvg_ic@meta.data
attempt_so_hvg_ic_nonmissing_names <- rownames(attempt_so_hvg_ic_meta)[!is.na(attempt_so_hvg_ic_meta$gfr_bsa_plasma_delta)]
attempt_so_hvg_ic_nonmissing <- attempt_so_hvg_ic[, attempt_so_hvg_ic_nonmissing_names, drop = FALSE]
counts_hvg_ic_nonmissing <- round(GetAssayData(attempt_so_hvg_ic_nonmissing, layer = "counts")) # load counts and round
...
ic_mgfr_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
...
# set the names of results based on gene names
names(ic_mgfr_bsa_pooled) <- sapply(ic_mgfr_bsa_pooled, function(x) x$gene)  # set names
ic_mgfr_bsa_pooled <- lapply(ic_mgfr_bsa_pooled, function(x) x$result)  # clean list back to just results
ic_mgfr_bsa_pooled <- Filter(Negate(is.null), ic_mgfr_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

ic_mgfr_bsa_filtered <- (length(genes_list)-length(ic_mgfr_bsa_pooled))/length(genes_list)
print(paste0(ic_mgfr_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_mgfr_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_bsa/ic_attempt_nebula_mgfr_bsa.rds')
```
...
attempt_so_hvg_ec_nonmissing_names <- rownames(attempt_so_hvg_ec_meta)[!is.na(attempt_so_hvg_ec_meta$gfr_raw_plasma_delta)]
attempt_so_hvg_ec_nonmissing <- attempt_so_hvg_ec[, attempt_so_hvg_ec_nonmissing_names, drop = FALSE]
...
ec_mgfr_raw_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_ec_nonmissing[g, , drop = FALSE]
...
# set the names of results based on gene names
names(ec_mgfr_raw_pooled) <- sapply(ec_mgfr_raw_pooled, function(x) x$gene)  # set names
ec_mgfr_raw_pooled <- lapply(ec_mgfr_raw_pooled, function(x) x$result)  # clean list back to just results
ec_mgfr_raw_pooled <- Filter(Negate(is.null), ec_mgfr_raw_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

ec_mgfr_raw_filtered <- (length(genes_list)-length(ec_mgfr_raw_pooled))/length(genes_list)
print(paste0(ec_mgfr_raw_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_mgfr_raw_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_raw/ec_attempt_nebula_mgfr_raw.rds')
```
...
# Subset the Seurat object to nonmissing gfr_bsa_plasma_delta
attempt_so_hvg_ec_meta <- attempt_so_hvg_ec@meta.data
attempt_so_hvg_ec_nonmissing_names <- rownames(attempt_so_hvg_ec_meta)[!is.na(attempt_so_hvg_ec_meta$gfr_bsa_plasma_delta)]
attempt_so_hvg_ec_nonmissing <- attempt_so_hvg_ec[, attempt_so_hvg_ec_nonmissing_names, drop = FALSE]
counts_hvg_ec_nonmissing <- round(GetAssayData(attempt_so_hvg_ec_nonmissing, layer = "counts")) # load counts and round
...
ec_mgfr_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
...
# set the names of results based on gene names
names(ec_mgfr_bsa_pooled) <- sapply(ec_mgfr_bsa_pooled, function(x) x$gene)  # set names
ec_mgfr_bsa_pooled <- lapply(ec_mgfr_bsa_pooled, function(x) x$result)  # clean list back to just results
ec_mgfr_bsa_pooled <- Filter(Negate(is.null), ec_mgfr_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

ec_mgfr_bsa_filtered <- (length(genes_list)-length(ec_mgfr_bsa_pooled))/length(genes_list)
print(paste0(ec_mgfr_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_mgfr_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_bsa/ec_attempt_nebula_mgfr_bsa.rds')
```
...
attempt_so_hvg_fibvsmc_nonmissing_names <- rownames(attempt_so_hvg_fibvsmc_meta)[!is.na(attempt_so_hvg_fibvsmc_meta$gfr_raw_plasma_delta)]
attempt_so_hvg_fibvsmc_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmc_nonmissing_names, drop = FALSE]
...
fibvsmc_mgfr_raw_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
                            ...
                            tryCatch({
                              count_gene <- counts_hvg_fibvsmc_nonmissing[g, , drop = FALSE]
...
## After the loop:
for (res in fibvsmc_mgfr_raw_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmc_mgfr_raw_pooled) <- sapply(fibvsmc_mgfr_raw_pooled, function(x) x$gene)  # set names
fibvsmc_mgfr_raw_pooled <- lapply(fibvsmc_mgfr_raw_pooled, function(x) x$result)  # clean list back to just results
fibvsmc_mgfr_raw_pooled <- Filter(Negate(is.null), fibvsmc_mgfr_raw_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmc_mgfr_raw_filtered <- (length(genes_list)-length(fibvsmc_mgfr_raw_pooled))/length(genes_list)
print(paste0(fibvsmc_mgfr_raw_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmc_mgfr_raw_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_raw/fibvsmc_attempt_nebula_mgfr_raw.rds')
```
...
# Subset the Seurat object to nonmissing gfr_bsa_plasma_delta
attempt_so_hvg_fibvsmc_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmc_nonmissing_names <- rownames(attempt_so_hvg_fibvsmc_meta)[!is.na(attempt_so_hvg_fibvsmc_meta$gfr_bsa_plasma_delta)]
attempt_so_hvg_fibvsmc_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmc_nonmissing_names, drop = FALSE]
counts_hvg_fibvsmc_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmc_nonmissing, layer = "counts")) # load counts and round
...
fibvsmc_mgfr_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                          .errorhandling = "pass") %dopar% {
                            warn <- NULL
                            err <- NULL
                            res <- NULL
...
## After the loop:
for (res in fibvsmc_mgfr_bsa_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmc_mgfr_bsa_pooled) <- sapply(fibvsmc_mgfr_bsa_pooled, function(x) x$gene)  # set names
fibvsmc_mgfr_bsa_pooled <- lapply(fibvsmc_mgfr_bsa_pooled, function(x) x$result)  # clean list back to just results
fibvsmc_mgfr_bsa_pooled <- Filter(Negate(is.null), fibvsmc_mgfr_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmc_mgfr_bsa_filtered <- (length(genes_list)-length(fibvsmc_mgfr_bsa_pooled))/length(genes_list)
print(paste0(fibvsmc_mgfr_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmc_mgfr_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_bsa/fibvsmc_attempt_nebula_mgfr_bsa.rds')
```
