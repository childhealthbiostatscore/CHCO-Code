...
temp_file <- tempfile(fileext = ".RDS")
s3$download_file("attempt", "Clinical Data/ATTEMPT_AC.RDS", temp_file)
attempt_dat2 <- readRDS(temp_file)
attempt_dat2[attempt_dat2 == ""] <- NA
attempt_dat2 <- attempt_dat2 %>%
  dplyr::select(-mgfr_jodal_bsa, -mgfr_jodal, -mgfr_ketone_iv, -mgfr_ketones_pre, -avg_k_r2, -avg_c_r2) %>%
  ...
  left_join(
    attempt_dat2 %>%
      dplyr::select(record_id, visit, mgfr_jodal_bsa, mgfr_jodal, mgfr_ketone_iv, mgfr_ketones_pre, avg_k_r2, avg_c_r2) %>%
...
delta_df <-  attempt_so_meta %>%
  ...
  summarise(tir_post = mean(cgm_tir[visit == "POST"], na.rm = T),
            ...
            avg_ketones_post = mean(c(mgfr_ketone_iv[visit == "POST"], mgfr_ketones_pre[visit == "POST"]), 
                                    na.rm = T),
            avg_ketones_pre  = mean(c(mgfr_ketone_iv[visit == "PRE"], mgfr_ketones_pre[visit == "PRE"]), 
                                    na.rm = T),
            avg_ketones_delta = avg_ketones_post - avg_ketones_pre,
            .groups = "drop") %>%
...
temp_file <- tempfile(fileext = ".rds")
s3$download_file("attempt", 'cleaned_data/     ', temp_file)
attempt_so <- readRDS(temp_file)
``` 
...
```{r echo = F}
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_pt_meta <- attempt_so_hvg_pt@meta.data
attempt_so_hvg_pt_nonmissing_names <- rownames(attempt_so_hvg_pt_meta)[!is.na(attempt_so_hvg_pt_meta$avg_ketones_delta)]
attempt_so_hvg_pt_nonmissing <- attempt_so_hvg_pt[, attempt_so_hvg_pt_nonmissing_names]
counts_hvg_pt_nonmissing <- round(GetAssayData(attempt_so_hvg_pt_nonmissing, layer = "counts")) # load counts and round
...
pt_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                              .errorhandling = "pass") %dopar% {
                                warn <- NULL
                                err <- NULL
                                res <- NULL
                                ...
                                tryCatch({
                                  count_gene <- counts_hvg_pt_nonmissing[g, , drop = F]
                                  meta_gene <- subset(attempt_so_hvg_pt_nonmissing, features = g)@meta.data
                                  pred_gene <- model.matrix(~avg_ketones_delta + treatment, data = meta_gene)
                                  data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
## After the loop:
for (res in pt_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}

# set the names of results based on gene names
names(pt_avg_ketones_pooled) <- sapply(pt_avg_ketones_pooled, function(x) x$gene)  # set names
pt_avg_ketones_pooled <- lapply(pt_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
pt_avg_ketones_pooled <- Filter(Negate(is.null), pt_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pt_avg_ketones_filtered <- (length(genes_list)-length(pt_avg_ketones_pooled))/length(genes_list)
print(paste0(pt_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pt_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/pt_attempt_nebula_avg_ketones.rds')
```
...
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_tal_meta <- attempt_so_hvg_tal@meta.data
attempt_so_hvg_tal_nonmissing_names <- rownames(attempt_so_hvg_tal_meta)[!is.na(attempt_so_hvg_tal_meta$avg_ketones_delta)]
attempt_so_hvg_tal_nonmissing <- attempt_so_hvg_tal[, attempt_so_hvg_tal_nonmissing_names]
counts_hvg_tal_nonmissing <- round(GetAssayData(attempt_so_hvg_tal_nonmissing, layer = "counts")) # load counts and round
...
tal_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                               .errorhandling = "pass") %dopar% {
                                 warn <- NULL
                                 err <- NULL
                                 res <- NULL
...
## After the loop:
for (res in tal_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(tal_avg_ketones_pooled) <- sapply(tal_avg_ketones_pooled, function(x) x$gene)  # set names
tal_avg_ketones_pooled <- lapply(tal_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
tal_avg_ketones_pooled <- Filter(Negate(is.null), tal_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

tal_avg_ketones_filtered <- (length(genes_list)-length(tal_avg_ketones_pooled))/length(genes_list)
print(paste0(tal_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(tal_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/tal_attempt_nebula_avg_ketones.rds')
```
...
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_immune_myeloid_meta <- attempt_so_hvg_immune_myeloid@meta.data
attempt_so_hvg_immune_myeloid_nonmissing_names <- rownames(attempt_so_hvg_immune_myeloid_meta)[!is.na(attempt_so_hvg_immune_myeloid_meta$avg_ketones_delta)]
attempt_so_hvg_immune_myeloid_nonmissing <- attempt_so_hvg_immune_myeloid[, attempt_so_hvg_immune_myeloid_nonmissing_names]
counts_hvg_immune_myeloid_nonmissing <- round(GetAssayData(attempt_so_hvg_immune_myeloid_nonmissing, layer = "counts")) # load counts and round
...
immune_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                  .errorhandling = "pass") %dopar% {
                                    warn <- NULL
                                    err <- NULL
                                    res <- NULL
...
## After the loop:
for (res in immune_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(immune_avg_ketones_pooled) <- sapply(immune_avg_ketones_pooled, function(x) x$gene)  # set names
immune_avg_ketones_pooled <- lapply(immune_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
immune_avg_ketones_pooled <- Filter(Negate(is.null), immune_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

immune_avg_ketones_filtered <- (length(genes_list)-length(immune_avg_ketones_pooled))/length(genes_list)
print(paste0(immune_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/immune_attempt_nebula_avg_ketones.rds')
```
...
immune_myeloid_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                          .errorhandling = "pass") %dopar% {
                                            warn <- NULL
                                            err <- NULL
                                            res <- NULL
...
## After the loop:
for (res in immune_myeloid_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(immune_myeloid_avg_ketones_pooled) <- sapply(immune_myeloid_avg_ketones_pooled, function(x) x$gene)  # set names
immune_myeloid_avg_ketones_pooled <- lapply(immune_myeloid_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
immune_myeloid_avg_ketones_pooled <- Filter(Negate(is.null), immune_myeloid_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

immune_myeloid_avg_ketones_filtered <- (length(genes_list)-length(immune_myeloid_avg_ketones_pooled))/length(genes_list)
print(paste0(immune_myeloid_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_myeloid_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/immune_myeloid_attempt_nebula_avg_ketones.rds')
```
...
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_immune_lymphoid_meta <- attempt_so_hvg_immune_lymphoid@meta.data
attempt_so_hvg_immune_lymphoid_nonmissing_names <- rownames(attempt_so_hvg_immune_lymphoid_meta)[!is.na(attempt_so_hvg_immune_lymphoid_meta$avg_ketones_delta)]
attempt_so_hvg_immune_lymphoid_nonmissing <- attempt_so_hvg_immune_lymphoid[, attempt_so_hvg_immune_lymphoid_nonmissing_names]
counts_hvg_immune_lymphoid_nonmissing <- round(GetAssayData(attempt_so_hvg_immune_lymphoid_nonmissing, layer = "counts")) # load counts and round
...
immune_lymphoid_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                           .errorhandling = "pass") %dopar% {
                                             warn <- NULL
                                             err <- NULL
                                             res <- NULL
...
## After the loop:
for (res in immune_lymphoid_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(immune_lymphoid_avg_ketones_pooled) <- sapply(immune_lymphoid_avg_ketones_pooled, function(x) x$gene)  # set names
immune_lymphoid_avg_ketones_pooled <- lapply(immune_lymphoid_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
immune_lymphoid_avg_ketones_pooled <- Filter(Negate(is.null), immune_lymphoid_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

immune_lymphoid_avg_ketones_filtered <- (length(genes_list)-length(immune_lymphoid_avg_ketones_pooled))/length(genes_list)
print(paste0(immune_lymphoid_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(immune_lymphoid_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/immune_lymphoid_attempt_nebula_avg_ketones.rds')
```
...
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_pc_meta <- attempt_so_hvg_pc@meta.data
attempt_so_hvg_pc_nonmissing_names <- rownames(attempt_so_hvg_pc_meta)[!is.na(attempt_so_hvg_pc_meta$avg_ketones_delta)]
attempt_so_hvg_pc_nonmissing <- attempt_so_hvg_pc[, attempt_so_hvg_pc_nonmissing_names]
counts_hvg_pc_nonmissing <- round(GetAssayData(attempt_so_hvg_pc_nonmissing, layer = "counts")) # load counts and round
...
pc_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                              .errorhandling = "pass") %dopar% {
                                warn <- NULL
                                err <- NULL
                                res <- NULL
...
## After the loop:
for (res in pc_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(pc_avg_ketones_pooled) <- sapply(pc_avg_ketones_pooled, function(x) x$gene)  # set names
pc_avg_ketones_pooled <- lapply(pc_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
pc_avg_ketones_pooled <- Filter(Negate(is.null), pc_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pc_avg_ketones_filtered <- (length(genes_list)-length(pc_avg_ketones_pooled))/length(genes_list)
print(paste0(pc_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pc_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/pc_attempt_nebula_avg_ketones.rds')
```
...
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_ic_meta <- attempt_so_hvg_ic@meta.data
attempt_so_hvg_ic_nonmissing_names <- rownames(attempt_so_hvg_ic_meta)[!is.na(attempt_so_hvg_ic_meta$avg_ketones_delta)]
attempt_so_hvg_ic_nonmissing <- attempt_so_hvg_ic[, attempt_so_hvg_ic_nonmissing_names]
counts_hvg_ic_nonmissing <- round(GetAssayData(attempt_so_hvg_ic_nonmissing, layer = "counts")) # load counts and round
...
ic_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                              .errorhandling = "pass") %dopar% {
                                warn <- NULL
                                err <- NULL
                                res <- NULL
...
## After the loop:
for (res in ic_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(ic_avg_ketones_pooled) <- sapply(ic_avg_ketones_pooled, function(x) x$gene)  # set names
ic_avg_ketones_pooled <- lapply(ic_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
ic_avg_ketones_pooled <- Filter(Negate(is.null), ic_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

ic_avg_ketones_filtered <- (length(genes_list)-length(ic_avg_ketones_pooled))/length(genes_list)
print(paste0(ic_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ic_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/ic_attempt_nebula_avg_ketones.rds')
```
...
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_ec_meta <- attempt_so_hvg_ec@meta.data
attempt_so_hvg_ec_nonmissing_names <- rownames(attempt_so_hvg_ec_meta)[!is.na(attempt_so_hvg_ec_meta$avg_ketones_delta)]
attempt_so_hvg_ec_nonmissing <- attempt_so_hvg_ec[, attempt_so_hvg_ec_nonmissing_names]
counts_hvg_ec_nonmissing <- round(GetAssayData(attempt_so_hvg_ec_nonmissing, layer = "counts")) # load counts and round
...
ec_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                              .errorhandling = "pass") %dopar% {
                                warn <- NULL
                                err <- NULL
                                res <- NULL
...
## After the loop:
for (res in ec_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(ec_avg_ketones_pooled) <- sapply(ec_avg_ketones_pooled, function(x) x$gene)  # set names
ec_avg_ketones_pooled <- lapply(ec_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
ec_avg_ketones_pooled <- Filter(Negate(is.null), ec_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

ec_avg_ketones_filtered <- (length(genes_list)-length(ec_avg_ketones_pooled))/length(genes_list)
print(paste0(ec_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(ec_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/ec_attempt_nebula_avg_ketones.rds')
```
...
```{r echo = F, eval = T}
# Subset the Seurat object to nonmissing mgfr_jodal_bsa_delta
attempt_so_hvg_fibvsmcp_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmcp_nonmissing_names <- rownames(attempt_so_hvg_fibvsmcp_meta)[!is.na(attempt_so_hvg_fibvsmcp_meta$mgfr_jodal_bsa_delta)]
attempt_so_hvg_fibvsmcp_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmcp_nonmissing_names]
counts_hvg_fibvsmcp_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmcp_nonmissing, layer = "counts")) # load counts and round

genes_list <- rownames(counts_hvg_fibvsmcp_nonmissing)
...
fibvsmcp_mgfr_jodal_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                         .errorhandling = "pass") %dopar% {
                                           warn <- NULL
                                           err <- NULL
                                           res <- NULL
                                           ...
                                           tryCatch({
                                             count_gene <- counts_hvg_fibvsmcp_nonmissing[g, , drop = F]
                                             meta_gene <- subset(attempt_so_hvg_fibvsmcp_nonmissing, features = g)@meta.data
                                             pred_gene <- model.matrix(~mgfr_jodal_bsa_delta + treatment, data = meta_gene)
                                             data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
## After the loop:
for (res in fibvsmcp_mgfr_jodal_bsa_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmcp_mgfr_jodal_bsa_pooled) <- sapply(fibvsmcp_mgfr_jodal_bsa_pooled, function(x) x$gene)  # set names
fibvsmcp_mgfr_jodal_bsa_pooled <- lapply(fibvsmcp_mgfr_jodal_bsa_pooled, function(x) x$result)  # clean list back to just results
fibvsmcp_mgfr_jodal_bsa_pooled <- Filter(Negate(is.null), fibvsmcp_mgfr_jodal_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmcp_mgfr_jodal_bsa_filtered <- (length(genes_list)-length(fibvsmcp_mgfr_jodal_bsa_pooled))/length(genes_list)
print(paste0(fibvsmcp_mgfr_jodal_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_mgfr_jodal_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_jodal_bsa/fibvsmcp_attempt_nebula_mgfr_jodal_bsa.rds')
```
...
# Subset the Seurat object to nonmissing mgfr_jodal_delta
attempt_so_hvg_fibvsmcp_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmcp_nonmissing_names <- rownames(attempt_so_hvg_fibvsmcp_meta)[!is.na(attempt_so_hvg_fibvsmcp_meta$mgfr_jodal_delta)]
attempt_so_hvg_fibvsmcp_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmcp_nonmissing_names]
counts_hvg_fibvsmcp_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmcp_nonmissing, layer = "counts")) # load counts and round
...
fibvsmcp_mgfr_jodal_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                     .errorhandling = "pass") %dopar% {
                                       warn <- NULL
                                       err <- NULL
                                       res <- NULL
...
## After the loop:
for (res in fibvsmcp_mgfr_jodal_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmcp_mgfr_jodal_pooled) <- sapply(fibvsmcp_mgfr_jodal_pooled, function(x) x$gene)  # set names
fibvsmcp_mgfr_jodal_pooled <- lapply(fibvsmcp_mgfr_jodal_pooled, function(x) x$result)  # clean list back to just results
fibvsmcp_mgfr_jodal_pooled <- Filter(Negate(is.null), fibvsmcp_mgfr_jodal_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmcp_mgfr_jodal_filtered <- (length(genes_list)-length(fibvsmcp_mgfr_jodal_pooled))/length(genes_list)
print(paste0(fibvsmcp_mgfr_jodal_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_mgfr_jodal_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_jodal/fibvsmcp_attempt_nebula_mgfr_jodal.rds')
```
...
# Subset the Seurat object to nonmissing tir_delta
attempt_so_hvg_fibvsmcp_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmcp_nonmissing_names <- rownames(attempt_so_hvg_fibvsmcp_meta)[!is.na(attempt_so_hvg_fibvsmcp_meta$tir_delta)]
attempt_so_hvg_fibvsmcp_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmcp_nonmissing_names]
counts_hvg_fibvsmcp_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmcp_nonmissing, layer = "counts")) # load counts and round
...
fibvsmcp_tir_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                              .errorhandling = "pass") %dopar% {
                                warn <- NULL
                                err <- NULL
                                res <- NULL
...
## After the loop:
for (res in fibvsmcp_tir_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmcp_tir_pooled) <- sapply(fibvsmcp_tir_pooled, function(x) x$gene)  # set names
fibvsmcp_tir_pooled <- lapply(fibvsmcp_tir_pooled, function(x) x$result)  # clean list back to just results
fibvsmcp_tir_pooled <- Filter(Negate(is.null), fibvsmcp_tir_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmcp_tir_filtered <- (length(genes_list)-length(fibvsmcp_tir_pooled))/length(genes_list)
print(paste0(fibvsmcp_tir_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_tir_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/tir/fibvsmcp_attempt_nebula_tir.rds')
```
...
# Subset the Seurat object to nonmissing hba1c_delta
attempt_so_hvg_fibvsmcp_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmcp_nonmissing_names <- rownames(attempt_so_hvg_fibvsmcp_meta)[!is.na(attempt_so_hvg_fibvsmcp_meta$hba1c_delta)]
attempt_so_hvg_fibvsmcp_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmcp_nonmissing_names]
counts_hvg_fibvsmcp_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmcp_nonmissing, layer = "counts")) # load counts and round
...
fibvsmcp_hba1c_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                .errorhandling = "pass") %dopar% {
                                  warn <- NULL
                                  err <- NULL
                                  res <- NULL
...
## After the loop:
for (res in fibvsmcp_hba1c_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmcp_hba1c_pooled) <- sapply(fibvsmcp_hba1c_pooled, function(x) x$gene)  # set names
fibvsmcp_hba1c_pooled <- lapply(fibvsmcp_hba1c_pooled, function(x) x$result)  # clean list back to just results
fibvsmcp_hba1c_pooled <- Filter(Negate(is.null), fibvsmcp_hba1c_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmcp_hba1c_filtered <- (length(genes_list)-length(fibvsmcp_hba1c_pooled))/length(genes_list)
print(paste0(fibvsmcp_hba1c_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_hba1c_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/hba1c/fibvsmcp_attempt_nebula_hba1c.rds')
```
...
# Subset the Seurat object to nonmissing weight_delta
attempt_so_hvg_fibvsmcp_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmcp_nonmissing_names <- rownames(attempt_so_hvg_fibvsmcp_meta)[!is.na(attempt_so_hvg_fibvsmcp_meta$weight_delta)]
attempt_so_hvg_fibvsmcp_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmcp_nonmissing_names]
counts_hvg_fibvsmcp_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmcp_nonmissing, layer = "counts")) # load counts and round
...
fibvsmcp_weight_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                 .errorhandling = "pass") %dopar% {
                                   warn <- NULL
                                   err <- NULL
                                   res <- NULL
...
## After the loop:
for (res in fibvsmcp_weight_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmcp_weight_pooled) <- sapply(fibvsmcp_weight_pooled, function(x) x$gene)  # set names
fibvsmcp_weight_pooled <- lapply(fibvsmcp_weight_pooled, function(x) x$result)  # clean list back to just results
fibvsmcp_weight_pooled <- Filter(Negate(is.null), fibvsmcp_weight_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmcp_weight_filtered <- (length(genes_list)-length(fibvsmcp_weight_pooled))/length(genes_list)
print(paste0(fibvsmcp_weight_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_weight_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/weight/fibvsmcp_attempt_nebula_weight.rds')
```
...
# Subset the Seurat object to nonmissing avg_c_r2_delta
attempt_so_hvg_fibvsmcp_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmcp_nonmissing_names <- rownames(attempt_so_hvg_fibvsmcp_meta)[!is.na(attempt_so_hvg_fibvsmcp_meta$avg_c_r2_delta)]
attempt_so_hvg_fibvsmcp_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmcp_nonmissing_names]
counts_hvg_fibvsmcp_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmcp_nonmissing, layer = "counts")) # load counts and round
...
fibvsmcp_avg_c_r2_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                   .errorhandling = "pass") %dopar% {
                                     warn <- NULL
                                     err <- NULL
                                     res <- NULL
...
## After the loop:
for (res in fibvsmcp_avg_c_r2_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmcp_avg_c_r2_pooled) <- sapply(fibvsmcp_avg_c_r2_pooled, function(x) x$gene)  # set names
fibvsmcp_avg_c_r2_pooled <- lapply(fibvsmcp_avg_c_r2_pooled, function(x) x$result)  # clean list back to just results
fibvsmcp_avg_c_r2_pooled <- Filter(Negate(is.null), fibvsmcp_avg_c_r2_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmcp_avg_c_r2_filtered <- (length(genes_list)-length(fibvsmcp_avg_c_r2_pooled))/length(genes_list)
print(paste0(fibvsmcp_avg_c_r2_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_avg_c_r2_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_c_r2/fibvsmcp_attempt_nebula_avg_c_r2.rds')
```
...
# Subset the Seurat object to nonmissing avg_k_r2_delta
attempt_so_hvg_fibvsmcp_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmcp_nonmissing_names <- rownames(attempt_so_hvg_fibvsmcp_meta)[!is.na(attempt_so_hvg_fibvsmcp_meta$avg_k_r2_delta)]
attempt_so_hvg_fibvsmcp_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmcp_nonmissing_names]
counts_hvg_fibvsmcp_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmcp_nonmissing, layer = "counts")) # load counts and round
...
fibvsmcp_avg_k_r2_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                   .errorhandling = "pass") %dopar% {
                                     warn <- NULL
                                     err <- NULL
                                     res <- NULL
...
## After the loop:
for (res in fibvsmcp_avg_k_r2_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmcp_avg_k_r2_pooled) <- sapply(fibvsmcp_avg_k_r2_pooled, function(x) x$gene)  # set names
fibvsmcp_avg_k_r2_pooled <- lapply(fibvsmcp_avg_k_r2_pooled, function(x) x$result)  # clean list back to just results
fibvsmcp_avg_k_r2_pooled <- Filter(Negate(is.null), fibvsmcp_avg_k_r2_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmcp_avg_k_r2_filtered <- (length(genes_list)-length(fibvsmcp_avg_k_r2_pooled))/length(genes_list)
print(paste0(fibvsmcp_avg_k_r2_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_avg_k_r2_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_k_r2/fibvsmcp_attempt_nebula_avg_k_r2.rds')
```
...
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_fibvsmcp_meta <- attempt_so_hvg_fibvsmc@meta.data
attempt_so_hvg_fibvsmcp_nonmissing_names <- rownames(attempt_so_hvg_fibvsmcp_meta)[!is.na(attempt_so_hvg_fibvsmcp_meta$avg_ketones_delta)]
attempt_so_hvg_fibvsmcp_nonmissing <- attempt_so_hvg_fibvsmc[, attempt_so_hvg_fibvsmcp_nonmissing_names]
counts_hvg_fibvsmcp_nonmissing <- round(GetAssayData(attempt_so_hvg_fibvsmcp_nonmissing, layer = "counts")) # load counts and round
...
fibvsmcp_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                   .errorhandling = "pass") %dopar% {
                                     warn <- NULL
                                     err <- NULL
                                     res <- NULL
...
## After the loop:
for (res in fibvsmcp_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(fibvsmcp_avg_ketones_pooled) <- sapply(fibvsmcp_avg_ketones_pooled, function(x) x$gene)  # set names
fibvsmcp_avg_ketones_pooled <- lapply(fibvsmcp_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
fibvsmcp_avg_ketones_pooled <- Filter(Negate(is.null), fibvsmcp_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

fibvsmcp_avg_ketones_filtered <- (length(genes_list)-length(fibvsmcp_avg_ketones_pooled))/length(genes_list)
print(paste0(fibvsmcp_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(fibvsmcp_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/fibvsmcp_attempt_nebula_avg_ketones.rds')
```
...
```{r echo = F}
pod_unexpected_transcripts <- attempt_unexpected_transcripts %>%
  filter(Cell_Type == "pod") %>%
  pull(Gene)
```

```{r echo = F}
# ATTEMPT BL only
attempt_so_pod_bl <- subset(attempt_so_hvg_pod, Visit == "BL")
attempt_so_pod_bl_subset <- attempt_so_pod_bl[pod_unexpected_transcripts,]

# HbA1c
pod_hba1c_select <- run_nebula_attempt(attempt_so_pod_bl_subset, trait = "hba1c", extra_covars = NULL,
                                            n_cores = 100,
                                            aws_s3 = s3, 
                                            s3_bucket = "attempt",
                                            s3_key = "associations/nebula/hba1c/pod_hba1c_select_transcripts.rds")

# TIR
pod_tir_select <- run_nebula_attempt(attempt_so_pod_bl_subset, trait = "cgm_tir", extra_covars = NULL,
                                          n_cores = 100,
                                          aws_s3 = s3, 
                                          s3_bucket = "attempt",
                                          s3_key = "associations/nebula/tir/pod_tir_select_transcripts.rds")

# mGFR
pod_mgfr_jodal_select <- run_nebula_attempt(attempt_so_pod_bl_subset, trait = "mgfr_jodal", extra_covars = NULL,
                                                 n_cores = 100,
                                                 aws_s3 = s3, 
                                                 s3_bucket = "attempt",
                                                 s3_key = "associations/nebula/mgfr_jodal/pod_mgfr_jodal_select_transcripts.rds")

# mGFR (BSA)
pod_mgfr_jodal_bsa_select <- run_nebula_attempt(attempt_so_pod_bl_subset, trait = "mgfr_jodal_bsa", extra_covars = NULL,
                                                     n_cores = 100,
                                                     aws_s3 = s3, 
                                                     s3_bucket = "attempt",
                                                     s3_key = "associations/nebula/mgfr_jodal_bsa/pod_mgfr_jodal_bsa_select_transcripts.rds")

# Weight
pod_weight_select <- run_nebula_attempt(attempt_so_pod_bl_subset, trait = "weight", extra_covars = NULL,
                                             n_cores = 100,
                                             aws_s3 = s3, 
                                             s3_bucket = "attempt",
                                             s3_key = "associations/nebula/weight/pod_weight_select_transcripts.rds")
...
```{r echo = F, eval = T}
# Subset the Seurat object to nonmissing mgfr_jodal_bsa_delta
attempt_so_hvg_pod_meta <- attempt_so_hvg_pod@meta.data
attempt_so_hvg_pod_nonmissing_names <- rownames(attempt_so_hvg_pod_meta)[!is.na(attempt_so_hvg_pod_meta$mgfr_jodal_bsa_delta)]
attempt_so_hvg_pod_nonmissing <- attempt_so_hvg_pod[, attempt_so_hvg_pod_nonmissing_names]
counts_hvg_pod_nonmissing <- round(GetAssayData(attempt_so_hvg_pod_nonmissing, layer = "counts")) # load counts and round

genes_list <- rownames(counts_hvg_pod_nonmissing)
...
pod_mgfr_jodal_bsa_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                         .errorhandling = "pass") %dopar% {
                                           warn <- NULL
                                           err <- NULL
                                           res <- NULL
                                           ...
                                           tryCatch({
                                             count_gene <- counts_hvg_pod_nonmissing[g, , drop = F]
                                             meta_gene <- subset(attempt_so_hvg_pod_nonmissing, features = g)@meta.data
                                             pred_gene <- model.matrix(~mgfr_jodal_bsa_delta + treatment, data = meta_gene)
                                             data_g_gene <- group_cell(count = count_gene, id = meta_gene$subject_id, pred = pred_gene)
...
## After the loop:
for (res in pod_mgfr_jodal_bsa_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(pod_mgfr_jodal_bsa_pooled) <- sapply(pod_mgfr_jodal_bsa_pooled, function(x) x$gene)  # set names
pod_mgfr_jodal_bsa_pooled <- lapply(pod_mgfr_jodal_bsa_pooled, function(x) x$result)  # clean list back to just results
pod_mgfr_jodal_bsa_pooled <- Filter(Negate(is.null), pod_mgfr_jodal_bsa_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pod_mgfr_jodal_bsa_filtered <- (length(genes_list)-length(pod_mgfr_jodal_bsa_pooled))/length(genes_list)
print(paste0(pod_mgfr_jodal_bsa_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pod_mgfr_jodal_bsa_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_jodal_bsa/pod_attempt_nebula_mgfr_jodal_bsa.rds')
```
...
# Subset the Seurat object to nonmissing mgfr_jodal_delta
attempt_so_hvg_pod_meta <- attempt_so_hvg_pod@meta.data
attempt_so_hvg_pod_nonmissing_names <- rownames(attempt_so_hvg_pod_meta)[!is.na(attempt_so_hvg_pod_meta$mgfr_jodal_delta)]
attempt_so_hvg_pod_nonmissing <- attempt_so_hvg_pod[, attempt_so_hvg_pod_nonmissing_names]
counts_hvg_pod_nonmissing <- round(GetAssayData(attempt_so_hvg_pod_nonmissing, layer = "counts")) # load counts and round
...
pod_mgfr_jodal_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                     .errorhandling = "pass") %dopar% {
                                       warn <- NULL
                                       err <- NULL
                                       res <- NULL
...
## After the loop:
for (res in pod_mgfr_jodal_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(pod_mgfr_jodal_pooled) <- sapply(pod_mgfr_jodal_pooled, function(x) x$gene)  # set names
pod_mgfr_jodal_pooled <- lapply(pod_mgfr_jodal_pooled, function(x) x$result)  # clean list back to just results
pod_mgfr_jodal_pooled <- Filter(Negate(is.null), pod_mgfr_jodal_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pod_mgfr_jodal_filtered <- (length(genes_list)-length(pod_mgfr_jodal_pooled))/length(genes_list)
print(paste0(pod_mgfr_jodal_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pod_mgfr_jodal_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/mgfr_jodal/pod_attempt_nebula_mgfr_jodal.rds')
```
...
# Subset the Seurat object to nonmissing tir_delta
attempt_so_hvg_pod_meta <- attempt_so_hvg_pod@meta.data
attempt_so_hvg_pod_nonmissing_names <- rownames(attempt_so_hvg_pod_meta)[!is.na(attempt_so_hvg_pod_meta$tir_delta)]
attempt_so_hvg_pod_nonmissing <- attempt_so_hvg_pod[, attempt_so_hvg_pod_nonmissing_names]
counts_hvg_pod_nonmissing <- round(GetAssayData(attempt_so_hvg_pod_nonmissing, layer = "counts")) # load counts and round
...
pod_tir_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                              .errorhandling = "pass") %dopar% {
                                warn <- NULL
                                err <- NULL
                                res <- NULL
...
# set the names of results based on gene names
names(pod_tir_pooled) <- sapply(pod_tir_pooled, function(x) x$gene)  # set names
pod_tir_pooled <- lapply(pod_tir_pooled, function(x) x$result)  # clean list back to just results
pod_tir_pooled <- Filter(Negate(is.null), pod_tir_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pod_tir_filtered <- (length(genes_list)-length(pod_tir_pooled))/length(genes_list)
print(paste0(pod_tir_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pod_tir_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/tir/pod_attempt_nebula_tir.rds')
```
...
# Subset the Seurat object to nonmissing hba1c_delta
attempt_so_hvg_pod_meta <- attempt_so_hvg_pod@meta.data
attempt_so_hvg_pod_nonmissing_names <- rownames(attempt_so_hvg_pod_meta)[!is.na(attempt_so_hvg_pod_meta$hba1c_delta)]
attempt_so_hvg_pod_nonmissing <- attempt_so_hvg_pod[, attempt_so_hvg_pod_nonmissing_names]
counts_hvg_pod_nonmissing <- round(GetAssayData(attempt_so_hvg_pod_nonmissing, layer = "counts")) # load counts and round
...
pod_hba1c_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                .errorhandling = "pass") %dopar% {
                                  warn <- NULL
                                  err <- NULL
                                  res <- NULL
...
# set the names of results based on gene names
names(pod_hba1c_pooled) <- sapply(pod_hba1c_pooled, function(x) x$gene)  # set names
pod_hba1c_pooled <- lapply(pod_hba1c_pooled, function(x) x$result)  # clean list back to just results
pod_hba1c_pooled <- Filter(Negate(is.null), pod_hba1c_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pod_hba1c_filtered <- (length(genes_list)-length(pod_hba1c_pooled))/length(genes_list)
print(paste0(pod_hba1c_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pod_hba1c_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/hba1c/pod_attempt_nebula_hba1c.rds')
```
...
# Subset the Seurat object to nonmissing weight_delta
attempt_so_hvg_pod_meta <- attempt_so_hvg_pod@meta.data
attempt_so_hvg_pod_nonmissing_names <- rownames(attempt_so_hvg_pod_meta)[!is.na(attempt_so_hvg_pod_meta$weight_delta)]
attempt_so_hvg_pod_nonmissing <- attempt_so_hvg_pod[, attempt_so_hvg_pod_nonmissing_names]
counts_hvg_pod_nonmissing <- round(GetAssayData(attempt_so_hvg_pod_nonmissing, layer = "counts")) # load counts and round
...
pod_weight_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                 .errorhandling = "pass") %dopar% {
                                   warn <- NULL
                                   err <- NULL
                                   res <- NULL
...
# set the names of results based on gene names
names(pod_weight_pooled) <- sapply(pod_weight_pooled, function(x) x$gene)  # set names
pod_weight_pooled <- lapply(pod_weight_pooled, function(x) x$result)  # clean list back to just results
pod_weight_pooled <- Filter(Negate(is.null), pod_weight_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pod_weight_filtered <- (length(genes_list)-length(pod_weight_pooled))/length(genes_list)
print(paste0(pod_weight_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pod_weight_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/weight/pod_attempt_nebula_weight.rds')
```
...
# Subset the Seurat object to nonmissing avg_c_r2_delta
attempt_so_hvg_pod_meta <- attempt_so_hvg_pod@meta.data
attempt_so_hvg_pod_nonmissing_names <- rownames(attempt_so_hvg_pod_meta)[!is.na(attempt_so_hvg_pod_meta$avg_c_r2_delta)]
attempt_so_hvg_pod_nonmissing <- attempt_so_hvg_pod[, attempt_so_hvg_pod_nonmissing_names]
counts_hvg_pod_nonmissing <- round(GetAssayData(attempt_so_hvg_pod_nonmissing, layer = "counts")) # load counts and round
...
pod_avg_c_r2_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                   .errorhandling = "pass") %dopar% {
                                     warn <- NULL
                                     err <- NULL
                                     res <- NULL
...
## After the loop:
for (res in pod_avg_c_r2_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(pod_avg_c_r2_pooled) <- sapply(pod_avg_c_r2_pooled, function(x) x$gene)  # set names
pod_avg_c_r2_pooled <- lapply(pod_avg_c_r2_pooled, function(x) x$result)  # clean list back to just results
pod_avg_c_r2_pooled <- Filter(Negate(is.null), pod_avg_c_r2_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pod_avg_c_r2_filtered <- (length(genes_list)-length(pod_avg_c_r2_pooled))/length(genes_list)
print(paste0(pod_avg_c_r2_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pod_avg_c_r2_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_c_r2/pod_attempt_nebula_avg_c_r2.rds')
```
...
# Subset the Seurat object to nonmissing avg_k_r2_delta
attempt_so_hvg_pod_meta <- attempt_so_hvg_pod@meta.data
attempt_so_hvg_pod_nonmissing_names <- rownames(attempt_so_hvg_pod_meta)[!is.na(attempt_so_hvg_pod_meta$avg_k_r2_delta)]
attempt_so_hvg_pod_nonmissing <- attempt_so_hvg_pod[, attempt_so_hvg_pod_nonmissing_names]
counts_hvg_pod_nonmissing <- round(GetAssayData(attempt_so_hvg_pod_nonmissing, layer = "counts")) # load counts and round
...
pod_avg_k_r2_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                   .errorhandling = "pass") %dopar% {
                                     warn <- NULL
                                     err <- NULL
                                     res <- NULL
...
## After the loop:
for (res in pod_avg_k_r2_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(pod_avg_k_r2_pooled) <- sapply(pod_avg_k_r2_pooled, function(x) x$gene)  # set names
pod_avg_k_r2_pooled <- lapply(pod_avg_k_r2_pooled, function(x) x$result)  # clean list back to just results
pod_avg_k_r2_pooled <- Filter(Negate(is.null), pod_avg_k_r2_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pod_avg_k_r2_filtered <- (length(genes_list)-length(pod_avg_k_r2_pooled))/length(genes_list)
print(paste0(pod_avg_k_r2_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pod_avg_k_r2_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_k_r2/pod_attempt_nebula_avg_k_r2.rds')
```
...
# Subset the Seurat object to nonmissing avg_ketones_delta
attempt_so_hvg_pod_meta <- attempt_so_hvg_pod@meta.data
attempt_so_hvg_pod_nonmissing_names <- rownames(attempt_so_hvg_pod_meta)[!is.na(attempt_so_hvg_pod_meta$avg_ketones_delta)]
attempt_so_hvg_pod_nonmissing <- attempt_so_hvg_pod[, attempt_so_hvg_pod_nonmissing_names]
counts_hvg_pod_nonmissing <- round(GetAssayData(attempt_so_hvg_pod_nonmissing, layer = "counts")) # load counts and round
...
pod_avg_ketones_pooled <- foreach(g = genes_list, .packages = c("nebula", "Matrix"),
                                   .errorhandling = "pass") %dopar% {
                                     warn <- NULL
                                     err <- NULL
                                     res <- NULL
...
## After the loop:
for (res in pod_avg_ketones_pooled) {
  if (!is.null(res$warning)) cat(sprintf("Warning for gene %s: %s\n", res$gene, res$warning))
  if (!is.null(res$error)) cat(sprintf("Error for gene %s: %s\n", res$gene, res$error))
}
...
# set the names of results based on gene names
names(pod_avg_ketones_pooled) <- sapply(pod_avg_ketones_pooled, function(x) x$gene)  # set names
pod_avg_ketones_pooled <- lapply(pod_avg_ketones_pooled, function(x) x$result)  # clean list back to just results
pod_avg_ketones_pooled <- Filter(Negate(is.null), pod_avg_ketones_pooled)  # remove NULLs first

end_time <- Sys.time()
print(end_time - start_time)
stopCluster(cl)

pod_avg_ketones_filtered <- (length(genes_list)-length(pod_avg_ketones_pooled))/length(genes_list)
print(paste0(pod_avg_ketones_filtered*100, "% filtered due to low expression"))
temp_file <- tempfile(fileext = "rds") # need to create a temporary file
saveRDS(pod_avg_ketones_pooled, temp_file) # save the temp file locally (will delete after R session)
s3$upload_file(temp_file, "attempt", 'associations/nebula/avg_ketones/pod_attempt_nebula_avg_ketones.rds')
```
