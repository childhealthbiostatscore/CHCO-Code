...
```{r echo = F}
plot_volcano_concordance <- function(clin_results, fc, p_col, title_suffix, 
                                     x_axis, y_axis, file_suffix, p_thresh = 0.05,
                                     positive_text = "Positive with Dapagliflozin", 
                                     negative_text = "Negative with Dapagliflozin",
                                     formula = "\u0394 treatment", 
                                     legend_position = c(0.3,0.8)) {
  ...
  # Get concordant genes (non-NC)
  concordant_genes <- clin_results %>%
    filter(concordance != "NC") %>%
    pull(Gene)
  
  # Top 5 concordant genes on the left (negative FC)
  top_5_left <- clin_results %>%
    filter(Gene %in% concordant_genes, !!sym(fc) < 0) %>%
    arrange(!!sym(p_col)) %>%
    slice_head(n = 5) %>%
    pull(Gene)
  
  # Top 5 concordant genes on the right (positive FC)
  top_5_right <- clin_results %>%
    filter(Gene %in% concordant_genes, !!sym(fc) > 0) %>%
    arrange(!!sym(p_col)) %>%
    slice_head(n = 5) %>%
    pull(Gene)
  ...
  # Combine for plotting
  top_label_genes <- c(top_5_left, top_5_right)
  
  message("Number of concordant genes: ", length(concordant_genes))
  message("Number of labeled genes: ", length(top_label_genes))
  
  clin_results <- clin_results %>%
    mutate(
      # Use concordance column for coloring
      color_var_plot = concordance,
      # Label only the top concordant genes
      top_lab = if_else(Gene %in% top_label_genes, Gene, "")
    )
  
  # Set factor levels for concordance
  clin_results$color_var_plot <- factor(clin_results$color_var_plot, 
                                        levels = c("+", "-", "NC"))
  ...
  p <- ggplot(clin_results, aes(x = !!sym(fc), y = -log10(!!sym(p_col)))) +
    geom_hline(yintercept = -log10(p_thresh), linetype = "dashed", color = "darkgrey") +
    geom_point(alpha = 0.3, aes(fill = color_var_plot, 
                                color = color_var_plot), size = 2) +
    # Updated color scheme for concordance
    scale_color_manual(values = c("+" = "#f28482", "-" = "#457b9d", "NC" = "#ced4da"),
                       labels = c("+" = "Positive", "-" = "Negative", "NC" = "Non-Concordant")) +
    scale_fill_manual(values = c("+" = "#f28482", "-" = "#457b9d", "NC" = "#ced4da"),
                      labels = c("+" = "Positive", "-" = "Negative", "NC" = "Non-Concordant")) +
    guides(color = guide_legend(title = "Concordance"), fill = "none") +
    geom_text_repel(aes(label = top_lab, color = color_var_plot),
                    size = 3, max.overlaps = Inf, force = 10,
                    segment.alpha = 0.5, segment.size = 0.4,
                    min.segment.length = 0, box.padding = 0.6, point.padding = 0.4,
