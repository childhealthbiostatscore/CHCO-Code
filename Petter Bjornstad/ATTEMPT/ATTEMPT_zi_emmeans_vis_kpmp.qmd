...
```{r echo = F}
# Top 2000 HVG
for (cell in celltype_groups[["PT"]])  {
  cell <- gsub("/", "_", cell)
...
```{r echo = F}
pt_kpmp_sig %>% left_join(pt_treatment_df) %>%
  arrange(fdr) %>%
  filter(`logFC_treatmentDapagliflozin:visitPOST`>0) %>%
  head(20)
```
...
tal1_hvg_kpmp <- tal1_nebula_processed$results
...
tal2_hvg_kpmp <- tal2_nebula_processed$results
...
tal3_hvg_kpmp <- tal3_nebula_processed$results
```
...
pc1_hvg_kpmp <- pc1_nebula_processed$results
...
pc2_hvg_kpmp <- pc2_nebula_processed$results
...
tpc_ic_hvg_kpmp <- tpc_ic_nebula_processed$results
```
...
mon_hvg_kpmp <- mon_nebula_processed$results
...
mac_hvg_kpmp <- mac_nebula_processed$results
...
t_hvg_kpmp <- t_nebula_processed$results
...
nkt_hvg_kpmp <- nkt_nebula_processed$results
...
b_hvg_kpmp <- b_nebula_processed$results
```
...
ic_a_hvg_kpmp <- ic_a_nebula_processed$results
...
ic_b_hvg_kpmp <- ic_b_nebula_processed$results
```
...
ec_aea_hvg_kpmp <- ec_aea_nebula_processed$results
...
ec_gc_hvg_kpmp <- ec_gc_nebula_processed$results
...
ec_lym_hvg_kpmp <- ec_lym_nebula_processed$results
...
ec_ptc_hvg_kpmp <- ec_ptc_nebula_processed$results
```
...
```{r echo =F}
plot_volcano(fibvsmcp_hvg_kpmp, "logFC_treatmentDapagliflozin:visitPOST", 
             "p_treatmentDapagliflozin:visitPOST",
             NULL,
             "logFC Treatment(Dapagliflozin):Visit(POST)", 
             "-log10(p-value)",
...
fib_hvg_kpmp <- fib_nebula_processed$results
...
p_hvg_kpmp <- p_nebula_processed$results
```

```{r echo = F}
fibvsmcp_plots <- create_gene_expression_plots(
  main_results = fibvsmcp_hvg_kpmp,
  subtype_results_list = list(
    "FIB" = fib_hvg_kpmp,
    "VSMC/P" = p_hvg_kpmp
  ),
