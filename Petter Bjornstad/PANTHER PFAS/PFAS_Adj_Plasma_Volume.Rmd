...
if (user == "leidholt") {
  ...
  keys <- fromJSON("/mmfs1/home/leidholt/keys.json")
    ...
    dir.results <- c("/Users/Shared/OneDrive - UW/Laura Pyle's files - Biostatistics Core Shared Drive/")
    ...
    dir.results <- c("/Users/savanahleidholt/OneDrive - UW/Laura Pyle's files - Biostatistics Core Shared Drive/")
...
meta_nopfas <- readr::read_csv(
  rawConnection(obj3)
)


obj4 <- aws.s3::get_object(
...
dat_baseline <- readr::read_csv(
  rawConnection(obj4)
)

obj5 <- aws.s3::get_object(
...
pfas_adj_blood_vars <- paste0(pfas_vars, "_adj_conc")
...
#-------------------------------------------------------
#nadler's vs lemmen's brodsky equation for lean controls
#-------------------------------------------------------
```{r}
...
```
#------------------------------------------------------------
#Do plasma volume's differ significantly by equation for LC
#------------------------------------------------------------
```{r}
...
beta_limit <- max(abs(hm_mod3d$beta), na.rm = TRUE)
...
# incident elevated uACR = first visit where acr_u >= 30
make_survival_data <- function(data,
                               outcome = "acr_u",
                               event_cutoff = 30,
                               id_var = "record_id",
                               time_var = "visit_num") {
  ...
  data %>%
    ...
    summarise(
      time = ifelse(
        any(.data[[outcome]] >= event_cutoff, na.rm = TRUE),
        min(.data[[time_var]][.data[[outcome]] >= event_cutoff], na.rm = TRUE),
        max(.data[[time_var]], na.rm = TRUE)
      ),
      event = as.integer(any(.data[[outcome]] >= event_cutoff, na.rm = TRUE)),
      
      # baseline covariates/exposures
      age = dplyr::first(age),
      sex = dplyr::first(sex),
      group = dplyr::first(group),
      tan_stage = dplyr::first(tan_stage),
      log_acr_u_bl = dplyr::first(log_acr_u_bl),
      across(all_of(pfas_group_vars), dplyr::first),
      .groups = "drop"
    ) %>%
...
```{r}
surv_dat_acr30 <- make_survival_data(
  data = dat_long,
  outcome = "acr_u",
  event_cutoff = 30
)
...
```{r}
run_pfas_cox <- function(data, exposure,
                         covars = c("age", "sex", "group")) {
  
  rhs <- c(exposure, covars)
  
  fml <- as.formula(
    paste0("Surv(time, event) ~ ", paste(rhs, collapse = " + "))
  )
...
cox_pfas_table <- cox_results_acr30 %>%
  ...
  select(exposure, n, n_events, estimate, conf.low, conf.high, HR_CI, p.value, p_fdr)
...
```
```{r}
cox_fit_example <- coxph(
  Surv(time, event) ~ ln_total_plasma_pfas_bl + age + sex + group,
  data = surv_dat_acr30
)

cox.zph(cox_fit_example)
plot(cox.zph(cox_fit_example))

zph <- cox.zph(cox_fit_example)
...
table(surv_dat_acr30$event)

table(surv_dat_acr30$time)

length(unique(surv_dat_acr30$time[surv_dat_acr30$event == 1]))
```

```{r}
cox_results_acr30_full <- map_dfr(
  pfas_group_vars,
  ~ run_pfas_cox(
    data = surv_dat_acr30,
    exposure = .x,
    covars = c("age", "sex", "group", "tan_stage", "log_acr_u_bl")
  )
)

cox_pfas_table_full <- cox_results_acr30_full %>%
  filter(term == exposure) %>%
  mutate(
    p_fdr = p.adjust(p.value, method = "fdr"),
    HR_CI = paste0(
...
```{r}
cox_fit_full <- coxph(
  Surv(time, event) ~ ln_total_plasma_pfas_bl + age + sex + group + tan_stage + log_acr_u_bl,
  data = surv_dat_acr30
)

cox.zph(cox_fit_full)
plot(cox.zph(cox_fit_full))
```
