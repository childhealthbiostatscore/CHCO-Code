...
# d) adjusted for uACR & tanner stage
#-------------------------------------------------------------------------------

```{r}
#---------------------only adjusting for age, sex, and group--------------------------------
...
beta_limit_unadjusted <- max(abs(heatmap_longitudinal_df_unadjusted$beta), na.rm = TRUE)

p_longitudinal_heatmap_unadjusted <- ggplot(
  heatmap_longitudinal_df_unadjusted,
  ...
  scale_fill_gradient2(
    ...
    high = "#B2182B",
    midpoint = 0,
    limits = c(-beta_limit_unadjusted, beta_limit_unadjusted),
    name = "Beta"
  ) +
...
p_longitudinal_heatmap_unadjusted
```


```{r}
#----------------------------baseline uACR adjusted-----------------------------
...
```
#-------------------------------------------------------------------------------
# Model 3c: baseline PFAS predicting longitudinal outcomes, Tanner-adjusted
#-------------------------------------------------------------------------------
```{r}
...
```{r}
heatmap_nominal_acr_tanner_df <- heatmap_longitudinal_df_acr_tanner  %>%
  mutate(
    sig_nominal = case_when(
      visit == "Baseline" & !is.na(pval_main) & pval_main < 0.05 ~ TRUE,
      visit %in% c("Year 1", "Year 2") & !is.na(pval_int) & pval_int < 0.05 ~ TRUE,
...
beta_limit <- max(abs(heatmap_nominal_acr_tanner_df$beta), na.rm = TRUE)

p_longitudinal_heatmap_acr_tanner_raw <- ggplot(
  heatmap_nominal_acr_tanner_df,
  aes(x = exposure, y = outcome, fill = beta)
)  +
  geom_tile(color = "white", linewidth = 0.4) +
...
p_longitudinal_heatmap_acr_tanner_raw 
