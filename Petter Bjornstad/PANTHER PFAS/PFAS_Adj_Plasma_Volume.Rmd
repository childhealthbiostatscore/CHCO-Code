...
dat_baseline2 <- dat_long %>% 
  select(record_id, visit, weight, height, hct, sex, group, bmi) %>%
...
dat_baseline2 <- dat_baseline2 %>%
  mutate(
    blood_vol_ml   = weight * (70 / sqrt(bmi / 22)),
    hct_prop       = hct / 100,
    LBB_plasma_ml  = blood_vol_ml * (1 - hct_prop),
...
plot_dat <- dat_baseline2 %>%
  select(record_id, LBB_plasma_ml, nadler_plasma_ml) %>%
  pivot_longer(
  ...
  mutate(
    equation_type = recode(
      equation_type,
      LBB_plasma = "Lemmens/Brodsky",
      nadler_plasma = "Nadler"
    )
  )
