...
```{r}
#Renal Clearance
#	erpf_bsa_plasma	BSA indexed ERPF from plasma clearance (mL/min/1.73m^2)	mL/min/1.73m^2		renal_clearance		
# erpf_bsa_plasma_urine	BSA indexed ERPF from plasma and urine clearance (mL/min/1.73m^2)	mL/min/1.73m^2		renal_clearance		
# erpf_raw_plasma	ERPF from plasma clearance	mL/min		renal_clearance		
# erpf_raw_plasma_urine	ERPF from plasma and urine clearance	mL/min		renal_clearance

#GFR

outcome_vars <- c("ERPF, GFR, Insulin Sensitivity, Renal Perfusion, renal oxygenation, adj. for baseline ACRu")
# ^ swap in whatever your actual outcome variable names are

# ── Define group variable and colors ─────────────────────────────────────────
group_var <- "group"   # your disease group column name
group_colors <- c("steelblue", "tomato", "forestgreen")  # one per group level
...
for (var in outcome_vars) {
  ...
  # Step 1: Summary — mean + 95% CI per visit per group
  summary_df <- dat_long %>%
    filter(!is.na(.data[[var]]), !is.na(.data[[group_var]])) %>%
    group_by(visit, .data[[group_var]]) %>%
    summarise(
  ...
  p <- ggplot() +
    ...
    geom_line(data = indiv_df,
              aes(x = visit, y = .data[[var]], 
                  group = record_id, color = group),
              alpha = 0.2, linewidth = 0.35) +
    geom_point(data = indiv_df,
               aes(x = visit, y = .data[[var]], color = group),
               alpha = 0.2, size = 0.7) +
    # Group mean + 95% CI
    geom_line(data = summary_df,
              aes(x = visit, y = mean_val, 
    ...
    labs(
      ...
      caption = "Faint lines: individual trajectories  |  Bold: mean ± 95% CI"
    ) +
...
plot_list[["hba1c"]]          # view one by name
plot_list[[1]]                # or by index

# ── Or print all to a PDF ─────────────────────────────────────────────────────
pdf("longitudinal_outcomes.pdf", width = 8, height = 5)
