...
heatmap_longitudinal_df <- longitudinal_results_fdr %>%
  ...
  select(
    ...
    pval_int_y1, pval_int_y1_fdr,
    pval_int_y2, pval_int_y2_fdr
  ) %>%
  pivot_longer(
  ...
  mutate(
    ...
    sig = case_when(
      ...
      visit == "Year 1"   & !is.na(pval_int_y1_fdr) & pval_int_y1_fdr < 0.001 ~ "***",
      visit == "Year 1"   & !is.na(pval_int_y1_fdr) & pval_int_y1_fdr < 0.01  ~ "**",
      visit == "Year 1"   & !is.na(pval_int_y1_fdr) & pval_int_y1_fdr < 0.05  ~ "*",
      visit == "Year 2"   & !is.na(pval_int_y2_fdr) & pval_int_y2_fdr < 0.001 ~ "***",
      visit == "Year 2"   & !is.na(pval_int_y2_fdr) & pval_int_y2_fdr < 0.01  ~ "**",
      visit == "Year 2"   & !is.na(pval_int_y2_fdr) & pval_int_y2_fdr < 0.05  ~ "*",
      TRUE ~ ""
    ),
    exposure = factor(
      exposure,
...
heatmap_longitudinal_df <- longitudinal_results_fdr %>%
  ...
  select(outcome, exposure, term_type, estimate, p.value, p_fdr) %>%
  pivot_wider(
    names_from = term_type,
    values_from = c(estimate, p.value, p_fdr),
    names_sep = "_"
  ) %>%
  mutate(
    beta_visit0 = estimate_main,
    beta_visit1 = estimate_main + estimate_int_y1,
    beta_visit2 = estimate_main + estimate_int_y2
  ) %>%
  pivot_longer(
    cols = c(beta_visit0, beta_visit1, beta_visit2),
    names_to = "visit",
  ...
  mutate(
    visit = recode(
      visit,
      beta_visit0 = "Baseline",
      beta_visit1 = "Year 1",
      beta_visit2 = "Year 2"
    ),
    visit = factor(visit, levels = c("Baseline", "Year 1", "Year 2")),
    sig = case_when(
      visit == "Baseline" & !is.na(p_fdr_main)   & p_fdr_main   < 0.001 ~ "***",
      visit == "Baseline" & !is.na(p_fdr_main)   & p_fdr_main   < 0.01  ~ "**",
      visit == "Baseline" & !is.na(p_fdr_main)   & p_fdr_main   < 0.05  ~ "*",
      visit == "Year 1"   & !is.na(p_fdr_int_y1) & p_fdr_int_y1 < 0.001 ~ "***",
      visit == "Year 1"   & !is.na(p_fdr_int_y1) & p_fdr_int_y1 < 0.01  ~ "**",
      visit == "Year 1"   & !is.na(p_fdr_int_y1) & p_fdr_int_y1 < 0.05  ~ "*",
      visit == "Year 2"   & !is.na(p_fdr_int_y2) & p_fdr_int_y2 < 0.001 ~ "***",
      visit == "Year 2"   & !is.na(p_fdr_int_y2) & p_fdr_int_y2 < 0.01  ~ "**",
      visit == "Year 2"   & !is.na(p_fdr_int_y2) & p_fdr_int_y2 < 0.05  ~ "*",
      TRUE ~ ""
    ),
    exposure = factor(
      exposure,
...
```{r}
heatmap_longitudinal_df_z <- heatmap_longitudinal_df %>%
  group_by(visit) %>%
  mutate(beta_z = as.numeric(scale(beta))) %>%
  ungroup()

z_limit <- max(abs(heatmap_longitudinal_df_z$beta_z), na.rm = TRUE)

p_longitudinal_heatmap_z <- ggplot(
  heatmap_longitudinal_df_z,
  ...
  labs(
    ...
    subtitle = paste0(
      "Tile color shows standardized effect size within visit panel.\n",
...
p_longitudinal_heatmap <- ggplot(
  ...
  labs(
    title = "Baseline PFAS associations with longitudinal outcomes",
    subtitle = paste0(
      "Baseline tiles show PFAS main effects. Year 1 and Year 2 tiles show ",
      "PFAS main effect + PFAS×visit interaction.\n",
      "* FDR<0.05  ** FDR<0.01  *** FDR<0.001"
    ),
    x = "Baseline PFAS",
    y = NULL,
