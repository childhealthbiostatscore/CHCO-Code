...
cat("baseline_ffa non-NA:",             sum(!is.na(so_subset@meta.data$baseline_ffa)), "\n")
cat("ffa_suppression_combined non-NA:", sum(!is.na(so_subset@meta.data$ffa_suppression_combined)), "\n")
...
demo_df <- so_subset@meta.data %>%
  distinct(record_id, .keep_all = TRUE) %>%
  filter(!is.na(baseline_ffa)) %>%
  mutate(across(c(age, bmi, hba1c, baseline_ffa,
                  ffa_suppression, ffa_suppression_combined, acr_u), as.numeric),
         across(c(epic_insulin_1, epic_sglti2_1, epic_glp1ra_1, epic_tzd_1,
                  epic_mfm_1, epic_raasi_1, epic_statin_1, epic_fibrate_1,
                  microalbuminuria), as.factor))
...
demo_table <- demo_df %>%
  dplyr::select(age, sex, bmi, hba1c,
                baseline_ffa, ffa_suppression_combined, acr_u, microalbuminuria,
                epic_insulin_1, epic_sglti2_1, epic_glp1ra_1, epic_tzd_1,
                epic_mfm_1, epic_raasi_1, epic_statin_1, epic_fibrate_1) %>%
  tbl_summary(
    type = list(acr_u ~ "continuous", microalbuminuria ~ "categorical",
                everything() ~ "categorical",
                age ~ "continuous", bmi ~ "continuous", hba1c ~ "continuous",
                baseline_ffa ~ "continuous", ffa_suppression_combined ~ "continuous"),
    statistic = list(all_continuous() ~ "{mean} ({sd})",
                     acr_u ~ "{median} ({p25}, {p75})",
                     all_categorical() ~ "{n} ({p}%)"),
    label = list(
      age ~ "Age, years", sex ~ "Sex", bmi ~ "BMI, kg/m²", hba1c ~ "HbA1c, %",
      baseline_ffa ~ "Baseline FFA, µmol/L",
      ffa_suppression_combined ~ "FFA suppression (combined), %",
      acr_u ~ "Urine albumin, mg/L", microalbuminuria ~ "Albuminuria category",
      epic_insulin_1 ~ "Insulin", epic_sglti2_1 ~ "SGLT2i",
      epic_glp1ra_1 ~ "GLP-1RA", epic_tzd_1 ~ "PPARγ agonist (TZD)",
      epic_mfm_1 ~ "Metformin", epic_raasi_1 ~ "ACEi/ARB",
      epic_statin_1 ~ "Statin", epic_fibrate_1 ~ "Fenofibrate"
    ),
    missing_text = "Missing"
  ) %>%
  modify_header(label ~ "**Characteristic**") %>%
...
run_nebula_ffa <- function(seurat_obj, cell_type, ffa_col,
  ...
  data_g <- group_cell(count  = count_mat, id = meta$record_id,
  ...
  res <- nebula(count = data_g$count, id = data_g$id, pred = data_g$pred,
                offset = offset_use, ncore = 1, reml = TRUE, model = "NBLMM",
                output_re = FALSE, covariance = TRUE)
...
# ══════════════════════════════════════════════════════════════════════════════
# 5. RUN NEBULA — BOTH EXPOSURES
# ══════════════════════════════════════════════════════════════════════════════

nefa_results <- lapply(cell_types_of_interest, function(ct)
  run_nebula_ffa(so_subset, ct, "baseline_ffa", covariates, "baseline_ffa")) %>%
  do.call(rbind, .) %>%
  group_by(cell_type) %>%
  mutate(FDR = p.adjust(p_ffa_scaled, method = "BH")) %>%
  ungroup() %>% arrange(cell_type, FDR)

suppression_results <- lapply(cell_types_of_interest, function(ct)
  run_nebula_ffa(so_subset, ct, "ffa_suppression_combined", covariates, "ffa_suppression_combined")) %>%
  do.call(rbind, .) %>%
  group_by(cell_type) %>%
  mutate(FDR = p.adjust(p_ffa_scaled, method = "BH")) %>%
  ungroup() %>% arrange(cell_type, FDR)

write.csv(nefa_results,        file.path(dir.results, "NEFA_NEBULA_baseline_ffa_FDR.csv"),                row.names = FALSE)
write.csv(suppression_results, file.path(dir.results, "NEFA_NEBULA_ffa_suppression_combined_FDR.csv"),    row.names = FALSE)

# ══════════════════════════════════════════════════════════════════════════════
# 6. SHARED THEME & PLOT HELPERS
# ══════════════════════════════════════════════════════════════════════════════
...
make_volcano <- function(df, ct, title_var, fdr_t = fdr_thresh, lfc_t = lfc_thresh) {
  ...
  top_genes <- if (sum(df_ct$sig) >= 1) df_ct %>% filter(sig) %>% slice_min(FDR, n = 20) else
    df_ct %>% slice_min(p_ffa_scaled, n = 5)
  ...
  ggplot(df_ct, aes(logFC_ffa_scaled, -log10(p_ffa_scaled), color = direction)) +
    ...
    geom_text_repel(data = top_genes, aes(label = gene),
                    size = 2.5, max.overlaps = 25, show.legend = FALSE) +
    scale_color_manual(values = setNames(
      c("#e63946", "#457b9d", "grey75"), c(up_lab, down_lab, "NS")
    ), name = NULL) +
    geom_hline(yintercept = -log10(0.05), linetype = "dashed", linewidth = 0.4, color = "grey50") +
    geom_vline(xintercept = c(-lfc_t, lfc_t), linetype = "dashed", linewidth = 0.4, color = "grey50") +
    labs(title    = paste0(title_var, " vs. Transcriptome — ", ct, " (T2D)"),
         subtitle = paste0("FDR < ", fdr_t, " | |logFC| > ", lfc_t, "  ·  Unadjusted"),
         x = paste("Log2 FC per 1 SD", title_var), y = expression(-log[10](p))) +
    theme_fig
}
...
make_gsea_dot <- function(df, ct, title_var, n_top = 20) {
  df_plot <- df %>%
    ...
    slice_min(pval, n = n_top) %>%
    mutate(
      collection = case_when(str_starts(pathway, "HALLMARK_") ~ "Hallmark",
  ...
  ggplot(df_plot, aes(NES, reorder(pathway, NES),
    ...
    scale_shape_manual(values = c("GO:BP" = 16, "Hallmark" = 17, "Other" = 15),
                       name = "Collection") +
    geom_vline(xintercept = 0, linetype = "dashed", linewidth = 0.4, color = "grey50") +
    labs(title    = paste0("GSEA — ", title_var, " — ", ct, " (T2D)"),
         subtitle = "Top 20 pathways | Hallmark + GO:BP",
         x = "Normalized Enrichment Score (NES)", y = NULL) +
    theme_fig + theme(axis.text.y = element_text(size = 7))
}

# ══════════════════════════════════════════════════════════════════════════════
# 7. GSEA — BOTH EXPOSURES
# ══════════════════════════════════════════════════════════════════════════════
...
run_gsea_ffa <- function(df, ct, label = "FFA") {
  ...
  set.seed(123)
  res <- fgsea(pathways = gene_sets, stats = ranked, minSize = 15, maxSize = 500)
  res$cell_type <- ct
  res$ffa_var   <- label
  res %>% arrange(pval)
}

gsea_baseline_df <- lapply(cell_types_of_interest, function(ct)
  run_gsea_ffa(nefa_results, ct, "baseline_ffa")) %>%
  do.call(rbind, .) %>%
  group_by(cell_type) %>% mutate(FDR_gsea = p.adjust(pval, method = "BH")) %>%
  ungroup() %>% arrange(cell_type, pval)

gsea_suppression_df <- lapply(cell_types_of_interest, function(ct)
  run_gsea_ffa(suppression_results, ct, "ffa_suppression_combined")) %>%
  do.call(rbind, .) %>%
  group_by(cell_type) %>% mutate(FDR_gsea = p.adjust(pval, method = "BH")) %>%
  ungroup() %>% arrange(cell_type, pval)
...
write.csv(gsea_baseline_df    %>% dplyr::select(-leadingEdge),
          file.path(dir.results, "GSEA_baseline_ffa.csv"), row.names = FALSE)
write.csv(gsea_suppression_df %>% dplyr::select(-leadingEdge),
          file.path(dir.results, "GSEA_ffa_suppression_combined.csv"), row.names = FALSE)

# ══════════════════════════════════════════════════════════════════════════════
# 8. GSEA CONCORDANCE COMPARISON
# ══════════════════════════════════════════════════════════════════════════════
...
gsea_compare <- gsea_baseline_df %>%
  ...
  mutate(
    sig_baseline    = !is.na(pval_baseline)    & pval_baseline    < 0.05,
    sig_suppression = !is.na(pval_suppression) & pval_suppression < 0.05,
    concordant      = sign(NES_baseline) == sign(NES_suppression),
    overlap_group   = case_when(
      sig_baseline & sig_suppression & concordant  ~ "Both (concordant)",
      sig_baseline & sig_suppression & !concordant ~ "Both (discordant)",
...
write.csv(gsea_compare,
          file.path(dir.results, "GSEA_comparison_baseline_vs_suppression_combined.csv"),
          row.names = FALSE)
...
# ══════════════════════════════════════════════════════════════════════════════
# 9. ASSEMBLE COMBINED FIGURE (panels A–E)
# ══════════════════════════════════════════════════════════════════════════════
...
for (ct in cell_types_of_interest) {
  ...
  pA <- make_volcano(nefa_results,        ct, "Baseline FFA")
  pB <- make_volcano(suppression_results, ct, "FFA Suppression (Combined)")
  pC <- make_gsea_dot(gsea_baseline_df,    ct, "Baseline FFA")
  pD <- make_gsea_dot(gsea_suppression_df, ct, "FFA Suppression (Combined)")
  ...
  pE <- ggplot(df_e, aes(NES_baseline, NES_suppression, color = overlap_group)) +
    ...
    geom_vline(xintercept = 0, linetype = "dotted", linewidth = 0.3, color = "grey60") +
    scale_color_manual(values = overlap_colors, name = NULL) +
    labs(title    = paste0("GSEA: Baseline FFA vs. FFA Suppression (Combined) — ", ct),
         subtitle = "Each point = one gene set  |  Diagonal = perfect concordance\nNote: exposures are directionally inverse — same-direction NES reflects biological divergence",
         x = "NES (Baseline FFA)", y = "NES (FFA Suppression Combined)") +
    theme_fig + theme(legend.position = "bottom", legend.text = element_text(size = 7))
  ...
  ggsave(file.path(dir.results, paste0("Figure_FFA_EC_Transcriptome_combined_", ct, ".pdf")),
         combined, width = 16, height = 20, device = cairo_pdf)
  ggsave(file.path(dir.results, paste0("Figure_FFA_EC_Transcriptome_combined_", ct, ".png")),
