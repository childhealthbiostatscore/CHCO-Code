...
harm_dat_combined <- harm_dat_collapsed %>%
  ...
  arrange(desc(pbmc)) %>%
  dplyr::select(record_id, study, group, visit, pbmc, urine_soma, blood_soma, clamp, pet, mri, mgfr, erpf, egfr,
                ends_with("_ID", ignore.case = F)) %>%
  filter(pbmc == 1)

# write.csv(harm_dat_combined, file.path(root_path, "CITE-seq/CITE_seq_PBMC_priority_list.csv"), na = "", row.names = F)

table(harm_dat_combined$group)

table(subset(harm_dat_combined, !is.na(Kit_ID))$group)
```
