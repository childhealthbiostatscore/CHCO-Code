...
# Study Design Overview Figure - VSG vs SMT vs Healthy Controls
# With complete participant verification and sample breakdown
...
# ============================================================================
# SECTION 6B: STUDY DESIGN INFO - Calculate sample size breakdowns
# ============================================================================

# Count paired samples and sample breakdowns for each group
vsg_breakdown <- improve@meta.data %>%
  group_by(record_id) %>%
  summarise(
    has_pre = any(pre_post == "pre"),
    has_post = any(pre_post == "post"),
    .groups = "drop"
  ) %>%
  summarise(
    baseline_only = sum(has_pre & !has_post),
    followup_only = sum(!has_pre & has_post),
    paired = sum(has_pre & has_post),
    total = n()
  )

smt_breakdown <- rh@meta.data %>%
  group_by(record_id) %>%
  summarise(
    has_pre = any(pre_post == "pre"),
    has_post = any(pre_post == "post"),
...
hc_breakdown <- healthy@meta.data %>%
  distinct(record_id) %>%
  summarise(
    baseline_only = n(),  # All HC are single timepoint
    followup_only = 0,
    paired = 0,
    total = n()
  )
...
cat("\n=== PAIRED SAMPLES FOR LONGITUDINAL ANALYSIS ===\n")
cat("VSG paired (pre + post):", vsg_breakdown$paired, "\n")
cat("SMT paired (pre + post):", smt_breakdown$paired, "\n")

# Split by group for demographics
hc <- patient_level_all %>% filter(treatment == "Healthy")
vsg <- patient_level_all %>% filter(treatment == "VSG")
smt <- patient_level_all %>% filter(treatment == "Standard")
