...
proteomics <- harm_dat %>% 
  dplyr::select(record_id, mrn, visit, group, study, age, bmi, seq.10000.28) %>%
  dplyr::summarise(across(where(negate(is.numeric)), ~ ifelse(all(is.na(.x)), NA_character_, last(na.omit(.x)))),
                   across(where(is.numeric), ~ ifelse(all(is.na(.x)), NA_real_, mean(.x, na.rm = TRUE))),
                   .by = c(record_id, visit)) %>%
...
t1d_proteomics <- proteomics %>%
  filter(study %in% c("CASPER", "CROCODILE", "ATTEMPT")) %>%
  filter(group == "Type 1 Diabetes" | is.na(group)) %>%
  distinct(mrn, seq.10000.28, .keep_all = T)
nrow(t1d_proteomics)
# N = 217  # CASPER proteomics not included here
