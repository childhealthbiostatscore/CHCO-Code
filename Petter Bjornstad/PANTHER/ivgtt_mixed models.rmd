...
glucose_long$glucose <- ifelse(glucose_long$glucose > 2000, NA, glucose_long$glucose)
...
check <- left_join(insulin_long, glucose_long, by = c("mrn", "time"))
check <- left_join(check, cpep_long, by = c("mrn", "time"))
```
...
mixed_noint <- function(data, outcome, grouping_var){
  data <- as.data.frame(data)
  data$time <- as.factor(data$time)
  form = as.formula(paste0(outcome,"~ ", grouping_var, "+ time"))
  mod <- lme(as.formula(form),random=~1|mrn,data = data,na.action = na.omit, correlation = corCompSymm(form = ~1|mrn))
...
mixed_adj_noint <- function(data, outcome, grouping_var){
  data <- as.data.frame(data)
  data$time <- as.factor(data$time)
  form = as.formula(paste0(outcome,"~ ", grouping_var, "+ time + group_risk"))
  mod <- lme(as.formula(form),random=~1|mrn,data = data,na.action = na.omit, correlation = corCompSymm(form = ~1|mrn))
...
plot_mixed_group_risk <- function(mixed_output, data, ylab) {
  ...
  p <- ggplot(emmeans_result, aes(x = time, y = emmean, color = group, fill = group, group = group)) +
...
### By Tanner stage - with interaction between Tanner and time
...
### By Tanner stage - no interaction

```{r echo=FALSE}
mod <- mixed_noint(data = insulin_long, outcome = "insulin", grouping_var = "tanner_combined")
```
...
### By Tanner stage - adjusted for risk group, no interaction

```{r echo=FALSE}
mod <- mixed_adj_noint(data = insulin_long, outcome = "insulin", grouping_var = "tanner_combined")
```
...
```{r echo=FALSE}
mod <- mixed_noint(data = glucose_long, outcome = "glucose", grouping_var = "tanner_combined")
```
...
```{r echo=FALSE}
mod <- mixed_adj_noint(data = glucose_long, outcome = "glucose", grouping_var = "tanner_combined")
```
...
```{r echo=FALSE}
mod <- mixed_noint(data = cpep_long, outcome = "Cpeptide", grouping_var = "tanner_combined")
```
...
```{r echo=FALSE}
mod <- mixed_adj_noint(data = cpep_long, outcome = "Cpeptide", grouping_var = "tanner_combined")
```
