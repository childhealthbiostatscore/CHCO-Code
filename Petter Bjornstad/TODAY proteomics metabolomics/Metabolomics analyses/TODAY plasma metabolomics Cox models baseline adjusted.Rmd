...
predictors <- colnames(base_log_scale[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_first_ckm_prog, progress_CKM)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log_scale[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_first_ckm_stage3_prog, progress_CKM_stage3)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log_scale[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_first_ckm_stage4_prog, progress_CKM_stage4)~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0")
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
