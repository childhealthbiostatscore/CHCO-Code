---
title: "TODAY plasma metabolomics baseline analyses - Cox models"
author: "Laura Pyle"
...
for (i in 1:length(predictors)) {
  ...
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  result_CKM <- rbind(result_CKM,tidymod)
}
result_CKM$adj.p.value <- p.adjust(result_CKM$p.value, "fdr")
result_CKM <- result_CKM[order(result_CKM$p.value),] 
...
for (i in 1:length(predictors)) {
  ...
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  result_CKM3 <- rbind(result_CKM3,tidymod)
}
result_CKM3$adj.p.value <- p.adjust(result_CKM3$p.value, "fdr")
result_CKM3 <- result_CKM3[order(result_CKM3$p.value),] 
...
for (i in 1:length(predictors)) {
  ...
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  result_CKM4 <- rbind(result_CKM4,tidymod)
}
result_CKM4$adj.p.value <- p.adjust(result_CKM4$p.value, "fdr")
result_CKM4 <- result_CKM4[order(result_CKM4$p.value),] 
...
writeData(wb,"CKM progression",result_CKM,rowNames = F)
addWorksheet(wb,"CKM progression stage 3")
writeData(wb,"CKM progression stage 3",result_CKM3,rowNames = F)
addWorksheet(wb,"CKM progression stage 4")
writeData(wb,"CKM progression stage 4",result_CKM4,rowNames = F)

saveWorkbook(wb,"./Results/Metabolomics Linear and Cox models/TODAY plasma metabolomics Cox models scaled baseline unadjusted_march2026.xlsx",overwrite = TRUE)
