...
predictors <- colnames(base_log_scale[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_first_ckm_prog, progress_CKM)~",predictors[i])
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log_scale[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_first_ckm_stage3_prog, progress_CKM_stage3)~",predictors[i])
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
predictors <- colnames(base_log_scale[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_first_ckm_stage4_prog, progress_CKM_stage4)~",predictors[i])
  mod <- coxph(data = base_log_scale, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
saveWorkbook(wb,"./Results/Metabolomics Linear and Cox models/TODAY urine metabolomics Cox models scaled baseline unadjusted_march2026.xlsx",overwrite = TRUE)
