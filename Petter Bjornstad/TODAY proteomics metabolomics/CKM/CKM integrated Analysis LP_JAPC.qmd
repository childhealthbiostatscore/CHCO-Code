...
#     )
#   )
# LP: I think this should be:
# want to double check this is working correctly
echo_keep <- echo_keep %>%
  mutate(
    lvmi = ifelse(!is.na(lvmass) & !is.na(BSA_dubois), lvmass / BSA_dubois, NA_real_),
    lvmi_score = case_when(
...
# LP: MAC0/MAC1 meets criteria for both stage 4 and stage 2 plus?
comorb_ckm2 <- comorb_ckm2 %>%
  mutate(
    CKM_syn = case_when(
      # Stage 4: most severe
...
# LP: would be more reproducible to use arsenal or tableone 
...
# LP: contingency tables should be chi-square or Fisher's exact
...
# LP: this doesn't work because there are AGEBASE.x and AGEBASE.y
