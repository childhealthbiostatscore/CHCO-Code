...
filename <- "/Clinical data/comorb_testing_CKM.Rdata"
full_path <- file.path(data_path, filename)
load(file = full_path)
...
# Use the 'days' column from ADDCBL, not visit_days
addcbl_1 <- long %>%
  filter(!is.na(days)) %>%
...
# AME event columns are already in comorb from make_clinical_data_add_CKM.R
# Rename releaseid to RELEASEID and reconstruct individual event data frames
# that downstream code expects
comorb <- comorb %>% rename(RELEASEID = releaseid)

arrhythmia <- comorb %>% filter(ARRHYTHMIA == 1) %>% select(RELEASEID, DAYSTOARRHYTHMIA) %>% mutate(ARRHYTHMIA = 1)
cad        <- comorb %>% filter(CAD == 1)        %>% select(RELEASEID, DAYSTOCAD)        %>% mutate(CAD = 1)
chf        <- comorb %>% filter(CHF == 1)        %>% select(RELEASEID, DAYSTOCHF)        %>% mutate(CHF = 1)
LVSD       <- comorb %>% filter(LVSD == 1)       %>% select(RELEASEID, DAYSTOLVSD)       %>% mutate(LVSD = 1)
mi         <- comorb %>% filter(MI == 1)         %>% select(RELEASEID, DAYSTOMI)         %>% mutate(MI = 1)
pad        <- comorb %>% filter(PAD == 1)        %>% select(RELEASEID, DAYSTOPAD)        %>% mutate(PAD = 1)
dvt        <- comorb %>% filter(DVT == 1)        %>% select(RELEASEID, DAYSTODVT)        %>% mutate(DVT = 1)
stroke     <- comorb %>% filter(STROKE == 1)     %>% select(RELEASEID, DAYSTOSTROKE)     %>% mutate(STROKE = 1)
tia        <- comorb %>% filter(TIA == 1)        %>% select(RELEASEID, DAYSTOTIA)        %>% mutate(TIA = 1)
ckd        <- comorb %>% filter(CKD == 1)        %>% select(RELEASEID, DAYSTOCKD)        %>% mutate(CKD = 1)
eskd       <- comorb %>% filter(ESKD == 1)       %>% select(RELEASEID, DAYSTOESKD)       %>% mutate(ESKD = 1)
death      <- comorb %>% filter(DEATH == 1)      %>% select(RELEASEID, DAYSTODEATH)      %>% mutate(DEATH = 1)
ulcer      <- comorb %>% filter(ULCER == 1)      %>% select(RELEASEID, DAYSTOULCER)      %>% mutate(ULCER = 1)

# Build ame_summary from comorb
ame_summary <- comorb %>%
...
# comorb_ckm - comorb already has AME columns so just add clin_cvd
comorb_ckm <- merge(comorb, ame_summary %>% select(RELEASEID, clin_cvd), by = "RELEASEID", all.x = TRUE)
...
# Biomarker extraction - sourced from long
# Use the 'days' column (from ADDCBL) not visit_days, as this matches what
# extract_biomarker_values_long() used in the original script
long_biom <- long %>%
  mutate(RELEASEID = toupper(releaseid),
         days = ifelse(!is.na(days) & days < 0, 0, days)) %>%
  filter(!is.na(days))
...
il1_vals       <- if ("il1" %in% names(long_biom)) long_biom %>% filter(!is.na(il1))  %>% select(releaseid, days, il1)  else data.frame(releaseid=character(), days=numeric(), il1=numeric())
...
mcp1_vals      <- if ("mcp1"      %in% names(long_biom)) long_biom %>% filter(!is.na(mcp1))      %>% select(releaseid, days, mcp1)      else data.frame(releaseid=character(), days=numeric(), mcp1=numeric())
tnfa_vals      <- if ("tnfa"      %in% names(long_biom)) long_biom %>% filter(!is.na(tnfa))      %>% select(releaseid, days, tnfa)      else data.frame(releaseid=character(), days=numeric(), tnfa=numeric())
tnfr1_vals     <- if ("tnfr1"     %in% names(long_biom)) long_biom %>% filter(!is.na(tnfr1))     %>% select(releaseid, days, tnfr1)     else data.frame(releaseid=character(), days=numeric(), tnfr1=numeric())
tnfr2_vals     <- if ("tnfr2"     %in% names(long_biom)) long_biom %>% filter(!is.na(tnfr2))     %>% select(releaseid, days, tnfr2)     else data.frame(releaseid=character(), days=numeric(), tnfr2=numeric())
icam1_vals     <- if ("icam1"     %in% names(long_biom)) long_biom %>% filter(!is.na(icam1))     %>% select(releaseid, days, icam1)     else data.frame(releaseid=character(), days=numeric(), icam1=numeric())
vcam1_vals     <- if ("vcam1"     %in% names(long_biom)) long_biom %>% filter(!is.na(vcam1))     %>% select(releaseid, days, vcam1)     else data.frame(releaseid=character(), days=numeric(), vcam1=numeric())
eselectin_vals <- if ("eselectin" %in% names(long_biom)) long_biom %>% filter(!is.na(eselectin)) %>% select(releaseid, days, eselectin) else data.frame(releaseid=character(), days=numeric(), eselectin=numeric())
vegf_vals      <- if ("vegf"      %in% names(long_biom)) long_biom %>% filter(!is.na(vegf))      %>% select(releaseid, days, vegf)      else data.frame(releaseid=character(), days=numeric(), vegf=numeric())
fgf23_vals     <- if ("fgf23"     %in% names(long_biom)) long_biom %>% filter(!is.na(fgf23))     %>% select(releaseid, days, fgf23)     else data.frame(releaseid=character(), days=numeric(), fgf23=numeric())
...
# Rename first, then clean - and skip clean_infinite on empty frames to avoid type coercion
...
biomarker_list <- lapply(biomarker_list, function(df) {
  if (nrow(df) == 0) df else clean_infinite(df)
})
...
# Note: _vals objects retain lowercase releaseid (the biomarker_list lapply doesn't modify them)
...
# DEATH is not in comorb_long so merge it in from comorb
death_keep <- comorb %>% select(RELEASEID, DEATH) %>% mutate(DEATH = as.integer(DEATH))
comorb_ckm2 <- comorb_ckm2 %>% ungroup() %>% left_join(death_keep, by = "RELEASEID")
...
# DEATH already comes through comorb - just set NAs to 0
...
cols_to_convert <- c("CKM_syn_base", "CKM_max", "progress_CKM", "tx", "num_prog")
prog_summary_final[cols_to_convert] <- lapply(prog_summary_final[cols_to_convert], as.factor)

# Binary variables need both levels explicitly defined
binary_vars <- c("HTN", "TGDLP", "MIC", "MAC", "GLYC", "DEATH")
prog_summary_final[binary_vars] <- lapply(prog_summary_final[binary_vars], function(x) factor(x, levels = c(0, 1)))
