...
# Core datasets - using testing versions from make_clinical_data_add_CKM.R
filename <- "/Clinical data/TODAY/clinical_data_long_testing_CKM.Rdata"
full_path <- file.path(data_path, filename)
load(full_path)

filename <- "/Clinical data/TODAY/baserisk_testing_CKM.Rdata"
full_path <- file.path(data_path, filename)
load(file = full_path)

filename <- "/Clinical data/comorb.Rdata"
full_path <- file.path(data_path, filename)
load(file = full_path)

# AME summary tables - still read raw AME for the display tables (event counts)
# AME event indicator columns are now in comorb
...
# BASELINE - still needed for days=0 merge in data integration section
filename <- "/Clinical data/TODAY/BASELINE.csv"
full_path <- file.path(data_path, filename)
baseline <- read.csv(full_path)

# Echo data - days-based, not in long
...
# Speckle and PWV - days-based, not in long
...
# TME - still needed for ulcer derivation
filename <- "/Clinical data/TODAY2/TME.csv"
full_path <- file.path(data_path, filename)
tme <- read.csv(full_path)

# Reconstruct addcbl_1 from long for downstream code that uses it directly
# (bnp lookup for HFpEF scoring, KDIGO gfr/uacr derivation)
addcbl_1 <- long %>%
  filter(!is.na(visit_days)) %>%
  mutate(releaseid = releaseid,
         days = ifelse(visit_days < 0, 0, visit_days)) %>%
  select(any_of(c("releaseid", "days", "bnp", "troponin", "adiponectin", "hmwa",
                  "edol", "shbg", "testosterone", "ckd_gfr", "eGFR_FAS", "insinv", "codi")))
```
...
# AME event columns are now in comorb - standardize RELEASEID casing and
# reconstruct the individual event data frames that downstream code expects
comorb$RELEASEID <- toupper(comorb$releaseid)

# Reconstruct individual AME event data frames from comorb for downstream code
arrhythmia <- comorb %>% filter(ARRHYTHMIA == 1) %>% select(RELEASEID, DAYSTOARRHYTHMIA) %>% rename(days = DAYSTOARRHYTHMIA) %>% mutate(ARRHYTHMIA = 1)
cad        <- comorb %>% filter(CAD == 1)        %>% select(RELEASEID, DAYSTOCAD)        %>% rename(days = DAYSTOCAD)        %>% mutate(CAD = 1)
chf        <- comorb %>% filter(CHF == 1)        %>% select(RELEASEID, DAYSTOCHF)        %>% rename(days = DAYSTOCHF)        %>% mutate(CHF = 1)
LVSD       <- comorb %>% filter(LVSD == 1)       %>% select(RELEASEID, DAYSTOLVSD)       %>% rename(days = DAYSTOLVSD)       %>% mutate(LVSD = 1)
mi         <- comorb %>% filter(MI == 1)         %>% select(RELEASEID, DAYSTOMI)         %>% rename(days = DAYSTOMI)         %>% mutate(MI = 1)
pad        <- comorb %>% filter(PAD == 1)        %>% select(RELEASEID, DAYSTOPAD)        %>% rename(days = DAYSTOPAD)        %>% mutate(PAD = 1)
dvt        <- comorb %>% filter(DVT == 1)        %>% select(RELEASEID, DAYSTODVT)        %>% rename(days = DAYSTODVT)        %>% mutate(DVT = 1)
stroke     <- comorb %>% filter(STROKE == 1)     %>% select(RELEASEID, DAYSTOSTROKE)     %>% rename(days = DAYSTOSTROKE)     %>% mutate(STROKE = 1)
tia        <- comorb %>% filter(TIA == 1)        %>% select(RELEASEID, DAYSTOTIA)        %>% rename(days = DAYSTOTIA)        %>% mutate(TIA = 1)
ckd        <- comorb %>% filter(CKD == 1)        %>% select(RELEASEID, DAYSTOCKD)        %>% rename(days = DAYSTOCKD)        %>% mutate(CKD = 1)
eskd       <- comorb %>% filter(ESKD == 1)       %>% select(RELEASEID, DAYSTOESKD)       %>% rename(days = DAYSTOESKD)       %>% mutate(ESKD = 1)
death      <- comorb %>% filter(DEATH == 1)      %>% select(RELEASEID, DAYSTODEATH)      %>% rename(days = DAYSTODEATH)      %>% mutate(DEATH = 1)
ulcer      <- comorb %>% filter(ULCER == 1)      %>% select(RELEASEID, DAYSTOULCER)      %>% rename(days = DAYSTOULCER)      %>% mutate(ULCER = 1)

# Build ame_summary from comorb directly
ame_summary <- comorb %>%
  select(RELEASEID, ARRHYTHMIA, DAYSTOARRHYTHMIA, CAD, DAYSTOCAD, CHF, DAYSTOCHF,
         LVSD, DAYSTOLVSD, MI, DAYSTOMI, PAD, DAYSTOPAD, DVT, DAYSTODVT,
         STROKE, DAYSTOSTROKE, TIA, DAYSTOTIA, CKD, DAYSTOCKD, ESKD, DAYSTOESKD,
         DEATH, DAYSTODEATH, ULCER, DAYSTOULCER)
ame_summary <- ame_summary %>%
  mutate(across(c(ARRHYTHMIA, CAD, CHF, LVSD, MI, PAD, DVT, STROKE, TIA, CKD, ESKD, DEATH, ULCER), as.factor))
...
# Merge with comorb data - comorb already has AME columns so just rename for consistency
comorb_ckm <- comorb %>% rename(RELEASEID = releaseid)
comorb_ckm <- merge(comorb_ckm, ame_summary %>% select(RELEASEID, clin_cvd), by = "RELEASEID", all.x = TRUE)
...
# Rebuild comorb_long from individual event frames (same structure as before)
HTN0 <- comorb_ckm %>% select(RELEASEID, HTN0) %>% rename(HTN = HTN0) %>% mutate(days = 0)
HTN  <- comorb_ckm %>% select(RELEASEID, DAYSTOHTN, HTN) %>% rename(days = DAYSTOHTN)
HTN  <- bind_rows(HTN0, HTN) %>% filter(HTN == 1) %>% unique()
TGDLP0 <- comorb_ckm %>% select(RELEASEID, TGDLP0) %>% rename(TGDLP = TGDLP0) %>% mutate(days = 0)
TGDLP  <- comorb_ckm %>% select(RELEASEID, DAYSTOTG, TGDLP) %>% rename(days = DAYSTOTG)
TGDLP  <- bind_rows(TGDLP0, TGDLP) %>% filter(TGDLP == 1) %>% unique()
...
# Biomarker extraction - now sourced from long instead of raw cbl/addcbl reads
# long has visit_days as the days variable; standardize to RELEASEID for downstream joins
long_biom <- long %>%
  mutate(RELEASEID = toupper(releaseid),
         days = ifelse(!is.na(visit_days) & visit_days < 0, 0, visit_days)) %>%
  filter(!is.na(days))

# ADDCBL biomarkers
bnp_vals         <- long_biom %>% filter(!is.na(bnp))         %>% select(releaseid, days, bnp)
troponin_vals    <- long_biom %>% filter(!is.na(troponin))    %>% select(releaseid, days, troponin)
adiponectin_vals <- long_biom %>% filter(!is.na(adiponectin)) %>% select(releaseid, days, adiponectin)
hmwa_vals        <- long_biom %>% filter(!is.na(hmwa))        %>% select(releaseid, days, hmwa)
edol_vals        <- long_biom %>% filter(!is.na(edol))        %>% select(releaseid, days, edol)
shbg_vals        <- long_biom %>% filter(!is.na(shbg))        %>% select(releaseid, days, shbg)
test_vals        <- long_biom %>% filter(!is.na(testosterone)) %>% select(releaseid, days, testosterone)
gfr_vals         <- long_biom %>% filter(!is.na(ckd_gfr))    %>% select(releaseid, days, ckd_gfr)
fas_vals         <- long_biom %>% filter(!is.na(eGFR_FAS))   %>% select(releaseid, days, eGFR_FAS)
codi_vals        <- long_biom %>% filter(!is.na(codi))        %>% select(releaseid, days, codi)
insinv_vals      <- long_biom %>% filter(!is.na(insinv))      %>% select(releaseid, days, insinv)

# CBL biomarkers
uacr_vals      <- long_biom %>% filter(!is.na(UAlbCreat)) %>% select(releaseid, days, UAlbCreat)
il6_vals       <- long_biom %>% filter(!is.na(IL6))       %>% select(releaseid, days, IL6)
il1_vals       <- long_biom %>% filter(!is.na(il1))       %>% select(releaseid, days, il1)
hscrp_vals     <- long_biom %>% filter(!is.na(hsCRP))     %>% select(releaseid, days, hsCRP)
apob_vals      <- long_biom %>% filter(!is.na(ApoB))      %>% select(releaseid, days, ApoB)
mcp1_vals      <- long_biom %>% filter(!is.na(mcp1))      %>% select(releaseid, days, mcp1)
tnfa_vals      <- long_biom %>% filter(!is.na(tnfa))      %>% select(releaseid, days, tnfa)
tnfr1_vals     <- long_biom %>% filter(!is.na(tnfr1))     %>% select(releaseid, days, tnfr1)
tnfr2_vals     <- long_biom %>% filter(!is.na(tnfr2))     %>% select(releaseid, days, tnfr2)
icam1_vals     <- long_biom %>% filter(!is.na(icam1))     %>% select(releaseid, days, icam1)
vcam1_vals     <- long_biom %>% filter(!is.na(vcam1))     %>% select(releaseid, days, vcam1)
eselectin_vals <- long_biom %>% filter(!is.na(eselectin)) %>% select(releaseid, days, eselectin)
vegf_vals      <- long_biom %>% filter(!is.na(vegf))      %>% select(releaseid, days, vegf)
fgf23_vals     <- long_biom %>% filter(!is.na(fgf23))     %>% select(releaseid, days, fgf23)
a1c_vals       <- long_biom %>% filter(!is.na(hba1c))     %>% select(releaseid, days, hba1c) %>% rename(HbA1c = hba1c)
...
# Apply cleaning and standardize RELEASEID
...
# Create consolidated biomarker dataset (same indices as original)
...
biom_ckm <- biom_ckm %>% filter(!if_all(all_of(cols_to_check), is.na))

biom_ckm <- biom_ckm %>%
  mutate(high_trop = ifelse(is.na(troponin), NA, ifelse(troponin > 0, 1, 0)),
         high_bnp  = ifelse(is.na(bnp),      NA, ifelse(bnp > 80,    1, 0)))

biom_ckm$UAlbCreat <- biom_ckm$UAlbCreat * 1000

# KDIGO - use long for gfr and uacr (days-based via visit_days)
all_gfr  <- long_biom %>% filter(!is.na(ckd_gfr))    %>% select(releaseid, days, ckd_gfr)
all_uacr <- long_biom %>% filter(!is.na(UAlbCreat))   %>% select(releaseid, days, UAlbCreat)
...
kdigo <- kdigo %>% group_by(RELEASEID) %>%
  fill(risk_level, .direction = "down") %>% fill(recommendation, .direction = "down") %>%
  ungroup() %>%
  select(RELEASEID, days, gfr_category, gfr_description, albumin_category, albumin_description, risk_level, recommendation)

# Lab means - sourced from long
a1c_means    <- a1c_vals        %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_a1c    = mean(HbA1c,       na.rm=T))
adipo_means  <- adiponectin_vals %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_adipo  = mean(adiponectin, na.rm=T))
il6_means    <- il6_vals        %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_il6    = mean(IL6,         na.rm=T))
il1_means    <- il1_vals        %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_il1    = mean(il1,         na.rm=T))
hscrp_means  <- hscrp_vals      %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_hscrp  = mean(hsCRP,       na.rm=T))
apob_means   <- apob_vals       %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_apob   = mean(ApoB,        na.rm=T))
tnfa_means   <- tnfa_vals       %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_tnfa   = mean(tnfa,        na.rm=T))
insinv_means <- insinv_vals     %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_insinv = mean(insinv,      na.rm=T))
codi_means   <- codi_vals       %>% rename(releaseid=RELEASEID) %>% group_by(releaseid) %>% summarise(mean_codi   = mean(codi,        na.rm=T))

lab_means <- a1c_means %>%
  full_join(adipo_means,  by="releaseid") %>%
...
# PedsQL - now in long, reconstruct the pedsQL_all structure downstream code expects
pedsQL_all <- long %>%
  mutate(RELEASEID = toupper(releaseid),
         days = ifelse(!is.na(visit_days) & visit_days < 0, 0, visit_days)) %>%
  filter(!is.na(days)) %>%
  select(any_of(c("RELEASEID", "days", "G01WALK", "G02RUN", "G03SPORT",
                  "G04LIFT", "G05BATH", "G06CHORE", "G07HURT", "G08ENERG"))) %>%
  filter(!is.na(G01WALK) | !is.na(G02RUN) | !is.na(G03SPORT) | !is.na(G04LIFT) |
         !is.na(G05BATH) | !is.na(G06CHORE) | !is.na(G07HURT) | !is.na(G08ENERG))

pedsQL_items <- c("G01WALK", "G02RUN", "G03SPORT", "G04LIFT",
                  "G05BATH", "G06CHORE", "G07HURT", "G08ENERG")
pedsQL_all <- pedsQL_all %>%
  rowwise() %>%
  mutate(physical_total = sum(c_across(any_of(pedsQL_items)), na.rm = TRUE)) %>%
  ungroup()
```
...
# pedsQL_all is already constructed from long in the data preparation chunk above
# Just need to compute the pedsQL_summary here
