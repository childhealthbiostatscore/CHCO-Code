...
#prog_summary_final_soma <- left_join(prog_summary_final_soma, baserisk, by = "RELEASEID")
#prog_summary_final_soma$sex_char <- as.factor(prog_summary_final_soma$sex_char)
...
```{r include=F}
t1 <- tableby( ~ CKM_syn_base + CKM_max_char + progress_CKM + 
                 num_prog + notest(days_to_ckm_prog, "Nmiss", "median", "q1q3"), 
...
predictors <- c("AGEBASE", "sex_char", "bmi", "HbA1c", "sbp", "codi", "si_1_ins0", "GLYC")
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_ckm_prog, as.numeric(progress_CKM))~",predictors[i])
  mod <- coxph(data = prog_summary_final_soma, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
...
result_clinical <- result_clinical %>% select(VarName, estimate, std.error,	statistic,	p.value,	conf.low,	conf.high)
```

```{r, include=FALSE}
# write all results to file
wb <- createWorkbook()
addWorksheet(wb,"Time to CKM Proteins")
writeData(wb,"Time to CKM Proteins",result_noBMI,rowNames = F)
addWorksheet(wb,"Time to CKM Clinical")
writeData(wb,"Time to CKM Clinical",result_clinical,rowNames = F)
...
```{r, include=FALSE, comment=""}
form = as.formula(paste0("Surv(days_to_ckm_prog, as.numeric(progress_CKM))~sex_char + AGEBASE"))
mod = coxph(formula = form,data = ckm_without_bmi_endata_nozero)
kable(tidy(mod, exponentiate = TRUE, conf.int = TRUE),digits = 3)
...
```{r, include=FALSE, comment=""}
form = as.formula(paste0("Surv(days_to_ckm_prog, as.numeric(progress_CKM))~sex_char + AGEBASE + bmi + HbA1c + sbp"))
mod = coxph(formula = form,data = ckm_without_bmi_endata_nozero)
kable(tidy(mod, exponentiate = TRUE, conf.int = TRUE),digits = 3)
...
```{r, include=FALSE, comment=""}
form = as.formula(paste0("Surv(days_to_ckm_prog, as.numeric(progress_CKM))~",paste0(ckm_without_bmi_select,collapse = "+"),paste0("+sex_char+AGEBASE")))
mod = coxph(formula = form,data = ckm_without_bmi_endata_nozero)
kable(tidy(mod, exponentiate = TRUE, conf.int = TRUE),digits = 3)
...
```{r, include=FALSE, comment=""}
form = as.formula(paste0("Surv(days_to_ckm_prog, as.numeric(progress_CKM))~",paste0(ckm_without_bmi_select,collapse = "+"),paste0("+HbA1c+log_trig+sbp+si_1_ins0+sex_char+bmi+AGEBASE+racedesc")))
mod = coxph(formula = form,data = ckm_without_bmi_endata_nozero)
kable(tidy(mod, exponentiate = TRUE, conf.int = TRUE),digits = 3)
