...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_ckm_prog, as.numeric(progress_CKM))~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0")
...
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_ckm_prog, as.numeric(progress_CKM2))~",predictors[i],"+HbA1c+log_trig+sbp+si_1_ins0")
  mod <- coxph(data = prog_summary_final_soma, formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
