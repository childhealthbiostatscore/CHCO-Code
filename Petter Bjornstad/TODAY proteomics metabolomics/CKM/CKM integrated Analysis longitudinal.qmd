...
```{r include=F}
my_controls <- tableby.control(cat.simplify = TRUE)
t2a <- tableby(CKM_max_char ~ max_clin_cvd + max_hfpef + max_ARRHYTHMIA + max_CAD + max_CHF + max_LVSD + max_MI + max_DVT + max_PAD + max_STROKE + 
              ...
              data = comorb_ckm2_criteria, total = F, control = my_controls)
# STOPPED HERE: trying to print just one row, but it's printing the zeroes
```
