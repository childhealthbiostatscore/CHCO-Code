...
prog_summary_final <- comorb_ckm2 %>% select(RELEASEID, fup_time, CKM_syn_base, CKM_max, progress_CKM, days_to_first_ckm_prog, num_prog, CKM_syn_first_progression, CKM_syn_num_first_progression) %>%
...
label(prog_summary_final$days_to_first_ckm_prog) <- "Days to first progression event, without BMI"
...
t1 <- tableby( ~ CKM_syn_base + CKM_max_char + progress_CKM + 
                 num_prog + notest(days_to_first_ckm_prog, "Nmiss", "median", "q1q3"), 
...
```{r include=F}
t2 <- tableby(CKM_max_char ~  days_to_first_ckm_prog + HTN + TGDLP + MIC + MAC + GLYC + HbA1c +
...
# predictors <- colnames(base_log[seq])
# for (i in 1:length(predictors)) {
#   form <- paste0("Surv(days_to_first_ckm_prog, as.numeric(progress_CKM2))~",predictors[i])
#   mod <- coxph(data = prog_summary_final_soma, formula = as.formula(form))
#   tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
#   tidymod <- tidymod[1,]
...
delta_contrast <- model.matrix(~ 1 + deltas_no_scale$progress_CKM)
ymat <- t(deltas_no_scale[predictors])
fit <- lmFit(ymat,delta_contrast)
fit <- eBayes(fit)
results_delta <- topTable(fit,coef = 2,number = nrow(ymat))
results_delta$AptName <- row.names(results_delta)
results_delta <- merge(results_delta,analytes,by="AptName",all.x = T, all.y = F)
results_delta <- results_delta[order(results_delta$P.Value),] 

# using paired limma
RELEASEID <- factor(soma_long$RELEASEID)
progress_CKM <- factor(soma_long$progress_CKM)
change_contrast <- model.matrix(~RELEASEID + progress_CKM)
ymat <- t(soma_long[predictors])
fit <- lmFit(ymat,change_contrast)
fit <- eBayes(fit)
```

```{r, include=FALSE}
# write delta results to file
wb <- createWorkbook()
addWorksheet(wb,"Protein delta CKM prog")
writeData(wb,"Protein delta CKM prog",results_delta,rowNames = F)
