...
```{r, include=FALSE}
# Run univariable Cox models for time to first progression for those whose max stage was 3
result_noBMI_3 <- NULL
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_first_ckm_prog, as.numeric(progress_CKM))~",predictors[i])
  mod <- coxph(data = prog_summary_final_soma[prog_summary_final_soma$CKM_max == 3,], formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
...
```{r, include=FALSE}
# Run univariable Cox models for time to first progression for those whose max stage was 4
result_noBMI_4 <- NULL
predictors <- colnames(base_log[seq])
for (i in 1:length(predictors)) {
  form <- paste0("Surv(days_to_first_ckm_prog, as.numeric(progress_CKM))~",predictors[i])
  mod <- coxph(data = prog_summary_final_soma[prog_summary_final_soma$CKM_max == 4,], formula = as.formula(form))
  tidymod <- tidy(mod, exponentiate=TRUE, conf.int = TRUE)
  tidymod <- tidymod[1,]
