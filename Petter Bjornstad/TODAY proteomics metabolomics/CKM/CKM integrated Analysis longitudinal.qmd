...
wb <- createWorkbook()
addWorksheet(wb,"Time to CKM CPH")
writeData(wb,"Time to CKM CPH",result_noBMI,rowNames = F)
...
addWorksheet(wb,"Time to CKM CPH")
writeData(wb,"Time to CKM CPH",result_noBMI,rowNames = F)
full_path <- file.path(data_path, "CKM/Results/IPA/TODAY somalogic Cox models scaled baseline adjusted time to CKM.xlsx")
saveWorkbook(wb, full_path, overwrite = TRUE)
...
# CKM without BMI
ckm_without_bmi_keep = read_excel(f,sheet = "Time to CKM CPH")
