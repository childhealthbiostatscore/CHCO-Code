...
```{r include=F}
t1data <- prog_summary_final %>% mutate(progress_CKM_char =
                                          case_when(progress_CKM == 0 ~ "Did not progress",
                                                    progress_CKM == 1 ~ "Progressed"))
t1 <- tableby( ~ CKM_syn_base + CKM_max_char + progress_CKM_char + 
...
```{r include=F}
t1a <- tableby( ~ progress_CKM_char + num_prog + notest(days_to_first_ckm_prog, "Nmiss", "median", "q1q3"), 
              data = t1data, total = F)
```

```{r results='asis'}
summary(t1a,  pfootnote = F)
```
...
t2a <- t2data %>%
  gtsummary::tbl_summary(
    ...
    missing = "no",
    include = c(
      progress_CKM_char, num_prog, days_to_first_ckm_prog
    ),
    type = list(
      c(days_to_first_ckm_prog) ~ "continuous",
      c(progress_CKM_char, num_prog) ~ "dichotomous"
    ),
    statistic = list(
      gtsummary::all_continuous() ~ "{median} ({q25}, {q75})",
      gtsummary::all_categorical() ~ "{n} ({p}%)"
    ),
    digits = list(
      # Continuous variables
      AGEBASE ~ 0,
      yrs_to_first_ckm_prog ~ 0,
      codi ~ 3,
  ...
  gtsummary::add_p(test = list(gtsummary::all_continuous() ~ "kruskal.test",  # or "aov" for ANOVA
...
# num_vars <- c("AGEBASE", "days_to_first_ckm_prog", "HbA1c", "log_trig", "sbp", "dbp", "uacid", "si_1_ins0", 
#               "UAlbCreat", "bmi", "HDL", "codi", "ALT", "AST")
# 
# my_controls <- tableby.control(
#   cat.simplify = TRUE
# )
...
#   digits = list(
#     AGEBASE = 0,
#     days_to_first_ckm_prog = 0
#   ),
#   digits.pct = 1,  # This is a different argument, so it's OK
#   digits.p = 3,
#   cat.simplify = TRUE
# )
# 
# t2 <- tableby(CKM_max_char ~  AGEBASE + sex_char + racedesc + 
#                 txdesc + days_to_first_ckm_prog + HTN + TGDLP + MIC + MAC + GLYC + HbA1c +
#                 log_trig + sbp + dbp + uacid + si_1_ins0 + UAlbCreat + bmi + HDL + codi + ALT + AST +  DEATH, 
#               data = prog_summary_final, total = F, control = my_controls)
...
t2a <- t2data %>%
  gtsummary::tbl_summary(
    ...
    include = c(
      ...
      HTN, TGDLP, MIC, MAC, GLYC, DEATH, CKM_syn_base, progress_CKM_char, yrs_to_first_ckm_prog, num_prog
    ),
    type = list(
      c(AGEBASE, yrs_to_first_ckm_prog, HbA1c, log_trig, sbp, dbp, 
...
t2b <- t2data %>%
  gtsummary::tbl_summary(
    ...
    missing = "no",
    include = c(
      AGEBASE, sex_char, racedesc, bmi, txdesc, yrs_to_first_ckm_prog, HbA1c, 
      si_1_ins0, UAlbCreat, 
      GLYC, DEATH, num_prog
    ),
    type = list(
      c(AGEBASE, yrs_to_first_ckm_prog, HbA1c, 
