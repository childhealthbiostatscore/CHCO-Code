                              ...
                              call_variants_per_sample = function(force_rerun = FALSE) {
                                ...
                                call_variants_single <- function(i) {
                                  ...
                                  tryCatch({
                                    ...
                                    cmd <- sprintf(
                                      "%s --java-options '-Xmx%s' HaplotypeCaller -R %s -I %s -O %s -ERC GVCF --native-pair-hmm-threads 1 --max-mnp-distance 0 --min-base-quality-score %d --minimum-mapping-quality %d --tmp-dir %s %s",
                                      self$gatk_path,
                                      self$memory,
                                ...
                                max_parallel <- min(16, self$threads, nrow(to_process))  # Limit parallel jobs for memory
                              ...
                              clean_gvcfs_for_genomicsdb = function(force_rerun = FALSE) {
                                ...
                                clean_single_gvcf <- function(i) {
                                  ...
                                  tryCatch({
                                    ...
                                    cmd <- sprintf(
                                      "%s view -i 'REF=\"A\" || REF=\"T\" || REF=\"C\" || REF=\"G\" || ALT=\"A\" || ALT=\"T\" || ALT=\"C\" || ALT=\"G\"' -O z -o %s %s",
                                      self$bcftools_path,
                                      row$cleaned_gvcf,
                                      row$gvcf_file
                                    )
                                    ...
                                    cmd <- sprintf("%s IndexFeatureFile -I %s", self$gatk_path, row$cleaned_gvcf)
                                    result <- system(cmd, intern = FALSE)
                                    if (result != 0) {
                                ...
                                # Use parallel processing
                                cl <- makeCluster(min(16, self$threads))
                            ...
                            private = list(
                              ...
                              joint_genotype_batched = function(available_samples, sample_map_file) {
                                ...
                                for (batch_i in 1:n_batches) {
                                  ...
                                  if(file.exists(batch_vcf) == FALSE)
                                  {
                                  # Get batch samples
                                    ...
                                    cmd <- sprintf(
                                      ...
                                      self$gatk_path, self$memory, batch_sample_map, genomicsdb_path, self$reference_genome,
                                    ...
                                    cmd <- sprintf(
                                      ...
                                      self$gatk_path, self$memory, self$reference_genome, genomicsdb_path, batch_vcf,
                                      file.path(self$output_dir, "temp")
                                    )
...
run_gatk_cram_pipeline <- function(cram_dir, output_dir, reference_genome,
  ...
  if (!is.null(intervals_file)) {
    file.copy(from = intervals_file, to = paste(output_dir, "intervals.list", sep = "/"))
...
fix_mnp_gvcfs <- function(sample_map_file, output_dir, bcftools_path = "bcftools") {
  ...
  clean_gvcf <- function(i) {
    ...
    tryCatch({
      # Remove MNPs using bcftools
      cmd <- sprintf("%s view -i 'REF=\"A\" || REF=\"T\" || REF=\"C\" || REF=\"G\" || ALT=\"A\" || ALT=\"T\" || ALT=\"C\" || ALT=\"G\"' -O z -o %s %s", 
  ...
  # Process in parallel
  cl <- makeCluster(min(16, detectCores()))
