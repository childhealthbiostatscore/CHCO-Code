...
run_qtl_analysis <- function(
    ...
    params_info <- data.frame(
        parameter = c(
            ...
            "maf_threshold", "hwe_threshold", "skip_completed", "outlier_removal", "temp_dir", 
        ...
        value = c(
            ...
            plink2_path, fdr_threshold, normalization, normalize_covariates, normalize_pcs,
            maf_threshold, hwe_threshold, skip_completed,
            if(is.null(outlier_removal)) "NULL" else outlier_removal,
    ...
    # Step 5: Run analysis for each phenotype
    cat("Step 5: Running PLINK2 analysis...\n")
    
    for(i in seq_along(pheno_cols)) {
        pheno_name <- pheno_cols[i]              # Original phenotype name
        clean_pheno_name <- clean_pheno_names[i]  # Clean phenotype name (already computed)
        
        # FIRST: Check if analysis already completed using CLEAN name for file path (RDS format)
        if(skip_completed) {
            existing_stats_file <- file.path(stats_dir, paste0(clean_pheno_name, "_summary_stats.rds"))
            
            if(file.exists(existing_stats_file)) {
                cat(sprintf("  - Skipping phenotype %d/%d: %s (summary stats file already exists)\n", i, length(pheno_cols), pheno_name))
                
                # Load existing results for final summary
                existing_results <- readRDS(existing_stats_file)
                existing_results$phenotype <- pheno_name  # Keep original name in data
                all_results[[pheno_name]] <- existing_results
                ...
                summary_stats <- rbind(summary_stats, data.frame(
                    phenotype = pheno_name,  # Use original name in summary
                ...
                # Set plot file paths using clean names (RDS format)
                plot_files[[pheno_name]] <- list(
                    manhattan = file.path(manhattan_dir, paste0(clean_pheno_name, "_manhattan.png")), 
                    qq = file.path(qq_dir, paste0(clean_pheno_name, "_qq.png")),
                    stats = existing_stats_file
                )
                
                next  # Skip to next phenotype immediately
            }
        }
        
        cat(sprintf("  - Analyzing phenotype %d/%d: %s (clean: %s)\n", i, length(pheno_cols), pheno_name, clean_pheno_name))

        # Debug: Check actual genotype file sample IDs and format
        psam_check <- fread(paste0(genotype_prefix, ".psam"), nrows = 5)
        psam_full <- fread(paste0(genotype_prefix, ".psam"))
        
        cat(sprintf("    - Genotype .psam structure: %d columns (%s)\n", 
                    ncol(psam_check), paste(colnames(psam_check), collapse = ", ")))
        
        if(grepl("#?IID", colnames(psam_check)[1], ignore.case = TRUE)) {
            # Format: IID SEX (no FID column)
            genotype_sample_ids <- psam_full[[1]]
            psam_format <- "IID_only"
            cat(sprintf("    - Detected .psam format: IID-only (no FID)\n"))
        } else if(ncol(psam_check) >= 2) {
            # Standard format: FID IID [SEX ...]
            genotype_sample_ids <- psam_full[[2]]
            psam_format <- "FID_IID"
            cat(sprintf("    - Detected .psam format: Standard FID IID\n"))
        } else {
            genotype_sample_ids <- psam_full[[1]]
            psam_format <- "unknown"
            cat(sprintf("    - Unknown .psam format, using first column\n"))
        }
        
        cat(sprintf("    - First few genotype IDs: %s\n", 
                    paste(head(genotype_sample_ids, 3), collapse = ", ")))
        
        # Prepare phenotype file for current phenotype - format to match .psam
        if(psam_format == "IID_only") {
            # .psam has only IID, so phenotype file should be: IID PHENO [covariates]
            current_pheno <- pheno_merged[, c("genotype_id", pheno_name, 
                                             pc_cols, available_covs), with = FALSE]
            colnames(current_pheno)[1:2] <- c("IID", "PHENO")
            cat(sprintf("    - Phenotype file format: IID PHENO (matching .psam)\n"))
        } else {
            # Standard format: FID IID PHENO [covariates]
            current_pheno <- pheno_merged[, c("genotype_id", "genotype_id", pheno_name, 
                                             pc_cols, available_covs), with = FALSE]
            colnames(current_pheno)[1:3] <- c("FID", "IID", "PHENO")
            cat(sprintf("    - Phenotype file format: FID IID PHENO (standard)\n"))
        }
        ...
        cat(sprintf("    - Prepared phenotype file: %d samples with complete data\n", nrow(current_pheno)))
        
        # Write phenotype file using clean name
        pheno_file <- file.path(temp_dir, paste0("pheno_", clean_pheno_name, ".txt"))
        fwrite(current_pheno, pheno_file, sep = "\t")
        
        # Debug: show first few rows of phenotype file
        cat(sprintf("    - First few sample IDs in phenotype file: %s\n", 
                    paste(head(current_pheno[[1]], 3), collapse = ", ")))
        
        # Debug: Check what's in the keep file - format to match .psam
        if(psam_format == "IID_only") {
            keep_data <- data.frame(IID = pheno_merged$genotype_id)
            cat(sprintf("    - Keep file format: IID (matching .psam)\n"))
        } else {
            keep_data <- data.frame(FID = pheno_merged$genotype_id, 
                                   IID = pheno_merged$genotype_id)
            cat(sprintf("    - Keep file format: FID IID (standard)\n"))
        }
        
        keep_file <- file.path(temp_dir, paste0("keep_samples_", clean_pheno_name, ".txt"))
        fwrite(keep_data, keep_file, sep = "\t", col.names = FALSE)
        
        cat(sprintf("    - First few sample IDs in keep file: %s\n", 
                    paste(head(keep_data[[1]], 3), collapse = ", ")))
        
        # Check for overlap
        overlap_count <- length(intersect(current_pheno[[1]], genotype_sample_ids))
        cat(sprintf("    - Overlap between phenotype and genotype files: %d/%d samples\n", 
                    overlap_count, nrow(current_pheno)))
        
        # Prepare covariate columns for PLINK2
        covar_cols <- c(pc_cols, available_covs)
        covar_string <- paste(covar_cols, collapse = ",")
        
        # Run PLINK2 using clean phenotype name
        output_file <- file.path(temp_dir, paste0("results_", clean_pheno_name))
        ...
        cat(sprintf("    - PLINK2 command: %s\n", plink2_cmd))
        
        # Execute PLINK2
        system_result <- system(plink2_cmd, intern = FALSE)
        ...
        # Check and standardize column names for plotting
        ...
        cat(sprintf("    - PLINK2 results columns: %s\n", paste(colnames(results), collapse = ", ")))
        
        # Apply FDR correction
        results$FDR <- p.adjust(results$P, method = "BH")
        results$phenotype <- pheno_name
        
        # Optimize results for storage: keep only essential columns and reduce precision
        ...
        # Check what columns are missing and available
        missing_essential_cols <- setdiff(essential_cols, colnames(results))
        if(length(missing_essential_cols) > 0) {
            cat(sprintf("    - Missing columns (will be skipped): %s\n", paste(missing_essential_cols, collapse = ", ")))
        }
        
        # Filter to essential columns
        results_optimized <- results[, available_essential_cols, with = FALSE]
        
        # Reduce precision to 4 significant figures for numeric columns
        ...
        cat(sprintf("    - Optimized results: %d variants, %d columns (was %d columns)\n", 
                    nrow(results_optimized), ncol(results_optimized), ncol(results)))

        # Store optimized results
        all_results[[pheno_name]] <- results_optimized
        
        # Update summary statistics using optimized results
        ...
        # Write individual summary statistics file using clean name for file path (RDS format)
        individual_stats_file <- file.path(stats_dir, paste0(clean_pheno_name, "_summary_stats.rds"))
        
        # Check if stats file already exists (shouldn't happen if skipping works, but double-check)
        if(file.exists(individual_stats_file)) {
            cat(sprintf("    - WARNING: Stats file already exists, skipping write: %s\n", basename(individual_stats_file)))
        } else {
            saveRDS(results_optimized, individual_stats_file, compress = TRUE)
            cat(sprintf("    - Saved summary statistics: %s\n", basename(individual_stats_file)))
        }
        
        # Clean up intermediate files for this phenotype (but keep processing folder)
        unlink(c(pheno_file, paste0(output_file, ".log"), paste0(output_file, ".PHENO.glm.linear")))
        
        cat(sprintf("    - Cleaned up intermediate files for %s\n", clean_pheno_name))
    }
...
example_usage <- function() {
    ...
    # Example: Analyze all phenotypes (default behavior)
    results_all <- run_qtl_analysis(
        phenotype_file = "path/to/phenotypes.txt",
        genotype_prefix = "path/to/genotypes",
        id_conversion_file = "path/to/id_conversion.txt",
        pca_file = "path/to/pca.txt",
        covariate_file = "path/to/covariates.txt",
        output_prefix = "output/qtl_analysis_all_folder",    # Directory for all results
        # phenotype_list = NULL,  # Default: analyze all phenotypes
        n_pcs = 10,
        covariate_columns = c("sex", "age", "batch"),
        threads = 8,
        memory = 16000,
        fdr_threshold = 0.05,
        normalization = "log_scale",          # Log + scaling for metabolomics
        normalize_covariates = TRUE,          # Always recommended
        normalize_pcs = TRUE,                 # Always recommended
        maf_threshold = 0.005,                # Slightly lower MAF for metabolomics
        hwe_threshold = 1e-6,                 # Standard HWE threshold
        skip_completed = TRUE,                # Skip already completed analyses
        outlier_removal = 3
    )
...
#' Diagnostic function to check ID matching between files
#'
diagnose_id_matching <- function(phenotype_file, genotype_prefix, id_conversion_file, 
                                pca_file, covariate_file) {
    
    cat("Diagnosing ID matching issues...\n")
    cat("=====================================\n")
    
    # Load all files
    phenotypes <- fread(phenotype_file)
    id_conv <- fread(id_conversion_file)
    pca_data <- fread(pca_file)
    covariates <- fread(covariate_file)
    psam <- fread(paste0(genotype_prefix, ".psam"))
    
    # Handle .psam file - check if it has headers or not
    cat(sprintf("Genotype .psam file structure:\n"))
    cat(sprintf("  Columns: %s\n", paste(colnames(psam), collapse = ", ")))
    cat(sprintf("  First row: %s\n", paste(head(as.character(psam[1,]), 3), collapse = ", ")))
    
    # PLINK2 .psam format can be: FID IID [optional columns] OR just IID [optional columns]
    # Check if first column header suggests it's IID only
    if(ncol(psam) >= 1) {
        first_col_header <- colnames(psam)[1]
        
        if(grepl("#?IID", first_col_header, ignore.case = TRUE)) {
            # Format: IID SEX (no FID column)
            iid_col <- psam[[1]]
            cat(sprintf("  Format detected: IID-only (no FID column)\n"))
            cat(sprintf("  IID (col 1): %s\n", paste(head(iid_col, 3), collapse = ", ")))
            genotype_ids <- iid_col[!is.na(iid_col)]
            
        } else if(ncol(psam) >= 2) {
            # Standard format: FID IID [SEX ...]
            fid_col <- psam[[1]]
            iid_col <- psam[[2]]
            cat(sprintf("  Format detected: Standard FID IID format\n"))
            cat(sprintf("  FID (col 1): %s\n", paste(head(fid_col, 3), collapse = ", ")))
            cat(sprintf("  IID (col 2): %s\n", paste(head(iid_col, 3), collapse = ", ")))
            genotype_ids <- iid_col[!is.na(iid_col)]
            
        } else {
            stop("PLINK2 .psam file format not recognized!")
        }
    } else {
        stop("PLINK2 .psam file has no columns - this is unexpected!")
    }
    
    cat(sprintf("ID conversion (genotype_id): %s\n", 
                paste(head(id_conv$genotype_id, 3), collapse = ", ")))
    cat(sprintf("ID conversion (phenotype_id): %s\n", 
                paste(head(id_conv$phenotype_id, 3), collapse = ", ")))
    
    # Check overlaps
    cat("\nChecking ID overlaps:\n")
    
    # Genotype IDs vs PCA IDs  
    pca_ids <- pca_data[[1]]
    overlap_geno_pca <- length(intersect(genotype_ids, pca_ids))
    cat(sprintf("Genotype vs PCA: %d/%d genotype samples, %d/%d PCA samples overlap\n", 
                overlap_geno_pca, length(genotype_ids), overlap_geno_pca, length(pca_ids)))
    
    # Genotype IDs vs ID conversion
    overlap_geno_conv <- length(intersect(genotype_ids, id_conv$genotype_id))
    cat(sprintf("Genotype vs ID conversion: %d/%d genotype samples, %d/%d conversion entries overlap\n", 
                overlap_geno_conv, length(genotype_ids), overlap_geno_conv, nrow(id_conv)))
    
    # Phenotype IDs vs ID conversion
    pheno_ids <- phenotypes[[1]]
    overlap_pheno_conv <- length(intersect(pheno_ids, id_conv$phenotype_id))
    cat(sprintf("Phenotype vs ID conversion: %d/%d phenotype samples, %d/%d conversion entries overlap\n", 
                overlap_pheno_conv, length(pheno_ids), overlap_pheno_conv, nrow(id_conv)))
    
    # Show actual sample IDs for comparison
    cat("\nActual sample ID examples:\n")
    cat(sprintf("Genotype IDs: %s\n", paste(head(genotype_ids, 5), collapse = ", ")))
    cat(sprintf("PCA IDs: %s\n", paste(head(pca_ids, 5), collapse = ", ")))
    cat(sprintf("Conversion genotype_id: %s\n", paste(head(id_conv$genotype_id, 5), collapse = ", ")))
    
    # Show mismatches if any
    if(overlap_geno_pca < min(length(genotype_ids), length(pca_ids))) {
        missing_in_pca <- setdiff(head(genotype_ids, 10), pca_ids)
        missing_in_geno <- setdiff(head(pca_ids, 10), genotype_ids)
        if(length(missing_in_pca) > 0) {
            cat(sprintf("\nFirst 3 genotype IDs missing in PCA: %s\n", paste(head(missing_in_pca, 3), collapse = ", ")))
        }
        if(length(missing_in_geno) > 0) {
            cat(sprintf("First 3 PCA IDs missing in genotypes: %s\n", paste(head(missing_in_geno, 3), collapse = ", ")))
        }
    }
    
    if(overlap_geno_conv < min(length(genotype_ids), nrow(id_conv))) {
        missing_in_conv <- setdiff(head(genotype_ids, 10), id_conv$genotype_id)
        missing_in_geno <- setdiff(head(id_conv$genotype_id, 10), genotype_ids)
        if(length(missing_in_conv) > 0) {
            cat(sprintf("\nFirst 3 genotype IDs missing in ID conversion: %s\n", paste(head(missing_in_conv, 3), collapse = ", ")))
        }
        if(length(missing_in_geno) > 0) {
            cat(sprintf("First 3 conversion IDs missing in genotypes: %s\n", paste(head(missing_in_geno, 3), collapse = ", ")))
        }
    }
