...
filename <- "/Results/Linear and Cox models/TODAY somalogic limma baseline unadjusted.xlsx"
full_path <- file.path(data_path, filename)
today_htn <- read_xlsx(full_path, sheet = "htn_moderated_FDR")
...
filename <- "Proteomics HTN/copy_of_old_analysis_dataset_for_HTN_response.RData"
full_path <- file.path(data_path, filename)
load(full_path)
df <- df[!is.na(df$htn),]
...
```{r include = F}
# read in top HTN proteins in TODAY 
filename <- "Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted new covariates.xlsx"
full_path <- file.path(data_path, filename)
today_htn <- read_xlsx(full_path, sheet = "HTN CPH")
...
filename <- "Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted HTN model selection.xlsx"
full_path <- file.path(data_path, filename)
today_htn_base_results <- read_xlsx(full_path, sheet = "HTN CPH base")
...
filename <- "Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted HTN model selection.xlsx"
full_path <- file.path(data_path, filename)
today_htn_base_BMI_results <- read_xlsx(full_path, sheet = "HTN CPH base+BMI")
...
* `r nrow(c)` protein (`r c$TargetFullName`) was significantly associated with time to HTN in models adjusted for HbA1c, sex, age, and ethnicity and retained significance after adjustment for BMI (refer to these associations as independent of BMI)
...
```{r results='asis', dpi=1200, include=FALSE}
filename <- "Results/Teen LABS change_from_m1.xlsx"
full_path <- file.path(teen_labs_path, filename)
teenlabs_results_m6 <- read_xlsx(full_path, sheet = "visitMonth 6")
teenlabs_results_m6$time <- "Month 6"
teenlabs_results_y1 <- read_xlsx(full_path, sheet = "visitYear 1")
teenlabs_results_y1$time <- "Year 1"
teenlabs_results_y2 <- read_xlsx(full_path, sheet = "visitYear 2")
teenlabs_results_y2$time <- "Year 2"
teenlabs_results_y3 <- read_xlsx(full_path, sheet = "visitYear 3")
teenlabs_results_y3$time <- "Year 3"
teenlabs_results_y4 <- read_xlsx(full_path, sheet = "visitYear 4")
teenlabs_results_y4$time <- "Year 4"
teenlabs_results_y5 <- read_xlsx(full_path, sheet = "visitYear 5")
...
filename <- "Results/Teen LABS change_from_m1 BMI modified.csv"
full_path <- file.path(teen_labs_path, filename)
write.csv(teenlabs_results, full_path, row.names = FALSE)
```
