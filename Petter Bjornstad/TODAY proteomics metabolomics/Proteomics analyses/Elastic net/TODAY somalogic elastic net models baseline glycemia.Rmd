...
home_dir = "/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward"
...
f = "./Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted.xlsx"
...
set.seed(3654)
source("/Users/kristenmiller/Documents/GitHub/CHCO-Code/Petter Bjornstad/TODAY proteomics metabolomics/Proteomics analyses/Elastic net/easy_elasticnet.R")
```
...
# tiff("/Users/kristenmiller/Library/CloudStorage/OneDrive-SharedLibraries-UW/bjornstad_pyle_tommerdahl_lab - Documents/Dissemination/Manuscripts/P028_TODAY_glycemia_km/02_Submitted/01_JCEM_r2/elastic_net_for_plot_p.tif",
#      res=300,units="in",width=12,height=12)
# elastic_net_for_plot_p
# dev.off()
```
...
# png('/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/Peds Endo/Petter Bjornstad/Proteomics and glycemia/Figures/forest_plot_elastic_net_glycemia_concordance.png', 
#     res = 300, width = 9, height = 3, units = "in")
# p
# dev.off()
...
# dev.off()
### export for JCEM: 
tiff("/Users/kristenmiller/Library/CloudStorage/OneDrive-SharedLibraries-UW/bjornstad_pyle_tommerdahl_lab - Documents/Dissemination/Manuscripts/P028_TODAY_glycemia_km/02_Submitted/01_JCEM_r2/fig2b_concordance.tif",
     res=300,units="in",width=12,height=3)
p
dev.off()
```
...
f = "/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward/Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted.xlsx"
...
glyc_forest_p <- forest(
  glyc_forest[, c("Target", " ", "HR (95% CI)")],
  est = glyc_forest$estimate,
  lower = glyc_forest$conf.low,
  upper = glyc_forest$conf.high,
  ci_column = 2,
  xlim = c(0, 3.5),
  ref_line = 1,
  theme = forest_theme(
    margin = unit(c(1, 1, 1, 1), "mm")  # top, right, bottom, left
  )
)

tiff("/Users/kristenmiller/Library/CloudStorage/OneDrive-SharedLibraries-UW/bjornstad_pyle_tommerdahl_lab - Documents/Dissemination/Manuscripts/P028_TODAY_glycemia_km/02_Submitted/01_JCEM_r2/fig2a_forestplot.tif",
     res=300,units="in",width=8,height=6)
glyc_forest_p
dev.off()

#### PANEL PLOT FOR JCEM RESUBMISSION (6/25/2026):
# Figure 2:
# convert both forest plots to grobs
glyc_forest_p <- forest(
  glyc_forest[, c("Target", " ", "HR (95% CI)")],
  est = glyc_forest$estimate,
  lower = glyc_forest$conf.low,
  ...
  ref_line = 1,
  theme = forest_theme(
    margin = unit(c(0.5, 0.5, 0.5, 0.5), "mm")  # top, right, bottom, left
  )
)

p <- forest(concordance_df_for_plot[, c(2, 7:8)], 
            est = concordance_df_for_plot$Concordance, 
            lower = concordance_df_for_plot$`Lower bound 95% CI`,
            upper = concordance_df_for_plot$`Upper bound 95% CI`, 
            ci_column = 2, 
            xlim = c(0,1),
            theme = forest_theme(
              margin = unit(c(0.5, 0.5, 0.5, 0.5), "mm")
            ))

grob_a <- ggplotify::as.grob(glyc_forest_p)
grob_b <- ggplotify::as.grob(p)

# combine into panel with labels
...
panel_plot <- arrangeGrob(
  arrangeGrob(textGrob("A", x = 0.1, hjust = 0, gp = gpar(fontsize = 14, fontface = "bold")), grob_a, ncol = 1, heights = unit(c(0.05, 1), c("cm", "null"))),
  arrangeGrob(textGrob("B", x = 0.1, hjust = 0, gp = gpar(fontsize = 14, fontface = "bold")), grob_b, ncol = 1, heights = unit(c(0.05, 1), c("cm", "null"))),
  ncol = 1,
  heights = unit(c(6, 2), "in")  # adjust to match actual plot heights
)

tiff("/Users/kristenmiller/Library/CloudStorage/OneDrive-SharedLibraries-UW/bjornstad_pyle_tommerdahl_lab - Documents/Dissemination/Manuscripts/P028_TODAY_glycemia_km/02_Submitted/01_JCEM_r2/fig2_panel.tif",
     res = 300, units = "in", width = 10, height = 8.5)
...
# helper function to build each forest plot
make_forest <- function(filepath) {
  glyc_forest <- read_excel(filepath, sheet = "GLYC CPH")
  glyc_forest <- glyc_forest %>% arrange(adj.p.value)
  glyc_forest <- glyc_forest[1:20, ]
  glyc_forest$Target <- ifelse(glyc_forest$Target == "carboxylesterase, liver", "Liver carboxylesterase", glyc_forest$Target)
  glyc_forest$Target <- ifelse(glyc_forest$Target == "aldolase C", "Aldolase C", glyc_forest$Target)
  ...
  forest(
    ...
    xlim = c(0, 3.5),
    ref_line = 1,
    theme = forest_theme(margin = unit(c(0.5, 0.5, 0.5, 0.5), "mm"))
  )
}

# build the three plots
p_hispanic <- make_forest("/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward/Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted Hispanic.xlsx")
p_nhb      <- make_forest("/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward/Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted NHB.xlsx")
p_nhw      <- make_forest("/Users/kristenmiller/Library/CloudStorage/OneDrive-UW/Laura Pyle's files - Biostatistics Core Shared Drive/TODAY subaward/Results/Linear and Cox models/TODAY somalogic Cox models scaled baseline adjusted NHW.xlsx")

# convert to grobs
grob_a <- ggplotify::as.grob(p_hispanic)
grob_b <- ggplotify::as.grob(p_nhb)
grob_c <- ggplotify::as.grob(p_nhw)

# combine with panel labels
panel_plot <- arrangeGrob(
  arrangeGrob(textGrob("A", x = 0.05, hjust = 0.3, gp = gpar(fontsize = 14, fontface = "bold")), grob_a, ncol = 1, heights = unit(c(0.3, 1), c("cm", "null"))),
  arrangeGrob(textGrob("B", x = 0.05, hjust = 0.3, gp = gpar(fontsize = 14, fontface = "bold")), grob_b, ncol = 1, heights = unit(c(0.3, 1), c("cm", "null"))),
  arrangeGrob(textGrob("C", x = 0.05, hjust = 0.3, gp = gpar(fontsize = 14, fontface = "bold")), grob_c, ncol = 1, heights = unit(c(0.3, 1), c("cm", "null"))),
  ncol = 1
)

tiff("/Users/kristenmiller/Library/CloudStorage/OneDrive-SharedLibraries-UW/bjornstad_pyle_tommerdahl_lab - Documents/Dissemination/Manuscripts/P028_TODAY_glycemia_km/02_Submitted/01_JCEM_r2/figs3_race_panel.tif",
     res = 300, units = "in", width = 8, height = 17)  # ~7*3 + a little for labels
grid.draw(panel_plot)
dev.off()
