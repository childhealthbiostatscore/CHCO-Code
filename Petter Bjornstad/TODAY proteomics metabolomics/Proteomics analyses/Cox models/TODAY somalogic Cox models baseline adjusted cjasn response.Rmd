...
saveWorkbook(wb,"/Users/pylell/Dropbox/TODAY HTN and DKD output/TODAY somalogic Cox models scaled baseline adjusted.xlsx",overwrite = TRUE)

# separate files for IPA
wb <- createWorkbook()
addWorksheet(wb,"MAC CPH")
writeData(wb,"MAC CPH",result_MAC,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted MAC.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"MIC CPH")
writeData(wb,"MIC CPH",result_MIC,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted MIC.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"MIC.OR.MAC CPH")
writeData(wb,"MIC.OR.MAC CPH",result_MIC.OR.MAC,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted MIC.OR.MAC.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"HYP CPH")
writeData(wb,"HYP CPH",result_HYP,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted HYP.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"RAPID CPH")
writeData(wb,"RAPID CPH",result_RAPID,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted RAPID.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"NEURO CPH")
writeData(wb,"NEURO CPH",result_NEURO,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted NEURO.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"RETINO CPH")
writeData(wb,"RETINO CPH",result_RETINO,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted RETINO.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"GLYC CPH")
writeData(wb,"GLYC CPH",result_GLYC,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted GLYC.xlsx",overwrite = TRUE)

wb <- createWorkbook()
addWorksheet(wb,"GLYC with A1c CPH")
writeData(wb,"GLYC with A1c CPH",results_glyc_a1c,rowNames = F)
saveWorkbook(wb,"./Results/Linear and Cox models/IPA/TODAY somalogic Cox models scaled baseline adjusted GLYC with A1c.xlsx",overwrite = TRUE)

# create dfs of significant proteins for Venn diagrams
# one version nominally significant, one version FDR adjusted
result_MIC.OR.MAC_nom_sig <- result_MIC.OR.MAC %>% arrange(AptName)
result_MIC.OR.MAC_nom_sig <- result_MIC.OR.MAC_nom_sig$p.value < 0.05
result_HYP_nom_sig <- result_HYP %>% arrange(AptName)
result_HYP_nom_sig <- result_HYP_nom_sig$p.value < 0.05
result_RAPID_nom_sig <- result_RAPID %>% arrange(AptName)
result_RAPID_nom_sig <- result_RAPID_nom_sig$p.value < 0.05
v_nom <- as.data.frame(cbind(result_MIC.OR.MAC_nom_sig, result_HYP_nom_sig, result_RAPID_nom_sig))
colnames(v_nom) <- c("Albuminuria", "Hyperfiltration", "Rapid eGFR decline")

result_MIC.OR.MAC_fdr_sig <- result_MIC.OR.MAC %>% arrange(AptName)
result_MIC.OR.MAC_fdr_sig <- result_MIC.OR.MAC_fdr_sig$adj.p.value < 0.05
result_HYP_fdr_sig <- result_HYP %>% arrange(AptName)
result_HYP_fdr_sig <- result_HYP_fdr_sig$p.value < 0.05
result_RAPID_fdr_sig <- result_RAPID %>% arrange(AptName)
result_RAPID_fdr_sig <- result_RAPID_fdr_sig$p.value < 0.05
v_fdr<- as.data.frame(cbind(result_MIC.OR.MAC_fdr_sig, result_HYP_fdr_sig, result_RAPID_fdr_sig))
colnames(v_fdr) <- c("Albuminuria", "Hyperfiltration", "Rapid eGFR decline")

# get list of proteins significant for all 3 outcomes
a <- result_MIC.OR.MAC[result_MIC.OR.MAC$p.value < 0.05,]
a$MIC.OR.MAC.sig <- TRUE
a <- a %>% select(AptName, MIC.OR.MAC.sig, Target)
b <- result_HYP[result_HYP$p.value < 0.05,]
b$HYP.sig <- TRUE
b <- b %>% select(AptName, HYP.sig)
c <- result_RAPID[result_RAPID$p.value < 0.05,]
c$RAPID.sig <- TRUE
c <- c %>% select(AptName, RAPID.sig)
ab <- left_join(a, b, by="AptName")
abc <- left_join(ab, c, by="AptName")

# pull out results for 7 proteins significant for all 3 DKD outcomes
abc_sig <- abc %>% filter(MIC.OR.MAC.sig & HYP.sig & RAPID.sig)
result_MIC.OR.MAC_intersect <- result_MIC.OR.MAC %>% filter(AptName %in% abc_sig$AptName)
result_MIC.OR.MAC_intersect$outcome <- "ALBUMINURIA"
result_HYP_intersect <- result_HYP %>% filter(AptName %in% abc_sig$AptName)
result_HYP_intersect$outcome <- "HYPERFILTRATION"
result_RAPID_intersect <- result_RAPID %>% filter(AptName %in% abc_sig$AptName)
result_RAPID_intersect$outcome <- "RAPID EGFR DECLINE"
result_intersect_temp <- rbind(result_MIC.OR.MAC_intersect, result_HYP_intersect, result_RAPID_intersect)
...
vennp_nom <- ggvenn(v_nom, 
  c("Albuminuria", "Hyperfiltration", "Rapid eGFR decline"),
  fill_color = c("#c2dfe3", "#fff9ec","#fcb1a6"),
  stroke_size = 0.5, set_name_size = 6
)
vennp_fdr <- ggvenn(v_fdr, 
  c("Albuminuria", "Hyperfiltration", "Rapid eGFR decline"),
  fill_color = c("#c2dfe3", "#fff9ec","#fcb1a6"),
  stroke_size = 0.5, set_name_size = 6
)

# plots including only significant proteins
v_nom_excl_ns <- v_nom %>% filter(Albuminuria=="TRUE" | Hyperfiltration=="TRUE" | `Rapid eGFR decline`=="TRUE")
v_fdr_excl_ns <- v_fdr %>% filter(Albuminuria=="TRUE" | Hyperfiltration=="TRUE" | `Rapid eGFR decline`=="TRUE")
vennp_nom_ns <- ggvenn(v_nom_excl_ns, 
  c("Albuminuria", "Hyperfiltration", "Rapid eGFR decline"),
  fill_color = c("#c2dfe3", "#fff9ec","#fcb1a6"),
  stroke_size = 0.5, set_name_size = 6
)
png(file = "/Users/pylell/Dropbox/TODAY HTN and DKD output/venn_nominally_significant_exclude_ns.png", res = 300,
    height = 2000, width = 2000)
vennp_nom_ns
dev.off()
vennp_fdr_ns <- ggvenn(v_fdr_excl_ns, 
  c("Albuminuria", "Hyperfiltration", "Rapid eGFR decline"),
  fill_color = c("#c2dfe3", "#fff9ec","#fcb1a6"),
  stroke_size = 0.5, set_name_size = 6
)
