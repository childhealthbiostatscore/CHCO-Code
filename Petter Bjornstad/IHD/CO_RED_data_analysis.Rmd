...
combined_clean <- read.csv('./IHD/Clean Data/combined_CO_RED_clean_11.19.25.csv')
...
label(combined_clean$fasting_ffa) <- "Fasting NEFA level 100 minutes after iohexol/PAH"
...
```{r additional data cleaning, include=FALSE}
## additional data cleaning, per Petter and Netherlands team 1.12.2026

# reclassify IT_22-PAN-08-T as being in IMPROVE instead of PANTHER
# this subject was co-enrolled in both studies, but put in PANTHER because their earliest visit was PANTHER
combined_clean$study[combined_clean$record_id == 'IT_22-PAN-08-T'] <- 'IMPROVE'

# reclassify RH-93-T-RH2-09-T as being in RENAL-HEIR instead of RENAL-HEIRitage since they were also co-enrolled
# and their insulin sensitivity data is actually from RENAL-HEIR
combined_clean$study[combined_clean$record_id == 'RH-93-T-RH2-09-T'] <- 'RENAL-HEIR'

# drop all PANTHER only (no co-enrollment) subjects since they have no clamp data
# also drop RH-99-T-RH2-53-T since they are missing data from RENAL-HEIR
combined_clean <- combined_clean %>% 
  filter(study != 'PANTHER') %>% 
  filter(record_id != 'RH-99-T-RH2-53-T')
```
...
pvalue <- function(x, variable, ...) {
  ...
  vars_to_skip <- c('study', 'acr_u', 'urine_microalb_quant_24hr', 'NEFA_delta',
...
table1::table1 (~ study + age + diabetes_duration + sex + ethnicity + height + weight + waistcm +
                  ...
                  steady_state_insulin + fasting_ffa + NEFA_delta + metformin_timepoint +
...
outcome_vars <- c('gfr_raw_plasma', 'gfr_bsa_plasma', 'erpf_raw_plasma', 'erpf_bsa_plasma', 
                  'glomerular_pressure', 'ra', 're', 'rvr', 'ff', 'rbf')
...
* (Intercept) = 159.394: Average gfr raw plasma in female Colorado subjects with an HbA1c of 0. (May not be meaningful to interpret).
* siteNetherlands = -118.668: Average difference in gfr raw plasma when Netherlands subjects are compared to Colorado subjects, after adjusting for sex and HbA1c.
* sexMale = 19.354: Average difference in gfr raw plasma when males at both sites are compared to females, after adjusting for the effect of site and HbA1c.
* hba1c = 7.388: Average change in gfr raw plasma associated with a 1 unit increase in HbA1c, after adjusting for site and sex.
