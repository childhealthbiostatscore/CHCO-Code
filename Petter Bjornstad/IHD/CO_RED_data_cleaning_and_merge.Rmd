...
CO_clean <- CO_clean %>% mutate(
  ...
  # divide fasting NEFA by 1000 to align with mmol/L units of Netherlands
  fasting_ffa = fasting_ffa/1000) 
```
...
Neth_clean <- Neth_clean %>% 
  mutate(sex = factor(sex, levels = c(1, 0), labels = c('Female', 'Male')), 
         ...
         ffa_suppression = (NEFA_delta/fasting_ffa)*100,
...
combined_dat <- full_join(CO_clean, Neth_clean, by=c('record_id', 'site', 'study', 'age', 'sex', 'ethnicity', 'group', 
                                                     ...
                                                     'ffa_suppression', 'gfr_bsa_plasma', 'erpf_bsa_plasma', 'steady_state_ffa', 
                                                     'rbf', 'fasting_ffa'))
```
...
combined_dat <- combined_dat %>% 
  select(c('record_id', 'site', 'study', 'group', 'age', 'diabetes_duration', 'sex', 'ethnicity', 'height', 
           ...
           'ffa_suppression', 'steady_state_insulin', 'fasting_ffa', 'NEFA_delta', 'INUorIOH',
