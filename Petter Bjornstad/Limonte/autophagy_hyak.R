...
source("nebula_pipeline.R")
# save results:
diabetes_ptcell_nebula_results<-full_results
# ---- Volcano Plot ----

# Create the volcano plot using ggplot
diabetes_ptcell_volcano_plot <- ggplot(full_results, aes(x = logFC_group1, y = PValue10, color = color)) +
...
# Create the volcano plot using ggplot
sglt2i_ptcell_volcano_plot <- ggplot(full_results, aes(x = logFC_group1, y = PValue10, color = color)) +
  geom_point(alpha = 0.7) +  # Add points with transparency
  scale_color_identity() +  # Use color directly from 'color' column
  theme_minimal() +  # Apply minimalistic theme
  labs(
...
# Create the volcano plot using ggplot
diabetes_allcell_volcano_plot <- ggplot(full_results, aes(x = logFC_group1, y = PValue10, color = color)) +
  geom_point(alpha = 0.7) +  # Add points with transparency
  scale_color_identity() +  # Use color directly from 'color' column
  theme_minimal() +  # Apply minimalistic theme
  labs(
...
# Create the volcano plot using ggplot
sglt2i_allcell_volcano_plot <- ggplot(full_results, aes(x = logFC_group1, y = PValue10, color = color)) +
  geom_point(alpha = 0.7) +  # Add points with transparency
  scale_color_identity() +  # Use color directly from 'color' column
  theme_minimal() +  # Apply minimalistic theme
  labs(
