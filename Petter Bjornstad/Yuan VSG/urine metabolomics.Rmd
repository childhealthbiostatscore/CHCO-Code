...
data_wide$ID <- str_replace(data_wide$ID, "_", "-")
...
# below is incorrect....need to make sure IDs and pre/post match across datasets
# or would it be better to merge RH/RH2 and IMPROVE separately, since RH/RH2 can be merged by record_id and IMPROVE merged by record_id and visit (but Long's data has pre/post for visit)
# keep only needed variables from Long's data
vsg_smt_patients_keep <- vsg_smt_patients %>% select(record_id, updated_record_id)
# merge in metabolomics, only keep those in Long's data
final <- left_join(vsg_smt_patients_keep, data_wide, by = "rec")
...
# identify columns corresponding to metabolites
