...
```{r, include=FALSE}
filename <- "RPC2 targeted normalised data combined PN.xlsx"
...
creat$time <- creat$baseline
creat$baseline <- NULL

data <- left_join(data, creat, by = c("record_id", "time"))

# just testing out who in Long's data has urine proteomics
...
test <- vsg_smt_patients %>% select(record_id, time)
test$in_vsg_smt <- 1
test <- left_join(test, creat, by = c("record_id", "time"))
# there are quite a significant number of people who have urinary metabolomics

# I have no idea where this "Dose" variable came from
