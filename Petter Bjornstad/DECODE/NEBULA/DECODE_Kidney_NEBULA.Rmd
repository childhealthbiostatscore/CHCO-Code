...
# Open the file in binary mode using Python built-in open()
# Prepare the Python command with the R temp file path inserted
py_cmd <- sprintf("f = open(r'%s', 'rb')", temp_file)

# Run it in Python via reticulate
py_run_string(py_cmd)

# Upload using upload_fileobj()
s3$upload_fileobj(py$f, bucket, "data_clean/Cleaned_DECODE_so.rds")
