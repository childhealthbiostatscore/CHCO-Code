...
setwd("/Users/hhampson/Library/CloudStorage/OneDrive-UW/Biostatistics Core Shared Drive/Adipose Tissue Transcriptomics and Liver Lipidomics")
library(DESeq2)
library(reshape2)
library(apeglm)
library(biomaRt)
...
table(coldata$group)
# CAT_healthy     CAT_T2D SAT_healthy     SAT_T2D VAT_healthy     VAT_T2D 
# 7           7           7           7           7           7 
...
# View available keytypes in the human database
keytypes(org.Hs.eg.db)
# Get gene-GO mappings directly from the package
# Example: Get all GO terms for human genes
go_mappings <- AnnotationDbi::select(org.Hs.eg.db, 
                                     keys = keys(org.Hs.eg.db, keytype = "SYMBOL"),
                                     columns = c("SYMBOL", "ENTREZID", "GO", "ONTOLOGY"),
                                     keytype = "SYMBOL")

# If you need it in a similar format to your file:
go_terms <- go_mappings %>%
  filter(!is.na(GO)) %>%
  dplyr::select(SYMBOL, ENTREZID, GO, ONTOLOGY) %>%
  unique()
...
###compare de
# setwd('G:/My Drive/lab files/Bryan Bergman/cohort 11-8-24 CAT VAT SAT/DESeq2 outputs/')
sat_de = read.csv('/DESeq2 Full results SAT_T2D over SAT_healthy.csv')
vat_de = read.csv('/DESeq2 Full results VAT_T2D over VAT_healthy.csv')
cat_de = read.csv('/DESeq2 Full results CAT_T2D over CAT_healthy.csv')
