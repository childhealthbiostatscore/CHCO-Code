...
for (i in 1:length(main_predictors)) {
  ...
  mod <- lm(as.formula(form), data = data, na.action = na.omit)
  temp <- summary(mod)$coef[2,]
  res <- rbind(res, temp)
}

outcome <- rep("Neonweight", length(main_predictors))
predictor <- rep(paste0(main_predictors[i]), length(main_predictors))
temp <- cbind(outcome, predictor)
res <- as.data.frame(cbind(temp, res))
...
filename <- "EDIPE_univariable_results.csv"
full_path <- file.path(results_path, filename)
write.csv(res, full_path, row.names = FALSE)
```
