...
simulate_cohort_specific <- function(n_controls = 5, 
                                     ...
                                     control_cells_var = 30,
                                     patient_cells_var = 30,
                                     correlation = 0.4,
                                     n_genes = 3000,
                                     batch_facLoc = 0.1,
                                     batch_facScale = 0.25) {
  ...
  control_counts <- round(rnorm(n_controls, control_cells_mean, control_cells_var))
  
  # Patients might have more variable/lower yields
  patient_counts <- round(rnorm(n_patients, patient_cells_mean, patient_cells_var))
