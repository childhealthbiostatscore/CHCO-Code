...
simulate_with_correlation <- function(n_individuals = 8,
  ...
  for(i in 1:n_individuals) {
    ...
    # Handle rowData to avoid conflicts when combining
    if(i == 1) {
      # Keep the rowData from the first simulation as reference
      keep_rowdata <- rowData(ind_sim)
    } else {
      # Clear rowData from subsequent simulations to avoid conflicts
      # The gene metadata should be the same across simulations anyway
      rowData(ind_sim) <- DataFrame(row.names = rownames(ind_sim))
    }
  ...
  # Restore the rowData from the first simulation
  if(!is.null(keep_rowdata)) {
    rowData(sim) <- keep_rowdata
  }
...
add_cell_state_correlation <- function(sim, 
  ...
  for(batch in unique(colData(sim)$Batch)) {
    ...
    if(state_correlation > 0) {
      ...
      # Create state-specific expression programs (reduced variance to avoid extreme values)
